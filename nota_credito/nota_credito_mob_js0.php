...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['nota_credito_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="nota_credito_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function sc_btn_btn_volver()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_volver(); }, 500);
      return;
    }
    sc_btn_btn_volver_ok();
}
function sc_btn_btn_volver_cancel()
{
}
function sc_btn_btn_volver_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_grid_facturaven_contratos_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo ""; ?>', 'inicio', '_self', '0', '0', 'grid_facturaven_contratos');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['nota_credito_mob']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_nota_credito_mob_navigate_form();
    }
} 
...
function scForm_submit_single(x) {
	if (x != "excluir")
	{
		...
		if ("incluir" == x || "muda_form" == x || "recarga" == x || "recarga_mobile" == x) {
			...
			Nm_Proc_Atualiz = true;
			do_ajax_nota_credito_mob_submit_form();
		}
	}
	if (Nm_submit_ok) {
		Nm_Proc_Atualiz = true;
...
function nm_saida_glo()
{
  ...
  document.F4.action = "nota_credito_mob_fim.php";
  document.F4.submit();
}
