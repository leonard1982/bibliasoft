...
$old_error_handler = set_error_handler("nota_credito_scriptcase_error_handler");

function nota_credito_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['nota_credito']['contr_erro']) && $_SESSION['scriptcase']['nota_credito']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            global $inicial_nota_credito;
            $inicial_nota_credito->contr_nota_credito->Apl_com_erro = true;
            if ($inicial_nota_credito->contr_nota_credito->NM_ajax_flag)
            {
                $inicial_nota_credito->contr_nota_credito->NM_ajax_info['errList']['geral_nota_credito'][] = $msg;
                return;
            }
            nota_credito_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function nota_credito_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['nota_credito'])  && '' != $_SESSION['scriptcase']['error_icon']['nota_credito'])  ? $_SESSION['scriptcase']['error_icon']['nota_credito']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['nota_credito']) && '' != $_SESSION['scriptcase']['error_close']['nota_credito']) ? $_SESSION['scriptcase']['error_close']['nota_credito'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['nota_credito']) && '' != $_SESSION['scriptcase']['error_span_title']['nota_credito']) ? $_SESSION['scriptcase']['error_span_title']['nota_credito'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['nota_credito']) && '' != $_SESSION['scriptcase']['error_icon_title']['nota_credito']) ? $_SESSION['scriptcase']['error_icon_title']['nota_credito'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['nota_credito']['error_buffer'] .= ob_get_flush();
}
