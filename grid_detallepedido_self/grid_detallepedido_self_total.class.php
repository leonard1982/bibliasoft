...
class grid_detallepedido_self_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_detallepedido_self']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_detallepedido_self']['campos_busca']))
      { 
   ...
   function quebra_geral_sc_free_total($res_limit=false)
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallepedido_self']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $nm_comando = "select distinct     iddet,     idpedid,     numfac,     remision,     idpro,     unidadmayor,     factor,     idbod,     costop,     cantidad,     valorunit,     valorpar,     iva,     descuento,     adicional,     adicional1,     devuelto,     colores,     tallas,     sabor,     estado_comanda,     usuario_comanda,     tercero_comanda,     hora_inicio,     hora_final,     observ,     cerrado,     obs,     descr from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallepedido_self']['where_pesq']; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallepedido_self']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(costop), sum(cantidad), sum(valorpar) from (" . $nm_comando . ") nm_sel_esp "; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(costop), sum(cantidad), sum(valorpar) from (" . $nm_comando . ") nm_sel_esp "; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallepedido_self']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallepedido_self']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallepedido_self']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallepedido_self']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallepedido_self']['tot_geral'][4] = $rt->fields[3]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallepedido_self']['contr_total_geral'] = "OK";
   } 
