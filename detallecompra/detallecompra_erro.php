...
$old_error_handler = set_error_handler("detallecompra_scriptcase_error_handler");

function detallecompra_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['detallecompra']['contr_erro']) && $_SESSION['scriptcase']['detallecompra']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_detallecompra->contr_detallecompra->Apl_com_erro = true;
            if ($inicial_detallecompra->contr_detallecompra->NM_ajax_flag)
            {
                $inicial_detallecompra->contr_detallecompra->NM_ajax_info['errList']['geral_detallecompra'][] = $msg;
                return;
            }
            detallecompra_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function detallecompra_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['detallecompra'])  && '' != $_SESSION['scriptcase']['error_icon']['detallecompra'])  ? $_SESSION['scriptcase']['error_icon']['detallecompra']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['detallecompra']) && '' != $_SESSION['scriptcase']['error_close']['detallecompra']) ? $_SESSION['scriptcase']['error_close']['detallecompra'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['detallecompra']) && '' != $_SESSION['scriptcase']['error_span_title']['detallecompra']) ? $_SESSION['scriptcase']['error_span_title']['detallecompra'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['detallecompra']) && '' != $_SESSION['scriptcase']['error_icon_title']['detallecompra']) ? $_SESSION['scriptcase']['error_icon_title']['detallecompra'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['detallecompra']['error_buffer'] .= ob_get_flush();
}
