...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['detallecompra_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="detallecompra_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_tallacolor() 
{ 
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_tallacolor(); }, 500);
      return;
    }
    if($(".css_colores__obj").css("display")=="none")
{
	$(".css_colores__obj").css("display","block");
	$(".css_colores__label").text("Color");
	
	$(".css_tallas__obj").css("display","block");
	$(".css_tallas__label").text("Talla");
	
	$(".css_sabor__obj").css("display","block");
	$(".css_sabor__label").text("Sabor");
}
else
{
	$(".css_colores__obj").css("display","none");
	$(".css_colores__label").text("");
	
	$(".css_tallas__obj").css("display","none");
	$(".css_tallas__label").text("");
	
	$(".css_sabor__obj").css("display","none");
	$(".css_sabor__label").text("");
...
?>
    ...
    {
        do_ajax_detallecompra_mob_navigate_form();
    }
} 
...
?>
    ...
    { 
       ...
       {
           Nm_Proc_Atualiz = true;
           do_ajax_detallecompra_mob_submit_form();
       }
    } 
    if (Nm_submit_ok)
    { 
