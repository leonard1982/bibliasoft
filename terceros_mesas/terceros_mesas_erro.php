...
$old_error_handler = set_error_handler("terceros_mesas_scriptcase_error_handler");

function terceros_mesas_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['terceros_mesas']['contr_erro']) && $_SESSION['scriptcase']['terceros_mesas']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            global $inicial_terceros_mesas;
            $inicial_terceros_mesas->contr_terceros_mesas->Apl_com_erro = true;
            if ($inicial_terceros_mesas->contr_terceros_mesas->NM_ajax_flag)
            {
                $inicial_terceros_mesas->contr_terceros_mesas->NM_ajax_info['errList']['geral_terceros_mesas'][] = $msg;
                return;
            }
            terceros_mesas_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function terceros_mesas_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['terceros_mesas'])  && '' != $_SESSION['scriptcase']['error_icon']['terceros_mesas'])  ? $_SESSION['scriptcase']['error_icon']['terceros_mesas']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['terceros_mesas']) && '' != $_SESSION['scriptcase']['error_close']['terceros_mesas']) ? $_SESSION['scriptcase']['error_close']['terceros_mesas'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['terceros_mesas']) && '' != $_SESSION['scriptcase']['error_span_title']['terceros_mesas']) ? $_SESSION['scriptcase']['error_span_title']['terceros_mesas'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['terceros_mesas']) && '' != $_SESSION['scriptcase']['error_icon_title']['terceros_mesas']) ? $_SESSION['scriptcase']['error_icon_title']['terceros_mesas'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['terceros_mesas']['error_buffer'] .= ob_get_flush();
}
