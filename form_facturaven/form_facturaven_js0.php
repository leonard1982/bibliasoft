<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idfacven" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idfacven']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_facturaven']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_facturaven"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_imprimir_par_numfacventa = "<?php echo $this->form_encode_input($this->nmgp_dados_form['idfacven']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_pdfreport_facturaven_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "par_numfacventa*scin' + var_btn_imprimir_par_numfacventa + '*scout"; ?>', 'inicio', '_blank', '0', '0', 'pdfreport_facturaven');
}
...
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_rc(); }, 500);
      return;
    }
    sc_btn_rc_ok();
}
function sc_btn_rc_cancel()
{
}
function sc_btn_rc_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_grid_reciboingreso_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo ""; ?>', 'inicio', '_self', '0', '0', 'grid_reciboingreso');
}
var Crtl_btn_autorizar = false;
function sc_btn_autorizar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_autorizar(); }, 500);
      return;
    }
    if (Crtl_btn_autorizar) {return;}
    sc_btn_autorizar_ok();
}
function sc_btn_autorizar_cancel()
{
}
function sc_btn_autorizar_ok()
{
    Crtl_btn_autorizar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?autorizar?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_facturaven']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_form_facturaven_navigate_form();
    }
} 
...
?>
    ...
    { 
       ...
       {
           Nm_Proc_Atualiz = true;
           do_ajax_form_facturaven_submit_form();
       }
    } 
    if (Nm_submit_ok)
    { 
...
{
    NovaJanela = window.open ("form_facturaven_help.php?nm_cod_campo=" + campo + "&language=<?php echo $this->Ini->path_lang . $this->Ini->str_lang?>&help_css=<?php echo '../_lib/css/' . $this->Ini->str_schema_all . '_form.css'; ?>", "ScriptCase", "height=100, width=300, resizable, scrollbars");
}
