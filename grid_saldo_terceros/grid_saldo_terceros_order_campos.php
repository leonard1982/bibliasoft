...
<?php
   ...
   $Ord_Cmp = new grid_saldo_terceros_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_saldo_terceros_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_select_orig'] = array();
   }
   ...
   $tab_labels["documento"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['labels']["documento"])) ? $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['labels']["documento"] : "Documento";
   $tab_ger_campos['nombres'] = "on";
   $tab_def_campos['nombres'] = "nombres";
   $tab_labels["nombres"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['labels']["nombres"])) ? $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['labels']["nombres"] : "Nombres";
   $tab_ger_campos['saldo_a_cobrar'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['saldo_a_cobrar'] = "saldo_a_cobrar";
   }
   else
   {
       $tab_def_campos['saldo_a_cobrar'] = "if(saldo<0,saldo*-1,saldo)";
   }
   $tab_labels["saldo_a_cobrar"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['labels']["saldo_a_cobrar"])) ? $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['labels']["saldo_a_cobrar"] : "Saldo a Cobrar";
   $tab_ger_campos['saldo_a_pagar'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['saldo_a_pagar'] = "saldo_a_pagar";
   }
   else
   {
       $tab_def_campos['saldo_a_pagar'] = "if(saldoapagar<0,saldoapagar*-1,saldoapagar)";
   }
   $tab_labels["saldo_a_pagar"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['labels']["saldo_a_pagar"])) ? $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['labels']["saldo_a_pagar"] : "Saldo a Pagar";
   $tab_ger_campos['diferencia'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['diferencia'] = "diferencia";
   }
   else
   {
       $tab_def_campos['diferencia'] = "(if(saldo<0,saldo*-1,saldo)-if(saldoapagar<0,saldoapagar*-1,saldoapagar))";
   }
   $tab_labels["diferencia"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['labels']["diferencia"])) ? $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['labels']["diferencia"] : "Diferencia";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_saldo_terceros_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_saldo_terceros']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
