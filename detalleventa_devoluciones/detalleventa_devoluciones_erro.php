...
$old_error_handler = set_error_handler("detalleventa_devoluciones_scriptcase_error_handler");

function detalleventa_devoluciones_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['detalleventa_devoluciones']['contr_erro']) && $_SESSION['scriptcase']['detalleventa_devoluciones']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_detalleventa_devoluciones->contr_detalleventa_devoluciones->Apl_com_erro = true;
            if ($inicial_detalleventa_devoluciones->contr_detalleventa_devoluciones->NM_ajax_flag)
            {
                $inicial_detalleventa_devoluciones->contr_detalleventa_devoluciones->NM_ajax_info['errList']['geral_detalleventa_devoluciones'][] = $msg;
                return;
            }
            detalleventa_devoluciones_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function detalleventa_devoluciones_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['detalleventa_devoluciones'])  && '' != $_SESSION['scriptcase']['error_icon']['detalleventa_devoluciones'])  ? $_SESSION['scriptcase']['error_icon']['detalleventa_devoluciones']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['detalleventa_devoluciones']) && '' != $_SESSION['scriptcase']['error_close']['detalleventa_devoluciones']) ? $_SESSION['scriptcase']['error_close']['detalleventa_devoluciones'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['detalleventa_devoluciones']) && '' != $_SESSION['scriptcase']['error_span_title']['detalleventa_devoluciones']) ? $_SESSION['scriptcase']['error_span_title']['detalleventa_devoluciones'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['detalleventa_devoluciones']) && '' != $_SESSION['scriptcase']['error_icon_title']['detalleventa_devoluciones']) ? $_SESSION['scriptcase']['error_icon_title']['detalleventa_devoluciones'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['detalleventa_devoluciones']['error_buffer'] .= ob_get_flush();
}
