...
class grid_DEKARDEX_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_DEKARDEX']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_DEKARDEX']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang , $matid, $bodid, $tipund;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_DEKARDEX']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_DEKARDEX']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum((PARCVTA/((PORCIVA/100)+1))), sum((PARCVTA-(PARCVTA/((PORCIVA/100)+1)))), sum(PARCVTA) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_DEKARDEX']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum((PARCVTA/((PORCIVA/100)+1))), sum((PARCVTA-(PARCVTA/((PORCIVA/100)+1)))), sum(PARCVTA) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_DEKARDEX']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_DEKARDEX']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_DEKARDEX']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_DEKARDEX']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_DEKARDEX']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_DEKARDEX']['tot_geral'][4] = $rt->fields[3]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_DEKARDEX']['contr_total_geral'] = "OK";
   } 

   //-----  porciva
   function quebra_porciva_sc_free_group_by($porciva, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $matid, $bodid, $tipund;
      $tot_porciva = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum((PARCVTA/((PORCIVA/100)+1))), sum((PARCVTA-(PARCVTA/((PORCIVA/100)+1)))), sum(PARCVTA) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_DEKARDEX']['where_pesq']; 
      ...
      $tot_porciva[0] = NM_encode_input(sc_strip_script($porciva)) ; 
      $tot_porciva[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_porciva[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_porciva[3] = (string)$rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $tot_porciva[4] = (string)$rt->fields[3]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_porciva;
   } 
