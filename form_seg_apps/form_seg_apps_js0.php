<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idapp" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idapp']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_seg_apps']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_seg_apps"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_seg_apps']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_form_seg_apps_navigate_form();
    }
} 
...
?>
    ...
    { 
       ...
       {
           Nm_Proc_Atualiz = true;
           do_ajax_form_seg_apps_submit_form();
       }
    } 
    if (Nm_submit_ok)
    { 
...
function sc_app_parentid_onchange()
{
   let opcion = document.F1.app_parentid.value;
if (opcion == 'NULL') {
	$('#hidden_field_label_color').closest('tr').show();
} else {
	$('#hidden_field_label_color').closest('tr').hide();
}
//console.log('opc: ' + opcion + ' - rta: ' + (opcion != 'NULL'))
}
function sc_app_icon_onfocus()
{
   $(".contenedor.card").removeClass('d-none');

$('#id_sc_field_app_icon').blur();
$('#buscar_icon').focus();

let x =document.querySelector('#id_sc_field_app_icon').getBoundingClientRect().x, y = document.querySelector('#id_sc_field_app_icon').getBoundingClientRect().y,
		winWidth = window.innerWidth,
		winHeight = window.innerHeight,
		cmWidth = document.querySelector('.contenedor.card').offsetWidth,
		cmHeight = document.querySelector('.contenedor.card').offsetHeight;

x = x > winWidth - cmWidth ? winWidth - cmWidth - 5 : x;
y = y > winHeight - cmHeight ? winHeight - cmHeight - 5 : y;

document.querySelector('.contenedor.card').style.left = x+`px`;
document.querySelector('.contenedor.card').style.top = y+`px`;



}
function sc_level_onblur()
{
...
function sc_form_onload()
{
   $('input[name="color"]').each(function(){
	let value =$(this).val();  
	console.log($(this).closest('.sc radio').find('label'))
	$(this).addClass('bg-'+value)
	$(this).closest('.sc.radio').find('label').addClass('d-block bg-'+value)
})
...
$('.item-icon').on('click', function(){
	$('#buscar_icon').val('')
	$('#id_sc_field_app_icon').val($(this).data('nombre'))
	$('.contenedor.card').addClass('d-none')
})

$('.card-header > summary').on('click', function(){
	$('#buscar_icon').val('')
	$('.contenedor.card').addClass('d-none')
})
