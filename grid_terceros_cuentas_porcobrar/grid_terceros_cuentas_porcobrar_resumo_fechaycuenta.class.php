...
class grid_terceros_cuentas_porcobrar_resumo
{
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      {
          $this->array_total_fecha = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['arr_total']['fecha'];
          $this->array_total_cod_cuenta = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['arr_total']['cod_cuenta'];
      }
      else
      {
          $this->array_total_fecha = array();
   ...
   function completeMatrix()
   {
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_order']['fechaycuenta']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_order']['fechaycuenta'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_order']['fechaycuenta'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['fechaycuenta'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['fechaycuenta'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['fechaycuenta'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['fechaycuenta'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['fechaycuenta'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['fechaycuenta'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['fechaycuenta'] as $i_sum => $d_sum)
           {
       ...
       //-----
       foreach ($this->array_total_fecha as $i_fecha => $d_fecha) {
           if (!in_array($i_fecha, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_fecha[4] ] = $d_fecha[3];
               $this->comp_label[0][ $d_fecha[4] ] = $d_fecha[3];
           }
           foreach ($this->array_total_cod_cuenta[$i_fecha] as $i_cod_cuenta => $d_cod_cuenta) {
               if (!in_array($i_cod_cuenta, $this->comp_label[1][ $d_fecha[4] ], true)) {
                   $this->comp_index[1][ $d_cod_cuenta[4] ] = $d_cod_cuenta[3];
                   $this->comp_label[1][ $d_fecha[4] ][ $d_cod_cuenta[4] ] = $d_cod_cuenta[3];
               }
           }
       }
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['pivot_order_level'] = -1;
       }
       ...
       //-----
       foreach ($this->comp_label[0] as $i_fecha => $l_fecha) {
           if (isset($this->array_total_fecha[$i_fecha])) {
               $this->comp_group[$i_fecha] = array();
           }
           foreach ($this->comp_label[1][$i_fecha] as $i_cod_cuenta => $l_cod_cuenta) {
               if (isset($this->array_total_cod_cuenta[$i_fecha][$i_cod_cuenta])) {
                   $this->comp_group[$i_fecha][$i_cod_cuenta] = array();
               }
           }
       }
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['proc_pdf_vert']){ 
   ...
   function totaliza()
   {
      $this->Total->Calc_resumo_fechaycuenta("res");
      ...
      krsort($this->array_total_fecha);
      ksort($this->array_total_cod_cuenta);
      if ("out" == $this->NM_tipo)
      {
         $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['SC_Ind_Groupby'];
         $this->Total->$Gb_geral();
...
//--- 
   ...
   //---- 
   function adiciona_registro($sum_valor_total, $quebra_fecha, $quebra_cod_cuenta, $quebra_fecha_orig, $quebra_cod_cuenta_orig)
   {
      //----- " . $this->Ini->Nm_lang['lang_othr_cons_title_YYYYMMDD2'] . "
      if (!isset($this->array_total_fecha[$quebra_fecha_orig]))
      {
         $this->array_total_fecha[$quebra_fecha_orig][0] = 1;
         $this->array_total_fecha[$quebra_fecha_orig][1] = $sum_valor_total;
         $this->array_total_fecha[$quebra_fecha_orig][2] = 1;
         $this->array_total_fecha[$quebra_fecha_orig][3] = $quebra_fecha;
         $this->array_total_fecha[$quebra_fecha_orig][4] = $quebra_fecha_orig;
      }
      else
      {
         $this->array_total_fecha[$quebra_fecha_orig][0]++;
         $this->array_total_fecha[$quebra_fecha_orig][1] += $sum_valor_total;
         $this->array_total_fecha[$quebra_fecha_orig][2]++;
      }
      //----- Cuenta
      if (!isset($this->array_total_cod_cuenta[$quebra_fecha_orig][$quebra_cod_cuenta_orig]))
      {
         $this->array_total_cod_cuenta[$quebra_fecha_orig][$quebra_cod_cuenta_orig][0] = 1;
         $this->array_total_cod_cuenta[$quebra_fecha_orig][$quebra_cod_cuenta_orig][1] = $sum_valor_total;
         $this->array_total_cod_cuenta[$quebra_fecha_orig][$quebra_cod_cuenta_orig][2] = 1;
         $this->array_total_cod_cuenta[$quebra_fecha_orig][$quebra_cod_cuenta_orig][3] = $quebra_cod_cuenta;
         $this->array_total_cod_cuenta[$quebra_fecha_orig][$quebra_cod_cuenta_orig][4] = $quebra_cod_cuenta_orig;
      }
      else
      {
         $this->array_total_cod_cuenta[$quebra_fecha_orig][$quebra_cod_cuenta_orig][0]++;
         $this->array_total_cod_cuenta[$quebra_fecha_orig][$quebra_cod_cuenta_orig][1] += $sum_valor_total;
         $this->array_total_cod_cuenta[$quebra_fecha_orig][$quebra_cod_cuenta_orig][2]++;
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     $campos_order = "";
     $format       = $this->Ini->Get_Gb_date_format('', 'fecha');
     $campos_order = $this->Ini->Get_date_order_groupby("fecha", 'desc', $format, $campos_order);
     $format       = $this->Ini->Get_Gb_date_format('', 'cod_cuenta');
     $campos_order = $this->Ini->Get_date_order_groupby("cod_cuenta", 'asc', $format, $campos_order);
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $_SESSION['scriptcase']['grid_terceros_cuentas_porcobrar']['contr_erro'] = 'on';
...
$_SESSION['scriptcase']['grid_terceros_cuentas_porcobrar']['contr_erro'] = 'off'; 
            ...
            $this->fecha_orig = $this->Ini->Get_arg_groupby($TP_Time . $this->fecha, $Format_tst);
            $this->fecha = $this->Ini->GB_date_format($TP_Time . $this->fecha, $Format_tst, $Prefix_dat);
            ...
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->valor_total, $this->fecha, $this->cod_cuenta, $this->fecha_orig,  $this->cod_cuenta_orig);
            }
   }
//
