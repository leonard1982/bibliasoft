...
class grid_terceros_cuentas_porcobrar_resumo
{
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      {
          $this->array_total_concepto = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['arr_total']['concepto'];
      }
      else
      {
          $this->array_total_concepto = array();
   ...
   function completeMatrix()
   {
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_order']['concepto']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_order']['concepto'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_order']['concepto'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['concepto'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['concepto'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['concepto'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['concepto'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['concepto'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['concepto'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['summarizing_fields_display']['concepto'] as $i_sum => $d_sum)
           {
       ...
       //-----
       foreach ($this->array_total_concepto as $i_concepto => $d_concepto) {
           if (!in_array($i_concepto, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_concepto[4] ] = $d_concepto[3];
               $this->comp_label[0][ $d_concepto[4] ] = $d_concepto[3];
           }
       }
       ...
       //-----
       foreach ($this->comp_label[0] as $i_concepto => $l_concepto) {
           if (isset($this->array_total_concepto[$i_concepto])) {
               $this->comp_group[$i_concepto] = array();
           }
       }
   ...
   function getCompData($level, $params = array()) {
       if (0 == $level) {
           $return = $this->array_total_concepto;
       }
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['proc_pdf_vert']){ 
   ...
   function totaliza()
   {
      $this->Total->Calc_resumo_concepto("res");
      ...
      ksort($this->array_total_concepto);
      if ("out" == $this->NM_tipo)
      {
         $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_cuentas_porcobrar']['SC_Ind_Groupby'];
         $this->Total->$Gb_geral();
...
//--- 
   ...
   //---- 
   function adiciona_registro($sum_valor_total, $quebra_concepto, $quebra_concepto_orig)
   {
      //----- Concepto
      if (!isset($this->array_total_concepto[$quebra_concepto_orig]))
      {
         $this->array_total_concepto[$quebra_concepto_orig][0] = 1;
         $this->array_total_concepto[$quebra_concepto_orig][1] = $sum_valor_total;
         $this->array_total_concepto[$quebra_concepto_orig][2] = 1;
         $this->array_total_concepto[$quebra_concepto_orig][3] = $quebra_concepto;
         $this->array_total_concepto[$quebra_concepto_orig][4] = $quebra_concepto_orig;
      }
      else
      {
         $this->array_total_concepto[$quebra_concepto_orig][0]++;
         $this->array_total_concepto[$quebra_concepto_orig][1] += $sum_valor_total;
         $this->array_total_concepto[$quebra_concepto_orig][2]++;
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     $campos_order = "";
     $format       = $this->Ini->Get_Gb_date_format('', 'concepto');
     $campos_order = $this->Ini->Get_date_order_groupby("concepto", 'asc', $format, $campos_order);
...
$_SESSION['scriptcase']['grid_terceros_cuentas_porcobrar']['contr_erro'] = 'off'; 
            ...
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->valor_total, $this->concepto, $this->concepto_orig);
            }
   }
//
