...
class grid_clientes_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_clientes']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_clientes']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang , $idmuni, $loatiende;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['contr_total_geral'] = "OK";
   } 

   //---- 
   function quebra_geral_ciudad($res_limit=false)
   {
      global $nada, $nm_lang , $idmuni, $loatiende;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['contr_total_geral'] == "OK") 
      { 
   ...
   //-----  loatiende
   function quebra_loatiende_vendedor($loatiende, $arg_sum_loatiende) 
   {
      global $tot_loatiende, $idmuni, $loatiende;
      $tot_loatiende = array() ;  
      $tot_loatiende[0] = $loatiende ; 
   }
   //-----  idmuni
   function quebra_idmuni_ciudad($idmuni, $arg_sum_idmuni) 
   {
      global $tot_idmuni, $idmuni, $loatiende;
      $tot_idmuni = array() ;  
      $tot_idmuni[0] = $idmuni ; 
   }
   ...
   {
      global $nm_lang, $facturas, $historial, $idmuni, $loatiende;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('loatiende' => "loatiende");
      $cmps_quebra_atual = array("loatiende");
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_vendedor($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('idtercero' => 'N','saldo' => 'N','idmuni' => 'N','loatiende' => 'N','dv' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['loatiende']['reg'] = "S";
      $format_dimensions['loatiende']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "loatiende") {
                  $this->Lookup->lookup_vendedor_loatiende($$Cada_dim,  $loatiende);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   //----- 
   function Calc_resumo_ciudad($destino_resumo)
   {
      global $nm_lang, $facturas, $historial, $idmuni, $loatiende;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['sql_tot_res']);
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('idmuni' => "idmuni");
      $cmps_quebra_atual = array("idmuni");
      $ult_cmp_quebra_atual = $cmps_quebra_atual[(count($cmps_quebra_atual) - 1)];
      $arr_tots = "";
      $join     = "";
      $group    = "";
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_ciudad($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_clientes']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
   ...
   function Res_Totaliza_ciudad($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      ...
      $format_dimensions = array();
      $format_dimensions['idmuni']['reg'] = "S";
      $format_dimensions['idmuni']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              $prep_look  = $Cada_dim . "_SC_look";
              $$prep_look = $rt->fields[$Ind_sql];
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'ciudad', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "idmuni") {
                  $this->Lookup->lookup_ciudad_idmuni($$Cada_dim,  $idmuni);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          {
              $where_ok = (empty($this->sc_where_atual)) ? " where " . $sql_where : $this->sc_where_atual . " and " . $sql_where;
              $vl_statistic = $this->Calc_statist_manual_ciudad($where_ok);
              foreach ($vl_statistic as $ind => $val)
              {
                  $rt->fields[$ind] = $val;
              }
