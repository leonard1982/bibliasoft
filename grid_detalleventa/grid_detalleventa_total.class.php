...
class grid_detalleventa_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_detalleventa']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_detalleventa']['campos_busca']))
      { 
   ...
   function quebra_geral_sc_free_total($res_limit=false)
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleventa']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleventa']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(cantidad), sum(iva), sum(descuento) from (SELECT      iddet,     numfac,     idpro,    unidadmayor,    factor,     idbod,     cantidad,     valorunit,     valorpar,     iva,     descuento,     adicional,     adicional1,     colores,     tallas,     sabor,     if(d.obs is null or d.obs='', (select p.nompro from productos p where p.idprod=d.idpro),concat((select p.nompro from productos p where p.idprod=d.idpro),': ',d.obs)) as descripcion_ampliada FROM      detalleventa d WHERE numfac=" . $_SESSION['par_numfacventa'] . " ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleventa']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(cantidad), sum(iva), sum(descuento) from (SELECT      iddet,     numfac,     idpro,    unidadmayor,    factor,     idbod,     cantidad,     valorunit,     valorpar,     iva,     descuento,     adicional,     adicional1,     colores,     tallas,     sabor,     if(d.obs is null or d.obs='', (select p.nompro from productos p where p.idprod=d.idpro),concat((select p.nompro from productos p where p.idprod=d.idpro),': ',d.obs)) as descripcion_ampliada FROM      detalleventa d WHERE numfac=" . $_SESSION['par_numfacventa'] . " ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleventa']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleventa']['tot_geral'][0] = "Total Productos Vendidos, IVA y Descuento de la venta"; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleventa']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleventa']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleventa']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleventa']['tot_geral'][4] = $rt->fields[3]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detalleventa']['contr_total_geral'] = "OK";
   } 
