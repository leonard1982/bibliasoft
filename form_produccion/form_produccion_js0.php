<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idmat" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idmat']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_produccion']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_produccion"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_Imprimir_par_numero = "<?php echo $this->form_encode_input($this->nmgp_dados_form['numero']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_pdfreport_produccion_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "par_numero*scin' + var_btn_Imprimir_par_numero + '*scout"; ?>', 'inicio', '_blank', '0', '0', 'pdfreport_produccion');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_produccion']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_form_produccion_navigate_form();
    }
} 
...
?>
    ...
    { 
       ...
       {
           Nm_Proc_Atualiz = true;
           do_ajax_form_produccion_submit_form();
       }
    } 
    if (Nm_submit_ok)
    { 
