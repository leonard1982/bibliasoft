...
function scEventControl_init(iSeqRow) {
  ...
  scEventControl_data["id_puc_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
...
function scEventControl_active(iSeqRow) {
  ...
  if (scEventControl_data["id_puc_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["id_puc_" + iSeqRow]["change"]) {
    return true;
  }
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_' + iSeqRow).bind('blur', function() { sc_form_puc_auxiliares_id__onblur(this, iSeqRow) })
                                 .bind('change', function() { sc_form_puc_auxiliares_id__onchange(this, iSeqRow) })
                                 .bind('focus', function() { sc_form_puc_auxiliares_id__onfocus(this, iSeqRow) });
  $('#id_sc_field_id_puc_' + iSeqRow).bind('blur', function() { sc_form_puc_auxiliares_id_puc__onblur(this, iSeqRow) })
                                     .bind('change', function() { sc_form_puc_auxiliares_id_puc__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_puc_auxiliares_id_puc__onfocus(this, iSeqRow) });
  $('#id_sc_field_codigo_' + iSeqRow).bind('blur', function() { sc_form_puc_auxiliares_codigo__onblur(this, iSeqRow) })
                                     .bind('change', function() { sc_form_puc_auxiliares_codigo__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_puc_auxiliares_codigo__onfocus(this, iSeqRow) });
  $('#id_sc_field_nombre_' + iSeqRow).bind('blur', function() { sc_form_puc_auxiliares_nombre__onblur(this, iSeqRow) })
                                     .bind('change', function() { sc_form_puc_auxiliares_nombre__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_puc_auxiliares_nombre__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_puc_auxiliares_id__onblur(oThis, iSeqRow) {
  do_ajax_form_puc_auxiliares_validate_id_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_puc_auxiliares_id__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_puc_auxiliares_id__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_puc_auxiliares_id_puc__onblur(oThis, iSeqRow) {
  do_ajax_form_puc_auxiliares_validate_id_puc_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_puc_auxiliares_id_puc__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  do_ajax_form_puc_auxiliares_event_id_puc__onchange(iSeqRow);
  nm_check_insert(iSeqRow);
}

function sc_form_puc_auxiliares_id_puc__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_puc_auxiliares_codigo__onblur(oThis, iSeqRow) {
  do_ajax_form_puc_auxiliares_validate_codigo_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_puc_auxiliares_codigo__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_puc_auxiliares_codigo__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_puc_auxiliares_nombre__onblur(oThis, iSeqRow) {
  do_ajax_form_puc_auxiliares_validate_nombre_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_puc_auxiliares_nombre__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_puc_auxiliares_nombre__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	...
	displayChange_field("id_puc_", "", status);
...
function displayChange_row(row, status) {
	...
	displayChange_field_id_puc_(row, status);
...
function displayChange_field_id_puc_(row, status) {
	if ("on" == status) {
		if ("all" == row) {
			var fieldList = $(".css_id_puc___obj");
			for (var i = 0; i < fieldList.length; i++) {
				$($(fieldList[i]).attr("id")).select2("destroy");
			}
		}
		else {
			$("#id_sc_field_id_puc_" + row).select2("destroy");
		}
		scJQSelect2Add(row, "id_puc_");
	}
}
...
function scRecreateSelect2() {
	displayChange_field_id_puc_("all", "on");
}
...
function scHidePage(pageNo) {
	$("#id_form_puc_auxiliares_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_puc_auxiliares')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
...
function scJQSelect2Add(seqRow, specificField) {
  if (null == specificField || "id_puc_" == specificField) {
    scJQSelect2Add_id_puc_(seqRow);
  }
} // scJQSelect2Add

function scJQSelect2Add_id_puc_(seqRow) {
  var elemSelector = "all" == seqRow ? ".css_id_puc__obj" : "#id_sc_field_id_puc_" + seqRow;
  $(elemSelector).select2(
    {
      containerCssClass: 'css_id_puc__obj',
      dropdownCssClass: 'css_id_puc__obj',
      language: {
        noResults: function() {
...
function scJQElementsAdd(iLine) {
  ...
  setTimeout(function () { if ('function' == typeof displayChange_field_id_puc_) { displayChange_field_id_puc_(iLine, "on"); } }, 150);
} // scJQElementsAdd
