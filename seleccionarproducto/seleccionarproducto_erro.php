...
$old_error_handler = set_error_handler("seleccionarproducto_scriptcase_error_handler");

function seleccionarproducto_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['seleccionarproducto']['contr_erro']) && $_SESSION['scriptcase']['seleccionarproducto']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_seleccionarproducto->contr_seleccionarproducto->Apl_com_erro = true;
            if ($inicial_seleccionarproducto->contr_seleccionarproducto->NM_ajax_flag)
            {
                $inicial_seleccionarproducto->contr_seleccionarproducto->NM_ajax_info['errList']['geral_seleccionarproducto'][] = $msg;
                return;
            }
            seleccionarproducto_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function seleccionarproducto_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['seleccionarproducto'])  && '' != $_SESSION['scriptcase']['error_icon']['seleccionarproducto'])  ? $_SESSION['scriptcase']['error_icon']['seleccionarproducto']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['seleccionarproducto']) && '' != $_SESSION['scriptcase']['error_close']['seleccionarproducto']) ? $_SESSION['scriptcase']['error_close']['seleccionarproducto'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['seleccionarproducto']) && '' != $_SESSION['scriptcase']['error_span_title']['seleccionarproducto']) ? $_SESSION['scriptcase']['error_span_title']['seleccionarproducto'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['seleccionarproducto']) && '' != $_SESSION['scriptcase']['error_icon_title']['seleccionarproducto']) ? $_SESSION['scriptcase']['error_icon_title']['seleccionarproducto'] : '';
?>
