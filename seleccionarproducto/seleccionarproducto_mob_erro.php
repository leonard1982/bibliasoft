...
$old_error_handler = set_error_handler("seleccionarproducto_mob_scriptcase_error_handler");

function seleccionarproducto_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['seleccionarproducto_mob']['contr_erro']) && $_SESSION['scriptcase']['seleccionarproducto_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            global $inicial_seleccionarproducto_mob;
            $inicial_seleccionarproducto_mob->contr_seleccionarproducto_mob->Apl_com_erro = true;
            if ($inicial_seleccionarproducto_mob->contr_seleccionarproducto_mob->NM_ajax_flag)
            {
                $inicial_seleccionarproducto_mob->contr_seleccionarproducto_mob->NM_ajax_info['errList']['geral_seleccionarproducto_mob'][] = $msg;
                return;
            }
            seleccionarproducto_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function seleccionarproducto_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['seleccionarproducto_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['seleccionarproducto_mob'])  ? $_SESSION['scriptcase']['error_icon']['seleccionarproducto_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['seleccionarproducto_mob']) && '' != $_SESSION['scriptcase']['error_close']['seleccionarproducto_mob']) ? $_SESSION['scriptcase']['error_close']['seleccionarproducto_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['seleccionarproducto_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['seleccionarproducto_mob']) ? $_SESSION['scriptcase']['error_span_title']['seleccionarproducto_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['seleccionarproducto_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['seleccionarproducto_mob']) ? $_SESSION['scriptcase']['error_icon_title']['seleccionarproducto_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['seleccionarproducto']['error_buffer'] .= ob_get_flush();
}
