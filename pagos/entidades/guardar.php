...
$nombreentidad = $_POST["Adenominacion"];
$codigoentidad = $_POST["Acodigoentidad"];

if ($accion == "alta") {
    $entidades->nombreentidad = $nombreentidad;
    $entidades->codigoentidad = $codigoentidad;
    $insertado = $entidades->insert();
    if ($insertado > 0) {
        $mensaje = "El registro ha sido insertado correctamente";
    }
    $identidad = $insertado;
    $cabecera2 = "NUEVA ENTIDAD BANCARIA";
}
...
if ($accion == "modificar") {
    ...
    $entidades->nombreentidad = $nombreentidad;
    $entidades->codigoentidad = $codigoentidad;
    $actualizado = $entidades->update($identidad);
    if ($actualizado == 0) {
