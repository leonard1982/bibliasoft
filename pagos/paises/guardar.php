...
$descpais = $_POST["Adenominacion"];

if ($accion == "alta") {
    $paises->descpais = $descpais;
    $insertado = $paises->insert();
    if ($insertado > 0) {
        $mensaje = "El registro ha sido insertado correctamente";
    }
    $idpais = $insertado;
    $cabecera2 = "NUEVO PAIS";
}

if ($accion == "modificar") {
    $idpais = $_POST["id"];
    $paises->descpais = $descpais;
    $actualizado = $paises->update($idpais);
    if ($actualizado == 0) {
        $mensaje = "El registro ha sido actualizado correctamente";
    }
    $cabecera2 = "MODIFICAR PAIS";
}
?>
