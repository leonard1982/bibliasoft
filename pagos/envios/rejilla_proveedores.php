        ...
        <tbody>
            ...
            while ($row = mysql_fetch_row($resultado)) {
                ...
                <tr class="<?= $fondolinea ?>">
                    ...
                    <td width="5%"><div align="center"><a href="#"><img src="../img/observaciones.png" width="16" height="16" border="0" onClick="sel_proveedor('<?= $row[0] ?>','<?= $row[1] ?>','<?= $row[23] ?>','<?= $row[16] ?>','<?= $row[21] ?>','<?= $row[22] ?>','<?= $row[3] ?>')" title="Visualizar"></a></div></td>
                </tr>
                <?php
                $contador++;
            }
