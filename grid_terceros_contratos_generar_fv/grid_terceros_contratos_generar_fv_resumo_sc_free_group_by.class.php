...
class grid_terceros_contratos_generar_fv_resumo
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   { 
	...
	function generateChartImages() {
		...
		$this->Graf         = new grid_terceros_contratos_generar_fv_grafico();
		$this->Graf->Db     = $this->Db;
		$this->Graf->Erro   = $this->Erro;
		$this->Graf->Ini    = $this->Ini;
		$this->Graf->Lookup = $this->Lookup;

		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_step']     = 'image';
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_count']    = 0;
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_total']    = 0;
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_step'] = 'end';
	...
	function zipFileList($fileList, $password = '') {
		$tempDir     = $this->Ini->root . $_SESSION['scriptcase']['grid_terceros_contratos_generar_fv']['glo_nm_path_imag_temp'] . '/';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['grid_terceros_contratos_generar_fv']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['where_resumo']);
      }
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['res_hrz'];
      $dir_raiz          = strrpos($_SERVER['PHP_SELF'],"/") ;  
      $dir_raiz          = substr($_SERVER['PHP_SELF'], 0, $dir_raiz + 1) ;  
      $this->nm_location = $this->Ini->sc_protocolo . $this->Ini->server . $dir_raiz; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['where_resumo'] = "";
      }
      $this->inicializa_vars();
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['SC_Ind_Groupby'];
      $this->Total->$Gb_geral();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['contr_total_geral'] == "OK")
      {
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
          {
              ...
              $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_total'][$cmp_gb];
          }
      }
      else
      {
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['tot_geral'][1];
      $this->array_total_geral[1] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['tot_geral'][2];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['tot_geral'][1] == 0)
      {
      ...
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] == 'print' || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] == 'pdf') && strpos(" " . $this->Ini->SC_module_export, "resume") === false)
      { }
      else
      {
          $this->drawMatrix();
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_total']['cliente'] = $this->array_total_cliente;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_total']['fecha_contrato'] = $this->array_total_fecha_contrato;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_total']['fecha_inicio'] = $this->array_total_fecha_inicio;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_total']['estado'] = $this->array_total_estado;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_total']['zona'] = $this->array_total_zona;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_total']['barrio'] = $this->array_total_barrio;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_total']['precinto'] = $this->array_total_precinto;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['json_use_label'])
       {
          $Lab_cliente = "cliente";
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['cliente'] = $Lab_cliente; 
       $prep_field['fecha_contrato'] = $Lab_fecha_contrato;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_contrato'] = $Lab_fecha_contrato; 
       $prep_field['fecha_inicio'] = $Lab_fecha_inicio;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_inicio'] = $Lab_fecha_inicio; 
       $prep_field['estado'] = $Lab_estado;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['estado'] = $Lab_estado; 
       $prep_field['zona'] = $Lab_zona;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['zona'] = $Lab_zona; 
       $prep_field['barrio'] = $Lab_barrio;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['barrio'] = $Lab_barrio; 
       $prep_field['precinto'] = $Lab_precinto;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['precinto'] = $Lab_precinto; 
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
          $Str_gb .= ($Str_gb == "") ? "" : ",";
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['numero_contrato']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['numero_contrato'] = "Contrato"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['cliente']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['cliente'] = "Cliente"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_inicio']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_inicio'] = "Fecha Inicio"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['zona']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['zona'] = "Zona"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['barrio2']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['barrio2'] = "Barrio"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['estado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['estado'] = "Estado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['mensualidad']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['mensualidad'] = "Mensualidad"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['detalle']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['detalle'] = "Detalle"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['novedades']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['novedades'] = "Novedades"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_factura']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_factura'] = "U.Factura"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['id_ter_cont']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['id_ter_cont'] = "Id Ter Cont"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_contrato']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_contrato'] = "Fecha Contrato"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_corte']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_corte'] = "Fecha Corte"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['activo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['activo'] = "Activo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['barrio']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['barrio'] = "Barrio"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['direccion']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['direccion'] = "Direccion"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['telefono']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['telefono'] = "Telefono"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['motivo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['motivo'] = "Motivo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_limitepago']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_limitepago'] = "Fecha Limitepago"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_ultimopago']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_ultimopago'] = "Fecha Ultimopago"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['valorpagado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['valorpagado'] = "Valorpagado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['saldoanterior']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['saldoanterior'] = "Saldoanterior"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['saldoactual']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['saldoactual'] = "Saldoactual"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['mesultimafactura']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['mesultimafactura'] = "Mesultimafactura"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['observaciones']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['observaciones'] = "Observaciones"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['valor_ultimafactura']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['valor_ultimafactura'] = "Valor Ultimafactura"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['precinto']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['precinto'] = "Precinto"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['correo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['correo'] = "Correo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['anio']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['anio'] = "Anio"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['periodo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['periodo'] = "Periodo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_documento']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['fecha_documento'] = "fecha_documento"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['creado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['creado'] = "Creado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['editado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['editado'] = "Editado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['usuario_crea']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['usuario_crea'] = "Usuario Crea"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['usuario_edita']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['labels']['usuario_edita'] = "Usuario Edita"; 
       }
       //-----
       $ix = 0;
       $this->comp_field_nm = array();
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['summarizing_fields_order']['sc_free_group_by']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['summarizing_fields_order']['sc_free_group_by'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['summarizing_fields_order']['sc_free_group_by'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['summarizing_fields_display']['sc_free_group_by'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['summarizing_fields_display']['sc_free_group_by'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['summarizing_fields_display']['sc_free_group_by'] as $i_sum => $d_sum)
           {
      ...
      eval ($Str_gb);
       ...
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['SC_Gb_Free_cmp'] as $cmp_gb => $col_sql)
       {
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_col'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_level'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_sort'] = '';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_col']  = 0;
           }
           ...
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_tabular_label'];
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@grid_terceros_contratos_generar_fv@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_export']['data']  = $this->build_data;
           return;
       }
      ...
      $TD_padding = (!$this->Print_All && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] == "pdf") ? " style=\"padding: 0px !important;\"" : "";
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               if (0 <= $columns['level'])
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['embutida'])
                   {
               ...
               $chart   = (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] != "print" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['embutida']) && isset($columns['chart']) && '' != $columns['chart'] && isset($this->comp_chart_data[ $columns['chart'] ]))
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   ...
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialDisplay = '; display: none';
                           $sInitialSrc     = '';
                       }
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['proc_pdf_vert']){ 
   ...
   function resumo_init()
   {
      ...
      if ("out" == $this->NM_tipo)
      {
         $this->monta_html_ini();
         $this->monta_cabecalho();
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] != "pdf" && !$_SESSION['scriptcase']['proc_mobile'])
         {
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
   ...
   function inicializa_vars()
   {
      ...
      if ($this->Print_All)
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] = "print";
      ...
      $this->Total   = new grid_terceros_contratos_generar_fv_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_total'][$cmp_gb];
      }
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['dashboard_refresh_after_chart'] = 'resumo';
...
$nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "grid_terceros_contratos_generar_fv/grid_terceros_contratos_generar_fv_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
      ...
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_terceros_contratos_generar_fv_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_grid_terceros_contratos_generar_fv_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_terceros_contratos_generar_fv_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['num_css'] . '.css');
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if (!$this->Ini->Export_html_zip && $this->Print_All && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['doc_word']) 
      {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['embutida'])
      {
   ...
   function monta_barra_top_normal()
   {
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['word'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bword", "", "", "Rword_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_terceros_contratos_generar_fv/grid_terceros_contratos_generar_fv_config_word.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_cor=AM&nm_res_cons=s&nm_ini_word_res=&nm_all_modules=grid&password=n&origem=res&language=es&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      if ($this->nmgp_botoes['groupby'] == "on" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['opc_psq'] && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
          $Q_free  = false;
          $Q_count = 0;
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['SC_All_Groupby'] as $QB => $Tp)
          {
              if (!in_array($QB, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['SC_Groupby_hide']) && $Tp == "all")
              {
                  $Q_count++;
      ...
      if ($this->nmgp_botoes['chart_detail'] == 'on' && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
         ...
         if (is_file("grid_terceros_contratos_generar_fv_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
   ...
   function monta_html_fim()
   {
      ...
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['grid_terceros_contratos_generar_fv']['glo_nm_path_prod'] . "/third/sweetalert/sweetalert2.all.min.js\"></script>\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['grid_terceros_contratos_generar_fv']['glo_nm_path_prod'] . "/third/sweetalert/polyfill.min.js\"></script>\r\n");
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'grid_terceros_contratos_generar_fv_config_pivot.php?nome_apl=grid_terceros_contratos_generar_fv&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=es&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   $nm_saida->saida("   }\r\n");
      ...
      $nm_saida->saida(" }\r\n");
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['SC_Gb_Free_cmp']))
      {
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_terceros_contratos_generar_fv_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "grid_terceros_contratos_generar_fv/grid_terceros_contratos_generar_fv_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
   ...
   {
      global $nm_saida;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['embutida'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['dashboard_info']['compact_mode'])
      { 
          return;
      } 
   ...
   function adiciona_registro($sum_mensualidad, $quebra_cliente, $quebra_fecha_contrato, $quebra_fecha_inicio, $quebra_estado, $quebra_zona, $quebra_barrio, $quebra_precinto, $quebra_cliente_orig, $quebra_fecha_contrato_orig, $quebra_fecha_inicio_orig, $quebra_estado_orig, $quebra_zona_orig, $quebra_barrio_orig, $quebra_precinto_orig)
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
      {
          $Name_orig  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['SC_Gb_Free_orig'][$cmp_gb])) ? $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['SC_Gb_Free_orig'][$cmp_gb] : $cmp_gb;
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->barrio2 = $rs_res->fields[4] ;  
            $this->estado = $rs_res->fields[5] ;  
            $this->mensualidad = $rs_res->fields[6] ;  
            $this->mensualidad =  str_replace(",", ".", $this->mensualidad);
            $this->fecha_factura = $rs_res->fields[7] ;  
            $this->id_ter_cont = $rs_res->fields[8] ;  
            $this->fecha_contrato = $rs_res->fields[9] ;  
            $this->fecha_corte = $rs_res->fields[10] ;  
            $this->activo = $rs_res->fields[11] ;  
            $this->barrio = $rs_res->fields[12] ;  
            $this->direccion = $rs_res->fields[13] ;  
            $this->telefono = $rs_res->fields[14] ;  
            $this->motivo = $rs_res->fields[15] ;  
            $this->fecha_limitepago = $rs_res->fields[16] ;  
            $this->fecha_ultimopago = $rs_res->fields[17] ;  
            $this->valorpagado = $rs_res->fields[18] ;  
            $this->valorpagado =  str_replace(",", ".", $this->valorpagado);
            $this->saldoanterior = $rs_res->fields[19] ;  
            $this->saldoanterior =  str_replace(",", ".", $this->saldoanterior);
            $this->saldoactual = $rs_res->fields[20] ;  
            $this->saldoactual =  str_replace(",", ".", $this->saldoactual);
            $this->mesultimafactura = $rs_res->fields[21] ;  
            $this->observaciones = $rs_res->fields[22] ;  
            $this->valor_ultimafactura = $rs_res->fields[23] ;  
            $this->valor_ultimafactura =  str_replace(",", ".", $this->valor_ultimafactura);
            $this->precinto = $rs_res->fields[24] ;  
            $this->correo = $rs_res->fields[25] ;  
            $this->anio = $rs_res->fields[26] ;  
            $this->periodo = $rs_res->fields[27] ;  
