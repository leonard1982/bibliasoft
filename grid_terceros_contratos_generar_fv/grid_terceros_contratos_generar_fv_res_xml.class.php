...
class grid_terceros_contratos_generar_fv_res_xml
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function monta_xml()
   {
      ...
      $this->grava_arquivo();
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['xml_res_grid']))
      {
          return;
      }
   ...
   function inicializa_vars()
   {
      ...
      $this->Res       = new grid_terceros_contratos_generar_fv_resumo("out");
      $this->prep_modulos("Res");
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['xml_res_grid']) && !$this->Ini->sc_export_ajax) {
   ...
   function grava_arquivo()
   {
      ...
      if ($this->New_Format)
      {
          fwrite($xml_f, "<grid_terceros_contratos_generar_fv>\r\n");
      }
      if ($this->Grava_view)
      {
          ...
          if ($this->New_Format)
          {
              fwrite($xml_v, "<grid_terceros_contratos_generar_fv>\r\n");
          }
      }
      ...
      $this->array_titulos = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_export']['label'];
      $this->array_linhas  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['arr_export']['data'];
      $contr_rowspan = array();
      $tit_rowspan   = array();
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['xml_res_grid']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['xml_res_file']['xml'] = $this->Xml_f;
          if ($this->Grava_view)
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_contratos_generar_fv']['xml_res_file']['view'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo_view;
          }
      }
   ...
   function grava_linha($xml_f)
   {
      ...
      foreach ($this->array_linhas as $lines)
      {
          ...
          if (!$this->New_Format)
          {
              $xml_registro = "<grid_terceros_contratos_generar_fv";
          }
          foreach ($lines as $columns)
          {
              if (0 <= $columns['level'])
