...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_pagar_pedido_CW_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_pagar_pedido_CW_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_pagarjs() 
{ 
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_pagarjs(); }, 500);
      return;
    }
    var vidpedido   = document.F1.idpedido.value;
	vidpedido   = fLimpiarNumero(vidpedido);
var vadicional2 = document.F1.adicional2.value;
	vadicional2 = fLimpiarNumero(vadicional2);
var vformapago  = document.F1.formapago.value;
var vadicional3 = document.F1.adicional3.value;
	vadicional3 = fLimpiarNumero(vadicional3);
var vobspago    = document.F1.obspago.value;
	
fPagarPed(vidpedido,vadicional2,vformapago,vadicional3,vobspago);
} 
function sc_btn_pagarjs_cancel()
{
}
function sc_btn_pagarjs_ok()
{
    var vidpedido   = document.F1.idpedido.value;
	vidpedido   = fLimpiarNumero(vidpedido);
var vadicional2 = document.F1.adicional2.value;
...
{
  nm_gp_submit('<?php echo $this->Ini->link_grid_pedidos_CW_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo ""; ?>', 'inicio', '_self', '0', '0', 'grid_pedidos_CW');
}
var Crtl_btn_pagar = false;
function sc_btn_pagar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_pagar(); }, 500);
      return;
    }
    if (Crtl_btn_pagar) {return;}
    scJs_confirm("<?php echo html_entity_decode('Desea continuar?..', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_pagar_ok, sc_btn_pagar_cancel)
}
function sc_btn_pagar_cancel()
{
}
function sc_btn_pagar_ok()
{
    Crtl_btn_pagar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?pagar?@?";
    document.F1.action = "form_pagar_pedido_CW_mob.php";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_pagar_pedido_CW_mob']['where_detal']))
  {
?>  
...
{
   nm_field_disabled("fechaven=disabled;numpedido=disabled;adicional=disabled;total=disabled;idcli=disabled", "U");
