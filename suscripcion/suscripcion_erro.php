...
$old_error_handler = set_error_handler("suscripcion_scriptcase_error_handler");

function suscripcion_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['suscripcion']['contr_erro']) && $_SESSION['scriptcase']['suscripcion']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_suscripcion->contr_suscripcion->Apl_com_erro = true;
            if ($inicial_suscripcion->contr_suscripcion->NM_ajax_flag)
            {
                $inicial_suscripcion->contr_suscripcion->NM_ajax_info['errList']['geral_suscripcion'][] = $msg;
                return;
            }
            suscripcion_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function suscripcion_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['suscripcion'])  && '' != $_SESSION['scriptcase']['error_icon']['suscripcion'])  ? $_SESSION['scriptcase']['error_icon']['suscripcion']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['suscripcion']) && '' != $_SESSION['scriptcase']['error_close']['suscripcion']) ? $_SESSION['scriptcase']['error_close']['suscripcion'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['suscripcion']) && '' != $_SESSION['scriptcase']['error_span_title']['suscripcion']) ? $_SESSION['scriptcase']['error_span_title']['suscripcion'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['suscripcion']) && '' != $_SESSION['scriptcase']['error_icon_title']['suscripcion']) ? $_SESSION['scriptcase']['error_icon_title']['suscripcion'] : '';
?>
