...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['suscripcion']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="suscripcion"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_btn_pagar() 
{ 
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_pagar(); }, 500);
      return;
    }
    scJs_confirm("<?php echo html_entity_decode('¿Desea realizar el pago de su suscripción?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_btn_pagar_ok, sc_btn_btn_pagar_cancel);
} 
function sc_btn_btn_pagar_cancel()
{
}
function sc_btn_btn_pagar_ok()
{
    fPagar();
}
...
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_salir(); }, 500);
      return;
    }
    sc_btn_btn_salir_ok();
}
function sc_btn_btn_salir_cancel()
{
}
function sc_btn_btn_salir_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_fin_sesion; ?>', '<?php echo $this->nm_location; ?>', '<?php echo ""; ?>', '', '_self', '0', '0', 'blank_fin_sesion');
}
var Crtl_btn_btn_validar_pago = false;
function sc_btn_btn_validar_pago()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_validar_pago(); }, 500);
      return;
    }
    if (Crtl_btn_btn_validar_pago) {return;}
    sc_btn_btn_validar_pago_ok();
}
function sc_btn_btn_validar_pago_cancel()
{
}
function sc_btn_btn_validar_pago_ok()
{
    Crtl_btn_btn_validar_pago = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?btn_validar_pago?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
function sc_btn_btn_continuar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_continuar(); }, 500);
      return;
    }
    sc_btn_btn_continuar_ok();
}
function sc_btn_btn_continuar_cancel()
{
}
function sc_btn_btn_continuar_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_menu; ?>', '<?php echo $this->nm_location; ?>', '<?php echo ""; ?>', '', '_self', '0', '0', 'menu');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['suscripcion']['where_detal']))
  {
?>  
...
{
  if (scFormHasChanged) {
    scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', function() { document.F4.target = "_self"; document.F4.action = "suscripcion_fim.php"; document.F4.submit(); }, function() {});
  } else {
    document.F4.target = "_self";
    document.F4.action = "suscripcion_fim.php";
    document.F4.submit();
  }
}
