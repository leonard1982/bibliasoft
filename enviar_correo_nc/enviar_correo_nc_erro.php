...
$old_error_handler = set_error_handler("enviar_correo_nc_scriptcase_error_handler");

function enviar_correo_nc_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['enviar_correo_nc']['contr_erro']) && $_SESSION['scriptcase']['enviar_correo_nc']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_enviar_correo_nc->contr_enviar_correo_nc->Apl_com_erro = true;
            if ($inicial_enviar_correo_nc->contr_enviar_correo_nc->NM_ajax_flag)
            {
                $inicial_enviar_correo_nc->contr_enviar_correo_nc->NM_ajax_info['errList']['geral_enviar_correo_nc'][] = $msg;
                return;
            }
            enviar_correo_nc_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function enviar_correo_nc_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['enviar_correo_nc'])  && '' != $_SESSION['scriptcase']['error_icon']['enviar_correo_nc'])  ? $_SESSION['scriptcase']['error_icon']['enviar_correo_nc']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['enviar_correo_nc']) && '' != $_SESSION['scriptcase']['error_close']['enviar_correo_nc']) ? $_SESSION['scriptcase']['error_close']['enviar_correo_nc'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['enviar_correo_nc']) && '' != $_SESSION['scriptcase']['error_span_title']['enviar_correo_nc']) ? $_SESSION['scriptcase']['error_span_title']['enviar_correo_nc'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['enviar_correo_nc']) && '' != $_SESSION['scriptcase']['error_icon_title']['enviar_correo_nc']) ? $_SESSION['scriptcase']['error_icon_title']['enviar_correo_nc'] : '';
?>
