...
$old_error_handler = set_error_handler("enviar_correo_nc_mob_scriptcase_error_handler");

function enviar_correo_nc_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['enviar_correo_nc_mob']['contr_erro']) && $_SESSION['scriptcase']['enviar_correo_nc_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_enviar_correo_nc_mob->contr_enviar_correo_nc_mob->Apl_com_erro = true;
            if ($inicial_enviar_correo_nc_mob->contr_enviar_correo_nc_mob->NM_ajax_flag)
            {
                $inicial_enviar_correo_nc_mob->contr_enviar_correo_nc_mob->NM_ajax_info['errList']['geral_enviar_correo_nc_mob'][] = $msg;
                return;
            }
            enviar_correo_nc_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function enviar_correo_nc_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['enviar_correo_nc_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['enviar_correo_nc_mob'])  ? $_SESSION['scriptcase']['error_icon']['enviar_correo_nc_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['enviar_correo_nc_mob']) && '' != $_SESSION['scriptcase']['error_close']['enviar_correo_nc_mob']) ? $_SESSION['scriptcase']['error_close']['enviar_correo_nc_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['enviar_correo_nc_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['enviar_correo_nc_mob']) ? $_SESSION['scriptcase']['error_span_title']['enviar_correo_nc_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['enviar_correo_nc_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['enviar_correo_nc_mob']) ? $_SESSION['scriptcase']['error_icon_title']['enviar_correo_nc_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['enviar_correo_nc']['error_buffer'] .= ob_get_flush();
}
