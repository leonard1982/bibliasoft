...
$old_error_handler = set_error_handler("detalle_devcompra_scriptcase_error_handler");

function detalle_devcompra_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['detalle_devcompra']['contr_erro']) && $_SESSION['scriptcase']['detalle_devcompra']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_detalle_devcompra->contr_detalle_devcompra->Apl_com_erro = true;
            if ($inicial_detalle_devcompra->contr_detalle_devcompra->NM_ajax_flag)
            {
                $inicial_detalle_devcompra->contr_detalle_devcompra->NM_ajax_info['errList']['geral_detalle_devcompra'][] = $msg;
                return;
            }
            detalle_devcompra_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function detalle_devcompra_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['detalle_devcompra'])  && '' != $_SESSION['scriptcase']['error_icon']['detalle_devcompra'])  ? $_SESSION['scriptcase']['error_icon']['detalle_devcompra']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['detalle_devcompra']) && '' != $_SESSION['scriptcase']['error_close']['detalle_devcompra']) ? $_SESSION['scriptcase']['error_close']['detalle_devcompra'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['detalle_devcompra']) && '' != $_SESSION['scriptcase']['error_span_title']['detalle_devcompra']) ? $_SESSION['scriptcase']['error_span_title']['detalle_devcompra'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['detalle_devcompra']) && '' != $_SESSION['scriptcase']['error_icon_title']['detalle_devcompra']) ? $_SESSION['scriptcase']['error_icon_title']['detalle_devcompra'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['detalle_devcompra']['error_buffer'] .= ob_get_flush();
}
