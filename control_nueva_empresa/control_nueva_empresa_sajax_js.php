  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_control_nueva_empresa" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_control_nueva_empresa_form();
	}
  ...
  function do_ajax_control_nueva_empresa_validate_copiar_desde()
  {
    var nomeCampo_copiar_desde = "copiar_desde";
    var var_copiar_desde = scAjaxGetFieldSelect(nomeCampo_copiar_desde);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_nueva_empresa_validate_copiar_desde(var_copiar_desde, var_script_case_init, do_ajax_control_nueva_empresa_validate_copiar_desde_cb);
  } // do_ajax_control_nueva_empresa_validate_copiar_desde

  function do_ajax_control_nueva_empresa_validate_copiar_desde_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_nueva_empresa_validate_copiar_desde_cb

  // ---------- Validate anio
  function do_ajax_control_nueva_empresa_validate_anio()
  {
    var nomeCampo_anio = "anio";
    var var_anio = scAjaxGetFieldSelect(nomeCampo_anio);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_nueva_empresa_validate_anio(var_anio, var_script_case_init, do_ajax_control_nueva_empresa_validate_anio_cb);
  } // do_ajax_control_nueva_empresa_validate_anio

  function do_ajax_control_nueva_empresa_validate_anio_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "anio";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_nueva_empresa_validate_anio_cb

  // ---------- Validate ruta_bd_tns
  function do_ajax_control_nueva_empresa_validate_ruta_bd_tns()
  {
    var nomeCampo_ruta_bd_tns = "ruta_bd_tns";
    var var_ruta_bd_tns = scAjaxGetFieldText(nomeCampo_ruta_bd_tns);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_nueva_empresa_validate_ruta_bd_tns(var_ruta_bd_tns, var_script_case_init, do_ajax_control_nueva_empresa_validate_ruta_bd_tns_cb);
  } // do_ajax_control_nueva_empresa_validate_ruta_bd_tns

  function do_ajax_control_nueva_empresa_validate_ruta_bd_tns_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_nueva_empresa_validate_ruta_bd_tns_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_control_nueva_empresa_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_control_nueva_empresa_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_control_nueva_empresa_submit_form(var_copiar_desde, var_anio, var_ruta_bd_tns, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_control_nueva_empresa_submit_form_cb);
  } // do_ajax_control_nueva_empresa_submit_form

  function do_ajax_control_nueva_empresa_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_control_nueva_empresa_submit_form_cb_after_alert);
  } // do_ajax_control_nueva_empresa_submit_form_cb
  function do_ajax_control_nueva_empresa_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_control_nueva_empresa_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_control_nueva_empresa_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_control_nueva_empresa_navigate_form, function() {});
    } else {
      perform_ajax_control_nueva_empresa_navigate_form();
    }
  }

  function perform_ajax_control_nueva_empresa_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
  ...
  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['control_nueva_empresa']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_control_nueva_empresa_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_control_nueva_empresa_navigate_form_cb_after_alert);
  } // do_ajax_control_nueva_empresa_navigate_form_cb
  function do_ajax_control_nueva_empresa_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_control_nueva_empresa_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_control_nueva_empresa_navigate_form_cb_after_alert
  function sc_hide_control_nueva_empresa_form()
  {
