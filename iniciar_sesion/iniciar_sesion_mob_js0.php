...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['iniciar_sesion_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="iniciar_sesion_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?>
    ...
    {
        do_ajax_iniciar_sesion_mob_navigate_form();
    }
} 
...
function scForm_submit_single(x) {
	if (x != "excluir")
	{
		...
		if ("incluir" == x || "muda_form" == x || "recarga" == x || "recarga_mobile" == x) {
			...
			Nm_Proc_Atualiz = true;
			do_ajax_iniciar_sesion_mob_submit_form();
		}
	}
	if (Nm_submit_ok) {
		Nm_Proc_Atualiz = true;
...
function nm_menu_link_administrar(valor, linkTarget) 
{ 
   document.F1.bok.value = "OK"; 
   document.F1.administrar.value = valor;
   document.F1.administrar_sc_target_name.value = linkTarget;
...
function nm_saida_glo()
{
  ...
  document.F4.action = "iniciar_sesion_mob_fim.php";
  document.F4.submit();
}
...
{
   nm_field_disabled("licencia=disabled", "");
