...
$old_error_handler = set_error_handler("iniciar_sesion_scriptcase_error_handler");

function iniciar_sesion_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['iniciar_sesion']['contr_erro']) && $_SESSION['scriptcase']['iniciar_sesion']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_iniciar_sesion->contr_iniciar_sesion->Apl_com_erro = true;
            if ($inicial_iniciar_sesion->contr_iniciar_sesion->NM_ajax_flag)
            {
                $inicial_iniciar_sesion->contr_iniciar_sesion->NM_ajax_info['errList']['geral_iniciar_sesion'][] = $msg;
                return;
            }
            iniciar_sesion_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function iniciar_sesion_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['iniciar_sesion'])  && '' != $_SESSION['scriptcase']['error_icon']['iniciar_sesion'])  ? $_SESSION['scriptcase']['error_icon']['iniciar_sesion']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['iniciar_sesion']) && '' != $_SESSION['scriptcase']['error_close']['iniciar_sesion']) ? $_SESSION['scriptcase']['error_close']['iniciar_sesion'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['iniciar_sesion']) && '' != $_SESSION['scriptcase']['error_span_title']['iniciar_sesion']) ? $_SESSION['scriptcase']['error_span_title']['iniciar_sesion'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['iniciar_sesion']) && '' != $_SESSION['scriptcase']['error_icon_title']['iniciar_sesion']) ? $_SESSION['scriptcase']['error_icon_title']['iniciar_sesion'] : '';
?>
