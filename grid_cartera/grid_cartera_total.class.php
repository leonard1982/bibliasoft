...
class grid_cartera_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_cartera']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_cartera']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_fechavencimiento($res_limit=false)
   {
      global $nada, $nm_lang , $idcli, $resolucion;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(saldo) from (SELECT      idfacven,     numfacven,     nremision,     credito,     fechaven,     fechavenc,     idcli,     subtotal,     valoriva,     total,     pagada,     asentada,     observaciones,     saldo,     adicional,     adicional2,     adicional3,     resolucion,    tipo,    concat((select r.prefijo from resdian r where r.Idres=f.resolucion limit 1),'/',f.numfacven) as documento FROM      facturaven f WHERE     numfacven!=0  AND  saldo>0 and tipo in ('FV','RS') and credito='1' ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(saldo) from (SELECT      idfacven,     numfacven,     nremision,     credito,     fechaven,     fechavenc,     idcli,     subtotal,     valoriva,     total,     pagada,     asentada,     observaciones,     saldo,     adicional,     adicional2,     adicional3,     resolucion,    tipo,    concat((select r.prefijo from resdian r where r.Idres=f.resolucion limit 1),'/',f.numfacven) as documento FROM      facturaven f WHERE     numfacven!=0  AND  saldo>0 and tipo in ('FV','RS') and credito='1' ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['contr_total_geral'] = "OK";
   } 
   ...
   //-----  fechavenc
   function quebra_fechavenc_fechavencimiento($fechavenc, $arg_sum_fechavenc) 
   {
      global $tot_fechavenc, $idcli, $resolucion;
      $tot_fechavenc = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(saldo) from (SELECT      idfacven,     numfacven,     nremision,     credito,     fechaven,     fechavenc,     idcli,     subtotal,     valoriva,     total,     pagada,     asentada,     observaciones,     saldo,     adicional,     adicional2,     adicional3,     resolucion,    tipo,    concat((select r.prefijo from resdian r where r.Idres=f.resolucion limit 1),'/',f.numfacven) as documento FROM      facturaven f WHERE     numfacven!=0  AND  saldo>0 and tipo in ('FV','RS') and credito='1' ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['where_pesq']; 
   ...
   {
      global $tot_idcli, $idcli, $resolucion;
      $tot_idcli = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(saldo) from (SELECT      idfacven,     numfacven,     nremision,     credito,     fechaven,     fechavenc,     idcli,     subtotal,     valoriva,     total,     pagada,     asentada,     observaciones,     saldo,     adicional,     adicional2,     adicional3,     resolucion,    tipo,    concat((select r.prefijo from resdian r where r.Idres=f.resolucion limit 1),'/',f.numfacven) as documento FROM      facturaven f WHERE     numfacven!=0  AND  saldo>0 and tipo in ('FV','RS') and credito='1' ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['where_pesq']; 
   ...
   //----- 
   function Calc_resumo_fechavencimiento($destino_resumo)
   {
      global $nm_lang, $prefijo, $idcli, $resolucion;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['SC_Ind_Groupby'];
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_fechavencimiento($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_fechavencimiento($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('idfacven' => 'N','numfacven' => 'N','nremision' => 'N','credito' => 'N','idcli' => 'N','subtotal' => 'N','valoriva' => 'N','total' => 'N','asentada' => 'N','saldo' => 'N','adicional' => 'N','adicional2' => 'N','adicional3' => 'N','resolucion' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(saldo)#@#cmps_quebras#@# from (SELECT      idfacven,     numfacven,     nremision,     credito,     fechaven,     fechavenc,     idcli,     subtotal,     valoriva,     total,     pagada,     asentada,     observaciones,     saldo,     adicional,     adicional2,     adicional3,     resolucion,    tipo,    concat((select r.prefijo from resdian r where r.Idres=f.resolucion limit 1),'/',f.numfacven) as documento FROM      facturaven f WHERE     numfacven!=0  AND  saldo>0 and tipo in ('FV','RS') and credito='1' ) sc_sel_esp " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select saldo as SC_metric1, " . $cmps_quebras1 . " from (SELECT      idfacven,     numfacven,     nremision,     credito,     fechaven,     fechavenc,     idcli,     subtotal,     valoriva,     total,     pagada,     asentada,     observaciones,     saldo,     adicional,     adicional2,     adicional3,     resolucion,    tipo,    concat((select r.prefijo from resdian r where r.Idres=f.resolucion limit 1),'/',f.numfacven) as documento FROM      facturaven f WHERE     numfacven!=0  AND  saldo>0 and tipo in ('FV','RS') and credito='1' ) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT      idfacven,     numfacven,     nremision,     credito,     fechaven,     fechavenc,     idcli,     subtotal,     valoriva,     total,     pagada,     asentada,     observaciones,     saldo,     adicional,     adicional2,     adicional3,     resolucion,    tipo,    concat((select r.prefijo from resdian r where r.Idres=f.resolucion limit 1),'/',f.numfacven) as documento FROM      facturaven f WHERE     numfacven!=0  AND  saldo>0 and tipo in ('FV','RS') and credito='1' ) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cartera']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'fechavencimiento', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_fechavencimiento($where_ok);
