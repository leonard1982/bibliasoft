...
<input type="hidden" name="iddev_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['iddev_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="detalleventa_dev"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['detalleventa_dev']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['detalleventa_dev']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['detalleventa_dev']['dashboard_info']['parent_widget'] . "']\")";
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_detalleventa_dev_submit_form(y, z);
...
{
   nm_field_disabled("idpro_=disabled;colores_=disabled;tallas_=disabled;sabor_=disabled;idbod_=disabled;unidadmayor_=disabled;valorunit_=disabled", "");
   nm_field_disabled("cantidad_=disabled", "I");
