...
$old_error_handler = set_error_handler("devcompra_scriptcase_error_handler");

function devcompra_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['devcompra']['contr_erro']) && $_SESSION['scriptcase']['devcompra']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            global $inicial_devcompra;
            $inicial_devcompra->contr_devcompra->Apl_com_erro = true;
            if ($inicial_devcompra->contr_devcompra->NM_ajax_flag)
            {
                $inicial_devcompra->contr_devcompra->NM_ajax_info['errList']['geral_devcompra'][] = $msg;
                return;
            }
            devcompra_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function devcompra_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['devcompra'])  && '' != $_SESSION['scriptcase']['error_icon']['devcompra'])  ? $_SESSION['scriptcase']['error_icon']['devcompra']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['devcompra']) && '' != $_SESSION['scriptcase']['error_close']['devcompra']) ? $_SESSION['scriptcase']['error_close']['devcompra'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['devcompra']) && '' != $_SESSION['scriptcase']['error_span_title']['devcompra']) ? $_SESSION['scriptcase']['error_span_title']['devcompra'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['devcompra']) && '' != $_SESSION['scriptcase']['error_icon_title']['devcompra']) ? $_SESSION['scriptcase']['error_icon_title']['devcompra'] : '';
?>
