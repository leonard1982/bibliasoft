...
$old_error_handler = set_error_handler("terceros_scriptcase_error_handler");

function terceros_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['terceros']['contr_erro']) && $_SESSION['scriptcase']['terceros']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_terceros->contr_terceros->Apl_com_erro = true;
            if ($inicial_terceros->contr_terceros->NM_ajax_flag)
            {
                $inicial_terceros->contr_terceros->NM_ajax_info['errList']['geral_terceros'][] = $msg;
                return;
            }
            terceros_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function terceros_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['terceros'])  && '' != $_SESSION['scriptcase']['error_icon']['terceros'])  ? $_SESSION['scriptcase']['error_icon']['terceros']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['terceros']) && '' != $_SESSION['scriptcase']['error_close']['terceros']) ? $_SESSION['scriptcase']['error_close']['terceros'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['terceros']) && '' != $_SESSION['scriptcase']['error_span_title']['terceros']) ? $_SESSION['scriptcase']['error_span_title']['terceros'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['terceros']) && '' != $_SESSION['scriptcase']['error_icon_title']['terceros']) ? $_SESSION['scriptcase']['error_icon_title']['terceros'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['terceros']['error_buffer'] .= ob_get_flush();
}
