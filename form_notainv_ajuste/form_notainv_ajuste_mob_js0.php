<form name="F2" method=post 
               ...
               target="_self"> 
<input type="hidden" name="idmov" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idmov']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_notainv_ajuste_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_notainv_ajuste_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_imprimir_par_numero = "<?php echo $this->form_encode_input($this->nmgp_dados_form['idmov']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_pdfreport_movimiento_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "par_numero*scin' + var_btn_imprimir_par_numero + '*scout"; ?>', 'inicio', '_blank', '0', '0', 'pdfreport_movimiento');
}
var Crtl_btn_ejecuta = false;
function sc_btn_ejecuta()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_ejecuta(); }, 500);
      return;
    }
    if (Crtl_btn_ejecuta) {return;}
    scJs_confirm("<?php echo html_entity_decode('¿Está seguro?, proceso no se puede deshacer', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_ejecuta_ok, sc_btn_ejecuta_cancel)
}
function sc_btn_ejecuta_cancel()
{
}
function sc_btn_ejecuta_ok()
{
    Crtl_btn_ejecuta = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?ejecuta?@?";
    document.F1.action = "form_notainv_ajuste_mob.php";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_notainv_ajuste_mob']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_form_notainv_ajuste_mob_navigate_form();
    }
} 
...
?>
    ...
    { 
       ...
       {
           Nm_Proc_Atualiz = true;
           do_ajax_form_notainv_ajuste_mob_submit_form();
       }
    } 
    if (Nm_submit_ok)
    { 
