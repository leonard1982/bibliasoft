<?php
class pdfreport_remision_lookup
{
//  
...
//  
   function lookup_docu_pedido(&$conteudo , $pedido, &$nm_array_retorno_lookup) 
   {
      $nm_array_retorno_lookup = array();
      if (trim($pedido) === "")
      { 
      ...
      $conteudo = "";
      $nm_comando = "SELECT  res.prefijo 
FROM pedidos ped
left join resdian res on ped.prefijo_ped=res.Idres
WHERE ped.idpedido = $pedido 
ORDER BY prefijo_ped" ; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rx = $this->Db->Execute($nm_comando)) 
      { 
...
//  
   function lookup_num_pedido(&$conteudo , $pedido, &$nm_array_retorno_lookup) 
   {
      $nm_array_retorno_lookup = array();
      if (trim($pedido) === "")
      { 
