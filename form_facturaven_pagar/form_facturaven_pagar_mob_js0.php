<form name="F2" method=post 
               action="form_facturaven_pagar_mob.php" 
               target="_self"> 
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_facturaven_pagar_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_facturaven_pagar_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_Imprimir_par_numfacventa = "<?php echo $this->form_encode_input($this->nmgp_dados_form['numfacven']); ?>";
function sc_btn_Imprimir()
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_blank_impresionPos; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "par_numfacventa?#?' + var_btn_Imprimir_par_numfacventa + '?@?"; ?>', '', '_blank', '0', '0', 'blank_impresionPos');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_facturaven_pagar']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_form_facturaven_pagar_mob_navigate_form();
    }
} 
...
?>
    ...
    { 
       ...
       {
           Nm_Proc_Atualiz = true;
           do_ajax_form_facturaven_pagar_mob_submit_form();
       }
    } 
    if (Nm_submit_ok)
    { 
...
{
   nm_field_disabled("numfacven=disabled;fechaven=disabled;idcli=disabled;total=disabled;adicional=disabled", "U");
