<form name="F2" method=post 
               ...
               target="_self"> 
<input type="hidden" name="iddatos" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['iddatos']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_datosemp_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_datosemp_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
  nm_gp_submit('<?php echo $this->Ini->link_grid_codigo_postal_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "NMSC_inicial*scininicio*scout"; ?>', 'inicio', '_blank', '0', '0', 'grid_codigo_postal');
}
function sc_btn_btn_recargar() 
{ 
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_recargar(); }, 500);
      return;
    }
    location.reload();
} 
function sc_btn_btn_recargar_cancel()
{
}
function sc_btn_btn_recargar_ok()
{
    location.reload();
}
 NM_tp_critica(1);
...
{ 
   var sob_iframe = '';
<?php 
if ($_SESSION['sc_session'][$this->Ini->sc_page]['form_datosemp_mob']['embutida_form']) {
...
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_datosemp_mob']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_datosemp_mob']['sc_modal']) {
    echo "   sob_iframe += 'parent.';\r\n";
}
?>
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_datosemp_mob']['where_detal']))
  {
?>  
...
?>
    ...
    if (x == "excluir") 
    { 
       if (confirm ("SEGURO, DATOS NO SE PUEDEN RECUPERAR"))  
       { 
           scAjaxProcOn();
...
function scForm_confirmDelete(callbackOk, callbackCancel) {
	scJs_confirm("SEGURO, DATOS NO SE PUEDEN RECUPERAR", callbackOk, callbackCancel);
...
function nm_mostra_doc(campo1, campo2, campo3)
{
    ...
    NovaJanela = window.open ("form_datosemp_mob_doc.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nm_cod_doc=" + campo1 + "&nm_nome_doc=" + encodeURIComponent(campo2) + "&nm_cod_apl=" + campo3, "sc_name_download_iframe", "resizable, scrollbars");
}
