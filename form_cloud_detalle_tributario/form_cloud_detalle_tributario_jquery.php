...
function scEventControl_init(iSeqRow) {
  ...
  scEventControl_data["descripcion_dt_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}
...
function scEventControl_active(iSeqRow) {
  ...
  if (scEventControl_data["descripcion_dt_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["descripcion_dt_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_detalle_trib_' + iSeqRow).bind('change', function() { sc_form_cloud_detalle_tributario_id_detalle_trib__onchange(this, iSeqRow) });
  $('#id_sc_field_codigo_' + iSeqRow).bind('blur', function() { sc_form_cloud_detalle_tributario_codigo__onblur(this, iSeqRow) })
                                     .bind('change', function() { sc_form_cloud_detalle_tributario_codigo__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_form_cloud_detalle_tributario_codigo__onfocus(this, iSeqRow) });
  $('#id_sc_field_descripcion_dt_' + iSeqRow).bind('blur', function() { sc_form_cloud_detalle_tributario_descripcion_dt__onblur(this, iSeqRow) })
                                             .bind('change', function() { sc_form_cloud_detalle_tributario_descripcion_dt__onchange(this, iSeqRow) })
                                             .bind('focus', function() { sc_form_cloud_detalle_tributario_descripcion_dt__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_cloud_detalle_tributario_id_detalle_trib__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}

function sc_form_cloud_detalle_tributario_codigo__onblur(oThis, iSeqRow) {
  do_ajax_form_cloud_detalle_tributario_validate_codigo_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_cloud_detalle_tributario_codigo__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_cloud_detalle_tributario_codigo__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_cloud_detalle_tributario_descripcion_dt__onblur(oThis, iSeqRow) {
  do_ajax_form_cloud_detalle_tributario_validate_descripcion_dt_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_cloud_detalle_tributario_descripcion_dt__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_cloud_detalle_tributario_descripcion_dt__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	...
	displayChange_field("descripcion_dt_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_codigo_(row, status);
	displayChange_field_descripcion_dt_(row, status);
}
...
function displayChange_field_descripcion_dt_(row, status) {
}
...
function scHidePage(pageNo) {
	$("#id_form_cloud_detalle_tributario_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_cloud_detalle_tributario')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
