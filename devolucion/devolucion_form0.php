<?php
class devolucion_form extends devolucion_apl
{
function Form_Init()
{
...
<HEAD>
 <TITLE><?php if ('novo' == $this->nmgp_opcao) { echo strip_tags("Devoluciones en Compra"); } else { echo strip_tags("Devoluciones en Compra"); } ?></TITLE>
...
<?php
if ($this->Embutida_form && isset($_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['sc_modal'] && isset($_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['sc_redir_atualiz']) && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['sc_redir_atualiz'] == 'ok')
{
?>
...
<script type="text/javascript" src="<?php echo $this->Ini->path_prod ?>/third/jquery_plugin/select2/js/select2.full.min.js"></script>
 ...
 <?php
 if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['embutida_pdf']))
 {
 ?>
...
?>
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>devolucion/devolucion_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
...
function navpage_atualiza(str_navpage)
{
 ...
 function nm_field_disabled(Fields, Opt, Seq, Opcao) {
  ...
  for (i=0; i < Field.length; i++)
  {
     ...
     if (F_name == "descuento_")
     {
        for (ii=ini; ii < xx; ii++)
        {
            cmp_name = "descuento_" + ii;
            $('input[name=' + cmp_name + ']').prop("disabled", F_opc);
            if (F_opc == "disabled" || F_opc == true) {
                $('input[name=' + cmp_name + ']').addClass("scFormInputDisabledMult");
            }
 ...
 function nm_field_disabled_reset() {
  for (var i = 0; i < iAjaxNewLine; i++) {
    ...
    nm_field_disabled("descuento_=enabled", "", i);
    nm_field_disabled("valorpar_=enabled", "", i);
    nm_field_disabled("iva_=enabled", "", i);
  }
 } // nm_field_disabled_reset
<?php

include_once('devolucion_jquery.php');
...
?>
 ...
 function addAutocomplete(elem) {
  ...
  $(".sc-ui-autocomp-idpro_", elem).on("focus", function() {
   ...
   var sId = $(this).attr("id").substr(6), sRow = "idpro_" != sId ? sId.substr(6) : "";
   sc_devolucion_idpro__onfocus("id_sc_field_" + sId, sRow);
  ...
  $(".sc-ui-autocomp-idbod_", elem).on("focus", function() {
   ...
   var sId = $(this).attr("id").substr(6), sRow = "idbod_" != sId ? sId.substr(6) : "";
   sc_devolucion_idbod__onfocus("id_sc_field_" + sId, sRow);
  }).on("select2:close", function(e) {
   var sId = $(this).attr("id").substr(6);
   $("#id_sc_field_" + sId).trigger("blur");
  }).on("select2:select", function(e) {
  ...
  $(".sc-ui-autocomp-colores_", elem).on("focus", function() {
   ...
   var sId = $(this).attr("id").substr(6), sRow = "colores_" != sId ? sId.substr(8) : "";
   sc_devolucion_colores__onfocus("id_sc_field_" + sId, sRow);
  }).on("select2:close", function(e) {
   var sId = $(this).attr("id").substr(6);
   $("#id_sc_field_" + sId).trigger("blur");
  }).on("select2:select", function(e) {
  ...
  $(".sc-ui-autocomp-tallas_", elem).on("focus", function() {
   ...
   var sId = $(this).attr("id").substr(6), sRow = "tallas_" != sId ? sId.substr(7) : "";
   sc_devolucion_tallas__onfocus("id_sc_field_" + sId, sRow);
  }).on("select2:close", function(e) {
   var sId = $(this).attr("id").substr(6);
   $("#id_sc_field_" + sId).trigger("blur");
  }).on("select2:select", function(e) {
  ...
  $(".sc-ui-autocomp-sabor_", elem).on("focus", function() {
   ...
   var sId = $(this).attr("id").substr(6), sRow = "sabor_" != sId ? sId.substr(6) : "";
   sc_devolucion_sabor__onfocus("id_sc_field_" + sId, sRow);
  }).on("select2:close", function(e) {
   var sId = $(this).attr("id").substr(6);
   $("#id_sc_field_" + sId).trigger("blur");
  }).on("select2:select", function(e) {
...
<?php
$str_iframe_body = ('F' == $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['run_iframe'] || 'R' == $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['run_iframe']) ? 'margin: 2px;' : '';
...
if ($GLOBALS["erro_incl"] == 1)
{
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['opc_ant'] = "novo";
    $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['recarga'] = "novo";
}
if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['recarga']))
{
    $opcao_botoes = $this->nmgp_opcao;
}
else
{
    $opcao_botoes = $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['recarga'];
}
if ('novo' == $opcao_botoes && $this->Embutida_form)
{
    $opcao_botoes = 'inicio';
}
    $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['dashboard_info']['remove_margin'] ? 'margin: 0; ' : '';
    $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['dashboard_info']['remove_border'] ? 'border-width: 0; ' : '';
    $vertical_center = '';
?>
...
<?php
 include_once("devolucion_js0.php");
?>
<script type="text/javascript"> 
...
<?php
$_SESSION['scriptcase']['error_span_title']['devolucion'] = $this->Ini->Error_icon_span;
$_SESSION['scriptcase']['error_icon_title']['devolucion'] = '' != $this->Ini->Err_ico_title ? $this->Ini->path_icones . '/' . $this->Ini->Err_ico_title : '';
?>
...
<?php
  if (!$this->Embutida_call && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['mostra_cab']) || $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['mostra_cab'] != "N") && (!$_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['dashboard_info']['under_dashboard'] || !$_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['dashboard_info']['compact_mode'] || $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['dashboard_info']['maximized']))
  {
?>
<tr><td>
<style>
...
<div style="width: 100%">
 ...
 <div style="height:37px; border-width:0px 0px 1px 0px;  border-style: dashed; border-color:#ddd; display: block">
 	<table style="width:100%; border-collapse:collapse; padding:0;">
    	<tr>
        	<td id="lin1_col1" class="scFormHeaderFont"><span><?php if ($this->nmgp_opcao == "novo") { echo "Devoluciones en Compra"; } else { echo "Devoluciones en Compra"; } ?></span></td>
...
<tr><td>
<?php
if (($this->Embutida_form || !$this->Embutida_call || $this->Grid_editavel || $this->Embutida_multi || ($this->Embutida_call && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['embutida_liga_form_btn_nav'])) && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['run_iframe'] != "F" && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['run_iframe'] != "R")
{
?>
 ...
 ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['descuento_']) && $this->nmgp_cmp_hidden['descuento_'] == 'off') { $sStyleHidden_descuento_ = 'display: none'; }
      ...
      if (!isset($this->nm_new_label['descuento_'])) {
          $this->nm_new_label['descuento_'] = "Desc X Und"; } ?>
...
{ 
   ...
   { 
       ob_start();
       $iStart = sizeof($this->form_vert_devolucion);
       $guarda_nmgp_opcao = $this->nmgp_opcao;
       $guarda_form_vert_devolucion = $this->form_vert_devolucion;
       $this->nmgp_opcao = 'novo';
   } 
   if ($this->Embutida_form && empty($this->form_vert_devolucion))
   {
       $sc_seq_vert = 0;
   }
   foreach ($this->form_vert_devolucion as $sc_seq_vert => $sc_lixo)
   {
       $this->loadRecordState($sc_seq_vert);
       $this->iddet_ = $this->form_vert_devolucion[$sc_seq_vert]['iddet_'];
       $this->idfaccom_ = $this->form_vert_devolucion[$sc_seq_vert]['idfaccom_'];
       ...
       elseif ($Line_Add)
       {
        ...
        $this->cantidad_ = $this->form_vert_devolucion[$sc_seq_vert]['cantidad_']; 
       ...
       $this->colores_ = $this->form_vert_devolucion[$sc_seq_vert]['colores_']; 
       ...
       $this->tallas_ = $this->form_vert_devolucion[$sc_seq_vert]['tallas_']; 
       ...
       $this->sabor_ = $this->form_vert_devolucion[$sc_seq_vert]['sabor_']; 
       $sabor_ = $this->sabor_; 
       $sStyleHidden_sabor_ = '';
       ...
       $this->idbod_ = $this->form_vert_devolucion[$sc_seq_vert]['idbod_']; 
       $idbod_ = $this->idbod_; 
       $sStyleHidden_idbod_ = '';
       ...
       $this->valorunit_ = $this->form_vert_devolucion[$sc_seq_vert]['valorunit_']; 
       ...
       $this->descuento_ = $this->form_vert_devolucion[$sc_seq_vert]['descuento_']; 
       ...
       $this->valorpar_ = $this->form_vert_devolucion[$sc_seq_vert]['valorpar_']; 
       ...
       $this->iva_ = $this->form_vert_devolucion[$sc_seq_vert]['iva_']; 
       $iva_ = $this->iva_; 
       $sStyleHidden_iva_ = '';
       ...
       $this->tasaiva_ = $this->form_vert_devolucion[$sc_seq_vert]['tasaiva_']; 
       ...
       $this->tasadesc_ = $this->form_vert_devolucion[$sc_seq_vert]['tasadesc_']; 
       ...
       $this->devuelto_ = $this->form_vert_devolucion[$sc_seq_vert]['devuelto_']; 
...
<?php echo nmButtonOutput($this->arr_buttons, "bmd_novo", "do_ajax_devolucion_add_new_line(" . $sc_seq_vert . ")", "do_ajax_devolucion_add_new_line(" . $sc_seq_vert . ")", "sc_new_line_" . $sc_seq_vert . "", "", "", "display: none", "", "", "", $this->Ini->path_botoes, "", "", "", "", "");?>
<?php }?>
<?php
  $Style_add_line = (!$Line_Add) ? "display: none" : "";
?>
<?php echo nmButtonOutput($this->arr_buttons, "bmd_cancelar", "do_ajax_devolucion_cancel_insert(" . $sc_seq_vert . ")", "do_ajax_devolucion_cancel_insert(" . $sc_seq_vert . ")", "sc_canceli_line_" . $sc_seq_vert . "", "", "", "" . $Style_add_line . "", "", "", "", $this->Ini->path_botoes, "", "", "", "", "");?>
<?php echo nmButtonOutput($this->arr_buttons, "bmd_cancelar", "do_ajax_devolucion_cancel_update(" . $sc_seq_vert . ")", "do_ajax_devolucion_cancel_update(" . $sc_seq_vert . ")", "sc_cancelu_line_" . $sc_seq_vert . "", "", "", "display: none", "", "", "", $this->Ini->path_botoes, "", "", "", "", "");?>
 </TD>
   <?php }?>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOddMult css_colores__line" id="hidden_field_data_colores_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_colores_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_colores__line" style="padding: 0px">
...
<span id="id_read_on_colores_<?php echo $sc_seq_vert ?>" class="sc-ui-readonly-colores_<?php echo $sc_seq_vert ?> css_colores__line" style="<?php echo $sStyleReadLab_colores_; ?>"><?php echo $this->form_format_readonly("colores_", str_replace("<", "&lt;", $colores__look)); ?></span><span id="id_read_off_colores_<?php echo $sc_seq_vert ?>" class="css_read_off_colores_<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_colores_; ?>">
 ...
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=3"; } ?> maxlength=3 style="display: none" alt="{datatype: 'text', maxLength: 3, allowedChars: '<?php echo $this->allowedCharsCharset("") ?>', lettersCase: '', enterTab: false, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddMultWm', maskChars: '(){}[].,;:-+/ '}" >
<?php
$aRecData['colores_'] = $this->colores_;
$aLookup = array();
if ('autocomp_' != substr($this->NM_ajax_opcao, 0, 9))
...
?>
<select id="id_ac_colores_<?php echo $sc_seq_vert; ?>" class="scFormObjectOddMult sc-ui-autocomp-colores_ css_colores__obj"><?php if ('' != $this->colores_) { ?><option value="<?php echo $this->colores_ ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select></span><?php } ?>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOddMult css_tallas__line" id="hidden_field_data_tallas_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_tallas_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_tallas__line" style="padding: 0px">
...
<span id="id_read_on_tallas_<?php echo $sc_seq_vert ?>" class="sc-ui-readonly-tallas_<?php echo $sc_seq_vert ?> css_tallas__line" style="<?php echo $sStyleReadLab_tallas_; ?>"><?php echo $this->form_format_readonly("tallas_", str_replace("<", "&lt;", $tallas__look)); ?></span><span id="id_read_off_tallas_<?php echo $sc_seq_vert ?>" class="css_read_off_tallas_<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_tallas_; ?>">
 ...
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=2"; } ?> maxlength=2 style="display: none" alt="{datatype: 'text', maxLength: 2, allowedChars: '<?php echo $this->allowedCharsCharset("") ?>', lettersCase: '', enterTab: false, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddMultWm', maskChars: '(){}[].,;:-+/ '}" >
<?php
$aRecData['tallas_'] = $this->tallas_;
$aLookup = array();
if ('autocomp_' != substr($this->NM_ajax_opcao, 0, 9))
...
?>
<select id="id_ac_tallas_<?php echo $sc_seq_vert; ?>" class="scFormObjectOddMult sc-ui-autocomp-tallas_ css_tallas__obj"><?php if ('' != $this->tallas_) { ?><option value="<?php echo $this->tallas_ ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select></span><?php } ?>
...
?>
<select id="id_ac_sabor_<?php echo $sc_seq_vert; ?>" class="scFormObjectOddMult sc-ui-autocomp-sabor_ css_sabor__obj"><?php if ('' != $this->sabor_) { ?><option value="<?php echo $this->sabor_ ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select></span><?php } ?>
...
<span id="id_read_on_idbod_<?php echo $sc_seq_vert ?>" class="sc-ui-readonly-idbod_<?php echo $sc_seq_vert ?> css_idbod__line" style="<?php echo $sStyleReadLab_idbod_; ?>"><?php echo $this->form_format_readonly("idbod_", str_replace("<", "&lt;", $idbod__look)); ?></span><span id="id_read_off_idbod_<?php echo $sc_seq_vert ?>" class="css_read_off_idbod_<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_idbod_; ?>">
 ...
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=10"; } ?> style="display: none" alt="{datatype: 'integer', maxLength: 10, thousandsSep: '<?php echo str_replace("'", "\'", $this->field_config['idbod_']['symbol_grp']); ?>', thousandsFormat: <?php echo $this->field_config['idbod_']['symbol_fmt']; ?>, allowNegative: false, onlyNegative: false, negativePos: <?php echo (4 == $this->field_config['idbod_']['format_neg'] ? "'suffix'" : "'prefix'") ?>, alignment: 'left', enterTab: false, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddMultWm', maskChars: '(){}[].,;:-+/ '}" >
<?php
$aRecData['idbod_'] = $this->idbod_;
$aLookup = array();
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
...
?>
<select id="id_ac_idbod_<?php echo $sc_seq_vert; ?>" class="scFormObjectOddMult sc-ui-autocomp-idbod_ css_idbod__obj"><?php if ('' != $unformatted_value_idbod_) { ?><option value="<?php echo $unformatted_value_idbod_ ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select></span><?php } ?>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOddMult css_valorpar__line" id="hidden_field_data_valorpar_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_valorpar_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_valorpar__line" style="vertical-align: top;padding: 0px">
<?php if ($bTestReadOnly_valorpar_ && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["valorpar_"]) &&  $this->nmgp_cmp_readonly["valorpar_"] == "on") { 

 ?>
<input type="hidden" name="valorpar_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($valorpar_) . "\">" . $valorpar_ . ""; ?>
<?php } else { ?>
<span id="id_read_on_valorpar_<?php echo $sc_seq_vert ?>" class="sc-ui-readonly-valorpar_<?php echo $sc_seq_vert ?> css_valorpar__line" style="<?php echo $sStyleReadLab_valorpar_; ?>"><?php echo $this->form_format_readonly("valorpar_", $this->form_encode_input($this->valorpar_)); ?></span><span id="id_read_off_valorpar_<?php echo $sc_seq_vert ?>" class="css_read_off_valorpar_<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_valorpar_; ?>">
 <input class="sc-js-input scFormObjectOddMult css_valorpar__obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_valorpar_<?php echo $sc_seq_vert ?>" type=text name="valorpar_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($valorpar_) ?>"
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=12"; } ?> alt="{datatype: 'currency', currencySymbol: '<?php echo $this->field_config['valorpar_']['symbol_mon']; ?>', currencyPosition: '<?php echo ((1 == $this->field_config['valorpar_']['format_pos'] || 3 == $this->field_config['valorpar_']['format_pos']) ? 'left' : 'right'); ?>', maxLength: 12, precision: 0, decimalSep: '<?php echo str_replace("'", "\'", $this->field_config['valorpar_']['symbol_dec']); ?>', thousandsSep: '<?php echo str_replace("'", "\'", $this->field_config['valorpar_']['symbol_grp']); ?>', thousandsFormat: <?php echo $this->field_config['valorpar_']['symbol_fmt']; ?>, manualDecimals: false, allowNegative: false, onlyNegative: false, negativePos: <?php echo (4 == $this->field_config['valorpar_']['format_neg'] ? "'suffix'" : "'prefix'") ?>, alignment: 'left', enterTab: false, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddMultWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
...
<span id="id_read_on_iva_<?php echo $sc_seq_vert ?>" class="sc-ui-readonly-iva_<?php echo $sc_seq_vert ?> css_iva__line" style="<?php echo $sStyleReadLab_iva_; ?>"><?php echo $this->form_format_readonly("iva_", $this->form_encode_input($this->iva_)); ?></span><span id="id_read_off_iva_<?php echo $sc_seq_vert ?>" class="css_read_off_iva_<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_iva_; ?>">
 ...
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=11"; } ?> alt="{datatype: 'currency', currencySymbol: '<?php echo $this->field_config['iva_']['symbol_mon']; ?>', currencyPosition: '<?php echo ((1 == $this->field_config['iva_']['format_pos'] || 3 == $this->field_config['iva_']['format_pos']) ? 'left' : 'right'); ?>', maxLength: 11, precision: 0, decimalSep: '<?php echo str_replace("'", "\'", $this->field_config['iva_']['symbol_dec']); ?>', thousandsSep: '<?php echo str_replace("'", "\'", $this->field_config['iva_']['symbol_grp']); ?>', thousandsFormat: <?php echo $this->field_config['iva_']['symbol_fmt']; ?>, manualDecimals: false, allowNegative: false, onlyNegative: false, negativePos: <?php echo (4 == $this->field_config['iva_']['format_neg'] ? "'suffix'" : "'prefix'") ?>, alignment: 'left', enterTab: false, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddMultWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
...
<?php   
   ...
   { 
       ...
       $this->form_vert_devolucion = $guarda_form_vert_devolucion;
   } 
   if ($Table_refresh) 
   { 
...
<?php if (('novo' != $this->nmgp_opcao || $this->Embutida_form) && !$this->nmgp_form_empty && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['run_iframe'] != "F") { if ('parcial' == $this->form_paginacao) {?><script>summary_atualiza(<?php echo ($_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['reg_start'] + 1). ", " . $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['reg_qtd'] . ", " . ($_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['total'] + 1)?>);</script><?php }} ?>
<?php if (('novo' != $this->nmgp_opcao || $this->Embutida_form) && !$this->nmgp_form_empty && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['run_iframe'] != "F") { if ('total' == $this->form_paginacao) {?><script>summary_atualiza(1, <?php echo $this->sc_max_reg . ", " . $this->sc_max_reg?>);</script><?php }} ?>
<?php if (('novo' != $this->nmgp_opcao || $this->Embutida_form) && !$this->nmgp_form_empty && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['run_iframe'] != "F") { ?><script>navpage_atualiza('<?php echo $this->SC_nav_page ?>');</script><?php } ?>
</td></tr> 
</table> 
</div> 
</td> 
...
<?php
if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['run_modal']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['run_modal'])
{
?>
...
<script>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['masterValue']))
{
...
<script>
 var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['dashboard_info']['parent_widget']; ?>']");
 dbParentFrame[0].contentWindow.scAjaxDetailStatus("devolucion");
</script>
<?php
    }
    else {
...
?>
<script>
 parent.scAjaxDetailStatus("devolucion");
 parent.scAjaxDetailHeight("devolucion", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
    }
}
...
<?php
$_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['buttonStatus'] = $this->nmgp_botoes;
?>
<script type="text/javascript">
