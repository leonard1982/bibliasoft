...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="devolucion"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['devolucion']['dashboard_info']['parent_widget'] . "']\")";
...
?> 
var Crtl_btn_Ejecutar = false;
function sc_btn_Ejecutar()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_Ejecutar(); }, 500);
      return;
    }
    if (Crtl_btn_Ejecutar) {return;}
    sc_btn_Ejecutar_ok();
}
function sc_btn_Ejecutar_cancel()
{
}
function sc_btn_Ejecutar_ok()
{
    Crtl_btn_Ejecutar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Ejecutar?@?";
    document.F1.action = "./";
...
?>
    ...
    {
        do_ajax_devolucion_navigate_form();
    }
    if ("ordem" == x)
    {
        scSetOrderColumn(y);
...
function scForm_submit_single(x) {
	if (x != "excluir")
	{
		...
		if ("incluir" == x || "muda_form" == x || "recarga" == x || "recarga_mobile" == x) {
			...
			Nm_Proc_Atualiz = true;
			do_ajax_devolucion_submit_form();
		}
	}
	if (Nm_submit_ok) {
		Nm_Proc_Atualiz = true;
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_devolucion_submit_form(y, z);
...
{
   nm_field_disabled("cantidad_=disabled;idpro_=disabled;colores_=disabled;tallas_=disabled;sabor_=disabled;idbod_=disabled;valorunit_=disabled;descuento_=disabled;valorpar_=disabled;iva_=disabled", "U");
