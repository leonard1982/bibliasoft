  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_bodegas" == sTestField)
        {
...
?>
  ...
  // ---------- Validate bodega_
  function do_ajax_form_bodegas_validate_bodega_(iNumLinha)
  {
    var nomeCampo_bodega_ = "bodega_" + iNumLinha;
    var var_bodega_ = scAjaxGetFieldText(nomeCampo_bodega_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_bodegas_validate_bodega_(var_bodega_, iNumLinha, var_script_case_init, do_ajax_form_bodegas_validate_bodega__cb);
  } // do_ajax_form_bodegas_validate_bodega_

  function do_ajax_form_bodegas_validate_bodega__cb(sResp)
  {
    ...
    if ("" != iLineNumber)
    {
      sFieldValid = "bodega_" + iLineNumber;
    }
    else
    {
      sFieldValid = "bodega_";
    ...
    if ("" == sFieldErrors)
    {
      ...
      scErrorLineOff(iLineNumber, "bodega_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "bodega_");
    }
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_bodegas_validate_bodega__cb

  // ---------- Validate idbodega_
  function do_ajax_form_bodegas_validate_idbodega_(iNumLinha)
  {
    var nomeCampo_idbodega_ = "idbodega_" + iNumLinha;
    var var_idbodega_ = scAjaxGetFieldHidden(nomeCampo_idbodega_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_bodegas_validate_idbodega_(var_idbodega_, iNumLinha, var_script_case_init, do_ajax_form_bodegas_validate_idbodega__cb);
  } // do_ajax_form_bodegas_validate_idbodega_

  function do_ajax_form_bodegas_validate_idbodega__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "idbodega_" + iLineNumber;
    }
    else
    {
      sFieldValid = "idbodega_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "idbodega_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "idbodega_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_bodegas_validate_idbodega__cb

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_form_bodegas_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_form_bodegas_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_form_bodegas_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_form_bodegas_add_new_line_cb);
  } // do_ajax_form_bodegas_add_new_line

  function do_ajax_form_bodegas_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
    if ("{" == sResp.substr(0, 1)) {
        oResp = scAjaxResponse(sResp);
  ...
  } // do_ajax_form_bodegas_add_new_line_cb

  // ---------- backup_line
  function do_ajax_form_bodegas_backup_line(iNumLinha)
  {
    var var_idbodega_ = scAjaxGetFieldHidden("idbodega_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_bodegas_backup_line(var_idbodega_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_form_bodegas_backup_line_cb);
  } // do_ajax_form_bodegas_backup_line

  function do_ajax_form_bodegas_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    if (!scAjaxHasError())
  ...
  } // do_ajax_form_bodegas_backup_line_cb

  function do_ajax_form_bodegas_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
  ...
  } // do_ajax_form_bodegas_cancel_insert

  function do_ajax_form_bodegas_cancel_update(iSeqVert)
  {
    do_ajax_form_bodegas_backup_line(iSeqVert);
...
?>
  } // do_ajax_form_bodegas_cancel_update

  function do_ajax_form_bodegas_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_form_bodegas_restore_buttons

  // ---------- table_refresh
  function do_ajax_form_bodegas_table_refresh()
  {
    var var_idbodega_ = document.F2.idbodega_.value;
    ...
    scAjaxProcOn();
    x_ajax_form_bodegas_table_refresh(var_idbodega_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_fast_search, var_nmgp_cond_fast_search, var_nmgp_arg_fast_search, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_bodegas_table_refresh_cb);
  } //  do_ajax_form_bodegas_table_refresh

  function do_ajax_form_bodegas_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    document.F2.idbodega_.value = scAjaxGetKeyValue("idbodega_");
    ...
    scAjaxAlert(do_ajax_form_bodegas_table_refresh_cb_after_alert);
  } // do_ajax_form_bodegas_table_refresh_cb
  function do_ajax_form_bodegas_table_refresh_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    ...
    sc_insert_open = false;
  } // do_ajax_form_bodegas_table_refresh_cb_after_alert
...
function scJs_sweetalert_params(params) {
  ...
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_form_bodegas_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_form_bodegas_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
    sc_num_ult_line = iNumLinha;
    sc_num_ult_tr   = indexTR;
    scAjaxHideMessage();
    var var_bodega_ = scAjaxGetFieldText("bodega_" + iNumLinha);
    var var_idbodega_ = scAjaxGetFieldHidden("idbodega_" + iNumLinha);
    var var_nm_form_submit = document.F1.nm_form_submit.value;
...
?>
    x_ajax_form_bodegas_submit_form(var_bodega_, var_idbodega_, var_nmgp_refresh_row, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_bodegas_submit_form_cb);
  } // do_ajax_form_bodegas_submit_form

  function do_ajax_form_bodegas_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
      ...
      scAjaxHideErrorDisplay("table");
      scAjaxHideErrorDisplay("bodega_" + sc_num_ult_line);
      scAjaxHideErrorDisplay("idbodega_" + sc_num_ult_line);
<?php
if (isset($this->Embutida_form) && $this->Embutida_form) {
?>
      scAjaxSetReadonly();
...
?>
    ...
    scAjaxSetLabel(true);
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_form_bodegas_submit_form_cb_after_alert);
  } // do_ajax_form_bodegas_submit_form_cb
  function do_ajax_form_bodegas_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_bodegas_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_bodegas_navigate_form()
  {
    perform_ajax_form_bodegas_navigate_form();
  }

  function perform_ajax_form_bodegas_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    if (sc_insert_open)
    {
        do_ajax_form_bodegas_cancel_insert(sc_num_ult_line);
    }
    nm_uncheck_delete();
    scAjaxHideMessage();
    scAjaxHideErrorDisplay("table");
    ...
    {
      scAjaxHideErrorDisplay("bodega_" + iNavForm);
      scAjaxHideErrorDisplay("idbodega_" + iNavForm);
    }
    var var_idbodega_ = document.F2.idbodega_.value;
    var var_nm_form_submit = document.F2.nm_form_submit.value;
    var var_nmgp_opcao = document.F2.nmgp_opcao.value;
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_form_bodegas_navigate_form(var_idbodega_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_bodegas_navigate_form_cb);
  } // do_ajax_form_bodegas_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_bodegas']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_bodegas_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.idbodega_.value = scAjaxGetKeyValue("idbodega_" + var_last_index);
    ...
    scErrorLineReset();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_bodegas_navigate_form_cb_after_alert);
  } // do_ajax_form_bodegas_navigate_form_cb
  function do_ajax_form_bodegas_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
?>
  ...
  } // do_ajax_form_bodegas_navigate_form_cb_after_alert
  function sc_hide_form_bodegas_form()
  {
...
?>
  ...
  {
    ajax_field_list[iTotCampos] = "bodega_" + iNumLinha;
    iTotCampos++;
    ajax_field_list[iTotCampos] = "idbodega_" + iNumLinha;
    iTotCampos++;
    ajax_error_list["bodega_" + iNumLinha] = {"label": "Bodega<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["idbodega_" + iNumLinha] = {"label": "Idbodega<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_field_mult["bodega_"][iNumLinha] = "bodega_" + iNumLinha;
    ajax_field_mult["idbodega_"][iNumLinha] = "idbodega_" + iNumLinha;
    ajax_field_id["bodega_" + iNumLinha] = new Array("hidden_field_label_bodega_", "hidden_field_data_bodega_" + iNumLinha);
    ajax_error_count["bodega_" + iNumLinha] = "off";
    ajax_error_count["idbodega_" + iNumLinha] = "off";
<?php
if (!$this->Grid_editavel)
{
?>
    ajax_read_only["bodega_" + iNumLinha] = "off";
    ajax_read_only["idbodega_" + iNumLinha] = "on";
<?php
}
else
{
?>
    ajax_read_only["bodega_" + iNumLinha] = "on";
    ajax_read_only["idbodega_" + iNumLinha] = "on";
<?php
}
?>
  ...
  {
    ajax_error_list["bodega_" + iNumLinha] = null;
    ajax_error_list["idbodega_" + iNumLinha] = null;
  }
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ajax_error_list["bodega_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["bodega_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["bodega_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["bodega_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["bodega_" + iNumLinha]["onfocus"] = new Array();
      ajax_error_list["idbodega_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["idbodega_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["idbodega_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["idbodega_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["idbodega_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
?>
  ...
  {
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['bodega_'])) ? $this->nmgp_cmp_readonly['bodega_'] : 'off';
?>
    scAjaxFieldRead("bodega_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['idbodega_'])) ? $this->nmgp_cmp_readonly['idbodega_'] : 'on';
?>
    scAjaxFieldRead("idbodega_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }

  function mdCloseObjects(iSeq)
  {
    scAjaxFieldRead("bodega_" + iSeq, "on");
    scAjaxFieldRead("idbodega_" + iSeq, "on");
  }
...
?>
  ...
  function scErrorLineOn(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("on" == ajax_error_count["bodega_" + iRow] || "on" == ajax_error_count["idbodega_" + iRow]))
    {
      $("#hidden_field_data_sc_seq" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_sc_actions" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_bodega_" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_idbodega_" + iRow).addClass("scFormErrorLine");
    }
  }
  ...
  function scErrorLineOff(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("off" == ajax_error_count["bodega_" + iRow] && "off" == ajax_error_count["idbodega_" + iRow]))
    {
      if (bErrorRow)
      {
        ajax_error_count["bodega_" + iRow] = "off";
        ajax_error_count["idbodega_" + iRow] = "off";
      }
      ...
      $("#hidden_field_data_bodega_" + iRow).removeClass("scFormErrorLine");
      $("#hidden_field_data_idbodega_" + iRow).removeClass("scFormErrorLine");
    }
  }
