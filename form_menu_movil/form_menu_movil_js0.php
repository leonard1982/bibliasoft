<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idmenu_movil_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idmenu_movil_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_menu_movil"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_menu_movil_submit_form(y, z);
...
{
    NovaJanela = window.open ("form_menu_movil_help.php?nm_cod_campo=" + campo + "&language=<?php echo $this->Ini->path_lang . $this->Ini->str_lang?>&help_css=<?php echo '../_lib/css/' . $this->Ini->str_schema_all . '_form.css'; ?>", "ScriptCase", "height=100, width=300, resizable, scrollbars");
}
