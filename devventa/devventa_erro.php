...
$old_error_handler = set_error_handler("devventa_scriptcase_error_handler");

function devventa_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['devventa']['contr_erro']) && $_SESSION['scriptcase']['devventa']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            global $inicial_devventa;
            $inicial_devventa->contr_devventa->Apl_com_erro = true;
            if ($inicial_devventa->contr_devventa->NM_ajax_flag)
            {
                $inicial_devventa->contr_devventa->NM_ajax_info['errList']['geral_devventa'][] = $msg;
                return;
            }
            devventa_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function devventa_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['devventa'])  && '' != $_SESSION['scriptcase']['error_icon']['devventa'])  ? $_SESSION['scriptcase']['error_icon']['devventa']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['devventa']) && '' != $_SESSION['scriptcase']['error_close']['devventa']) ? $_SESSION['scriptcase']['error_close']['devventa'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['devventa']) && '' != $_SESSION['scriptcase']['error_span_title']['devventa']) ? $_SESSION['scriptcase']['error_span_title']['devventa'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['devventa']) && '' != $_SESSION['scriptcase']['error_icon_title']['devventa']) ? $_SESSION['scriptcase']['error_icon_title']['devventa'] : '';
?>
