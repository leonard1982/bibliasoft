...
$old_error_handler = set_error_handler("devventa_mob_scriptcase_error_handler");

function devventa_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['devventa_mob']['contr_erro']) && $_SESSION['scriptcase']['devventa_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_devventa_mob->contr_devventa_mob->Apl_com_erro = true;
            if ($inicial_devventa_mob->contr_devventa_mob->NM_ajax_flag)
            {
                $inicial_devventa_mob->contr_devventa_mob->NM_ajax_info['errList']['geral_devventa_mob'][] = $msg;
                return;
            }
            devventa_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function devventa_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['devventa_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['devventa_mob'])  ? $_SESSION['scriptcase']['error_icon']['devventa_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['devventa_mob']) && '' != $_SESSION['scriptcase']['error_close']['devventa_mob']) ? $_SESSION['scriptcase']['error_close']['devventa_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['devventa_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['devventa_mob']) ? $_SESSION['scriptcase']['error_span_title']['devventa_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['devventa_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['devventa_mob']) ? $_SESSION['scriptcase']['error_icon_title']['devventa_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['devventa']['error_buffer'] .= ob_get_flush();
}
