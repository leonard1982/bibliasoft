...
$old_error_handler = set_error_handler("recalcular_mob_scriptcase_error_handler");

function recalcular_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['recalcular_mob']['contr_erro']) && $_SESSION['scriptcase']['recalcular_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            global $inicial_recalcular_mob;
            $inicial_recalcular_mob->contr_recalcular_mob->Apl_com_erro = true;
            if ($inicial_recalcular_mob->contr_recalcular_mob->NM_ajax_flag)
            {
                $inicial_recalcular_mob->contr_recalcular_mob->NM_ajax_info['errList']['geral_recalcular_mob'][] = $msg;
                return;
            }
            recalcular_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function recalcular_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['recalcular_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['recalcular_mob'])  ? $_SESSION['scriptcase']['error_icon']['recalcular_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['recalcular_mob']) && '' != $_SESSION['scriptcase']['error_close']['recalcular_mob']) ? $_SESSION['scriptcase']['error_close']['recalcular_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['recalcular_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['recalcular_mob']) ? $_SESSION['scriptcase']['error_span_title']['recalcular_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['recalcular_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['recalcular_mob']) ? $_SESSION['scriptcase']['error_icon_title']['recalcular_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['recalcular']['error_buffer'] .= ob_get_flush();
}
