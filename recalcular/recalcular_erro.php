...
$old_error_handler = set_error_handler("recalcular_scriptcase_error_handler");

function recalcular_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['recalcular']['contr_erro']) && $_SESSION['scriptcase']['recalcular']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            global $inicial_recalcular;
            $inicial_recalcular->contr_recalcular->Apl_com_erro = true;
            if ($inicial_recalcular->contr_recalcular->NM_ajax_flag)
            {
                $inicial_recalcular->contr_recalcular->NM_ajax_info['errList']['geral_recalcular'][] = $msg;
                return;
            }
            recalcular_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function recalcular_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['recalcular'])  && '' != $_SESSION['scriptcase']['error_icon']['recalcular'])  ? $_SESSION['scriptcase']['error_icon']['recalcular']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['recalcular']) && '' != $_SESSION['scriptcase']['error_close']['recalcular']) ? $_SESSION['scriptcase']['error_close']['recalcular'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['recalcular']) && '' != $_SESSION['scriptcase']['error_span_title']['recalcular']) ? $_SESSION['scriptcase']['error_span_title']['recalcular'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['recalcular']) && '' != $_SESSION['scriptcase']['error_icon_title']['recalcular']) ? $_SESSION['scriptcase']['error_icon_title']['recalcular'] : '';
?>
