  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_recalcular_mob" == sTestField)
        {
...
?>
  ...
  // ---------- Validate hola
  function do_ajax_recalcular_mob_validate_hola()
  {
    var nomeCampo_hola = "hola";
    var var_hola = scAjaxGetFieldText(nomeCampo_hola);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_recalcular_mob_validate_hola(var_hola, var_script_case_init, do_ajax_recalcular_mob_validate_hola_cb);
  } // do_ajax_recalcular_mob_validate_hola

  function do_ajax_recalcular_mob_validate_hola_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_recalcular_mob_validate_hola_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_recalcular_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_recalcular_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_recalcular_mob_submit_form(var_hola, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_recalcular_mob_submit_form_cb);
  } // do_ajax_recalcular_mob_submit_form

  function do_ajax_recalcular_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    ...
    if (scAjaxIsOk())
    {
      ...
      scLigEditLookupCall();
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['recalcular_mob']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['recalcular_mob']['dashboard_info']['under_dashboard']) {
?>
      var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['recalcular_mob']['dashboard_info']['parent_widget']; ?>']");
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_recalcular_mob_submit_form_cb_after_alert);
  } // do_ajax_recalcular_mob_submit_form_cb
  function do_ajax_recalcular_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_recalcular_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_recalcular_mob_navigate_form()
  {
    perform_ajax_recalcular_mob_navigate_form();
  }

  function perform_ajax_recalcular_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
  ...
  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['recalcular_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_recalcular_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_recalcular_mob_navigate_form_cb_after_alert);
  } // do_ajax_recalcular_mob_navigate_form_cb
  function do_ajax_recalcular_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_recalcular_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_recalcular_mob_navigate_form_cb_after_alert
  function sc_hide_recalcular_mob_form()
  {
