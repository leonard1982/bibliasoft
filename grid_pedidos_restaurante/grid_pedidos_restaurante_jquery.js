function grid_pedidos_restaurante_total_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_total" + seqRow).html();
}

function grid_pedidos_restaurante_total_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_total" + seqRow).html(newValue);
}

function grid_pedidos_restaurante_mesa_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_mesa" + seqRow).find("i").attr("src");
}

function grid_pedidos_restaurante_mesa_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_mesa" + seqRow).find("img").attr("src", newValue);
}

function grid_pedidos_restaurante_idcli_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_idcli" + seqRow).html();
}

function grid_pedidos_restaurante_idcli_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_idcli" + seqRow).html(newValue);
}

function grid_pedidos_restaurante_vendedor_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_vendedor" + seqRow).html();
}

function grid_pedidos_restaurante_vendedor_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_vendedor" + seqRow).html(newValue);
}

function grid_pedidos_restaurante_cambiar_cliente_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_cambiar_cliente" + seqRow).find("i").attr("src");
}

function grid_pedidos_restaurante_cambiar_cliente_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_cambiar_cliente" + seqRow).find("img").attr("src", newValue);
}

function grid_pedidos_restaurante_imprimir_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_imprimir" + seqRow).find("i").attr("src");
}

function grid_pedidos_restaurante_imprimir_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_imprimir" + seqRow).find("img").attr("src", newValue);
}

function grid_pedidos_restaurante_prefijo_ped_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_prefijo_ped" + seqRow).html();
}

function grid_pedidos_restaurante_prefijo_ped_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_prefijo_ped" + seqRow).html(newValue);
}

function grid_pedidos_restaurante_getValue(field, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  var SC_tmp = "";
  if ("total" == field) {
    SC_tmp = grid_pedidos_restaurante_total_getValue(seqRow);
  }
  if ("mesa" == field) {
    SC_tmp = grid_pedidos_restaurante_mesa_getValue(seqRow);
  }
  if ("idcli" == field) {
    SC_tmp = grid_pedidos_restaurante_idcli_getValue(seqRow);
  }
  if ("vendedor" == field) {
    SC_tmp = grid_pedidos_restaurante_vendedor_getValue(seqRow);
  }
  if ("cambiar_cliente" == field) {
    SC_tmp = grid_pedidos_restaurante_cambiar_cliente_getValue(seqRow);
  }
  if ("imprimir" == field) {
    SC_tmp = grid_pedidos_restaurante_imprimir_getValue(seqRow);
  }
  if ("prefijo_ped" == field) {
    SC_tmp = grid_pedidos_restaurante_prefijo_ped_getValue(seqRow);
  }
...
function grid_pedidos_restaurante_setValue(field, newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  if ("total" == field) {
    grid_pedidos_restaurante_total_setValue(newValue, seqRow);
  }
  if ("mesa" == field) {
    grid_pedidos_restaurante_mesa_setValue(newValue, seqRow);
  }
  if ("idcli" == field) {
    grid_pedidos_restaurante_idcli_setValue(newValue, seqRow);
  }
  if ("vendedor" == field) {
    grid_pedidos_restaurante_vendedor_setValue(newValue, seqRow);
  }
  if ("cambiar_cliente" == field) {
    grid_pedidos_restaurante_cambiar_cliente_setValue(newValue, seqRow);
  }
  if ("imprimir" == field) {
    grid_pedidos_restaurante_imprimir_setValue(newValue, seqRow);
  }
  if ("prefijo_ped" == field) {
    grid_pedidos_restaurante_prefijo_ped_setValue(newValue, seqRow);
  }
  if ("idpedido_Hidden" == field) {
    grid_pedidos_restaurante_idpedido_Hidden_setValue(newValue, seqRow);
  }
  if ("asentada_Hidden" == field) {
    grid_pedidos_restaurante_asentada_Hidden_setValue(newValue, seqRow);
  }
}
...
function scJQAddEvents(seqRow) {
  ...
  $("#id_sc_field_idpedido" + seqRow).click(function () {
    $.ajax({
      type: "POST",
      url: "index.php",
      data: "nmgp_opcao=ajax_event&nmgp_event=idpedido_onClick&script_case_init=" + document.F3.script_case_init.value + "&"
     })
     .done(function(jsonReturn) {
        ...
        if (oResp["setValue"]) {
          for (i = 0; i < oResp["setValue"].length; i++) {
            grid_pedidos_restaurante_setValue(oResp["setValue"][i]["field"], oResp["setValue"][i]["value"], seqRow);
          }
        }
...
function ajax_navigate(opc, parm)
{
    ...
    return new Promise(function(resolve, reject) {$.ajax({
     ...
     .done(function(jsonNavigate) {
        ...
        document.getElementById('nmsc_iframe_liga_A_grid_pedidos_restaurante').src = 'NM_Blank_Page.htm';
        document.getElementById('nmsc_iframe_liga_E_grid_pedidos_restaurante').src = 'NM_Blank_Page.htm';
        document.getElementById('nmsc_iframe_liga_D_grid_pedidos_restaurante').src = 'NM_Blank_Page.htm';
        document.getElementById('nmsc_iframe_liga_B_grid_pedidos_restaurante').src = 'NM_Blank_Page.htm';
        document.getElementById('nmsc_iframe_liga_A_grid_pedidos_restaurante').style.height = '0px';
        document.getElementById('nmsc_iframe_liga_E_grid_pedidos_restaurante').style.height = '0px';
        document.getElementById('nmsc_iframe_liga_D_grid_pedidos_restaurante').style.height = '0px';
        document.getElementById('nmsc_iframe_liga_B_grid_pedidos_restaurante').style.height = '0px';
        document.getElementById('nmsc_iframe_liga_A_grid_pedidos_restaurante').style.width  = '0px';
        document.getElementById('nmsc_iframe_liga_E_grid_pedidos_restaurante').style.width  = '0px';
        document.getElementById('nmsc_iframe_liga_D_grid_pedidos_restaurante').style.width  = '0px';
        document.getElementById('nmsc_iframe_liga_B_grid_pedidos_restaurante').style.width  = '0px';
        if (oResp["setValue"]) {
          for (i = 0; i < oResp["setValue"].length; i++) {
