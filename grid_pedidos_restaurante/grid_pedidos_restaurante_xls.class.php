...
class grid_pedidos_restaurante_xls
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function inicializa_vars()
   {
      ...
      { 
          ...
          if (!is_file($this->Ini->root . $this->Ini->path_link . "grid_pedidos_restaurante/grid_pedidos_restaurante_res_xls.class.php"))
          {
              $this->Tem_xls_res  = false;
          }
          if ($this->Tem_xls_res)
          { 
              require_once($this->Ini->path_aplicacao . "grid_pedidos_restaurante_res_xls.class.php");
              $this->Res_xls = new grid_pedidos_restaurante_res_xls();
              $this->prep_modulos("Res_xls");
          } 
          $this->Arquivo    = "sc_xls";
          $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
          $this->Arq_zip    = $this->Arquivo . "_grid_pedidos_restaurante.zip";
          $this->Arquivo   .= "_grid_pedidos_restaurante" . $this->Xls_tp;
      ...
      require_once($this->Ini->path_aplicacao . "grid_pedidos_restaurante_total.class.php"); 
      ...
      {
          $this->sum_total = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['tot_geral'][2];
      }
   ...
   function grava_arquivo()
   {
      ...
      $nm_raiz_img  = ""; 
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['field_order'] as $Cada_cmp)
      {
...
$_SESSION['scriptcase']['grid_pedidos_restaurante']['contr_erro'] = 'off'; 
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['embutida_label']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['embutida_label'])
      { 
          $this->count_span = 0;
          $this->Xls_row++;
          $this->proc_label();
      ...
      while (!$rs->EOF)
      {
         ...
         $this->arg_sum_prefijo_ped = ($this->prefijo_ped == "") ? " is null " : " = " . $this->prefijo_ped;
          ...
          {  
              ...
              if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['embutida'] && !$prim_gb && $this->groupby_show == "S") {
...
$_SESSION['scriptcase']['grid_pedidos_restaurante']['contr_erro'] = 'off'; 
         ...
         if (isset($this->NM_Row_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['embutida'])
         { 
      ...
      $this->xls_set_style();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['embutida'] && $prim_reg)
      { 
          $this->proc_label();
          $this->xls_sub_cons_copy_label($this->Xls_row);
      ...
      if (isset($this->NM_Col_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['embutida'])
      { 
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   { 
      ...
      { 
          ...
          if ($Cada_col == "copiar_a_fv" && (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off"))
          {
              $this->count_span++;
              $current_cell_ref = $this->calc_cell($this->Xls_col);
              $SC_Label = NM_charset_to_utf8($SC_Label);
   ...
   function NM_export_mesa()
   {
         ...
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->mesa, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->mesa, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- idcli
   ...
   function NM_export_copiar_a_fv()
   {
         ...
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->copiar_a_fv, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->copiar_a_fv, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- total
   ...
   //----- mesa
   function NM_sub_cons_mesa()
   {
         $this->mesa = NM_charset_to_utf8($this->mesa);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->mesa;
   ...
   //----- copiar_a_fv
   function NM_sub_cons_copiar_a_fv()
   {
         $this->copiar_a_fv = NM_charset_to_utf8($this->copiar_a_fv);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->copiar_a_fv;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "center";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
         $this->Xls_col++;
   ...
   {
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['nolabel']) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['nolabel'])
       {
 ...
 function quebra_vendedor_vendedor($vendedor) 
 {
   ...
   function quebra_prefijo_ped_prefijo_bot()
   {
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['field_order'] as $Cada_cmp)
       {
           ...
           elseif (!isset($this->NM_cmp_hidden[$Cada_cmp]) || $this->NM_cmp_hidden[$Cada_cmp] != "off")
           {
               ...
               elseif ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['embutida']) {
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = "";
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "left";
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
                       $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
   ...
   function quebra_geral_prefijo_bot()
   {
       ...
       $prim_cmp = true;
       $mens_tot = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['tot_geral'][0];
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['field_order'] as $Cada_cmp)
       {
           if ($Cada_cmp == "total" && (!isset($this->NM_cmp_hidden['total']) || $this->NM_cmp_hidden['total'] != "off"))
           {
               $Format_Num = "#,##0.00";
               $Vl_Tot     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_restaurante']['tot_geral'][2];
               $prim_cmp = false;
