...
class grid_caja_resumo
{
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
       ...
       $idcaja_2 = $Busca_temp['idcaja_input_2']; 
       $this->idcaja_2 = $Busca_temp['idcaja_input_2']; 
      ...
      {
          $this->array_total_detalle = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_caja']['arr_total']['detalle'];
      }
      else
      {
          $this->array_total_detalle = array();
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_caja']['arr_total']['detalle'] = $this->array_total_detalle;
   ...
   function completeMatrix()
   {
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_caja']['summarizing_fields_control']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_caja']['summarizing_fields_control'] = array(
               ...
               'cantidad' => array(
                   ...
                   'label_field' => "Valor Ingreso/Egreso",
                   'options' => array(
       ...
       foreach ($this->array_total_detalle as $i_detalle => $d_detalle) {
           if (!in_array($i_detalle, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_detalle[3] ] = $d_detalle[2];
               $this->comp_label[0][ $d_detalle[3] ] = $d_detalle[2];
           }
       }
   ...
   function buildChart()
   {
       ...
       $this->comp_chart_config = array(
           '0|0' => array(
               ...
               'show_sub' => true,
               'subtitle' => "",
               'label_x'  => $this->getChartText(strip_tags("Detalle")),
...
$nm_saida->saida("     " . $res_sem_reg . "</TD> </TR>\r\n");
   ...
   function totaliza()
   {
      $this->total->resumo_detalle("res", $this->array_total_detalle);
...
//--- 
   ...
   function adiciona_registro($sum_cantidad, $quebra_detalle, $quebra_detalle_orig)
   {
      ...
      if (!isset($this->array_total_detalle[$quebra_detalle_orig]))
      {
         ...
         $this->array_total_detalle[$quebra_detalle_orig][2] = $quebra_detalle;
         $this->array_total_detalle[$quebra_detalle_orig][3] = $quebra_detalle_orig;
      }
      else
      {
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     { 
         $nmgp_select = "SELECT str_replace (convert(char(10),fecha,102), '.', '-') + ' ' + convert(char(8),fecha,20), jornada, detalle, nota, resolucion, documento, cantidad, idrc, idcaja from " . $this->Ini->nm_tabela; 
     } 
     elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mysql))
     { 
         $nmgp_select = "SELECT fecha, jornada, detalle, nota, resolucion, documento, cantidad, idrc, idcaja from " . $this->Ini->nm_tabela; 
     } 
     else 
     { 
         $nmgp_select = "SELECT fecha, jornada, detalle, nota, resolucion, documento, cantidad, idrc, idcaja from " . $this->Ini->nm_tabela; 
     ...
     $nmgp_order_by = " order by  detalle asc"; 
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->jornada = $rs_res->fields[1] ;  
            $this->detalle = $rs_res->fields[2] ;  
            $this->nota = $rs_res->fields[3] ;  
            $this->resolucion = $rs_res->fields[4] ;  
...
if (!isset($_SESSION['gPar_Idfac'])) {$_SESSION['gPar_Idfac'] = "";}
if (!isset($this->sc_temp_gPar_Idfac)) {$this->sc_temp_gPar_Idfac = (isset($_SESSION['gPar_Idfac'])) ? $_SESSION['gPar_Idfac'] : "";}
 if($this->detalle ='FAC. CONTADO')
	{
	$num=$this->documento ;
	 
      $nm_select = "select idfacven from facturaven where numfacven=$num and resolucion=$this->resolucion  "; 
...
;
	if(isset($this->da[0][0]))
		{
		$this->sc_temp_gPar_Idfac=$this->da[0][0];
		}
	else
		{
		$this->sc_temp_gPar_Idfac=0;
		}
	}
else
	{
...
if (isset($this->sc_temp_gPar_Idfac)) {$_SESSION['gPar_Idfac'] = $this->sc_temp_gPar_Idfac;}
