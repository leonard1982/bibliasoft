<form name="F2" method=post 
               ...
               target="_self"> 
<input type="hidden" name="idprod" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idprod']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_productos_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_productos_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
  tb_show('', '<?php echo $this->Ini->link_form_colores_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scoutsc_redir_atualiz*scinok*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=400&width=450&modal=true', '');
}
...
{
  tb_show('', '<?php echo $this->Ini->link_form_tallas_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scoutsc_redir_atualiz*scinok*scoutsc_redir_insert*scinok*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=500&width=450&modal=true', '');
}
function sc_btn_sc_btn_2()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_sc_btn_2(); }, 500);
      return;
    }
    sc_btn_sc_btn_2_ok();
}
function sc_btn_sc_btn_2_cancel()
{
}
function sc_btn_sc_btn_2_ok()
{
  tb_show('', '<?php echo $this->Ini->link_form_sabores_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scoutsc_redir_atualiz*scinok*scoutsc_redir_insert*scinok*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=500&width=450&modal=true', '');
}
var var_btn_escalas_gidpro = "<?php echo $this->form_encode_input($this->nmgp_dados_form['idprod']); ?>";
function sc_btn_escalas()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_escalas(); }, 500);
      return;
    }
    sc_btn_escalas_ok();
}
function sc_btn_escalas_cancel()
{
}
function sc_btn_escalas_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_form_escalas_productos_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "gidpro*scin' + var_btn_escalas_gidpro + '*scoutNM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>', '', '_blank', '0', '0', 'form_escalas_productos');
}
var Crtl_btn_recalcular = false;
function sc_btn_recalcular()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_recalcular(); }, 500);
      return;
    }
    if (Crtl_btn_recalcular) {return;}
    sc_btn_recalcular_ok();
}
function sc_btn_recalcular_cancel()
{
}
function sc_btn_recalcular_ok()
{
    Crtl_btn_recalcular = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?recalcular?@?";
    document.F1.action = "form_productos_mob.php";
...
{ 
   var sob_iframe = '';
<?php 
if ($_SESSION['sc_session'][$this->Ini->sc_page]['form_productos_mob']['embutida_form']) {
    echo "   sob_iframe += 'parent.';\r\n";
}
if ($_SESSION['sc_session'][$this->Ini->sc_page]['form_productos_mob']['run_iframe'] == "F" || $_SESSION['sc_session'][$this->Ini->sc_page]['form_productos_mob']['run_iframe'] == "R") {
    echo "   sob_iframe += 'parent.';\r\n";
}
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_productos_mob']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_productos_mob']['sc_modal']) {
    echo "   sob_iframe += 'parent.';\r\n";
}
?>
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_productos_mob']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_form_productos_mob_navigate_form();
    }
} 
...
?>
    ...
    { 
       ...
       {
           Nm_Proc_Atualiz = true;
           do_ajax_form_productos_mob_submit_form();
       }
    } 
    if (Nm_submit_ok)
    { 
...
{
    NovaJanela = window.open ("form_productos_mob_help.php?nm_cod_campo=" + campo + "&language=<?php echo $this->Ini->path_lang . $this->Ini->str_lang?>&help_css=<?php echo '../_lib/css/' . $this->Ini->str_schema_all . '_form.css'; ?>", "ScriptCase", "height=100, width=300, resizable, scrollbars");
}
...
{
   nm_field_disabled("stockmen=disabled", "");
   
}
function sc_exib_ocult_pag(N_pagina)
{
    document.getElementById('form_productos_mob_form0').style.width='1px';
    document.getElementById('form_productos_mob_form0').style.height='0px';
    document.getElementById('form_productos_mob_form0').style.display='none';
    document.getElementById('form_productos_mob_form0').style.overflow='scroll';
    document.getElementById('form_productos_mob_form1').style.width='1px';
    document.getElementById('form_productos_mob_form1').style.height='0px';
    document.getElementById('form_productos_mob_form1').style.display='none';
    document.getElementById('form_productos_mob_form1').style.overflow='scroll';
<?php
    if (!$_SESSION['sc_session'][$this->Ini->sc_page]['form_productos_mob']['pdf_view']) {
?>
