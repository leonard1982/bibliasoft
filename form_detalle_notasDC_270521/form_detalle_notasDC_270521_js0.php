<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="iddet_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['iddet_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_detalle_notasDC_270521"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalle_notasDC_270521']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalle_notasDC_270521']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalle_notasDC_270521']['dashboard_info']['parent_widget'] . "']\")";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalle_notasDC_270521']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_form_detalle_notasDC_270521_navigate_form();
    }
    if ("ordem" == x)
    {
        scSetOrderColumn(y);
...
function scForm_submit_single(x) {
	if (x != "excluir")
	{
		...
		if ("incluir" == x || "muda_form" == x || "recarga" == x || "recarga_mobile" == x) {
			...
			Nm_Proc_Atualiz = true;
			do_ajax_form_detalle_notasDC_270521_submit_form();
		}
	}
	if (Nm_submit_ok) {
		Nm_Proc_Atualiz = true;
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_detalle_notasDC_270521_submit_form(y, z);
...
{
    NovaJanela = window.open ("form_detalle_notasDC_270521_help.php?nm_cod_campo=" + campo + "&language=<?php echo $this->Ini->path_lang . $this->Ini->str_lang?>&help_css=<?php echo '../_lib/css/' . $this->Ini->str_schema_all . '_form.css'; ?>", "ScriptCase", "height=100, width=300, resizable, scrollbars");
}
...
{
   nm_field_disabled("elcolor_=disabled;latalla_=disabled;elsabor_=disabled", "I");
