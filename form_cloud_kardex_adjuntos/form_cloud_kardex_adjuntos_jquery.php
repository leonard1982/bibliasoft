...
function scEventControl_init(iSeqRow) {
  scEventControl_data["id_adjunto_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["id_cloud_kardex_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["adjunto_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["id_adjunto_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["id_adjunto_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["id_cloud_kardex_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["id_cloud_kardex_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_adjunto_' + iSeqRow).bind('blur', function() { sc_form_cloud_kardex_adjuntos_id_adjunto__onblur(this, iSeqRow) })
                                         .bind('change', function() { sc_form_cloud_kardex_adjuntos_id_adjunto__onchange(this, iSeqRow) })
                                         .bind('focus', function() { sc_form_cloud_kardex_adjuntos_id_adjunto__onfocus(this, iSeqRow) });
  $('#id_sc_field_id_cloud_kardex_' + iSeqRow).bind('blur', function() { sc_form_cloud_kardex_adjuntos_id_cloud_kardex__onblur(this, iSeqRow) })
                                              .bind('change', function() { sc_form_cloud_kardex_adjuntos_id_cloud_kardex__onchange(this, iSeqRow) })
                                              .bind('focus', function() { sc_form_cloud_kardex_adjuntos_id_cloud_kardex__onfocus(this, iSeqRow) });
  $('#id_sc_field_adjunto_' + iSeqRow).bind('blur', function() { sc_form_cloud_kardex_adjuntos_adjunto__onblur(this, iSeqRow) })
                                      .bind('change', function() { sc_form_cloud_kardex_adjuntos_adjunto__onchange(this, iSeqRow) })
                                      .bind('focus', function() { sc_form_cloud_kardex_adjuntos_adjunto__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_cloud_kardex_adjuntos_id_adjunto__onblur(oThis, iSeqRow) {
  do_ajax_form_cloud_kardex_adjuntos_validate_id_adjunto_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_cloud_kardex_adjuntos_id_adjunto__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_cloud_kardex_adjuntos_id_adjunto__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_cloud_kardex_adjuntos_id_cloud_kardex__onblur(oThis, iSeqRow) {
  do_ajax_form_cloud_kardex_adjuntos_validate_id_cloud_kardex_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_cloud_kardex_adjuntos_id_cloud_kardex__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_cloud_kardex_adjuntos_id_cloud_kardex__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_cloud_kardex_adjuntos_adjunto__onblur(oThis, iSeqRow) {
  scCssBlur(oThis, iSeqRow);
}

function sc_form_cloud_kardex_adjuntos_adjunto__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_cloud_kardex_adjuntos_adjunto__onfocus(oThis, iSeqRow) {
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("id_adjunto_", "", status);
	displayChange_field("id_cloud_kardex_", "", status);
	displayChange_field("adjunto_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_id_adjunto_(row, status);
	displayChange_field_id_cloud_kardex_(row, status);
	displayChange_field_adjunto_(row, status);
}

function displayChange_field(field, row, status) {
	if ("id_adjunto_" == field) {
		displayChange_field_id_adjunto_(row, status);
	}
	if ("id_cloud_kardex_" == field) {
		displayChange_field_id_cloud_kardex_(row, status);
	}
	if ("adjunto_" == field) {
		displayChange_field_adjunto_(row, status);
...
function displayChange_field_id_adjunto_(row, status) {
}

function displayChange_field_id_cloud_kardex_(row, status) {
}

function displayChange_field_adjunto_(row, status) {
}
...
function scHidePage(pageNo) {
	$("#id_form_cloud_kardex_adjuntos_form" + pageNo).hide();
}
...
function scJQUploadAdd(iSeqRow) {
  $("#id_sc_field_adjunto_" + iSeqRow).fileupload({
    datatype: "json",
    url: "form_cloud_kardex_adjuntos_ul_save.php",
    ...
    progress: function(e, data) {
      ...
      if (data.lengthComputable && window.FormData !== undefined) {
        ...
        loaderContent = $("#id_img_loader_adjunto_" + iSeqRow + " .scProgressBarLoading");
        loaderContent.html("&nbsp;");
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_cloud_kardex_adjuntos')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
