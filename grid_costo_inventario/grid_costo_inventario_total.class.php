...
class grid_costo_inventario_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_costo_inventario']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_costo_inventario']['campos_busca']))
      { 
   ...
   function quebra_geral_grupo($res_limit=false)
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(stockmen), sum((costomen*stockmen)) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['contr_total_geral'] = "OK";
   } 
   ...
   //-----  idgrup
   function quebra_idgrup_grupo($idgrup, $arg_sum_idgrup) 
   {
      global $tot_idgrup, $idgrup;
      $tot_idgrup = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(stockmen), sum((costomen*stockmen)) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['where_pesq']; 
      ...
      { 
         $nm_comando .= " where idgrup" . $arg_sum_idgrup ; 
      } 
      else 
      { 
         $nm_comando .= " and idgrup" . $arg_sum_idgrup ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_idgrup[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
   } 
   ...
   function quebra_existencia_sc_free_group_by($existencia, $Where_qb, $Cmp_Name) 
   {
      ...
      $tot_existencia[0] = NM_encode_input(sc_strip_script($existencia)) ; 
      $tot_existencia[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_existencia[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_existencia[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_existencia;
   } 
   ...
   function quebra_unimen_sc_free_group_by($unimen, $Where_qb, $Cmp_Name) 
   {
      ...
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_unimen[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_unimen;
   } 

   //-----  costou
   function quebra_costou_sc_free_group_by($costou, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $idgrup;
      $tot_costou = array() ;  
      ...
      $tot_costou[0] = NM_encode_input(sc_strip_script($costou)) ; 
      $tot_costou[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_costou[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_costou[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_costou;
   } 

   //-----  costo_total
   function quebra_costo_total_sc_free_group_by($costo_total, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $idgrup;
      $tot_costo_total = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(stockmen), sum((costomen*stockmen)) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['where_pesq']; 
      } 
      ...
      $tot_costo_total[0] = NM_encode_input(sc_strip_script($costo_total)) ; 
      $tot_costo_total[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_costo_total[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_costo_total[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_costo_total;
   } 
   ...
   function Calc_resumo_grupo($destino_resumo)
   {
      ...
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('idgrup' => "idgrup");
      $cmps_quebra_atual = array("idgrup");
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_grupo($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('idgrup' => 'N','existencia' => 'N','costou' => 'N','costo_total' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(stockmen), sum((costomen*stockmen))#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2)#@#cmps_quebras#@# from (";
         $comando .= "select stockmen as SC_metric1,(costomen*stockmen) as SC_metric2, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "idgrup") {
                  $this->Lookup->lookup_grupo_idgrup($$Cada_dim,  $idgrup);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('idgrup' => "idgrup",'producto' => "concat(codigoprod,'-',nompro)",'existencia' => "stockmen",'unimen' => "unimen",'costou' => "costomen",'costo_total' => "(costomen*stockmen)");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_costo_inventario']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $format_dimensions['costou']['reg'] = "S";
      $format_dimensions['costou']['msk'] = "";
      $format_dimensions['costo_total']['reg'] = "S";
      $format_dimensions['costo_total']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
