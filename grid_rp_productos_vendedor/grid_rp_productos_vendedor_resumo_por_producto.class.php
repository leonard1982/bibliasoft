...
class grid_rp_productos_vendedor_resumo
{
   ...
   var $sum_cantidad;
   var $sum_total;
   var $sc_proc_quebra_p_nompro;
   var $count_p_nompro;
   var $sum_p_nompro_cantidad;
   var $sum_p_nompro_total;
   ...
   function __construct($tipo = "")
   {
      ...
      $this->array_total_p_nompro = array();
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      {
          $this->array_total_p_nompro = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['arr_total']['p_nompro'];
      }
      else
      {
          $this->array_total_p_nompro = array();
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['arr_total']['p_nompro'] = $this->array_total_p_nompro;
   ...
   function completeMatrix()
   {
       ...
       {
          $Lab_p_nompro = "p.nompro";
       }
       else
       {
       $Lab_p_nompro = "Producto";
       }
       $this->comp_field = array(
           $Lab_p_nompro,
       );
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['labels']['p_nompro'] = $Lab_p_nompro; 
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_order']['por_producto']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_order']['por_producto'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_order']['por_producto'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_producto'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_producto'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_producto'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_producto'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_producto'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_producto'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_producto'] as $i_sum => $d_sum)
           {
       ...
       //-----
       foreach ($this->array_total_p_nompro as $i_p_nompro => $d_p_nompro) {
           if (!in_array($i_p_nompro, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_p_nompro[5] ] = $d_p_nompro[4];
               $this->comp_label[0][ $d_p_nompro[5] ] = $d_p_nompro[4];
           }
       }
       ...
       //-----
       $this->comp_links_fl = array(
           array('name' => 'p_nompro', 'prot' => '@aspass@'),
       );
       ...
       //-----
       foreach ($this->comp_label[0] as $i_p_nompro => $l_p_nompro) {
           if (isset($this->array_total_p_nompro[$i_p_nompro])) {
               $this->comp_group[$i_p_nompro] = array();
           }
       }
   ...
   function getCompData($level, $params = array()) {
       if (0 == $level) {
           $return = $this->array_total_p_nompro;
       }
   ...
   function buildChart()
   {
       ...
       $this->comp_chart_config = array(
           '0|1' => array(
               ...
               'label_x'  => $this->getChartText(strip_tags($Lab_p_nompro)),
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['proc_pdf_vert']){ 
   ...
   function totaliza()
   {
      ...
      ksort($this->array_total_p_nompro);
      if ("out" == $this->NM_tipo)
      {
         $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['SC_Ind_Groupby'];
         $this->Total->$Gb_geral();
...
//--- 
   ...
   //---- 
   function adiciona_registro($sum_cantidad, $sum_total, $quebra_p_nompro, $quebra_p_nompro_orig)
   {
      //----- Producto
      if (!isset($this->array_total_p_nompro[$quebra_p_nompro_orig]))
      {
         $this->array_total_p_nompro[$quebra_p_nompro_orig][0] = 1;
         $this->array_total_p_nompro[$quebra_p_nompro_orig][1] = $sum_cantidad;
         $this->array_total_p_nompro[$quebra_p_nompro_orig][2] = $sum_total;
         $this->array_total_p_nompro[$quebra_p_nompro_orig][3] = 1;
         $this->array_total_p_nompro[$quebra_p_nompro_orig][4] = $quebra_p_nompro;
         $this->array_total_p_nompro[$quebra_p_nompro_orig][5] = $quebra_p_nompro_orig;
      }
      else
      {
         $this->array_total_p_nompro[$quebra_p_nompro_orig][0]++;
         $this->array_total_p_nompro[$quebra_p_nompro_orig][1] += $sum_cantidad;
         $this->array_total_p_nompro[$quebra_p_nompro_orig][2] += $sum_total;
         $this->array_total_p_nompro[$quebra_p_nompro_orig][3]++;
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     $campos_order = "";
     $format       = $this->Ini->Get_Gb_date_format('', 'p_nompro');
     $campos_order = $this->Ini->Get_date_order_groupby("p.nompro", 'asc', $format, $campos_order);
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->p_nompro_orig = $this->p_nompro;
            nmgp_Form_Num_Val($this->periodo, "", "", "0", "", "", "", "N:", "") ; 
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->cantidad, $this->total, $this->p_nompro, $this->p_nompro_orig);
            }
   }
//
