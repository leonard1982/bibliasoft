...
class grid_rp_productos_vendedor_resumo
{
   ...
   function completeMatrix()
   {
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_order']['por_asesor_producto']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_order']['por_asesor_producto'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_order']['por_asesor_producto'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_asesor_producto'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_asesor_producto'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_asesor_producto'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_asesor_producto'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_asesor_producto'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_asesor_producto'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['summarizing_fields_display']['por_asesor_producto'] as $i_sum => $d_sum)
           {
       ...
       foreach ($this->array_total_v_nombres as $i_v_nombres => $d_v_nombres) {
           ...
           foreach ($this->array_total_p_nompro[$i_v_nombres] as $i_p_nompro => $d_p_nompro) {
               if (!in_array($i_p_nompro, $this->comp_label[1][ $d_v_nombres[5] ], true)) {
                   $this->comp_index[1][ $d_p_nompro[5] ] = $d_p_nompro[4];
                   $this->comp_label[1][ $d_v_nombres[5] ][ $d_p_nompro[5] ] = $d_p_nompro[4];
               }
           }
       }
       ...
       foreach ($this->comp_label[0] as $i_v_nombres => $l_v_nombres) {
           ...
           foreach ($this->comp_label[1][$i_v_nombres] as $i_p_nompro => $l_p_nompro) {
               if (isset($this->array_total_p_nompro[$i_v_nombres][$i_p_nompro])) {
                   $this->comp_group[$i_v_nombres][$i_p_nompro] = array();
               }
           }
       }
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_rp_productos_vendedor']['proc_pdf_vert']){ 
   ...
   function totaliza()
   {
      $this->Total->Calc_resumo_por_asesor_producto("res");
...
//--- 
   ...
   //---- 
   function adiciona_registro($sum_cantidad, $sum_total, $quebra_v_nombres, $quebra_p_nompro, $quebra_v_nombres_orig, $quebra_p_nompro_orig)
   {
      //----- Vendedor
      ...
      //----- Producto
      if (!isset($this->array_total_p_nompro[$quebra_v_nombres_orig][$quebra_p_nompro_orig]))
      {
         $this->array_total_p_nompro[$quebra_v_nombres_orig][$quebra_p_nompro_orig][0] = 1;
         $this->array_total_p_nompro[$quebra_v_nombres_orig][$quebra_p_nompro_orig][1] = $sum_cantidad;
         $this->array_total_p_nompro[$quebra_v_nombres_orig][$quebra_p_nompro_orig][2] = $sum_total;
         $this->array_total_p_nompro[$quebra_v_nombres_orig][$quebra_p_nompro_orig][3] = 1;
         $this->array_total_p_nompro[$quebra_v_nombres_orig][$quebra_p_nompro_orig][4] = $quebra_p_nompro;
         $this->array_total_p_nompro[$quebra_v_nombres_orig][$quebra_p_nompro_orig][5] = $quebra_p_nompro_orig;
      }
      else
      {
         $this->array_total_p_nompro[$quebra_v_nombres_orig][$quebra_p_nompro_orig][0]++;
         $this->array_total_p_nompro[$quebra_v_nombres_orig][$quebra_p_nompro_orig][1] += $sum_cantidad;
         $this->array_total_p_nompro[$quebra_v_nombres_orig][$quebra_p_nompro_orig][2] += $sum_total;
         $this->array_total_p_nompro[$quebra_v_nombres_orig][$quebra_p_nompro_orig][3]++;
      }
   }
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->cantidad, $this->total, $this->v_nombres, $this->p_nompro, $this->v_nombres_orig,  $this->p_nompro_orig);
            }
   }
//
