...
class grid_facturas_TNS_pesq
{
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_facturas_TNS_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_facturas_TNS_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_facturas_TNS_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == 'autocomp_factura')
      {
          $factura = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_factura($factura);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_c_nit')
      {
          $c_nit = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_c_nit($c_nit);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
   ...
   function lookup_ajax_cliente($cliente)
   {
      ...
      $nm_comando = "select distinct C.NOMBRE from " . $this->Ini->nm_tabela . " where K.FECASENTAD IS NOT NULL AND K.FECANULADO IS NULL AND K.TOTAL >0 AND K.CODCOMP='FV' and  C.NOMBRE like '%" . $cliente . "%' order by C.NOMBRE"; 
      unset($cmp1,$cmp2);
   ...
   function lookup_ajax_factura($factura)
   {
      $factura = substr($this->Db->qstr($factura), 1, -1);
            $factura_look = substr($this->Db->qstr($factura), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct K.CODPREFIJO||' '||K.NUMERO from " . $this->Ini->nm_tabela . " where K.FECASENTAD IS NOT NULL AND K.FECANULADO IS NULL AND K.TOTAL >0 AND K.CODCOMP='FV' and  K.CODPREFIJO||' '||K.NUMERO like '%" . $factura . "%' order by K.CODPREFIJO||' '||K.NUMERO"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
   ...
   function lookup_ajax_c_nit($c_nit)
   {
      $c_nit = substr($this->Db->qstr($c_nit), 1, -1);
            $c_nit_look = substr($this->Db->qstr($c_nit), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct C.NIT from " . $this->Ini->nm_tabela . " where K.FECASENTAD IS NOT NULL AND K.FECANULADO IS NULL AND K.TOTAL >0 AND K.CODCOMP='FV' and  C.NIT like '%" . $c_nit . "%' order by C.NIT"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_facturas_TNS/grid_facturas_TNS_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
<?php
$vertical_center = '';
?>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_facturas_TNS_ajax_search.js"></script>
 <script type="text/javascript" src="grid_facturas_TNS_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_facturas_TNS_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_facturas_TNS']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_facturas_TNS']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
function scJQCalendarAdd() {
  $("#sc_k_fecha_jq").datepicker({
    beforeShow: function(input, inst) {
          var_dt_ini  = document.getElementById('SC_k_fecha_dia').value + '/';
          var_dt_ini += document.getElementById('SC_k_fecha_mes').value + '/';
          var_dt_ini += document.getElementById('SC_k_fecha_ano').value;
          document.getElementById('sc_k_fecha_jq').value = var_dt_ini;
    },
    onClose: function(dateText, inst) {
          aParts  = dateText.split("/");
          document.getElementById('SC_k_fecha_dia').value = aParts[0];
          document.getElementById('SC_k_fecha_mes').value = aParts[1];
          document.getElementById('SC_k_fecha_ano').value = aParts[2];
    },
    showWeek: true,
    numberOfMonths: 1,
    changeMonth: true,
  ...
  $("#sc_k_fecha_jq2").datepicker({
    beforeShow: function(input, inst) {
          var_dt_ini  = document.getElementById('SC_k_fecha_input_2_dia').value + '/';
          var_dt_ini += document.getElementById('SC_k_fecha_input_2_mes').value + '/';
          var_dt_ini += document.getElementById('SC_k_fecha_input_2_ano').value;
          document.getElementById('sc_k_fecha_jq2').value = var_dt_ini;
    },
    onClose: function(dateText, inst) {
          aParts  = dateText.split("/");
          document.getElementById('SC_k_fecha_input_2_dia').value = aParts[0];
          document.getElementById('SC_k_fecha_input_2_mes').value = aParts[1];
          document.getElementById('SC_k_fecha_input_2_ano').value = aParts[2];
    },
    showWeek: true,
    numberOfMonths: 1,
    changeMonth: true,
 ...
 function nm_save_form(pos)
 {
  ...
  str_out += 'SC_factura_cond#NMF#' + search_get_sel_txt('SC_factura_cond') + '@NMF@';
  str_out += 'SC_factura#NMF#' + search_get_text('SC_factura') + '@NMF@';
  str_out += 'id_ac_factura#NMF#' + search_get_title('select2-id_ac_factura-container') + '@NMF@';
  str_out += 'SC_k_fecha_cond#NMF#' + search_get_sel_txt('SC_k_fecha_cond') + '@NMF@';
  str_out += 'SC_k_fecha_dia#NMF#' + search_get_sel_txt('SC_k_fecha_dia') + '@NMF@';
  str_out += 'SC_k_fecha_mes#NMF#' + search_get_sel_txt('SC_k_fecha_mes') + '@NMF@';
  str_out += 'SC_k_fecha_ano#NMF#' + search_get_sel_txt('SC_k_fecha_ano') + '@NMF@';
  str_out += 'SC_k_fecha_input_2_dia#NMF#' + search_get_sel_txt('SC_k_fecha_input_2_dia') + '@NMF@';
  str_out += 'SC_k_fecha_input_2_mes#NMF#' + search_get_sel_txt('SC_k_fecha_input_2_mes') + '@NMF@';
  str_out += 'SC_k_fecha_input_2_ano#NMF#' + search_get_sel_txt('SC_k_fecha_input_2_ano') + '@NMF@';
  str_out += 'SC_k_fecha_hor#NMF#' + search_get_sel_txt('SC_k_fecha_hor') + '@NMF@';
  str_out += 'SC_k_fecha_min#NMF#' + search_get_sel_txt('SC_k_fecha_min') + '@NMF@';
  str_out += 'SC_k_fecha_seg#NMF#' + search_get_sel_txt('SC_k_fecha_seg') + '@NMF@';
  str_out += 'SC_k_fecha_input_2_hor#NMF#' + search_get_sel_txt('SC_k_fecha_input_2_hor') + '@NMF@';
  str_out += 'SC_k_fecha_input_2_min#NMF#' + search_get_sel_txt('SC_k_fecha_input_2_min') + '@NMF@';
  str_out += 'SC_k_fecha_input_2_seg#NMF#' + search_get_sel_txt('SC_k_fecha_input_2_seg') + '@NMF@';
  str_out += 'SC_c_nit_cond#NMF#' + search_get_sel_txt('SC_c_nit_cond') + '@NMF@';
  str_out += 'SC_c_nit#NMF#' + search_get_text('SC_c_nit') + '@NMF@';
  str_out += 'id_ac_c_nit#NMF#' + search_get_title('select2-id_ac_c_nit-container') + '@NMF@';
...
<script type="text/javascript">
 $(function() {
  $(".sc-ui-autocomp-factura").on("focus", function() {
  }).on("blur", function() {
   ...
   $("#SC_factura").val(e.params.data.id);
  });
  $(".sc-ui-autocomp-c_nit").on("focus", function() {
  }).on("blur", function() {
  }).on("keydown", function(e) {
   if(e.keyCode == $.ui.keyCode.TAB && $(".ui-autocomplete").filter(":visible").length) {
    e.keyCode = $.ui.keyCode.DOWN;
   ...
   ajax: {
    ...
    data: function (params) {
     var query = {
      q: params.term,
      nmgp_opcao: "ajax_autocomp",
      nmgp_parms: "NM_ajax_opcao?#?autocomp_c_nit",
      max_itens: "10",
      script_case_init: <?php echo $this->Ini->sc_page ?>
     }
     return query;
...
<?php
   ...
   {
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div style="width: 100%">
 ...
 <div style="height:37px; border-width:0px 0px 1px 0px;  border-style: dashed; border-color:#ddd; display: block">
 	<table style="width:100%; border-collapse:collapse; padding:0;">
    	<tr>
        	<td id="lin1_col1" class="scFilterHeaderFont"><span>Facturas TNS</span></td>
...
<?php
   ...
   function monta_form()
   {
      global 
             ...
             $factura_cond, $factura, $factura_autocomp,
             $k_fecha_cond, $k_fecha, $k_fecha_dia, $k_fecha_mes, $k_fecha_ano, $k_fecha_hor, $k_fecha_min, $k_fecha_seg, $k_fecha_input_2_dia, $k_fecha_input_2_mes, $k_fecha_input_2_ano, $k_fecha_input_2_min, $k_fecha_input_2_hor, $k_fecha_input_2_seg,
             $c_nit_cond, $c_nit, $c_nit_autocomp,
             $nm_url_saida, $nm_apl_dependente, $nmgp_parms, $bprocessa, $nmgp_save_name, $NM_operador, $NM_filters, $nmgp_save_option, $NM_filters_del, $Script_BI;
      $Script_BI = "";
      $this->nmgp_botoes['clear'] = "on";
      $this->nmgp_botoes['save'] = "on";
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_facturas_TNS']['btn_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['grid_facturas_TNS']['btn_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['grid_facturas_TNS']['btn_display'] as $NM_cada_btn => $NM_cada_opc)
          {
      ...
      if (isset($_SESSION['scriptcase']['sc_aba_iframe']))
      {
          ...
          {
              if (in_array("grid_facturas_TNS", $apls_aba))
              {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['iframe_menu'] && (!isset($_SESSION['scriptcase']['menu_mobile']) || empty($_SESSION['scriptcase']['menu_mobile'])))
      {
          $this->aba_iframe = true;
      }
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $cliente = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['cliente']; 
          $cliente_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['cliente_cond']; 
          $factura = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['factura']; 
          $factura_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['factura_cond']; 
          $k_fecha_dia = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_dia']; 
          $k_fecha_mes = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_mes']; 
          $k_fecha_ano = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_ano']; 
          $k_fecha_input_2_dia = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2_dia']; 
          $k_fecha_input_2_mes = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2_mes']; 
          $k_fecha_input_2_ano = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2_ano']; 
          $k_fecha_hor = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_hor']; 
          $k_fecha_min = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_min']; 
          $k_fecha_seg = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_seg']; 
          $k_fecha_input_2_hor = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2_hor']; 
          $k_fecha_input_2_min = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2_min']; 
          $k_fecha_input_2_seg = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2_seg']; 
          $k_fecha_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_cond']; 
          $c_nit = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['c_nit']; 
          $c_nit_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['c_nit_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['NM_operador']; 
      } 
      if (!isset($cliente_cond) || empty($cliente_cond))
      {
         $cliente_cond = "qp";
      }
      if (!isset($factura_cond) || empty($factura_cond))
      {
         $factura_cond = "qp";
      }
      if (!isset($k_fecha_cond) || empty($k_fecha_cond))
      {
         $k_fecha_cond = "bw";
      }
      if (!isset($c_nit_cond) || empty($c_nit_cond))
      {
         $c_nit_cond = "qp";
      }
      ...
      $str_hide_factura = (in_array($factura_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_k_fecha = (in_array($k_fecha_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_c_nit = (in_array($c_nit_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;

      $str_display_cliente = ('bw' == $cliente_cond) ? $display_aberto : $display_fechado;
      $str_display_factura = ('bw' == $factura_cond) ? $display_aberto : $display_fechado;
      $str_display_k_fecha = ('bw' == $k_fecha_cond) ? $display_aberto : $display_fechado;
      $str_display_c_nit = ('bw' == $c_nit_cond) ? $display_aberto : $display_fechado;
      ...
      {
         $tmp_pos = strpos($factura, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $factura = substr($factura, 0, $tmp_pos);
         }
      }
      if (!isset($k_fecha) || $k_fecha == "")
      {
          $k_fecha = "";
      }
      if (isset($k_fecha) && !empty($k_fecha))
      {
         $tmp_pos = strpos($k_fecha, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $k_fecha = substr($k_fecha, 0, $tmp_pos);
         }
      }
      if (!isset($c_nit) || $c_nit == "")
      {
          $c_nit = "";
      }
      if (isset($c_nit) && !empty($c_nit))
      {
         $tmp_pos = strpos($c_nit, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $c_nit = substr($c_nit, 0, $tmp_pos);
         }
      }
?>
 <TR align="center">
...
<?php
      ...
      $cliente_look = substr($this->Db->qstr($cliente), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct C.NOMBRE from " . $this->Ini->nm_tabela . " where K.FECASENTAD IS NOT NULL AND K.FECANULADO IS NULL AND K.TOTAL >0 AND K.CODCOMP='FV' and C.NOMBRE = '$cliente_look' order by C.NOMBRE"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
...
?>
<INPUT  type="text" id="SC_cliente" name="cliente" value="<?php echo NM_encode_input($cliente) ?>"  size=50 alt="{datatype: 'text', maxLength: 120, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectOdd sc-ui-autocomp-cliente" id="id_ac_cliente" name="cliente_autocomp"><?php if ('' !=  $cliente) { ?><option value="<?php echo $cliente ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
      ...
      <TD id='SC_factura_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['factura'])) ? $this->New_label['factura'] : "FACTURA"; ?></TD>
      
      <INPUT type="hidden" id="SC_factura_cond" name="factura_cond" value="qp">
 
     <TD colspan=2 class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_factura" <?php echo $str_hide_factura?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['factura'])) ? $this->New_label['factura'] : "FACTURA";
 $nmgp_tab_label .= "factura?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $factura_look = substr($this->Db->qstr($factura), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct K.CODPREFIJO||' '||K.NUMERO from " . $this->Ini->nm_tabela . " where K.FECASENTAD IS NOT NULL AND K.FECANULADO IS NULL AND K.TOTAL >0 AND K.CODCOMP='FV' and K.CODPREFIJO||' '||K.NUMERO = '$factura_look' order by K.CODPREFIJO||' '||K.NUMERO"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$factura]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$factura];
      }
      else
      {
          $sAutocompValue = $factura;
      }
?>
<INPUT  type="text" id="SC_factura" name="factura" value="<?php echo NM_encode_input($factura) ?>"  size=11 alt="{datatype: 'text', maxLength: 11, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectEven sc-ui-autocomp-factura" id="id_ac_factura" name="factura_autocomp"><?php if ('' !=  $factura) { ?><option value="<?php echo $factura ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
      ...
      <TD id='SC_k_fecha_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['k_fecha'])) ? $this->New_label['k_fecha'] : "FECHA"; ?></TD>
      
      <INPUT type="hidden" id="SC_k_fecha_cond" name="k_fecha_cond" value="bw">
 
     <TD colspan=2 class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_k_fecha" <?php echo $str_hide_k_fecha?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['k_fecha'])) ? $this->New_label['k_fecha'] : "FECHA";
 $nmgp_tab_label .= "k_fecha?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
  ...
  {
?>
<span id='id_date_part_k_fecha_DD' style='display: inline-block'>
<INPUT class="sc-js-input scFilterObjectOdd" type="text" id="SC_k_fecha_dia" name="k_fecha_dia" value="<?php echo NM_encode_input($k_fecha_dia); ?>" size="2" alt="{datatype: 'mask', maskList: '99', alignRight: true, maxLength: 2, autoTab: true, enterTab: false}">
</span>
...
<?php
  ...
  {
?>
<span id='id_date_part_k_fecha_MM' style='display: inline-block'>
<INPUT class="sc-js-input scFilterObjectOdd" type="text" id="SC_k_fecha_mes" name="k_fecha_mes" value="<?php echo NM_encode_input($k_fecha_mes); ?>" size="2" alt="{datatype: 'mask', maskList: '99', alignRight: true, maxLength: 2, autoTab: true, enterTab: false}">
</span>
...
<?php
  ...
  {
?>
<span id='id_date_part_k_fecha_AAAA' style='display: inline-block'>
<INPUT class="sc-js-input scFilterObjectOdd" type="text" id="SC_k_fecha_ano" name="k_fecha_ano" value="<?php echo NM_encode_input($k_fecha_ano); ?>" size="4" alt="{datatype: 'mask', maskList: '9999', alignRight: true, maxLength: 4, autoTab: true, enterTab: false}">
 <INPUT type="hidden" id="sc_k_fecha_jq">
</span>
...
?>
        <SPAN id="id_css_k_fecha"  class="scFilterFieldFontOdd">
 <?php echo $date_format_show ?>         </SPAN>
                 </TD>
       </TR>
       <TR valign="top">
        <TD id="id_vis_k_fecha"  <?php echo $str_display_k_fecha; ?> class="scFilterFieldFontOdd">
...
<?php
  ...
  {
?>
<span id='id_date_part_k_fecha_input_2_DD' style='display: inline-block'>
<INPUT class="sc-js-input scFilterObjectOdd" type="text" id="SC_k_fecha_input_2_dia" name="k_fecha_input_2_dia" value="<?php echo NM_encode_input($k_fecha_input_2_dia); ?>" size="2" alt="{datatype: 'mask', maskList: '99', alignRight: true, maxLength: 2, autoTab: true, enterTab: false}">
</span>
...
<?php
  ...
  {
?>
<span id='id_date_part_k_fecha_input_2_MM' style='display: inline-block'>
<INPUT class="sc-js-input scFilterObjectOdd" type="text" id="SC_k_fecha_input_2_mes" name="k_fecha_input_2_mes" value="<?php echo NM_encode_input($k_fecha_input_2_mes); ?>" size="2" alt="{datatype: 'mask', maskList: '99', alignRight: true, maxLength: 2, autoTab: true, enterTab: false}">
</span>
...
<?php
  ...
  {
?>
<span id='id_date_part_k_fecha_input_2_AAAA' style='display: inline-block'>
<INPUT class="sc-js-input scFilterObjectOdd" type="text" id="SC_k_fecha_input_2_ano" name="k_fecha_input_2_ano" value="<?php echo NM_encode_input($k_fecha_input_2_ano); ?>" size="4" alt="{datatype: 'mask', maskList: '9999', alignRight: true, maxLength: 4, autoTab: true, enterTab: false}">
 <INPUT type="hidden" id="sc_k_fecha_jq2">
</span>
...
?>
      ...
      <TD id='SC_c_nit_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['c_nit'])) ? $this->New_label['c_nit'] : "NIT"; ?></TD>
      
      <INPUT type="hidden" id="SC_c_nit_cond" name="c_nit_cond" value="qp">
 
     <TD colspan=2 class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_c_nit" <?php echo $str_hide_c_nit?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['c_nit'])) ? $this->New_label['c_nit'] : "NIT";
 $nmgp_tab_label .= "c_nit?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $c_nit_look = substr($this->Db->qstr($c_nit), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct C.NIT from " . $this->Ini->nm_tabela . " where K.FECASENTAD IS NOT NULL AND K.FECANULADO IS NULL AND K.TOTAL >0 AND K.CODCOMP='FV' and C.NIT = '$c_nit_look' order by C.NIT"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$c_nit]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$c_nit];
      }
      else
      {
          $sAutocompValue = $c_nit;
      }
?>
<INPUT  type="text" id="SC_c_nit" name="c_nit" value="<?php echo NM_encode_input($c_nit) ?>"  size=20 alt="{datatype: 'text', maxLength: 20, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectEven sc-ui-autocomp-c_nit" id="id_ac_c_nit" name="c_nit_autocomp"><?php if ('' !=  $c_nit) { ?><option value="<?php echo $c_nit ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
...
?>
<?php
   if (is_file("grid_facturas_TNS_help.txt"))
   {
      $Arq_WebHelp = file("grid_facturas_TNS_help.txt"); 
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_facturas_TNS']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_facturas_TNS']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
?>
   <INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.factura_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_factura'), document.F1.factura_cond, 'factura');
   document.F1.factura.value = "";
   document.F1.factura_autocomp.value = "";
   $('#select2-id_ac_factura-container').html('<?php echo $this->Val_init_factura['desc'] ?>');;
   document.F1.k_fecha_cond.value = 'bw';
   nm_campos_between(document.getElementById('id_vis_k_fecha'), document.F1.k_fecha_cond, 'k_fecha');
   document.F1.k_fecha_dia.value = "";
   document.F1.k_fecha_mes.value = "";
   document.F1.k_fecha_ano.value = "";
   document.F1.k_fecha_input_2_dia.value = "";
   document.F1.k_fecha_input_2_mes.value = "";
   document.F1.k_fecha_input_2_ano.value = "";
   document.F1.c_nit_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_c_nit'), document.F1.c_nit_cond, 'c_nit');
   document.F1.c_nit.value = "";
   document.F1.c_nit_autocomp.value = "";
   $('#select2-id_ac_c_nit-container').html('<?php echo $this->Val_init_c_nit['desc'] ?>');;
   Sc_carga_select2('all');
 }
 ...
 {
    $('#SC_k_fecha_dia').bind('change', function() {sc_grid_facturas_TNS_valida_dia(this)});
    $('#SC_k_fecha_input_2_dia').bind('change', function() {sc_grid_facturas_TNS_valida_dia(this)});
    $('#SC_k_fecha_input_2_mes').bind('change', function() {sc_grid_facturas_TNS_valida_mes(this)});
    $('#SC_k_fecha_mes').bind('change', function() {sc_grid_facturas_TNS_valida_mes(this)});
 }
 function sc_grid_facturas_TNS_valida_dia(obj)
 {
 ...
 function sc_grid_facturas_TNS_valida_mes(obj)
 {
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "FACILWEB_V2/grid_facturas_TNS";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function recupera_filtro($NM_filters)
   {
      ...
      $tp_fields['SC_factura_cond'] = 'cond';
      $tp_fields['SC_factura'] = 'text_aut';
      $tp_fields['id_ac_factura'] = 'select2_aut';
      $tp_fields['SC_k_fecha_cond'] = 'cond';
      $tp_fields['SC_k_fecha_dia'] = 'text';
      $tp_fields['SC_k_fecha_mes'] = 'text';
      $tp_fields['SC_k_fecha_ano'] = 'text';
      $tp_fields['SC_k_fecha_input_2_dia'] = 'text';
      $tp_fields['SC_k_fecha_input_2_mes'] = 'text';
      $tp_fields['SC_k_fecha_input_2_ano'] = 'text';
      $tp_fields['SC_k_fecha_hor'] = 'text';
      $tp_fields['SC_k_fecha_min'] = 'text';
      $tp_fields['SC_k_fecha_seg'] = 'text';
      $tp_fields['SC_k_fecha_input_2_hor'] = 'text';
      $tp_fields['SC_k_fecha_input_2_min'] = 'text';
      $tp_fields['SC_k_fecha_input_2_seg'] = 'text';
      $tp_fields['SC_c_nit_cond'] = 'cond';
      $tp_fields['SC_c_nit'] = 'text_aut';
      $tp_fields['id_ac_c_nit'] = 'select2_aut';
   ...
   {
      global $cliente_cond, $cliente, $cliente_autocomp,
             $factura_cond, $factura, $factura_autocomp,
             $k_fecha_cond, $k_fecha, $k_fecha_dia, $k_fecha_mes, $k_fecha_ano, $k_fecha_hor, $k_fecha_min, $k_fecha_seg, $k_fecha_input_2_dia, $k_fecha_input_2_mes, $k_fecha_input_2_ano, $k_fecha_input_2_min, $k_fecha_input_2_hor, $k_fecha_input_2_seg,
             $c_nit_cond, $c_nit, $c_nit_autocomp, $nmgp_tab_label;
      ...
      if (!empty($factura_autocomp) && empty($factura))
      {
          $factura = $factura_autocomp;
      }
      if (!empty($c_nit_autocomp) && empty($c_nit))
      {
          $c_nit = $c_nit_autocomp;
      }
      ...
      $factura_cond_salva = $factura_cond; 
      if (!isset($factura_input_2) || $factura_input_2 == "")
      {
          $factura_input_2 = $factura;
      }
      $k_fecha_cond_salva = $k_fecha_cond; 
      if (!isset($k_fecha_input_2_dia) || $k_fecha_input_2_dia == "")
      {
          $k_fecha_input_2_dia = $k_fecha_dia;
      }
      if (!isset($k_fecha_input_2_mes) || $k_fecha_input_2_mes == "")
      {
          $k_fecha_input_2_mes = $k_fecha_mes;
      }
      if (!isset($k_fecha_input_2_ano) || $k_fecha_input_2_ano == "")
      {
          $k_fecha_input_2_ano = $k_fecha_ano;
      }
      if (!isset($k_fecha_input_2_hor) || $k_fecha_input_2_hor == "")
      {
          $k_fecha_input_2_hor = $k_fecha_hor;
      }
      if (!isset($k_fecha_input_2_min) || $k_fecha_input_2_min == "")
      {
          $k_fecha_input_2_min = $k_fecha_min;
      }
      if (!isset($k_fecha_input_2_seg) || $k_fecha_input_2_seg == "")
      {
          $k_fecha_input_2_seg = $k_fecha_seg;
      }
      $c_nit_cond_salva = $c_nit_cond; 
      if (!isset($c_nit_input_2) || $c_nit_input_2 == "")
      {
          $c_nit_input_2 = $c_nit;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['cliente'] = $cliente; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['cliente_cond'] = $cliente_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['factura'] = $factura; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['factura_cond'] = $factura_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_dia'] = $k_fecha_dia; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_mes'] = $k_fecha_mes; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_ano'] = $k_fecha_ano; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2_dia'] = $k_fecha_input_2_dia; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2_mes'] = $k_fecha_input_2_mes; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2_ano'] = $k_fecha_input_2_ano; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_hor'] = $k_fecha_hor; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_min'] = $k_fecha_min; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_seg'] = $k_fecha_seg; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2_hor'] = $k_fecha_input_2_hor; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2_min'] = $k_fecha_input_2_min; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2_seg'] = $k_fecha_input_2_seg; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_cond'] = $k_fecha_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['c_nit'] = $c_nit; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['c_nit_cond'] = $c_nit_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca'] = $Temp_Busca;
      }
      if (!empty($this->Campos_Mens_erro)) 
      {
          return;
    ...
    if ($factura != '') {
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['factura'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['factura'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['factura'] = $factura;
      }
      $nmgp_def_dados = array();
    if ($c_nit != '') {
      $c_nit_look = substr($this->Db->qstr($c_nit), 1, -1); 
      ...
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          ...
          $this->cmp_formatado['c_nit'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['c_nit'] = $c_nit;
      }
      ...
      {
         $this->monta_condicao("C.NOMBRE", $cliente_cond, $cliente, "", "cliente");
      }

      //----- $factura
      $this->Date_part = false;
      if (isset($factura) || $factura_cond == "nu" || $factura_cond == "nn" || $factura_cond == "ep" || $factura_cond == "ne")
      {
         $this->monta_condicao("K.CODPREFIJO||' '||K.NUMERO", $factura_cond, $factura, "", "factura");
      }

      //----- $k_fecha
      $this->Date_part = false;
      if ($k_fecha_cond != "bi_TP")
      {
          $k_fecha_cond = strtoupper($k_fecha_cond);
          $Dtxt = "";
          $val  = array();
          $Dtxt .= $k_fecha_ano;
          $Dtxt .= $k_fecha_mes;
          ...
          $Dtxt .= $k_fecha_min;
          $Dtxt .= $k_fecha_seg;
          $val[0]['ano'] = $k_fecha_ano;
          $val[0]['mes'] = $k_fecha_mes;
          $val[0]['dia'] = $k_fecha_dia;
          $val[0]['hor'] = $k_fecha_hor;
          $val[0]['min'] = $k_fecha_min;
          $val[0]['seg'] = $k_fecha_seg;
          if ($k_fecha_cond == "BW")
          {
              $val[1]['ano'] = $k_fecha_input_2_ano;
              $val[1]['mes'] = $k_fecha_input_2_mes;
              $val[1]['dia'] = $k_fecha_input_2_dia;
              $val[1]['hor'] = $k_fecha_input_2_hor;
              $val[1]['min'] = $k_fecha_input_2_min;
              $val[1]['seg'] = $k_fecha_input_2_seg;
          }
          $this->Operador_date_part = "";
          $this->Lang_date_part     = "";
          $this->nm_prep_date($val, "DH", "TIMESTAMP", $k_fecha_cond, "", "datahora");
          if (!$this->Date_part) {
              $val[0] = $this->Ini->sc_Date_Protect($val[0]);
          }
          $k_fecha = $val[0];
          $this->cmp_formatado['k_fecha'] = $val[0];
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha'] = $val[0];
          $this->nm_data->SetaData($this->cmp_formatado['k_fecha'], "YYYY-MM-DD HH:II:SS");
          $this->cmp_formatado['k_fecha'] = $this->nm_data->FormataSaida($this->nm_data->FormatRegion("DH", "dmY"));
          if ($k_fecha_cond == "BW")
          {
              if (!$this->Date_part) {
                  $val[1] = $this->Ini->sc_Date_Protect($val[1]);
              }
              $k_fecha_input_2     = $val[1];
              $this->cmp_formatado['k_fecha_input_2'] = $val[1];
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']['k_fecha_input_2'] = $val[1];
              $this->nm_data->SetaData($this->cmp_formatado['k_fecha_input_2'], "YYYY-MM-DD HH:II:SS");
              $this->cmp_formatado['k_fecha_input_2'] = $this->nm_data->FormataSaida($this->nm_data->FormatRegion("DH", "dmY"));
          }
          if (!empty($Dtxt) || $k_fecha_cond == "NU" || $k_fecha_cond == "NN"|| $k_fecha_cond == "EP"|| $k_fecha_cond == "NE")
          {
              $this->monta_condicao("K.FECHA", $k_fecha_cond, $k_fecha, $k_fecha_input_2, 'k_fecha', 'TIMESTAMP');
          }
      }

      //----- $c_nit
      $this->Date_part = false;
      if (isset($c_nit) || $c_nit_cond == "nu" || $c_nit_cond == "nn" || $c_nit_cond == "ep" || $c_nit_cond == "ne")
      {
         $this->monta_condicao("C.NIT", $c_nit_cond, $c_nit, "", "c_nit");
      }
   }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturas_TNS']['cond_pesq'] .= $this->NM_operador;
