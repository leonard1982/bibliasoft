<?php
   ...
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['grid_reciboingreso_seleccionar_factura']['glo_nm_path_imag_temp'] . "/sc_apl_default_FACILWEBv_2022.txt"))
       {
?>
...
<?php
   ...
   $Ord_Cmp = new grid_reciboingreso_seleccionar_factura_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_reciboingreso_seleccionar_factura_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_select_orig'] = array();
   }
   ...
   if ($use_alias == "S")
   {
       $tab_def_campos['f_fechaven'] = "f_fechaven";
   }
   else
   {
       $tab_def_campos['f_fechaven'] = "f.fechaven";
   }
   $tab_labels["f_fechaven"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['labels']["f_fechaven"])) ? $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['labels']["f_fechaven"] : "Fecha";
   ...
   {
       $tab_def_campos['documento'] = "concat(f.tipo,'/',r.prefijo,'/',f.numfacven)";
   }
   $tab_labels["documento"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['labels']["documento"])) ? $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['labels']["documento"] : "Documento";
   ...
   {
       $tab_def_campos['cliente'] = "concat(t.documento,'--',t.nombres)";
   }
   $tab_labels["cliente"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['labels']["cliente"])) ? $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['labels']["cliente"] : "Cliente";
   $tab_ger_campos['f_total'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['f_total'] = "f_total";
   }
   else
   {
       $tab_def_campos['f_total'] = "f.total";
   }
   $tab_labels["f_total"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['labels']["f_total"])) ? $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['labels']["f_total"] : "Total";
   $tab_ger_campos['fp'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['fp'] = "fp";
   }
   else
   {
       $tab_def_campos['fp'] = "if(f.credito=2,'CONTADO','CREDITO')";
   }
   $tab_labels["fp"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['labels']["fp"])) ? $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['labels']["fp"] : "Fp";
   ...
   {
       $tab_def_campos['vencimiento'] = "f.fechavenc";
   }
   $tab_labels["vencimiento"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['labels']["vencimiento"])) ? $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['labels']["vencimiento"] : "Vencimiento";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
<HEAD>
 <TITLE>Remisiones/Facturas</TITLE>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_reciboingreso_seleccionar_factura_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_reciboingreso_seleccionar_factura']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
