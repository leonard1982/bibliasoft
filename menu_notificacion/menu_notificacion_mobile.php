...
<html>
    <head>
        ...
        <script src="<?php echo $_SESSION["scriptcase"]["menu_notificacion"]["glo_nm_path_prod"]; ?>/third/jquery.mobile/jquery.mobile-1.2.0.min.js"></script>
    <style>
    <?php
    if(isset($menu_notificacion_menuData["data"]) && is_array($menu_notificacion_menuData["data"]) && !empty($menu_notificacion_menuData["data"]))
    {
        foreach($menu_notificacion_menuData["data"] as $arr_item)
        {
    ...
    <body>
    <?php
        escreveMenuMobile($menu_notificacion_menuData["data"], $this->path_imag_apl);
    ?>
	<?php
