...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['control_codbarras_filtro_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="control_codbarras_filtro_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function sc_btn_resetar() 
{ 
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_resetar(); }, 500);
      return;
    }
    scJs_confirm("<?php echo html_entity_decode('¿Desea limpiar el formulario?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_resetar_ok, sc_btn_resetar_cancel);
} 
function sc_btn_resetar_cancel()
{
}
function sc_btn_resetar_ok()
{
    location.href="../control_codbarras_filtro";
}
 NM_tp_critica(1);
...
function nm_saida_glo()
{
  ...
  document.F4.action = "control_codbarras_filtro_mob_fim.php";
  document.F4.submit();
}
