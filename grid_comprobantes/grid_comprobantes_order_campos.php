...
<?php
   ...
   $Ord_Cmp = new grid_comprobantes_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_comprobantes_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_select_orig'] = array();
   }
   $this->restore = isset($_POST['restore']) ? true : false;
   if ($this->restore && !class_exists('Services_JSON')) {
       include_once("grid_comprobantes_json.php");
   }
   ...
   $tab_labels["periodo"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["periodo"])) ? $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["periodo"] : "Periodo";
   $tab_ger_campos['creado'] = "on";
   $tab_def_campos['creado'] = "creado";
   $tab_labels["creado"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["creado"])) ? $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["creado"] : "Creado";
   $tab_ger_campos['fecha'] = "on";
   $tab_def_campos['fecha'] = "fecha";
   $tab_labels["fecha"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["fecha"])) ? $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["fecha"] : "Fecha";
   $tab_ger_campos['tipo'] = "on";
   $tab_def_campos['tipo'] = "tipo";
   $tab_labels["tipo"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["tipo"])) ? $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["tipo"] : "Tipo";
   $tab_ger_campos['prefijo'] = "on";
   $tab_def_campos['prefijo'] = "prefijo";
   $tab_labels["prefijo"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["prefijo"])) ? $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["prefijo"] : "PJ";
   $tab_ger_campos['numero'] = "on";
   $tab_def_campos['numero'] = "numero";
   $tab_labels["numero"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["numero"])) ? $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["numero"] : "N°";
   $tab_ger_campos['total_debito'] = "on";
   $tab_def_campos['total_debito'] = "total_debito";
   $tab_labels["total_debito"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["total_debito"])) ? $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["total_debito"] : "Total";
   $tab_ger_campos['observaciones'] = "on";
   $tab_def_campos['observaciones'] = "observaciones";
   $tab_labels["observaciones"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["observaciones"])) ? $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["observaciones"] : "Observaciones";
   $tab_ger_campos['importado'] = "on";
   $tab_def_campos['importado'] = "importado";
   $tab_labels["importado"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["importado"])) ? $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['labels']["importado"] : "Exportado";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_comprobantes_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_comprobantes']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
