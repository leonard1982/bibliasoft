...
class grid_comprobantes_resumo
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   var $conv_col;
   var $count_ger;
   var $sum_total_debito;
   var $sum_total_credito;
   var $sc_proc_quebra_fecha;
   var $count_fecha;
   var $sum_fecha_total_debito;
   var $sum_fecha_total_credito;
   ...
   function initializeButtons()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['dashboard_info']['under_dashboard'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['dashboard_info']['maximized']) {
          $tmpDashboardApp = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['dashboard_info']['dashboard_app'];
          if (isset($_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['grid_comprobantes'])) {
              $tmpDashboardButtons = $_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['grid_comprobantes'];
      ...
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_comprobantes']['btn_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['grid_comprobantes']['btn_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['grid_comprobantes']['btn_display'] as $NM_cada_btn => $NM_cada_opc)
          {
   ...
   { 
	...
	function generateChartImages() {
		...
		$this->Graf         = new grid_comprobantes_grafico();
		$this->Graf->Db     = $this->Db;
		$this->Graf->Erro   = $this->Erro;
		$this->Graf->Ini    = $this->Ini;
		$this->Graf->Lookup = $this->Lookup;

		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_step']     = 'image';
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_count']    = 0;
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_total']    = 0;
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_step'] = 'end';
	...
	function zipFileList($fileList, $password = '') {
		$tempDir     = $this->Ini->root . $_SESSION['scriptcase']['grid_comprobantes']['glo_nm_path_imag_temp'] . '/';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['grid_comprobantes']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
   ...
   function monta_resumo($b_export = false)
   {
      ...
      $this->sc_where_orig   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['where_orig'];
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['where_pesq'];
      $this->sc_where_filtro = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['where_pesq_filtro'];
     if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['campos_busca']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['campos_busca']))
     { 
         $Busca_temp = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['campos_busca'];
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] != "pdf")
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['where_resumo']);
      }
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['res_hrz'];
      $dir_raiz          = strrpos($_SERVER['PHP_SELF'],"/") ;  
      $dir_raiz          = substr($_SERVER['PHP_SELF'], 0, $dir_raiz + 1) ;  
      $this->nm_location = $this->Ini->sc_protocolo . $this->Ini->server . $dir_raiz; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['where_resumo'] = "";
      }
      $this->inicializa_vars();
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['SC_Ind_Groupby'];
      $this->Total->$Gb_geral();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['contr_total_geral'] == "OK")
      {
          $this->array_total_fecha = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['arr_total']['fecha'];
      }
      else
      {
          $this->array_total_fecha = array();
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['tot_geral'][1];
      $this->array_total_geral[1] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['tot_geral'][2];
      $this->array_total_geral[2] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['tot_geral'][3];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['tot_geral'][1] == 0)
      {
      ...
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] == 'print' || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] == 'pdf') && strpos(" " . $this->Ini->SC_module_export, "resume") === false)
      { }
      else
      {
          $this->drawMatrix();
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['arr_total']['fecha'] = $this->array_total_fecha;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['json_use_label'])
       {
          $Lab_fecha = "fecha";
       }
       else
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['periodo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['periodo'] = "Periodo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['creado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['creado'] = "Creado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['fecha']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['fecha'] = "Fecha"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['tipo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['tipo'] = "Tipo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['prefijo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['prefijo'] = "PJ"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['numero']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['numero'] = "N°"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['total_debito']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['total_debito'] = "Total"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['observaciones']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['observaciones'] = "Observaciones"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['detalle']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['detalle'] = ""; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['importado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['importado'] = "Exportado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['idcomprobante']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['idcomprobante'] = "Idcomprobante"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['total_credito']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['total_credito'] = "Total Credito"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['asentada']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['asentada'] = "Asentada"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['sucursal']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['sucursal'] = "Sucursal"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['usuario']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['usuario'] = "Usuario"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['editado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['labels']['editado'] = "Editado"; 
       }
       //-----
       $this->comp_field_nm = array(
           'fecha' => 0,
       ...
       $this->comp_sum = array(
           ...
           4 => "Total Credito(" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sumt'] . ")",
       );
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_order']['fecha']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_order']['fecha'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_order']['fecha'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['fecha'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['fecha'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['fecha'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['fecha'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['fecha'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['fecha'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['fecha'] as $i_sum => $d_sum)
           {
       ...
       $this->comp_sum_lnk = array(
           ...
           3 => array('field' => "total_debito", 'show' => true),
           4 => array('field' => "total_credito", 'show' => true),
       );

       //-----
       $this->comp_sum_css = array(
           2 => "css___nm_count_nm___cnt",
           3 => "css_total_debito_sum",
           4 => "css_total_credito_sum",
       );
       ...
       foreach ($this->array_total_fecha as $i_fecha => $d_fecha) {
           if (!in_array($i_fecha, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_fecha[5] ] = $d_fecha[4];
               $this->comp_label[0][ $d_fecha[5] ] = $d_fecha[4];
           }
       }
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_col'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_level'] = -1;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_sort'] = '';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_col']  = 0;
           }
           ...
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_tabular_label'];
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@grid_comprobantes@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function makeTabular(&$a_data)
   {
       ...
       {
           $a_labels = array();
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['proc_pdf_vert'])
   {
   ...
   function buildChart()
   {
       ...
       $this->comp_chart_config = array(
           ...
           '0|3' => array(
               'title'    => $this->getChartText(strip_tags("Total Credito(" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sumt'] . ")")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'fecha',
               'field_y'  => $this->comp_sum_nm[4],
               'label_x'  => $this->getChartText(strip_tags($Lab_fecha)),
               'label_y'  => $this->getChartText(strip_tags("Total Credito(" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sumt'] . ")")),
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_charts']    = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_orig']      = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['comb_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['comb_table_def']  = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['anlt_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['anlt_table_def']  = array();
	...
	function filterChartsByGroupbyLevel() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['chart_level']) && '' !== $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['chart_level']) {
			$applyFilter = true;
		}
		elseif (isset($nmgp_opcao) && 'image_res' == $nmgp_opcao && isset($_POST['chart_level']) && '' !== $_POST['chart_level']) {
			$applyFilter = true;
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['chart_level'] = $_POST['chart_level'];
		}
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_charts'] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_orig'];

		if ($applyFilter) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_charts'] as $chartIndex => $chartData) {
				...
				if ($groupbyLevel > $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['chart_level']) {
					unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_charts'][$chartIndex]);
				}
			}
		}

		$this->comp_chart_data = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_charts'];
	}
           ...
           foreach ($a_totals as $i_total => $values)
           {
               ...
               if (isset($this->comp_chart_config[$key_config]))
               {
                   ...
                   foreach ($a_keys as $tmp_key)
                   {
                       ...
                       $xml_links[]  = $this->getFusionLink('sc_grid_comprobantes_' . session_id() . '_!!!' . implode('_', array_merge(array($group + 1, $i_total), $link_index)) . '!!!.json');
                   }
                   ...
                   $this->comp_chart_data[$key_data]['xml']        = $this->getFusionLink('sc_grid_comprobantes_' . session_id() . '_!!!' . implode('_', array_merge(array($group, $i_total), $param)) . '!!!.json');
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['arr_export']['data']  = $this->build_data;
           return;
       }

       $nm_saida->saida("<tr id=\"summary_body\">\r\n");
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['ajax_nav'])
       { 
           $_SESSION['scriptcase']['saida_html'] = "";
       } 
      $TD_padding = (!$this->Print_All && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] == "pdf") ? " style=\"padding: 0px !important;\"" : "";
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               if (0 <= $columns['level'])
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['embutida'])
                   {
               ...
               $chart   = (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] != "print" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['embutida']) && isset($columns['chart']) && '' != $columns['chart'] && isset($this->comp_chart_data[ $columns['chart'] ]))
   ...
   function drawMatrixLabels()
   {
       ...
       $nm_saida->saida("    <script type=\"text/javascript\">\r\n");
       if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['embutida'])
       { 
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   ...
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialDisplay = '; display: none';
                           $sInitialSrc     = '';
                       }
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['proc_pdf_vert']){ 
   ...
   function resumo_init()
   {
      ...
      if ("out" == $this->NM_tipo)
      {
         $this->monta_html_ini();
         $this->monta_cabecalho();
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] != "pdf" && !$_SESSION['scriptcase']['proc_mobile'])
         {
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['grid_comprobantes']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['grid_comprobantes']) . "_";
               } 
           } 
           else 
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['grid_comprobantes']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['grid_comprobantes']) . "_";
               } 
           }
       }
   ...
   function inicializa_vars()
   {
      ...
      if ("out" == $this->NM_tipo)
      {
         require_once($this->Ini->path_aplicacao . "grid_comprobantes_total.class.php"); 
      }
      $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['print_all'];
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['doc_word'] || $this->Ini->sc_export_ajax_img)
      { 
      ...
      if ($this->Print_All)
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] = "print";
      ...
      $this->Total   = new grid_comprobantes_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['arr_total'][$cmp_gb];
      }
...
if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['doc_word'])
{
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['dashboard_refresh_after_chart'] = 'resumo';
      $nm_saida->saida("<HTML" . $_SESSION['scriptcase']['reg_conf']['html_dir'] . ">\r\n");
      $nm_saida->saida("<HEAD>\r\n");
      $nm_saida->saida(" <TITLE>Comprobantes de Contabilidad</TITLE>\r\n");
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['doc_word'])
{
      ...
      { 
           ...
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"grid_comprobantes_ajax.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida(" </script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"grid_comprobantes_jquery-3.6.0.min.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"grid_comprobantes_message.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput2.js\"></script>\r\n");
...
$nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "grid_comprobantes/grid_comprobantes_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
           $nm_saida->saida("   <link rel=\"stylesheet\" href=\"" . $this->Ini->path_prod . "/third/jquery_plugin/thickbox/thickbox.css\" type=\"text/css\" media=\"screen\" />\r\n");
           $nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/buttons/" . $this->Ini->Str_btn_css . "\" /> \r\n");
      } 
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['num_css']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['num_css'] = rand(0, 1000);
      }
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_comprobantes_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_grid_comprobantes_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_comprobantes_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['num_css'] . '.css');
...
$nm_saida->saida("      url: \"grid_comprobantes_save_grid.php\",\r\n");
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if (!$this->Ini->Export_html_zip && $this->Print_All && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['doc_word']) 
      {
      ...
      {
          $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['dashboard_info']['remove_margin'] ? 'margin: 0;' : '';
          $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['dashboard_info']['remove_border'] ? 'border-width: 0;' : '';
          $vertical_center = '';
          $nm_saida->saida(" <BODY id=\"grid_summary\" class=\"" . $this->css_scGridPage . "\" style=\"" . $remove_margin . $vertical_center . "\">\r\n");
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['embutida'])
      {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['embutida'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['doc_word'])
      { 
          $nm_saida->saida("      <STYLE>\r\n");
   ...
   function monta_barra_top_normal()
   {
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['word'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bword", "", "", "Rword_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_comprobantes/grid_comprobantes_config_word.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_cor=AM&nm_res_cons=s&nm_ini_word_res=resume&nm_all_modules=grid,resume,chart&password=n&origem=res&language=es&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['print'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Rprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_comprobantes/grid_comprobantes_config_print.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_opc=resumo&nm_cor=PB&nm_res_cons=s&nm_ini_prt_res=resume&nm_all_modules=grid,resume,chart&password=n&origem=res&language=es&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      if ($this->nmgp_botoes['chart_conf'] == 'on' && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bconf_graf", "", "", "Rgraf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_comprobantes/grid_comprobantes_config_graf_flash.php?nome_apl=grid_comprobantes&campo_apl=nm_resumo_graf&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=es&tp_apl=cns_htm&KeepThis=true&TB_iframe=true&height=450&width=400&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
      }
      ...
      if ($this->nmgp_botoes['groupby'] == "on" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['opc_psq'] && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
          $Q_free  = false;
          $Q_count = 0;
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['SC_All_Groupby'] as $QB => $Tp)
          {
              if (!in_array($QB, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['SC_Groupby_hide']) && $Tp == "all")
              {
                  $Q_count++;
          ...
          if ($Q_count > 1 || $Q_free)
          {
             $this->nm_btn_exist['groupby'][] = "sel_groupby_top";
              $Cod_Btn = nmButtonOutput($this->arr_buttons, "bgroupby", "scBtnGroupByShow('" . $this->Ini->path_link . "grid_comprobantes/grid_comprobantes_sel_groupby.php?opc_ret=resumo&path_img=" . $this->Ini->path_img_global . "&path_btn=" . $this->Ini->path_botoes . "&script_case_init=" . NM_encode_input($this->Ini->sc_page) . "&embbed_groupby=Y&toolbar_pos=top', 'top');", "scBtnGroupByShow('" . $this->Ini->path_link . "grid_comprobantes/grid_comprobantes_sel_groupby.php?opc_ret=resumo&path_img=" . $this->Ini->path_img_global . "&path_btn=" . $this->Ini->path_botoes . "&script_case_init=" . NM_encode_input($this->Ini->sc_page) . "&embbed_groupby=Y&toolbar_pos=top', 'top');", "sel_groupby_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "", "", "", "only_text", "text_right", "", "", "", "", "", "", "");
              $nm_saida->saida("           $Cod_Btn \r\n");
          }
              $NM_btn = true;
      }
      if ($this->nmgp_botoes['chart_detail'] == 'on' && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
         ...
         if (is_file("grid_comprobantes_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
            $Arq_WebHelp = file("grid_comprobantes_help.txt"); 
   ...
   function monta_html_fim()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['print_all'])
      {
          $this->grafico_pdf();
      }
      $nm_saida->saida("</TABLE>\r\n");
      $nm_saida->saida("<link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/css/" . $this->Ini->str_schema_all . "_sweetalert.css\" />\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['grid_comprobantes']['glo_nm_path_prod'] . "/third/sweetalert/sweetalert2.all.min.js\"></script>\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['grid_comprobantes']['glo_nm_path_prod'] . "/third/sweetalert/polyfill.min.js\"></script>\r\n");
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'grid_comprobantes_config_pivot.php?nome_apl=grid_comprobantes&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=es&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   $nm_saida->saida("                  action=\"grid_comprobantes_iframe_prt.php\" \r\n");
   ...
   $nm_saida->saida("   }\r\n");
      ...
      $nm_saida->saida("               nm_submit_modal(\"" . $this->Ini->path_link . "grid_comprobantes/grid_comprobantes_export_email.php?script_case_init={$this->Ini->sc_page}&path_img={$this->Ini->path_img_global}&path_btn={$this->Ini->path_botoes}&sType=\"+ str_type +\"&sAdd=__E__nmgp_cor_word=\" + cor + \"__E__SC_module_export=\" + SC_module_export + \"__E__nmgp_password=\" + password + \"&KeepThis=true&TB_iframe=true&modal=true\", bol_param);\r\n");
      $nm_saida->saida("       }\r\n");
      $nm_saida->saida("       else\r\n");
      $nm_saida->saida("       {\r\n");
      ...
      $nm_saida->saida("               document.Fdoc_word.action = \"grid_comprobantes_export_ctrl.php\";\r\n");
      ...
      $nm_saida->saida("               document.Fprint.action = \"grid_comprobantes_export_ctrl.php\";\r\n");
      $nm_saida->saida("           }\r\n");
      $nm_saida->saida("           else\r\n");
      $nm_saida->saida("           {\r\n");
      ...
      $nm_saida->saida("               document.Fexport.action = \"grid_comprobantes_export_ctrl.php\";\r\n");
      ...
      $nm_saida->saida("       document.F3.SC_module_export.value = z;\r\n");
      $nm_saida->saida("  }\r\n");
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['grid_comprobantes_iframe_params'] = array(
      ...
      $Md5_pdf    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@grid_comprobantes@SC_par@" . md5($prep_parm_pdf);
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['Md5_pdf'][md5($prep_parm_pdf)] = $prep_parm_pdf;
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_comprobantes_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "grid_comprobantes/grid_comprobantes_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['print_all'];
       if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pdf_res'])
       {
           $nm_saida->saida("<TR>\r\n");
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
...
//--- 
 function grafico_pdf()
 {
   ...
   $this->Graf->Ini    = $this->Ini;
   $this->Graf->Lookup = $this->Lookup;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_charts']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['skip_charts']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['skip_charts']))
   {
       ...
       $iChartCount = 1;
       $iChartTotal = sizeof($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_charts']);
 ...
 { 
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['pivot_charts'] as $chart_index => $chart_data)
       {
           if (!$prim_graf)
           {
 ...
 { 
           ...
           if ($this->progress_fp)
           {
               grid_comprobantes_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "...\n", $this->Ini->Nm_lang, true);
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['this_chart_label'] = '';
       ...
       if ($bChartFP)
       {
           ...
           grid_comprobantes_pdf_progress_call(100 . "_#NM#_" . 90 . "_#NM#_" . $lang_protect . "...\n", $this->Ini->Nm_lang);
           fwrite($this->progress_fp, 90 . "_#NM#_" . $lang_protect . "...\n");
           fclose($this->progress_fp);
       }
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['charts_html'])
       {
            $nm_saida->saida("<script type=\"text/javascript\">\r\n");
            $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['charts_html']}\r\n");
            $nm_saida->saida("</script>\r\n");
       }
   }
       $nm_saida->saida("</body>\r\n");
...
//--- 
 ...
 {
   global $nm_saida, $nm_lang;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['chart_list']))
   {
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['chart_list'] as $arr_chart)
       {
           $nm_saida->saida("<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>\r\n");
       $nm_saida->saida("<b><div style=\"height:1px;overflow:hidden\"><H1 style=\"font-size:0;padding:1px\">" . $this->Ini->Nm_lang['lang_btns_chrt_pdff_hint'] . "</H1></div></b>\r\n");
           $nm_saida->saida("<table><tr><td>\r\n");
...
//--- 
   ...
   {
      global $nm_saida;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['embutida'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['dashboard_info']['compact_mode'])
      { 
          return;
      } 
      ...
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['cond_pesq']))
      {  
          $pos       = 0;
          $trab_pos  = false;
          $pos_tmp   = true; 
          $tmp       = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['cond_pesq'];
          while ($pos_tmp)
          {
             $pos = strpos($tmp, "##*@@", $pos);
             if ($pos !== false)
          ...
          $nm_cond_filtro_or  = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['cond_pesq'], $trab_pos + 5) == "or")  ? " " . trim($this->Ini->Nm_lang['lang_srch_orr_cond']) . " " : "";
          $nm_cond_filtro_and = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['cond_pesq'], $trab_pos + 5) == "and") ? " " . trim($this->Ini->Nm_lang['lang_srch_and_cond']) . " " : "";
          $nm_cab_filtro   = substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['cond_pesq'], 0, $trab_pos);
     ...
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['proc_pdf_vert']) {
      ...
      $nm_saida->saida("          Comprobantes de Contabilidad\r\n");
      $nm_saida->saida("        </TD>\r\n");
   ...
   //---- 
   function adiciona_registro($sum_total_debito, $sum_total_credito, $quebra_fecha, $quebra_fecha_orig)
   {
      ...
      if (!isset($this->array_total_fecha[$quebra_fecha_orig]))
      {
         ...
         $this->array_total_fecha[$quebra_fecha_orig][1] = $sum_total_debito;
         $this->array_total_fecha[$quebra_fecha_orig][2] = $sum_total_credito;
         $this->array_total_fecha[$quebra_fecha_orig][3] = 1;
         $this->array_total_fecha[$quebra_fecha_orig][4] = $quebra_fecha;
         $this->array_total_fecha[$quebra_fecha_orig][5] = $quebra_fecha_orig;
      }
      else
      {
         ...
         $this->array_total_fecha[$quebra_fecha_orig][1] += $sum_total_debito;
         $this->array_total_fecha[$quebra_fecha_orig][2] += $sum_total_credito;
         $this->array_total_fecha[$quebra_fecha_orig][3]++;
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     { 
         $nmgp_select = "SELECT periodo, str_replace (convert(char(10),creado,102), '.', '-') + ' ' + convert(char(8),creado,20), str_replace (convert(char(10),fecha,102), '.', '-') + ' ' + convert(char(8),fecha,20), tipo, prefijo, numero, total_debito, observaciones, importado, idcomprobante, total_credito from " . $this->Ini->nm_tabela; 
     } 
     elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mysql))
     { 
         $nmgp_select = "SELECT periodo, creado, fecha, tipo, prefijo, numero, total_debito, observaciones, importado, idcomprobante, total_credito from " . $this->Ini->nm_tabela; 
     } 
     elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
     { 
         $nmgp_select = "SELECT periodo, convert(char(23),creado,121), convert(char(23),fecha,121), tipo, prefijo, numero, total_debito, observaciones, importado, idcomprobante, total_credito from " . $this->Ini->nm_tabela; 
     } 
     else 
     { 
         $nmgp_select = "SELECT periodo, creado, fecha, tipo, prefijo, numero, total_debito, observaciones, importado, idcomprobante, total_credito from " . $this->Ini->nm_tabela; 
     } 
     $nmgp_select .= " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['where_pesq']; 
     $campos_order = "";
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->prefijo = $rs_res->fields[4] ;  
            $this->numero = $rs_res->fields[5] ;  
            $this->total_debito = $rs_res->fields[6] ;  
            $this->total_debito =  str_replace(",", ".", $this->total_debito);
            $this->observaciones = $rs_res->fields[7] ;  
            $this->importado = $rs_res->fields[8] ;  
            $this->idcomprobante = $rs_res->fields[9] ;  
            $this->total_credito = $rs_res->fields[10] ;  
            $this->total_credito =  str_replace(",", ".", $this->total_credito);
            $this->look_periodo = $this->periodo; 
            $this->Lookup->lookup_periodo($this->look_periodo); 
            $_SESSION['scriptcase']['grid_comprobantes']['contr_erro'] = 'on';
 if($this->importado =='SI')
{
	$this->NM_field_style["importado"] = "background-color:#33ff99;font-size:15px;color:#000000;font-family:arial;font-weight:sans-serif;";
}

switch($this->tipo )
{
	case 'FV':
		$this->NM_field_style["tipo"] = "background-color:#29abe2;font-size:15px;color:#000000;font-family:arial;font-weight:sans-serif;";
	break;
...
$_SESSION['scriptcase']['grid_comprobantes']['contr_erro'] = 'off'; 
            ...
            $this->periodo = $this->look_periodo ;
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->total_debito, $this->total_credito, $this->fecha, $this->fecha_orig);
            }
   }
//
