...
class grid_comprobantes_resumo
{
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      {
          $this->array_total_prefijo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['arr_total']['prefijo'];
      }
      else
      {
          $this->array_total_prefijo = array();
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['arr_total']['prefijo'] = $this->array_total_prefijo;
   ...
   function completeMatrix()
   {
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_order']['prefijo']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_order']['prefijo'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_order']['prefijo'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['prefijo'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['prefijo'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['prefijo'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['prefijo'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['prefijo'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['prefijo'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['summarizing_fields_display']['prefijo'] as $i_sum => $d_sum)
           {
       ...
       //-----
       foreach ($this->array_total_prefijo as $i_prefijo => $d_prefijo) {
           if (!in_array($i_prefijo, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_prefijo[5] ] = $d_prefijo[4];
               $this->comp_label[0][ $d_prefijo[5] ] = $d_prefijo[4];
           }
       }
       ...
       //-----
       $this->comp_links_fl = array(
           array('name' => 'prefijo', 'prot' => '@aspass@'),
       );
       ...
       //-----
       foreach ($this->comp_label[0] as $i_prefijo => $l_prefijo) {
           if (isset($this->array_total_prefijo[$i_prefijo])) {
               $this->comp_group[$i_prefijo] = array();
           }
       }
   ...
   function getCompData($level, $params = array()) {
       if (0 == $level) {
           $return = $this->array_total_prefijo;
       }
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['proc_pdf_vert']){ 
   ...
   function totaliza()
   {
      ...
      ksort($this->array_total_prefijo);
      if ("out" == $this->NM_tipo)
      {
         $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_comprobantes']['SC_Ind_Groupby'];
         $this->Total->$Gb_geral();
...
//--- 
   ...
   //---- 
   function adiciona_registro($sum_total_debito, $sum_total_credito, $quebra_prefijo, $quebra_prefijo_orig)
   {
      //----- PJ
      if (!isset($this->array_total_prefijo[$quebra_prefijo_orig]))
      {
         $this->array_total_prefijo[$quebra_prefijo_orig][0] = 1;
         $this->array_total_prefijo[$quebra_prefijo_orig][1] = $sum_total_debito;
         $this->array_total_prefijo[$quebra_prefijo_orig][2] = $sum_total_credito;
         $this->array_total_prefijo[$quebra_prefijo_orig][3] = 1;
         $this->array_total_prefijo[$quebra_prefijo_orig][4] = $quebra_prefijo;
         $this->array_total_prefijo[$quebra_prefijo_orig][5] = $quebra_prefijo_orig;
      }
      else
      {
         $this->array_total_prefijo[$quebra_prefijo_orig][0]++;
         $this->array_total_prefijo[$quebra_prefijo_orig][1] += $sum_total_debito;
         $this->array_total_prefijo[$quebra_prefijo_orig][2] += $sum_total_credito;
         $this->array_total_prefijo[$quebra_prefijo_orig][3]++;
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     $campos_order = "";
     $format       = $this->Ini->Get_Gb_date_format('', 'prefijo');
     $campos_order = $this->Ini->Get_date_order_groupby("prefijo", 'asc', $format, $campos_order);
...
$_SESSION['scriptcase']['grid_comprobantes']['contr_erro'] = 'off'; 
            ...
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->total_debito, $this->total_credito, $this->prefijo, $this->prefijo_orig);
            }
   }
//
