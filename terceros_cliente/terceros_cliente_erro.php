...
$old_error_handler = set_error_handler("terceros_cliente_scriptcase_error_handler");

function terceros_cliente_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['terceros_cliente']['contr_erro']) && $_SESSION['scriptcase']['terceros_cliente']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_terceros_cliente->contr_terceros_cliente->Apl_com_erro = true;
            if ($inicial_terceros_cliente->contr_terceros_cliente->NM_ajax_flag)
            {
                $inicial_terceros_cliente->contr_terceros_cliente->NM_ajax_info['errList']['geral_terceros_cliente'][] = $msg;
                return;
            }
            terceros_cliente_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function terceros_cliente_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['terceros_cliente'])  && '' != $_SESSION['scriptcase']['error_icon']['terceros_cliente'])  ? $_SESSION['scriptcase']['error_icon']['terceros_cliente']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['terceros_cliente']) && '' != $_SESSION['scriptcase']['error_close']['terceros_cliente']) ? $_SESSION['scriptcase']['error_close']['terceros_cliente'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['terceros_cliente']) && '' != $_SESSION['scriptcase']['error_span_title']['terceros_cliente']) ? $_SESSION['scriptcase']['error_span_title']['terceros_cliente'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['terceros_cliente']) && '' != $_SESSION['scriptcase']['error_icon_title']['terceros_cliente']) ? $_SESSION['scriptcase']['error_icon_title']['terceros_cliente'] : '';
?>
