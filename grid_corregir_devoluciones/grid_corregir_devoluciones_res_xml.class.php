...
class grid_corregir_devoluciones_res_xml
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function monta_xml()
   {
      ...
      $this->grava_arquivo();
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_res_grid']))
      {
          return;
      }
   ...
   function inicializa_vars()
   {
      ...
      $this->Arquivo      .= ".xml";
      $this->Tit_doc       = "grid_corregir_devoluciones.xml";
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_name']))
      {
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_name'], ".");
          if ($Pos === false) {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_name'] .= ".xml";
          }
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_name']);
      }
      if (!$this->Grava_view)
      {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_res_grid']) && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_return']);
   ...
   function grava_arquivo()
   {
      ...
      if ($this->New_Format)
      {
          fwrite($xml_f, "<grid_corregir_devoluciones>\r\n");
      }
      if ($this->Grava_view)
      {
          ...
          if ($this->New_Format)
          {
              fwrite($xml_v, "<grid_corregir_devoluciones>\r\n");
          }
      }
      ...
      $this->array_titulos = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['arr_export']['label'];
      $this->array_linhas  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['arr_export']['data'];
      $contr_rowspan = array();
      $tit_rowspan   = array();
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_res_grid']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_res_file']['xml'] = $this->Xml_f;
          if ($this->Grava_view)
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_res_file']['view'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo_view;
          }
      }
   ...
   function grava_linha($xml_f)
   {
      ...
      foreach ($this->array_linhas as $lines)
      {
          ...
          if (!$this->New_Format)
          {
              $xml_registro = "<grid_corregir_devoluciones";
          }
          foreach ($lines as $columns)
          {
              if (0 <= $columns['level'])
...
<INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['grid_corregir_devoluciones']['xml_return']); ?>"> 
</FORM> 
</td></tr></table>
</BODY>
</HTML>
...
if(!empty($ds[0][0]))
	...
	$desc=$ds[0][2];
     ...
     $nm_select = "UPDATE devventa SET vunit=$total, vparc=$sub, viva=$iva, vdesc=$desc WHERE numerodev=$a order by numerodev desc limit 1"; 
         $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_select;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
         $rf = $this->Db->Execute($nm_select);
         if ($rf === false)
