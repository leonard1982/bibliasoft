...
class grid_tmp_insertar_defisico_pesq
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_tmp_insertar_defisico_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_tmp_insertar_defisico_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_tmp_insertar_defisico_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == 'autocomp_fisicoid')
      {
          $fisicoid = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_fisicoid($fisicoid);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_matid')
      {
          $matid = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_matid($matid);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_canlista')
      {
          $canlista = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_canlista($canlista);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_tipund')
      {
          $tipund = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_tipund($tipund);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
   ...
   function lookup_ajax_fisicoid($fisicoid)
   {
      $fisicoid = substr($this->Db->qstr($fisicoid), 1, -1);
            $fisicoid_look = substr($this->Db->qstr($fisicoid), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct FISICOID from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where  FISICOID like '%" . $fisicoid . "%'"; 
      unset($cmp1,$cmp2);
   ...
   function lookup_ajax_matid($matid)
   {
      $matid = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $matid);
      $matid = substr($this->Db->qstr($matid), 1, -1);
            $matid_look = substr($this->Db->qstr($matid), 1, -1); 
      $nmgp_def_dados = array(); 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct MATID from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where   CAST (MATID AS TEXT)  like '%" . $matid . "%'"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct MATID from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where   CAST (MATID AS VARCHAR)  like '%" . $matid . "%'"; 
      }
      else
      {
          $nm_comando = "select distinct MATID from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where  MATID like '%" . $matid . "%'"; 
      }
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
   ...
   function lookup_ajax_canlista($canlista)
   {
      $canlista = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $canlista);
      $canlista = substr($this->Db->qstr($canlista), 1, -1);
            $canlista_look = substr($this->Db->qstr($canlista), 1, -1); 
      $nmgp_def_dados = array(); 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct CANLISTA from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where   CAST (CANLISTA AS TEXT)  like '%" . $canlista . "%'"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct CANLISTA from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where   CAST (CANLISTA AS VARCHAR)  like '%" . $canlista . "%'"; 
      }
      else
      {
          $nm_comando = "select distinct CANLISTA from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where  CANLISTA like '%" . $canlista . "%'"; 
      }
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
   ...
   function lookup_ajax_tipund($tipund)
   {
      $tipund = substr($this->Db->qstr($tipund), 1, -1);
            $tipund_look = substr($this->Db->qstr($tipund), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct TIPUND from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where  TIPUND like '%" . $tipund . "%'"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
   ...
   function monta_condicao($nome, $condicao, $campo, $campo2 = "", $nome_campo="", $tp_campo="")
   {
      ...
      $nm_ini_lower = "";
      $nm_fim_lower = "";
      $Nm_numeric[] = "matid";$Nm_numeric[] = "canlista";$Nm_numeric[] = "existencia";$Nm_numeric[] = "canmat";
      ...
      if (in_array($campo_join, $Nm_numeric))
      {
         ...
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['decimal_db'] == ".")
         {
            $nm_aspas  = "";
            $nm_aspas1 = "";
         }
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
<HTML>
<HEAD>
 <TITLE>TMP Insertar defisico tns</TITLE>
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_tmp_insertar_defisico/grid_tmp_insertar_defisico_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
<?php
$vertical_center = '';
?>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_tmp_insertar_defisico_ajax_search.js"></script>
 <script type="text/javascript" src="grid_tmp_insertar_defisico_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_tmp_insertar_defisico_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_tmp_insertar_defisico']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_tmp_insertar_defisico']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_fisicoid_cond#NMF#' + search_get_sel_txt('SC_fisicoid_cond') + '@NMF@';
  str_out += 'SC_fisicoid#NMF#' + search_get_text('SC_fisicoid') + '@NMF@';
  str_out += 'id_ac_fisicoid#NMF#' + search_get_title('select2-id_ac_fisicoid-container') + '@NMF@';
  str_out += 'SC_matid_cond#NMF#' + search_get_sel_txt('SC_matid_cond') + '@NMF@';
  str_out += 'SC_matid#NMF#' + search_get_text('SC_matid') + '@NMF@';
  str_out += 'id_ac_matid#NMF#' + search_get_title('select2-id_ac_matid-container') + '@NMF@';
  str_out += 'SC_matid_input_2#NMF#' + search_get_text('SC_matid_input_2') + '@NMF@';
  str_out += 'SC_canlista_cond#NMF#' + search_get_sel_txt('SC_canlista_cond') + '@NMF@';
  str_out += 'SC_canlista#NMF#' + search_get_text('SC_canlista') + '@NMF@';
  str_out += 'id_ac_canlista#NMF#' + search_get_title('select2-id_ac_canlista-container') + '@NMF@';
  str_out += 'SC_tipund_cond#NMF#' + search_get_sel_txt('SC_tipund_cond') + '@NMF@';
  str_out += 'SC_tipund#NMF#' + search_get_text('SC_tipund') + '@NMF@';
  str_out += 'id_ac_tipund#NMF#' + search_get_title('select2-id_ac_tipund-container') + '@NMF@';
...
<script type="text/javascript">
 $(function() {
  $(".sc-ui-autocomp-fisicoid").on("focus", function() {
  }).on("blur", function() {
   ...
   ajax: {
    ...
    data: function (params) {
     var query = {
      ...
      nmgp_parms: "NM_ajax_opcao?#?autocomp_fisicoid",
      max_itens: "10",
   ...
   $("#SC_fisicoid").val(e.params.data.id);
  });
  $(".sc-ui-autocomp-matid").on("focus", function() {
  }).on("blur", function() {
  }).on("keydown", function(e) {
   if(e.keyCode == $.ui.keyCode.TAB && $(".ui-autocomplete").filter(":visible").length) {
    e.keyCode = $.ui.keyCode.DOWN;
   ...
   ajax: {
    ...
    data: function (params) {
     var query = {
      q: params.term,
      nmgp_opcao: "ajax_autocomp",
      nmgp_parms: "NM_ajax_opcao?#?autocomp_matid",
      max_itens: "10",
      script_case_init: <?php echo $this->Ini->sc_page ?>
     }
     return query;
   ...
   $("#SC_matid").val(e.params.data.id);
  });
  $(".sc-ui-autocomp-tipund").on("focus", function() {
  }).on("blur", function() {
  }).on("keydown", function(e) {
   if(e.keyCode == $.ui.keyCode.TAB && $(".ui-autocomplete").filter(":visible").length) {
    e.keyCode = $.ui.keyCode.DOWN;
   ...
   ajax: {
    ...
    data: function (params) {
     var query = {
      q: params.term,
      nmgp_opcao: "ajax_autocomp",
      nmgp_parms: "NM_ajax_opcao?#?autocomp_tipund",
      max_itens: "10",
      script_case_init: <?php echo $this->Ini->sc_page ?>
     }
     return query;
   ...
   $("#SC_tipund").val(e.params.data.id);
  });
  $(".sc-ui-autocomp-canlista").on("focus", function() {
  }).on("blur", function() {
  }).on("keydown", function(e) {
   if(e.keyCode == $.ui.keyCode.TAB && $(".ui-autocomplete").filter(":visible").length) {
    e.keyCode = $.ui.keyCode.DOWN;
   ...
   ajax: {
    ...
    data: function (params) {
     var query = {
      q: params.term,
      nmgp_opcao: "ajax_autocomp",
      nmgp_parms: "NM_ajax_opcao?#?autocomp_canlista",
      max_itens: "10",
      script_case_init: <?php echo $this->Ini->sc_page ?>
     }
     return query;
   ...
   $("#SC_canlista").val(e.params.data.id);
  });
 });
</script>
...
<?php
   ...
   {
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div style="width: 100%">
 ...
 <div style="height:37px; border-width:0px 0px 1px 0px;  border-style: dashed; border-color:#ddd; display: block">
 	<table style="width:100%; border-collapse:collapse; padding:0;">
    	<tr>
        	<td id="lin1_col1" class="scFilterHeaderFont"><span>TMP Insertar defisico tns</span></td>
...
<?php
   ...
   {
      global 
             $fisicoid_cond, $fisicoid, $fisicoid_autocomp,
             $matid_cond, $matid, $matid_input_2, $matid_autocomp,
             $canlista_cond, $canlista, $canlista_autocomp,
             $tipund_cond, $tipund, $tipund_autocomp,
      ...
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $fisicoid = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['fisicoid']; 
          $fisicoid_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['fisicoid_cond']; 
          $matid = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['matid']; 
          $matid_input_2 = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['matid_input_2']; 
          $matid_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['matid_cond']; 
          $canlista = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['canlista']; 
          $canlista_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['canlista_cond']; 
          $tipund = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['tipund']; 
          $tipund_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['tipund_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['NM_operador']; 
      } 
      if (!isset($fisicoid_cond) || empty($fisicoid_cond))
      {
         $fisicoid_cond = "qp";
      }
      ...
      if (!isset($canlista_cond) || empty($canlista_cond))
      {
         $canlista_cond = "qp";
      }
      if (!isset($tipund_cond) || empty($tipund_cond))
      {
         $tipund_cond = "qp";
      }
      ...
      $str_hide_fisicoid = (in_array($fisicoid_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_matid = (in_array($matid_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_canlista = (in_array($canlista_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_tipund = (in_array($tipund_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;

      $str_display_fisicoid = ('bw' == $fisicoid_cond) ? $display_aberto : $display_fechado;
      $str_display_matid = ('bw' == $matid_cond) ? $display_aberto : $display_fechado;
      $str_display_canlista = ('bw' == $canlista_cond) ? $display_aberto : $display_fechado;
      $str_display_tipund = ('bw' == $tipund_cond) ? $display_aberto : $display_fechado;

      if (!isset($fisicoid) || $fisicoid == "")
      {
          $fisicoid = "";
      }
      if (isset($fisicoid) && !empty($fisicoid))
      {
         $tmp_pos = strpos($fisicoid, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $fisicoid = substr($fisicoid, 0, $tmp_pos);
         }
      }
      ...
      if (!isset($canlista) || $canlista == "")
      {
          $canlista = "";
      }
      if (isset($canlista) && !empty($canlista))
      {
         $tmp_pos = strpos($canlista, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $canlista = substr($canlista, 0, $tmp_pos);
         }
      }
      if (!isset($tipund) || $tipund == "")
      {
          $tipund = "";
      }
      if (isset($tipund) && !empty($tipund))
      {
         $tmp_pos = strpos($tipund, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $tipund = substr($tipund, 0, $tmp_pos);
         }
      }
?>
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr>
      ...
      <TD id='SC_fisicoid_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['fisicoid'])) ? $this->New_label['fisicoid'] : "FISICOID"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_fisicoid_cond" name="fisicoid_cond" onChange="nm_campos_between(document.getElementById('id_vis_fisicoid'), this, 'fisicoid')">
       <OPTION value="qp" <?php if ("qp" == $fisicoid_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $fisicoid_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $fisicoid_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $fisicoid_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_fisicoid" <?php echo $str_hide_fisicoid?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['fisicoid'])) ? $this->New_label['fisicoid'] : "FISICOID";
 $nmgp_tab_label .= "fisicoid?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
...
<?php
      ...
      if (isset($nmgp_def_dados[0][$fisicoid]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$fisicoid];
      }
      else
      {
          $sAutocompValue = $fisicoid;
      }
?>
<INPUT  type="text" id="SC_fisicoid" name="fisicoid" value="<?php echo NM_encode_input($fisicoid) ?>"  size=1 alt="{datatype: 'text', maxLength: 1, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectOdd sc-ui-autocomp-fisicoid" id="id_ac_fisicoid" name="fisicoid_autocomp"><?php if ('' !=  $fisicoid) { ?><option value="<?php echo $fisicoid ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
      ...
      <TD id='SC_matid_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['matid'])) ? $this->New_label['matid'] : "MATID"; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_matid_cond" name="matid_cond" onChange="nm_campos_between(document.getElementById('id_vis_matid'), this, 'matid')">
       ...
       <OPTION value="bw" <?php if ("bw" == $matid_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_betw'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
...
<?php
   ...
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct MATID from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where MATID = $matid_look"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct MATID from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where MATID = $matid_look"; 
      ...
      if (isset($nmgp_def_dados[0][$matid]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$matid];
      }
      else
      {
            nmgp_Form_Num_Val($matid, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          $sAutocompValue = $matid;
      }
?>
<INPUT  type="text" id="SC_matid" name="matid" value="<?php echo NM_encode_input($matid) ?>" size=10 alt="{datatype: 'integer', maxLength: 10, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectEven sc-ui-autocomp-matid" id="id_ac_matid" name="matid_autocomp"><?php if ('' !=  $matid) { ?><option value="<?php echo $matid ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>        </TD>
       </TR>
       <TR valign="top">
        <TD id="id_vis_matid"  <?php echo $str_display_matid; ?> class="scFilterFieldFontEven">
         <?php echo $date_sep_bw ?>&nbsp;
         <BR>
         <INPUT type="text" id="SC_matid_input_2" name="matid_input_2" value="<?php echo NM_encode_input($matid_input_2) ?>"  size=10 alt="{datatype: 'integer', maxLength: 10, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" class="sc-js-input sc-js-input scFilterObjectEven">
      ...
      <TD id='SC_canlista_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['canlista'])) ? $this->New_label['canlista'] : "CANLISTA"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_canlista_cond" name="canlista_cond" onChange="nm_campos_between(document.getElementById('id_vis_canlista'), this, 'canlista')">
       <OPTION value="qp" <?php if ("qp" == $canlista_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $canlista_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $canlista_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_canlista" <?php echo $str_hide_canlista?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['canlista'])) ? $this->New_label['canlista'] : "CANLISTA";
 $nmgp_tab_label .= "canlista?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $canlista_look = substr($this->Db->qstr($canlista), 1, -1); 
      $nmgp_def_dados = array(); 
   if (is_numeric($canlista))
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct CANLISTA from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where CANLISTA = $canlista_look"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct CANLISTA from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where CANLISTA = $canlista_look"; 
      ...
      if (isset($nmgp_def_dados[0][$canlista]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$canlista];
      }
      else
      {
            nmgp_Form_Num_Val($canlista, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          $sAutocompValue = $canlista;
      }
?>
<INPUT  type="text" id="SC_canlista" name="canlista" value="<?php echo NM_encode_input($canlista) ?>" size=8 alt="{datatype: 'integer', maxLength: 8, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectOdd sc-ui-autocomp-canlista" id="id_ac_canlista" name="canlista_autocomp"><?php if ('' !=  $canlista) { ?><option value="<?php echo $canlista ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
      ...
      <TD id='SC_tipund_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['tipund'])) ? $this->New_label['tipund'] : "TIPUND"; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_tipund_cond" name="tipund_cond" onChange="nm_campos_between(document.getElementById('id_vis_tipund'), this, 'tipund')">
       <OPTION value="qp" <?php if ("qp" == $tipund_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $tipund_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $tipund_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $tipund_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_tipund" <?php echo $str_hide_tipund?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['tipund'])) ? $this->New_label['tipund'] : "TIPUND";
 $nmgp_tab_label .= "tipund?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $tipund_look = substr($this->Db->qstr($tipund), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct TIPUND from (SELECT '5' AS FISICOID, D.MATID AS MATID, SUM(D.CANLISTA) AS CANLISTA, D.TIPUND AS TIPUND, (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1') AS EXISTENCIA, (SUM(D.CANLISTA)-(SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')) AS CANMAT FROM KARDEX K INNER JOIN DEKARDEX D ON K.KARDEXID=D.KARDEXID WHERE K.CODCOMP='FV' AND K.CODPREFIJO='FW' AND K.FECASENTAD IS NULL GROUP BY D.MATID,D.TIPUND HAVING (SELECT SUM(S.EXISTENC) FROM SALMATERIAL S WHERE S.MATID=D.MATID AND S.BODID='1')<SUM(D.CANLISTA)) nm_sel_esp where TIPUND = '$tipund_look'"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$tipund]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$tipund];
      }
      else
      {
          $sAutocompValue = $tipund;
      }
?>
<INPUT  type="text" id="SC_tipund" name="tipund" value="<?php echo NM_encode_input($tipund) ?>"  size=1 alt="{datatype: 'text', maxLength: 1, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectEven sc-ui-autocomp-tipund" id="id_ac_tipund" name="tipund_autocomp"><?php if ('' !=  $tipund) { ?><option value="<?php echo $tipund ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
...
?>
<?php
   if (is_file("grid_tmp_insertar_defisico_help.txt"))
   {
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_tmp_insertar_defisico']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_tmp_insertar_defisico']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_tmp_insertar_defisico']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_tmp_insertar_defisico']['start'] == 'filter')
   {
?>
...
?>
   <INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.fisicoid_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_fisicoid'), document.F1.fisicoid_cond, 'fisicoid');
   document.F1.fisicoid.value = "";
   document.F1.fisicoid_autocomp.value = "";
   $('#select2-id_ac_fisicoid-container').html('<?php echo $this->Val_init_fisicoid['desc'] ?>');;
   ...
   document.F1.matid_autocomp.value = "";
   $('#select2-id_ac_matid-container').html('<?php echo $this->Val_init_matid['desc'] ?>');;
   document.F1.matid_input_2.value = "";
   document.F1.canlista_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_canlista'), document.F1.canlista_cond, 'canlista');
   document.F1.canlista.value = "";
   document.F1.canlista_autocomp.value = "";
   $('#select2-id_ac_canlista-container').html('<?php echo $this->Val_init_canlista['desc'] ?>');;
   document.F1.tipund_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_tipund'), document.F1.tipund_cond, 'tipund');
   document.F1.tipund.value = "";
   document.F1.tipund_autocomp.value = "";
   $('#select2-id_ac_tipund-container').html('<?php echo $this->Val_init_tipund['desc'] ?>');;
   Sc_carga_select2('all');
 }
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "FACILWEB_V2/grid_tmp_insertar_defisico";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function salva_filtro($nmgp_save_origem)
   {
      global $NM_filters_save, $nmgp_save_name, $nmgp_save_option, $script_case_init;
          ...
          $NM_patch .= "grid_tmp_insertar_defisico/";
          if (!is_dir($NM_patch))
          {
              $NMdir = mkdir($NM_patch, 0755);
          }
   ...
   function recupera_filtro($NM_filters)
   {
      ...
      $tp_fields['SC_fisicoid_cond'] = 'cond';
      $tp_fields['SC_fisicoid'] = 'text_aut';
      $tp_fields['id_ac_fisicoid'] = 'select2_aut';
      $tp_fields['SC_matid_cond'] = 'cond';
      $tp_fields['SC_matid'] = 'text_aut';
      $tp_fields['id_ac_matid'] = 'select2_aut';
      $tp_fields['SC_matid_input_2'] = 'text';
      $tp_fields['SC_canlista_cond'] = 'cond';
      $tp_fields['SC_canlista'] = 'text_aut';
      $tp_fields['id_ac_canlista'] = 'select2_aut';
      $tp_fields['SC_tipund_cond'] = 'cond';
      $tp_fields['SC_tipund'] = 'text_aut';
      $tp_fields['id_ac_tipund'] = 'select2_aut';
   ...
   {
      global $fisicoid_cond, $fisicoid, $fisicoid_autocomp,
             $matid_cond, $matid, $matid_input_2, $matid_autocomp,
             $canlista_cond, $canlista, $canlista_autocomp,
             $tipund_cond, $tipund, $tipund_autocomp, $nmgp_tab_label;

      $C_formatado = true;
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_gp_limpa.php", "F", "nm_limpa_valor") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_conv_dados.php", "F", "nm_conv_limpa_dado") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_edit.php", "F", "nmgp_Form_Num_Val") ; 
      if (!empty($fisicoid_autocomp) && empty($fisicoid))
      {
          $fisicoid = $fisicoid_autocomp;
      }
      if (!empty($matid_autocomp) && empty($matid))
      {
          $matid = $matid_autocomp;
      }
      if (!empty($canlista_autocomp) && empty($canlista))
      {
          $canlista = $canlista_autocomp;
      }
      if (!empty($tipund_autocomp) && empty($tipund))
      {
          $tipund = $tipund_autocomp;
      }
      $fisicoid_cond_salva = $fisicoid_cond; 
      if (!isset($fisicoid_input_2) || $fisicoid_input_2 == "")
      {
          $fisicoid_input_2 = $fisicoid;
      }
      ...
      $canlista_cond_salva = $canlista_cond; 
      if (!isset($canlista_input_2) || $canlista_input_2 == "")
      {
          $canlista_input_2 = $canlista;
      }
      $tipund_cond_salva = $tipund_cond; 
      if (!isset($tipund_input_2) || $tipund_input_2 == "")
      {
          $tipund_input_2 = $tipund;
      }
      if ($matid_cond != "in")
      {
      ...
      if ($matid_cond != "in")
      {
          nm_limpa_numero($matid_input_2, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      if ($canlista_cond != "in")
      {
          nm_limpa_numero($canlista, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['fisicoid'] = $fisicoid; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['fisicoid_cond'] = $fisicoid_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['matid'] = $matid; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['matid_input_2'] = $matid_input_2; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['matid_cond'] = $matid_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['canlista'] = $canlista; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['canlista_cond'] = $canlista_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['tipund'] = $tipund; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['tipund_cond'] = $tipund_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca'] = $Temp_Busca;
      }
      if ($matid_cond != "in" && $matid_cond != "bw" && !empty($matid) && !is_numeric($matid))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : MATID";
      }
      if ($matid_cond == "bw" && ((!empty($matid) && !is_numeric($matid)) || (!empty($matid_input_2) && !is_numeric($matid_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : MATID";
      }
      if ($canlista_cond != "in" && $canlista_cond != "bw" && !empty($canlista) && !is_numeric($canlista))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : CANLISTA";
      }
      if ($canlista_cond == "bw" && ((!empty($canlista) && !is_numeric($canlista)) || (!empty($canlista_input_2) && !is_numeric($canlista_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : CANLISTA";
      }
      if (!empty($this->Campos_Mens_erro)) 
    ...
    if ($fisicoid != '') {
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['fisicoid'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          ...
          $this->cmp_formatado['fisicoid'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['fisicoid'] = $fisicoid;
      }
      $nmgp_def_dados = array();
    if ($matid != '') {
      $matid_look = substr($this->Db->qstr($matid), 1, -1); 
   ...
   { 
      ...
      $this->cmp_formatado['matid_input_2'] = $Conteudo;
      $nmgp_def_dados = array();
    if ($canlista != '') {
      $canlista_look = substr($this->Db->qstr($canlista), 1, -1); 
      $nmgp_def_dados = array(); 
   ...
   { 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['canlista'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['canlista'] = $cmp1;
      }
      else
      {
          $cmp1 = $canlista;
    ...
    if ($tipund != '') {
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['tipund'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['tipund'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['tipund'] = $tipund;
      }

      //----- $fisicoid
      $this->Date_part = false;
      if (isset($fisicoid) || $fisicoid_cond == "nu" || $fisicoid_cond == "nn" || $fisicoid_cond == "ep" || $fisicoid_cond == "ne")
      {
         $this->monta_condicao("FISICOID", $fisicoid_cond, $fisicoid, "", "fisicoid");
      }
      ...
      {
         $this->monta_condicao("MATID", $matid_cond, $matid, $matid_input_2, "matid");
      }

      //----- $canlista
      $this->Date_part = false;
      if (isset($canlista) || $canlista_cond == "nu" || $canlista_cond == "nn" || $canlista_cond == "ep" || $canlista_cond == "ne")
      {
         $this->monta_condicao("CANLISTA", $canlista_cond, $canlista, "", "canlista");
      }

      //----- $tipund
      $this->Date_part = false;
      if (isset($tipund) || $tipund_cond == "nu" || $tipund_cond == "nn" || $tipund_cond == "ep" || $tipund_cond == "ne")
      {
         $this->monta_condicao("TIPUND", $tipund_cond, $tipund, "", "tipund");
      }
   }
   ...
   {
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['where_pesq_fast'] = "";
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['fast_search']);
      if ("" == $this->comando_filtro)
      {
          $this->comando = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['where_orig'];
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['where_pesq']         = $this->comando . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['opcao']              = "pesq";
      if ("" == $this->comando_filtro)
      {
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['cond_pesq'] .= $this->NM_operador;
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['where_pesq_ant'] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tmp_insertar_defisico']['where_pesq'];
