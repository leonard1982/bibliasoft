  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_prefijos" == sTestField)
        {
...
?>
  ...
  // ---------- Validate id_prefijo_
  function do_ajax_form_prefijos_validate_id_prefijo_(iNumLinha)
  {
    var nomeCampo_id_prefijo_ = "id_prefijo_" + iNumLinha;
    var var_id_prefijo_ = scAjaxGetFieldHidden(nomeCampo_id_prefijo_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_prefijos_validate_id_prefijo_(var_id_prefijo_, iNumLinha, var_script_case_init, do_ajax_form_prefijos_validate_id_prefijo__cb);
  } // do_ajax_form_prefijos_validate_id_prefijo_

  function do_ajax_form_prefijos_validate_id_prefijo__cb(sResp)
  {
    ...
    if ("" != iLineNumber)
    {
      sFieldValid = "id_prefijo_" + iLineNumber;
    }
    else
    {
      sFieldValid = "id_prefijo_";
    }
    ...
    if ("" == sFieldErrors)
    {
      ...
      scErrorLineOff(iLineNumber, "id_prefijo_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "id_prefijo_");
    }
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_prefijos_validate_id_prefijo__cb

  // ---------- Validate elprefijo_
  function do_ajax_form_prefijos_validate_elprefijo_(iNumLinha)
  {
    var nomeCampo_elprefijo_ = "elprefijo_" + iNumLinha;
    var var_elprefijo_ = scAjaxGetFieldText(nomeCampo_elprefijo_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_prefijos_validate_elprefijo_(var_elprefijo_, iNumLinha, var_script_case_init, do_ajax_form_prefijos_validate_elprefijo__cb);
  } // do_ajax_form_prefijos_validate_elprefijo_

  function do_ajax_form_prefijos_validate_elprefijo__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "elprefijo_" + iLineNumber;
    }
    else
    {
      sFieldValid = "elprefijo_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "elprefijo_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "elprefijo_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_prefijos_validate_elprefijo__cb

  // ---------- Validate comentario_
  function do_ajax_form_prefijos_validate_comentario_(iNumLinha)
  {
    var nomeCampo_comentario_ = "comentario_" + iNumLinha;
    var var_comentario_ = scAjaxGetFieldText(nomeCampo_comentario_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_prefijos_validate_comentario_(var_comentario_, iNumLinha, var_script_case_init, do_ajax_form_prefijos_validate_comentario__cb);
  } // do_ajax_form_prefijos_validate_comentario_

  function do_ajax_form_prefijos_validate_comentario__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "comentario_" + iLineNumber;
    }
    else
    {
      sFieldValid = "comentario_";
    }
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "comentario_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "comentario_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_prefijos_validate_comentario__cb

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_form_prefijos_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_form_prefijos_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_form_prefijos_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_form_prefijos_add_new_line_cb);
  } // do_ajax_form_prefijos_add_new_line

  function do_ajax_form_prefijos_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
    if ("{" == sResp.substr(0, 1)) {
        oResp = scAjaxResponse(sResp);
  ...
  } // do_ajax_form_prefijos_add_new_line_cb

  // ---------- backup_line
  function do_ajax_form_prefijos_backup_line(iNumLinha)
  {
    var var_id_prefijo_ = scAjaxGetFieldHidden("id_prefijo_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_prefijos_backup_line(var_id_prefijo_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_form_prefijos_backup_line_cb);
  } // do_ajax_form_prefijos_backup_line

  function do_ajax_form_prefijos_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    if (!scAjaxHasError())
  ...
  } // do_ajax_form_prefijos_backup_line_cb

  function do_ajax_form_prefijos_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
  ...
  } // do_ajax_form_prefijos_cancel_insert

  function do_ajax_form_prefijos_cancel_update(iSeqVert)
  {
    do_ajax_form_prefijos_backup_line(iSeqVert);
...
?>
  } // do_ajax_form_prefijos_cancel_update

  function do_ajax_form_prefijos_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_form_prefijos_restore_buttons

  // ---------- table_refresh
  function do_ajax_form_prefijos_table_refresh()
  {
    var var_id_prefijo_ = document.F2.id_prefijo_.value;
    ...
    scAjaxProcOn();
    x_ajax_form_prefijos_table_refresh(var_id_prefijo_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_prefijos_table_refresh_cb);
  } //  do_ajax_form_prefijos_table_refresh

  function do_ajax_form_prefijos_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    document.F2.id_prefijo_.value = scAjaxGetKeyValue("id_prefijo_");
    ...
    scAjaxAlert(do_ajax_form_prefijos_table_refresh_cb_after_alert);
  } // do_ajax_form_prefijos_table_refresh_cb
  function do_ajax_form_prefijos_table_refresh_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    ...
    sc_insert_open = false;
  } // do_ajax_form_prefijos_table_refresh_cb_after_alert
...
function scJs_sweetalert_params(params) {
  ...
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_form_prefijos_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_form_prefijos_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
    ...
    scAjaxHideMessage();
    var var_id_prefijo_ = scAjaxGetFieldHidden("id_prefijo_" + iNumLinha);
    var var_elprefijo_ = scAjaxGetFieldText("elprefijo_" + iNumLinha);
    var var_comentario_ = scAjaxGetFieldText("comentario_" + iNumLinha);
    var var_nm_form_submit = document.F1.nm_form_submit.value;
...
?>
    x_ajax_form_prefijos_submit_form(var_id_prefijo_, var_elprefijo_, var_comentario_, var_nmgp_refresh_row, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_prefijos_submit_form_cb);
  } // do_ajax_form_prefijos_submit_form

  function do_ajax_form_prefijos_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
      ...
      scAjaxHideErrorDisplay("table");
      scAjaxHideErrorDisplay("id_prefijo_" + sc_num_ult_line);
      scAjaxHideErrorDisplay("elprefijo_" + sc_num_ult_line);
      scAjaxHideErrorDisplay("comentario_" + sc_num_ult_line);
<?php
if (isset($this->Embutida_form) && $this->Embutida_form) {
?>
      scAjaxSetReadonly();
...
?>
    ...
    scAjaxSetLabel(true);
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_form_prefijos_submit_form_cb_after_alert);
  } // do_ajax_form_prefijos_submit_form_cb
  function do_ajax_form_prefijos_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_prefijos_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_prefijos_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_form_prefijos_navigate_form, function() {});
    } else {
      perform_ajax_form_prefijos_navigate_form();
    }
  }

  function perform_ajax_form_prefijos_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    if (sc_insert_open)
    {
        do_ajax_form_prefijos_cancel_insert(sc_num_ult_line);
    }
    nm_uncheck_delete();
    scAjaxHideMessage();
    scAjaxHideErrorDisplay("table");
    ...
    {
      scAjaxHideErrorDisplay("id_prefijo_" + iNavForm);
      scAjaxHideErrorDisplay("elprefijo_" + iNavForm);
      scAjaxHideErrorDisplay("comentario_" + iNavForm);
    }
    var var_id_prefijo_ = document.F2.id_prefijo_.value;
    var var_nm_form_submit = document.F2.nm_form_submit.value;
    var var_nmgp_opcao = document.F2.nmgp_opcao.value;
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_form_prefijos_navigate_form(var_id_prefijo_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_prefijos_navigate_form_cb);
  } // do_ajax_form_prefijos_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_prefijos']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_prefijos_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.id_prefijo_.value = scAjaxGetKeyValue("id_prefijo_" + var_last_index);
    ...
    scErrorLineReset();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_prefijos_navigate_form_cb_after_alert);
  } // do_ajax_form_prefijos_navigate_form_cb
  function do_ajax_form_prefijos_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
...
?>
  ...
  } // do_ajax_form_prefijos_navigate_form_cb_after_alert
  function sc_hide_form_prefijos_form()
  {
...
?>
  ...
  {
    ajax_field_list[iTotCampos] = "id_prefijo_" + iNumLinha;
    iTotCampos++;
    ajax_field_list[iTotCampos] = "elprefijo_" + iNumLinha;
    iTotCampos++;
    ajax_field_list[iTotCampos] = "comentario_" + iNumLinha;
    iTotCampos++;
    ajax_error_list["id_prefijo_" + iNumLinha] = {"label": "Id Prefijo<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["elprefijo_" + iNumLinha] = {"label": "Prefijo<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["comentario_" + iNumLinha] = {"label": "Comentario<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_field_mult["id_prefijo_"][iNumLinha] = "id_prefijo_" + iNumLinha;
    ajax_field_mult["elprefijo_"][iNumLinha] = "elprefijo_" + iNumLinha;
    ajax_field_mult["comentario_"][iNumLinha] = "comentario_" + iNumLinha;
    ajax_field_id["elprefijo_" + iNumLinha] = new Array("hidden_field_label_elprefijo_", "hidden_field_data_elprefijo_" + iNumLinha);
    ajax_field_id["comentario_" + iNumLinha] = new Array("hidden_field_label_comentario_", "hidden_field_data_comentario_" + iNumLinha);
    ajax_error_count["id_prefijo_" + iNumLinha] = "off";
    ajax_error_count["elprefijo_" + iNumLinha] = "off";
    ajax_error_count["comentario_" + iNumLinha] = "off";
<?php
if (!$this->Grid_editavel)
{
?>
    ajax_read_only["id_prefijo_" + iNumLinha] = "on";
    ajax_read_only["elprefijo_" + iNumLinha] = "off";
    ajax_read_only["comentario_" + iNumLinha] = "off";
<?php
}
else
{
?>
    ajax_read_only["id_prefijo_" + iNumLinha] = "on";
    ajax_read_only["elprefijo_" + iNumLinha] = "on";
    ajax_read_only["comentario_" + iNumLinha] = "on";
<?php
}
?>
  }
  ...
  {
    ajax_error_list["id_prefijo_" + iNumLinha] = null;
    ajax_error_list["elprefijo_" + iNumLinha] = null;
    ajax_error_list["comentario_" + iNumLinha] = null;
  }
  ...
  var ajax_field_mult = {
    "id_prefijo_": new Array(),
    "elprefijo_": new Array(),
    "comentario_": new Array()
  };
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ajax_error_list["id_prefijo_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["id_prefijo_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["id_prefijo_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["id_prefijo_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["id_prefijo_" + iNumLinha]["onfocus"] = new Array();
      ajax_error_list["elprefijo_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["elprefijo_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["elprefijo_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["elprefijo_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["elprefijo_" + iNumLinha]["onfocus"] = new Array();
      ajax_error_list["comentario_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["comentario_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["comentario_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["comentario_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["comentario_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
?>
    ...
    mdOpenObjects(iSeq);
    Obj_Focus = eval("document.getElementById('id_sc_field_elprefijo_" + iSeq + "')");
    if (Obj_Focus)
    {
        Obj_Focus.focus();
    }
  ...
  {
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['id_prefijo_'])) ? $this->nmgp_cmp_readonly['id_prefijo_'] : 'on';
?>
    scAjaxFieldRead("id_prefijo_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['elprefijo_'])) ? $this->nmgp_cmp_readonly['elprefijo_'] : 'off';
?>
    scAjaxFieldRead("elprefijo_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['comentario_'])) ? $this->nmgp_cmp_readonly['comentario_'] : 'off';
?>
    scAjaxFieldRead("comentario_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }

  function mdCloseObjects(iSeq)
  {
    scAjaxFieldRead("id_prefijo_" + iSeq, "on");
    scAjaxFieldRead("elprefijo_" + iSeq, "on");
    scAjaxFieldRead("comentario_" + iSeq, "on");
    rerunHeaderDisplay = 1;
    scSetFixedHeaders(true);
  }
...
?>
  ...
  function scErrorLineOn(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("on" == ajax_error_count["id_prefijo_" + iRow] || "on" == ajax_error_count["elprefijo_" + iRow] || "on" == ajax_error_count["comentario_" + iRow]))
    {
      $("#hidden_field_data_sc_seq" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_sc_actions" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_id_prefijo_" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_elprefijo_" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_comentario_" + iRow).addClass("scFormErrorLine");
    }
  }
  ...
  function scErrorLineOff(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("off" == ajax_error_count["id_prefijo_" + iRow] && "off" == ajax_error_count["elprefijo_" + iRow] && "off" == ajax_error_count["comentario_" + iRow]))
    {
      if (bErrorRow)
      {
        ajax_error_count["id_prefijo_" + iRow] = "off";
        ajax_error_count["elprefijo_" + iRow] = "off";
        ajax_error_count["comentario_" + iRow] = "off";
      }
      ...
      $("#hidden_field_data_id_prefijo_" + iRow).removeClass("scFormErrorLine");
      $("#hidden_field_data_elprefijo_" + iRow).removeClass("scFormErrorLine");
      $("#hidden_field_data_comentario_" + iRow).removeClass("scFormErrorLine");
    }
  }
