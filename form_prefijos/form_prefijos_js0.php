<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="id_prefijo_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['id_prefijo_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_prefijos"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?>
    ...
    {
        do_ajax_form_prefijos_navigate_form();
    }
    if ("ordem" == x)
    {
        scSetOrderColumn(y);
...
function scForm_submit_single(x) {
	if (x != "excluir")
	{
		...
		if ("incluir" == x || "muda_form" == x || "recarga" == x || "recarga_mobile" == x) {
			...
			Nm_Proc_Atualiz = true;
			do_ajax_form_prefijos_submit_form();
		}
	}
	if (Nm_submit_ok) {
		Nm_Proc_Atualiz = true;
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_prefijos_submit_form(y, z);
