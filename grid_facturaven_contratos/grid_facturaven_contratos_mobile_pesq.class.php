...
class grid_facturaven_contratos_pesq
{
   var $Db;
   var $Erro;
   var $Ini;
...
if(isset($this->sc_temp_ganio))
{
	if($this->sc_temp_ganio>0)
	{
		$anio  = $this->sc_temp_ganio;
	}
}

if(isset($this->sc_temp_gperiodo))
{
	if($this->sc_temp_gperiodo>0)
	{
		$periodo  = $this->sc_temp_gperiodo;
	}
}

if(isset($this->sc_temp_gcodzona))
{
	if(!empty($this->sc_temp_gcodzona))
	{
		$vsql = "select nombre from zona_clientes where codigo='".$this->sc_temp_gcodzona."' limit 1";
      ...
      { 
          ...
          { 
                 ...
                 { 
                        $vNomZona[$SCy] [$SCx] = $SCrx->fields[$SCx];
                        $vnomzona[$SCy] [$SCx] = $SCrx->fields[$SCx];
                 }
                 $SCy++; 
                 $SCrx->MoveNext();
      ...
      { 
          $vNomZona = false;
          $vNomZona_erro = $this->Db->ErrorMsg();
          $vnomzona = false;
          $vnomzona_erro = $this->Db->ErrorMsg();
      } 
;
		if(isset($vnomzona[0][0]))
		{
			$zona  = $vnomzona[0][0];
		}
	}
...
if(isset($this->sc_temp_gresolucion))
{
	if(!empty($this->sc_temp_gresolucion))
	{
		$resolucion  = $this->sc_temp_gresolucion;
	}
}
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_facturaven_contratos_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_facturaven_contratos_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_facturaven_contratos_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == "ajax_refresh_field")
      {
          ...
          if (in_array("barrio", $NMcmp_refr))
          {
              $list = array();
              $nmgp_def_dados = $this->lookup_ajax_barrio($zona);
              ...
              $this->Arr_result['set_option'][] = array('field' => 'SC_barrio', 'value' => $list);
          }
      }
   ...
   function lookup_ajax_barrio($zona)
   {
      $tmp_pos = strpos($zona, "##@@");
      if ($tmp_pos !== false)
      {
      ...
      $nmgp_def_dados[] = array("" => NM_charset_to_utf8("TODOS")); 
      $nm_comando = "SELECT b.descripcion FROM barrios b inner join zona_clientes z on b.cod_zona=z.codigo WHERE z.nombre='$zona' ORDER BY b.descripcion ASC"; 
      ...
      { 
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['barrio'] = array();
         while (!$rs->EOF) 
         { 
            $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['barrio'][] = trim($rs->fields[0]);
   ...
   function monta_condicao($nome, $condicao, $campo, $campo2 = "", $nome_campo="", $tp_campo="")
   {
      ...
      if (in_array($campo_join, $Nm_datas))
      {
          ...
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['SC_sep_date']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['SC_sep_date']))
          {
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_facturaven_contratos/grid_facturaven_contratos_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
<?php
$vertical_center = '';
?>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_facturaven_contratos_ajax_search.js"></script>
 <script type="text/javascript" src="grid_facturaven_contratos_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_facturaven_contratos_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_facturaven_contratos']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_facturaven_contratos']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
?>
 ...
 function nm_save_form(pos)
 {
  ...
  str_out += 'SC_barrio#NMF#' + search_get_select('SC_barrio') + '@NMF@';
  ...
  str_out += 'SC_numcontrato_cond#NMF#' + search_get_sel_txt('SC_numcontrato_cond') + '@NMF@';
  str_out += 'SC_numcontrato#NMF#' + search_get_text('SC_numcontrato') + '@NMF@';
  ...
  str_out += 'SC_enviada_cond#NMF#' + search_get_sel_txt('SC_enviada_cond') + '@NMF@';
  str_out += 'SC_enviada#NMF#' + search_get_select('SC_enviada') + '@NMF@';
 ...
 function nm_refresh_zona(Proc_on)
 {
   var parms  = "";
   var fields = "barrio";
   parms += 'zona#NMF#' + search_get_select('SC_zona') + '@NMF@';
...
<?php
   ...
   {
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div style="width: 100%">
 ...
 <div style="height:37px; border-width:0px 0px 1px 0px;  border-style: dashed; border-color:#ddd; display: block">
 	<table style="width:100%; border-collapse:collapse; padding:0;">
    	<tr>
        	<td id="lin1_col1" class="scFilterHeaderFont"><span>Facturas de venta contrato</span></td>
...
<?php
   ...
   function monta_form()
   {
      ...
      if (isset($_SESSION['scriptcase']['sc_aba_iframe']))
      {
          ...
          {
              if (in_array("grid_facturaven_contratos", $apls_aba))
              {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['iframe_menu'] && (!isset($_SESSION['scriptcase']['menu_mobile']) || empty($_SESSION['scriptcase']['menu_mobile'])))
      {
          $this->aba_iframe = true;
      }
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $anio = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['anio']; 
          $anio_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['anio_cond']; 
          $periodo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['periodo']; 
          $periodo_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['periodo_cond']; 
          $zona = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['zona']; 
          $zona_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['zona_cond']; 
          $barrio = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['barrio']; 
          $barrio_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['barrio_cond']; 
          $fechaven_dia = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_dia']; 
          $fechaven_mes = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_mes']; 
          $fechaven_ano = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_ano']; 
          $fechaven_input_2_dia = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_input_2_dia']; 
          $fechaven_input_2_mes = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_input_2_mes']; 
          $fechaven_input_2_ano = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_input_2_ano']; 
          $fechaven_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_cond']; 
          $numcontrato = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['numcontrato']; 
          $numcontrato_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['numcontrato_cond']; 
          $idcli = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['idcli']; 
          $idcli_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['idcli_cond']; 
          $asentada = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['asentada']; 
          $asentada_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['asentada_cond']; 
          $resolucion = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['resolucion']; 
          $resolucion_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['resolucion_cond']; 
          $enviada = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['enviada']; 
          $enviada_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['enviada_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['NM_operador']; 
          if (strtoupper($numcontrato_cond) != "II" && strtoupper($numcontrato_cond) != "QP" && strtoupper($numcontrato_cond) != "NP" && strtoupper($numcontrato_cond) != "IN") 
          { 
              nmgp_Form_Num_Val($numcontrato, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "1", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          } 
      } 
      ...
      if (!isset($numcontrato_cond) || empty($numcontrato_cond))
      {
         $numcontrato_cond = "eq";
      }
      ...
      if (!isset($enviada_cond) || empty($enviada_cond))
      {
         $enviada_cond = "eq";
      }
      ...
      $str_hide_numcontrato = (in_array($numcontrato_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      ...
      $str_hide_enviada = (in_array($enviada_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      ...
      $str_display_numcontrato = ('bw' == $numcontrato_cond) ? $display_aberto : $display_fechado;
      ...
      $str_display_enviada = ('bw' == $enviada_cond) ? $display_aberto : $display_fechado;
      ...
      if (!isset($numcontrato) || $numcontrato == "")
      {
          $numcontrato = "";
      }
      if (isset($numcontrato) && !empty($numcontrato))
      {
      ...
      if (!isset($enviada) || $enviada == "")
      {
          $enviada = "";
      }
      if (isset($enviada) && !empty($enviada))
      {
    ...
    <TR>
     <TD colspan="1" height="20" class="scFilterBlockBack">
      <TABLE border="0" cellpadding="0" cellspacing="0" width="100%" height="100%">
       <TR>
        <TD class="scFilterBlockFont css_blk_0"><?php echo $Img_tit_blk_i ?>Año, periodo, zona y barrio<?php echo $Img_tit_blk_f ?></TD>
...
<?php
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['anio'] = array();
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['anio'][] = "2020";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['anio'][] = "2021";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['anio'][] = "2022";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['anio'][] = "2023";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['anio'][] = "2024";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['anio'][] = "2025";
 ?>
...
<?php
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'] = array();
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'][] = "1";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'][] = "2";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'][] = "3";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'][] = "4";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'][] = "5";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'][] = "6";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'][] = "7";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'][] = "8";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'][] = "9";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'][] = "10";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'][] = "11";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'][] = "12";
 ?>
...
?>
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span> <span class="scFilterRequiredOdd">*</span><br><span id="id_hide_zona"  <?php echo $str_hide_zona?>>
<?php
      ...
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['zona'] = array();
         while (!$rs->EOF) 
         { 
            $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['zona'][] = trim($rs->fields[0]);
...
?>
   <span id="idAjaxSelect_zona">
      <SELECT class="scFilterObjectOdd" id="SC_zona" name="zona" onChange="nm_refresh_zona();" size="1">
...
?>
      ...
      <INPUT type="hidden" id="SC_barrio_cond" name="barrio_cond" value="eq">
...
?>
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br><span id="id_hide_barrio"  <?php echo $str_hide_barrio?>>
<?php
      $barrio_look = substr($this->Db->qstr($barrio), 1, -1); 
      $nmgp_def_dados = "" ; 
      $nm_comando = "SELECT b.descripcion FROM barrios b inner join zona_clientes z on b.cod_zona=z.codigo WHERE z.nombre='$zona' ORDER BY b.descripcion ASC"; 
...
?>
   <span id="idAjaxSelect_barrio">
      <SELECT class="scFilterObjectEven" id="SC_barrio" name="barrio"  size="1">
...
<?php
      ...
      foreach ($nm_opcoes as $nm_opcao)
      {
         if (!empty($nm_opcao))
         {
            ...
            if ("" != $barrio)
            {
                    $barrio_sel = ($nm_opc_cod === $barrio) ? "selected" : "";
            }
            else
            {
               $barrio_sel = ("S" == $nm_opc_sel) ? "selected" : "";
            }
            $nm_sc_valor = $nm_opc_val;
            $nm_opc_val = $nm_sc_valor;
?>
       <OPTION value="<?php echo NM_encode_input($nm_opc_cod . $delimitador . $nm_opc_val); ?>" <?php echo $barrio_sel; ?>><?php echo $nm_opc_val; ?></OPTION>
<?php
         }
      }
?>
...
?>
    ...
    <TR>
     <TD colspan="1" height="20" class="scFilterBlockBack">
      <TABLE border="0" cellpadding="0" cellspacing="0" width="100%" height="100%">
       <TR>
        <TD class="scFilterBlockFont css_blk_1"><?php echo $Img_tit_blk_i ?>grid_facturaven_pos<?php echo $Img_tit_blk_f ?></TD>
...
?>
      ...
      <INPUT type="hidden" id="SC_numcontrato_cond" name="numcontrato_cond" value="eq">

    <TD nowrap class="scFilterLabelEven" style="vertical-align: top" > <?php
 $SC_Label = (isset($this->New_label['numcontrato'])) ? $this->New_label['numcontrato'] : "Número Contrato (CodCliente)";
 $nmgp_tab_label .= "numcontrato?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
?>
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br><span id="id_hide_numcontrato"  <?php echo $str_hide_numcontrato?>><INPUT  type="text" id="SC_numcontrato" name="numcontrato" value="<?php echo NM_encode_input($numcontrato) ?>" size=20 alt="{datatype: 'integer', maxLength: 20, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" class="sc-js-input scFilterObjectEven">
 </TD>
...
?>
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br><span id="id_hide_idcli"  <?php echo $str_hide_idcli?>><?php
      if ($idcli != "")
      {
      $idcli_look = substr($this->Db->qstr($idcli), 1, -1); 
      $nmgp_def_dados = array(); 
...
?>
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br><span id="id_hide_asentada"  <?php echo $str_hide_asentada?>> 
<?php
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['asentada'] = array();
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['asentada'][] = "1";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['asentada'][] = "0";
 ?>
...
?>
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br><span id="id_hide_resolucion"  <?php echo $str_hide_resolucion?>>
<?php
      $resolucion_look = substr($this->Db->qstr($resolucion), 1, -1); 
      $nmgp_def_dados = "" ; 
      $nm_comando = "SELECT Idres,prefijo  FROM resdian  WHERE activa='SI' and resolucion>0 and prefijo_fe='FE' and pref_factura='SI' ORDER BY prefijo"; 
      foreach ($this->Ini->nm_col_dinamica as $nm_cada_col => $nm_nova_col)
      {
          $nm_comando = str_replace($nm_cada_col, $nm_nova_col, $nm_comando); 
      }
      ...
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['resolucion'] = array();
         while (!$rs->EOF) 
         { 
            $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['resolucion'][] = trim($rs->fields[0]);
...
?>
   <span id="idAjaxSelect_resolucion">
      <SELECT class="scFilterObjectOdd" id="SC_resolucion" name="resolucion"  size="1">
...
?>
      ...
      <INPUT type="hidden" id="SC_enviada_cond" name="enviada_cond" value="eq">

    <TD nowrap class="scFilterLabelEven" style="vertical-align: top" > <?php
 $SC_Label = (isset($this->New_label['enviada'])) ? $this->New_label['enviada'] : "Enviada";
 $nmgp_tab_label .= "enviada?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
?>
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br><span id="id_hide_enviada"  <?php echo $str_hide_enviada?>> 
<?php
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['enviada'] = array();
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['enviada'][] = "SI";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['enviada'][] = "NO";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['enviada'][] = "PT";
  $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['enviada'][] = "PR";
 ?>

 <SELECT class="scFilterObjectEven" id="SC_enviada"  name="enviada"  size="1">
 <OPTION value="">TODAS</option>
 <OPTION value="SI##@@SI"<?php if ($enviada == "SI") { echo " selected" ;} ?>>SI</option>
 <OPTION value="NO##@@NO"<?php if ($enviada == "NO") { echo " selected" ;} ?>>NO</option>
 <OPTION value="PT##@@PENDIENTE"<?php if ($enviada == "PT") { echo " selected" ;} ?>>PENDIENTE</option>
 <OPTION value="PR##@@PROCESO"<?php if ($enviada == "PR") { echo " selected" ;} ?>>PROCESO</option>
 </SELECT>
 </TD>
...
?>
<?php
   if (is_file("grid_facturaven_contratos_help.txt"))
   {
...
?>
<?php
   if ($nm_apl_dependente == 1 || (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['opc_psq'] && !$this->aba_iframe))
   {
...
<?php
       ...
       elseif (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dashboard_info']['under_dashboard'])
       { }
       else 
       { 
?>
...
<?php
   ...
   elseif ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['opc_psq'])
   {
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['sc_modal'])
       {
?>
...
?>
   ...
   <INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.barrio_cond.value = 'eq';
   ...
   document.F1.numcontrato_cond.value = 'eq';
   nm_campos_between(document.getElementById('id_vis_numcontrato'), document.F1.numcontrato_cond, 'numcontrato');
   document.F1.numcontrato.value = "";
   ...
   document.F1.enviada_cond.value = 'eq';
   nm_campos_between(document.getElementById('id_vis_enviada'), document.F1.enviada_cond, 'enviada');
   Sc_carga_select2('all');
 }
 function Sc_carga_select2(Field)
 {
    ...
    if (Field == 'all' || Field == 'barrio') {
       Sc_carga_select2_barrio();
    }
    if (Field == 'all' || Field == 'zona') {
       Sc_carga_select2_zona();
    }
    if (Field == 'all' || Field == 'enviada') {
       Sc_carga_select2_enviada();
    }
 }
 ...
 function Sc_carga_select2_barrio()
 {
  $("#SC_barrio").select2(
    {
      language: {
 ...
 function Sc_carga_select2_enviada()
 {
  $("#SC_enviada").select2(
    {
      language: {
 ...
 {
    $('#SC_fechaven_dia').bind('change', function() {sc_grid_facturaven_contratos_valida_dia(this)});
    $('#SC_fechaven_input_2_dia').bind('change', function() {sc_grid_facturaven_contratos_valida_dia(this)});
    $('#SC_fechaven_input_2_mes').bind('change', function() {sc_grid_facturaven_contratos_valida_mes(this)});
    $('#SC_fechaven_mes').bind('change', function() {sc_grid_facturaven_contratos_valida_mes(this)});
 }
 function sc_grid_facturaven_contratos_valida_dia(obj)
 {
 ...
 function sc_grid_facturaven_contratos_valida_mes(obj)
 {
...
<?php
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function recupera_filtro($NM_filters)
   {
      ...
      $tp_fields['SC_numcontrato_cond'] = 'cond';
      $tp_fields['SC_numcontrato'] = 'text';
      ...
      $tp_fields['SC_enviada_cond'] = 'cond';
      $tp_fields['SC_enviada'] = 'select';
   ...
   function trata_campos()
   {
      global $anio_cond, $anio,
             ...
             $asentada_cond, $asentada,
             $resolucion_cond, $resolucion,
             $enviada_cond, $enviada, $nmgp_tab_label;
      ...
      $numcontrato_cond_salva = $numcontrato_cond; 
      if (!isset($numcontrato_input_2) || $numcontrato_input_2 == "")
      {
          $numcontrato_input_2 = $numcontrato;
      }
      ...
      $enviada_cond_salva = $enviada_cond; 
      if (!isset($enviada_input_2) || $enviada_input_2 == "")
      {
          $enviada_input_2 = $enviada;
      }
      $tmp_pos = strpos($anio, "##@@");
      if ($tmp_pos === false) {
          $L_lookup = $anio;
      ...
      if ($this->NM_ajax_opcao != "ajax_grid_search_change_fil" && !empty($L_lookup) && !in_array($L_lookup, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['anio'])) {
      ...
      if ($this->NM_ajax_opcao != "ajax_grid_search_change_fil" && !empty($L_lookup) && !in_array($L_lookup, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['periodo'])) {
      ...
      if ($this->NM_ajax_opcao != "ajax_grid_search_change_fil" && !empty($L_lookup) && !in_array($L_lookup, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['zona'])) {
      ...
      if ($tmp_pos === false) {
          ...
          $L_lookup = substr($barrio, 0, $tmp_pos);
      }
      if ($this->NM_ajax_opcao != "ajax_grid_search_change_fil" && !empty($L_lookup) && !in_array($L_lookup, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['barrio'])) {
          if (!empty($this->Campos_Mens_erro)) {$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= "Barrio : " . $this->Ini->Nm_lang['lang_errm_ajax_data'];
      }
      if ($numcontrato_cond != "in")
      {
          nm_limpa_numero($numcontrato, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      $tmp_pos = strpos($asentada, "##@@");
      if ($tmp_pos === false) {
          $L_lookup = $asentada;
      ...
      if ($this->NM_ajax_opcao != "ajax_grid_search_change_fil" && !empty($L_lookup) && !in_array($L_lookup, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['asentada'])) {
      ...
      if ($this->NM_ajax_opcao != "ajax_grid_search_change_fil" && !empty($L_lookup) && !in_array($L_lookup, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['resolucion'])) {
      ...
      if ($tmp_pos === false) {
          ...
          $L_lookup = substr($enviada, 0, $tmp_pos);
      }
      if ($this->NM_ajax_opcao != "ajax_grid_search_change_fil" && !empty($L_lookup) && !in_array($L_lookup, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['psq_check_ret']['enviada'])) {
          if (!empty($this->Campos_Mens_erro)) {$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= "Enviada : " . $this->Ini->Nm_lang['lang_errm_ajax_data'];
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['anio'] = $anio; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['anio_cond'] = $anio_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['periodo'] = $periodo; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['periodo_cond'] = $periodo_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['zona'] = $zona; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['zona_cond'] = $zona_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['barrio'] = $barrio; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['barrio_cond'] = $barrio_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_dia'] = $fechaven_dia; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_mes'] = $fechaven_mes; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_ano'] = $fechaven_ano; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_input_2_dia'] = $fechaven_input_2_dia; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_input_2_mes'] = $fechaven_input_2_mes; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_input_2_ano'] = $fechaven_input_2_ano; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_cond'] = $fechaven_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['numcontrato'] = $numcontrato; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['numcontrato_cond'] = $numcontrato_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['idcli'] = $idcli; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['idcli_cond'] = $idcli_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['asentada'] = $asentada; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['asentada_cond'] = $asentada_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['resolucion'] = $resolucion; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['resolucion_cond'] = $resolucion_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['enviada'] = $enviada; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['enviada_cond'] = $enviada_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca'] = $Temp_Busca;
      }
...
if (!isset($_SESSION['gperiodo2'])) {$_SESSION['gperiodo2'] = "";}
if (!isset($this->sc_temp_gperiodo2)) {$this->sc_temp_gperiodo2 = (isset($_SESSION['gperiodo2'])) ? $_SESSION['gperiodo2'] : "";}
if (!isset($_SESSION['ganio2'])) {$_SESSION['ganio2'] = "";}
if (!isset($this->sc_temp_ganio2)) {$this->sc_temp_ganio2 = (isset($_SESSION['ganio2'])) ? $_SESSION['ganio2'] : "";}
  $this->sc_temp_ganio2    = $anio ;
$this->sc_temp_gperiodo2 = $periodo ;


$vsql = "select codigo from zona_clientes where nombre='".$zona ."' limit 1";
      ...
      { 
          ...
          { 
                 for ($SCx = 0; $SCx < $nm_count; $SCx++)
                 { 
                        $vCodZona[$SCy] [$SCx] = $SCrx->fields[$SCx];
                        $vcodzona[$SCy] [$SCx] = $SCrx->fields[$SCx];
                 }
                 $SCy++; 
                 $SCrx->MoveNext();
      ...
      { 
          $vCodZona = false;
          $vCodZona_erro = $this->Db->ErrorMsg();
          $vcodzona = false;
          $vcodzona_erro = $this->Db->ErrorMsg();
      } 
;
if(isset($vcodzona[0][0]))
{
	$this->sc_temp_gcodzona2 = $vcodzona[0][0];
}

$vsql = "select codigo from zona_clientes where resolucion='".$resolucion ."'";
...
if (isset($this->sc_temp_ganio2)) {$_SESSION['ganio2'] = $this->sc_temp_ganio2;}
if (isset($this->sc_temp_gperiodo2)) {$_SESSION['gperiodo2'] = $this->sc_temp_gperiodo2;}
...
$_SESSION['scriptcase']['grid_facturaven_contratos']['contr_erro'] = 'off'; 
      ...
      if ($barrio_LKP == $barrio)
      {
          $barrio = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['barrio'];
      }
      if ($asentada_LKP == $asentada)
      {
          $asentada = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['asentada'];
      }
      ...
      $this->cmp_formatado['barrio'] = $Conteudo;
      $Conteudo = $numcontrato;
      if (strtoupper($numcontrato_cond) != "II" && strtoupper($numcontrato_cond) != "QP" && strtoupper($numcontrato_cond) != "NP" && strtoupper($numcontrato_cond) != "IN") 
      { 
      ...
      $this->cmp_formatado['numcontrato'] = $Conteudo;
      $nmgp_def_dados = array();
    if ($idcli != '') {
      $idcli_look = substr($this->Db->qstr($idcli), 1, -1); 
      $nmgp_def_dados = array(); 
   ...
   { 
      ...
      $this->cmp_formatado['enviada'] = $Conteudo;
      ...
      if ($fechaven_cond != "bi_TP")
      {
          ...
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven'] = $val[0];
          ...
          if ($fechaven_cond == "BW")
          {
              ...
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']['fechaven_input_2'] = $val[1];
      ...
      //----- $numcontrato
      $this->Date_part = false;
      if (isset($numcontrato) || $numcontrato_cond == "nu" || $numcontrato_cond == "nn" || $numcontrato_cond == "ep" || $numcontrato_cond == "ne")
      {
         $this->monta_condicao("numcontrato", $numcontrato_cond, $numcontrato, "", "numcontrato");
      }
      ...
      if (isset($enviada))
      {
         $this->monta_condicao("enviada", $enviada_cond, $enviada, "", "enviada");
      }
   }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_contratos']['cond_pesq'] .= $this->NM_operador;
...
?>
   ...
   function css_obj_select_ajax($Obj)
   {
      switch ($Obj)
      {
         ...
         case "barrio" : return ('class="scFilterObjectEven"'); break;
