<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idpedido" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idpedido']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_pedido_14082020']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_pedido_14082020"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_imprimir_par_pedido = "<?php echo $this->form_encode_input($this->nmgp_dados_form['idpedido']); ?>";
function sc_btn_imprimir()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_imprimir(); }, 500);
      return;
    }
    sc_btn_imprimir_ok();
}
function sc_btn_imprimir_cancel()
{
}
function sc_btn_imprimir_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_pdfreport_pedidos_cti_proforma_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "par_pedido*scin' + var_btn_imprimir_par_pedido + '*scout"; ?>', 'inicio', '_blank', '0', '0', 'pdfreport_pedidos_cti_proforma');
}
...
function sc_btn_Eliminar()
{
    ...
    scJs_confirm("<?php echo html_entity_decode('¿Desea borrar el pedido?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_Eliminar_ok, sc_btn_Eliminar_cancel)
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_pedido_14082020']['where_detal']))
  {
?>  
...
function lookup_numfacven(campo)
{
   dados  = document.F1.numfacven.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_pedido_14082020')?>/form_pedido_14082020.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_numfacven, "numfacven", dados, false);
}
function recebe_numfacven(string)
{
   document.getElementById('id_lookup_numfacven').innerHTML = string;
}
<?php 
...
{
   nm_field_disabled("facturado=disabled", "");
   nm_field_disabled("numfacven=disabled", "U");
