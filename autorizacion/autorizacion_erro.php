...
$old_error_handler = set_error_handler("autorizacion_scriptcase_error_handler");

function autorizacion_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['autorizacion']['contr_erro']) && $_SESSION['scriptcase']['autorizacion']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_autorizacion->contr_autorizacion->Apl_com_erro = true;
            if ($inicial_autorizacion->contr_autorizacion->NM_ajax_flag)
            {
                $inicial_autorizacion->contr_autorizacion->NM_ajax_info['errList']['geral_autorizacion'][] = $msg;
                return;
            }
            autorizacion_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function autorizacion_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['autorizacion'])  && '' != $_SESSION['scriptcase']['error_icon']['autorizacion'])  ? $_SESSION['scriptcase']['error_icon']['autorizacion']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['autorizacion']) && '' != $_SESSION['scriptcase']['error_close']['autorizacion']) ? $_SESSION['scriptcase']['error_close']['autorizacion'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['autorizacion']) && '' != $_SESSION['scriptcase']['error_span_title']['autorizacion']) ? $_SESSION['scriptcase']['error_span_title']['autorizacion'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['autorizacion']) && '' != $_SESSION['scriptcase']['error_icon_title']['autorizacion']) ? $_SESSION['scriptcase']['error_icon_title']['autorizacion'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['autorizacion']['error_buffer'] .= ob_get_flush();
}
