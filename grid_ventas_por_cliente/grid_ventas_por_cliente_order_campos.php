...
<?php
   ...
   $Ord_Cmp = new grid_ventas_por_cliente_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_ventas_por_cliente_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_select_orig'] = array();
   }
   ...
   if ($use_alias == "S")
   {
       $tab_def_campos['t_documento'] = "t_documento";
       $tab_def_seq[10] = "t_documento";
   }
   else
   {
       $tab_def_campos['t_documento'] = "t.documento";
       $tab_def_seq[10] = "t.documento";
   }
   $tab_labels["t_documento"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['labels']["t_documento"])) ? $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['labels']["t_documento"] : "Documento";
   $tab_ger_campos['cliente'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['cliente'] = "cliente";
       $tab_def_seq[20] = "cliente";
   }
   else
   {
   ...
   if ($use_alias == "S")
   {
       ...
       $tab_def_seq[30] = "fecha";
   }
   else
   {
       $tab_def_campos['fecha'] = "f.fechaven";
       $tab_def_seq[30] = "f.fechaven";
   }
   $tab_labels["fecha"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['labels']["fecha"])) ? $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['labels']["fecha"] : "Fecha";
   $tab_ger_campos['factura'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['factura'] = "factura";
       $tab_def_seq[40] = "factura";
   }
   else
   {
       $tab_def_campos['factura'] = "concat(r.prefijo,'/',f.numfacven)";
       $tab_def_seq[40] = "concat(r.prefijo,'/',f.numfacven)";
   }
   $tab_labels["factura"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['labels']["factura"])) ? $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['labels']["factura"] : "Factura";
   $tab_ger_campos['f_subtotal'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['f_subtotal'] = "f_subtotal";
       $tab_def_seq[50] = "f_subtotal";
   }
   else
   {
       $tab_def_campos['f_subtotal'] = "f.subtotal";
       $tab_def_seq[50] = "f.subtotal";
   }
   $tab_labels["f_subtotal"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['labels']["f_subtotal"])) ? $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['labels']["f_subtotal"] : "Base";
   $tab_ger_campos['f_valoriva'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['f_valoriva'] = "f_valoriva";
       $tab_def_seq[60] = "f_valoriva";
   }
   else
   {
       $tab_def_campos['f_valoriva'] = "f.valoriva";
       $tab_def_seq[60] = "f.valoriva";
   }
   $tab_labels["f_valoriva"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['labels']["f_valoriva"])) ? $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['labels']["f_valoriva"] : "IVA";
   $tab_ger_campos['f_total'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['f_total'] = "f_total";
       $tab_def_seq[70] = "f_total";
   }
   else
   {
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
function Sel_processa_out_regr($sel_regra)
{
   ...
   foreach ($regras_ord as $cada_regra)
   {
       ...
       {
           $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_select'][$tab_def_seq[$seq]] = $ordem;
       }
   }
?>
    <script language="javascript"> 
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_ventas_por_cliente_order_campos.php",
    data: {
   ...
   <table class="<?php echo ($embbed)? '':'scGridTabela'; ?>" style="border-width: 0; border-collapse: collapse; width:100%;" cellspacing=0 cellpadding=0>
    ...
    <td colspan=5>
     <select  name="sel_regra" id="id_sel_regra_sel_ord" size=1 class="scAppDivToolbarInput">
       <OPTION value=""></OPTION>
       <OPTION value="+;30;fecha">Fecha</OPTION>
    </select>
<?php
   if (!$embbed)
   {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_ventas_por_cliente']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
