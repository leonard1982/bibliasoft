function grid_pedidos_CW_prefijo_ped_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_prefijo_ped" + seqRow).html();
}

function grid_pedidos_CW_prefijo_ped_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_prefijo_ped" + seqRow).html(newValue);
}

function grid_pedidos_CW_numpedido_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_numpedido" + seqRow).html();
}

function grid_pedidos_CW_numpedido_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_numpedido" + seqRow).html(newValue);
}

function grid_pedidos_CW_asentada_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_Hidden_asentada" + seqRow).html();
}

function grid_pedidos_CW_asentada_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_asentada" + seqRow).html(newValue);
}
function grid_pedidos_CW_asentada_Hidden_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_Hidden_asentada" + seqRow).html(newValue);
}

function grid_pedidos_CW_facturado_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_facturado" + seqRow).html();
}

function grid_pedidos_CW_facturado_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_facturado" + seqRow).html(newValue);
}

function grid_pedidos_CW_idcli_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_idcli" + seqRow).html();
}

function grid_pedidos_CW_idcli_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_idcli" + seqRow).html(newValue);
}

function grid_pedidos_CW_fechaven_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fechaven" + seqRow).html();
}

function grid_pedidos_CW_fechaven_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fechaven" + seqRow).html(newValue);
}

function grid_pedidos_CW_creado_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_creado" + seqRow).html();
}

function grid_pedidos_CW_creado_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_creado" + seqRow).html(newValue);
}

function grid_pedidos_CW_fechavenc_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_fechavenc" + seqRow).html();
}

function grid_pedidos_CW_fechavenc_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_fechavenc" + seqRow).html(newValue);
}

function grid_pedidos_CW_total_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_total" + seqRow).html();
}

function grid_pedidos_CW_total_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_total" + seqRow).html(newValue);
}

function grid_pedidos_CW_vendedor_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_vendedor" + seqRow).html();
}

function grid_pedidos_CW_vendedor_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_vendedor" + seqRow).html(newValue);
}

function grid_pedidos_CW_imprimir_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_imprimir" + seqRow).find("i").attr("src");
}

function grid_pedidos_CW_imprimir_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_imprimir" + seqRow).find("img").attr("src", newValue);
}

function grid_pedidos_CW_pos_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_pos" + seqRow).html();
}

function grid_pedidos_CW_pos_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_pos" + seqRow).html(newValue);
}

function grid_pedidos_CW_cobrar_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_cobrar" + seqRow).find("i").attr("src");
}

function grid_pedidos_CW_cobrar_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_cobrar" + seqRow).find("img").attr("src", newValue);
}

function grid_pedidos_CW_observaciones_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_observaciones" + seqRow).html();
}

function grid_pedidos_CW_observaciones_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_observaciones" + seqRow).html(newValue);
}

function grid_pedidos_CW_idpedido_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_Hidden_idpedido" + seqRow).html();
}

function grid_pedidos_CW_idpedido_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_idpedido" + seqRow).html(newValue);
}
function grid_pedidos_CW_idpedido_Hidden_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_Hidden_idpedido" + seqRow).html(newValue);
}

function grid_pedidos_CW_numfacven_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_numfacven" + seqRow).html();
}

function grid_pedidos_CW_numfacven_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_numfacven" + seqRow).html(newValue);
}

function grid_pedidos_CW_nremision_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nremision" + seqRow).html();
}

function grid_pedidos_CW_nremision_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nremision" + seqRow).html(newValue);
}

function grid_pedidos_CW_credito_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_credito" + seqRow).html();
}

function grid_pedidos_CW_credito_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_credito" + seqRow).html(newValue);
}

function grid_pedidos_CW_getValue(field, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  var SC_tmp = "";
  if ("prefijo_ped" == field) {
    SC_tmp = grid_pedidos_CW_prefijo_ped_getValue(seqRow);
  }
  if ("numpedido" == field) {
    SC_tmp = grid_pedidos_CW_numpedido_getValue(seqRow);
  }
  if ("asentada" == field) {
    SC_tmp = grid_pedidos_CW_asentada_getValue(seqRow);
  }
  if ("facturado" == field) {
    SC_tmp = grid_pedidos_CW_facturado_getValue(seqRow);
  }
  if ("idcli" == field) {
    SC_tmp = grid_pedidos_CW_idcli_getValue(seqRow);
  }
  if ("fechaven" == field) {
    SC_tmp = grid_pedidos_CW_fechaven_getValue(seqRow);
  }
  if ("creado" == field) {
    SC_tmp = grid_pedidos_CW_creado_getValue(seqRow);
  }
  if ("fechavenc" == field) {
    SC_tmp = grid_pedidos_CW_fechavenc_getValue(seqRow);
  }
  if ("total" == field) {
    SC_tmp = grid_pedidos_CW_total_getValue(seqRow);
  }
  if ("vendedor" == field) {
    SC_tmp = grid_pedidos_CW_vendedor_getValue(seqRow);
  }
  if ("imprimir" == field) {
    SC_tmp = grid_pedidos_CW_imprimir_getValue(seqRow);
  }
  if ("pos" == field) {
    SC_tmp = grid_pedidos_CW_pos_getValue(seqRow);
  }
  if ("cobrar" == field) {
    SC_tmp = grid_pedidos_CW_cobrar_getValue(seqRow);
  }
  if ("observaciones" == field) {
    SC_tmp = grid_pedidos_CW_observaciones_getValue(seqRow);
  }
  if ("idpedido" == field) {
    SC_tmp = grid_pedidos_CW_idpedido_getValue(seqRow);
  }
  if ("numfacven" == field) {
    SC_tmp = grid_pedidos_CW_numfacven_getValue(seqRow);
  }
  if ("nremision" == field) {
    SC_tmp = grid_pedidos_CW_nremision_getValue(seqRow);
  }
  if ("credito" == field) {
    SC_tmp = grid_pedidos_CW_credito_getValue(seqRow);
  }
...
function grid_pedidos_CW_setValue(field, newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  if ("prefijo_ped" == field) {
    grid_pedidos_CW_prefijo_ped_setValue(newValue, seqRow);
  }
  if ("numpedido" == field) {
    grid_pedidos_CW_numpedido_setValue(newValue, seqRow);
  }
  if ("asentada" == field) {
    grid_pedidos_CW_asentada_setValue(newValue, seqRow);
  }
  if ("facturado" == field) {
    grid_pedidos_CW_facturado_setValue(newValue, seqRow);
  }
  if ("idcli" == field) {
    grid_pedidos_CW_idcli_setValue(newValue, seqRow);
  }
  if ("fechaven" == field) {
    grid_pedidos_CW_fechaven_setValue(newValue, seqRow);
  }
  if ("creado" == field) {
    grid_pedidos_CW_creado_setValue(newValue, seqRow);
  }
  if ("fechavenc" == field) {
    grid_pedidos_CW_fechavenc_setValue(newValue, seqRow);
  }
  if ("total" == field) {
    grid_pedidos_CW_total_setValue(newValue, seqRow);
  }
  if ("vendedor" == field) {
    grid_pedidos_CW_vendedor_setValue(newValue, seqRow);
  }
  if ("imprimir" == field) {
    grid_pedidos_CW_imprimir_setValue(newValue, seqRow);
  }
  if ("pos" == field) {
    grid_pedidos_CW_pos_setValue(newValue, seqRow);
  }
  if ("cobrar" == field) {
    grid_pedidos_CW_cobrar_setValue(newValue, seqRow);
  }
  if ("observaciones" == field) {
    grid_pedidos_CW_observaciones_setValue(newValue, seqRow);
  }
  if ("idpedido" == field) {
    grid_pedidos_CW_idpedido_setValue(newValue, seqRow);
  }
  if ("numfacven" == field) {
    grid_pedidos_CW_numfacven_setValue(newValue, seqRow);
  }
  if ("nremision" == field) {
    grid_pedidos_CW_nremision_setValue(newValue, seqRow);
  }
  if ("credito" == field) {
    grid_pedidos_CW_credito_setValue(newValue, seqRow);
  }
  if ("asentada_Hidden" == field) {
    grid_pedidos_CW_asentada_Hidden_setValue(newValue, seqRow);
  }
  if ("idpedido_Hidden" == field) {
    grid_pedidos_CW_idpedido_Hidden_setValue(newValue, seqRow);
  }
}
...
function scJQAddEvents(seqRow) {
  ...
  $("#id_sc_field_idpedido" + seqRow).click(function () {
    $.ajax({
      type: "POST",
      url: "index.php",
      data: "nmgp_opcao=ajax_event&nmgp_event=idpedido_onClick&script_case_init=" + document.F3.script_case_init.value + "&idpedido=" + grid_pedidos_CW_getValue("idpedido", seqRow) + "&asentada=" + grid_pedidos_CW_getValue("asentada", seqRow) + ""
     })
     .done(function(jsonReturn) {
        ...
        if (oResp["setValue"]) {
          for (i = 0; i < oResp["setValue"].length; i++) {
            grid_pedidos_CW_setValue(oResp["setValue"][i]["field"], oResp["setValue"][i]["value"], seqRow);
          }
        }
...
function ajax_navigate(opc, parm)
{
    ...
    return new Promise(function(resolve, reject) {$.ajax({
     ...
     .done(function(jsonNavigate) {
        ...
        document.getElementById('nmsc_iframe_liga_A_grid_pedidos_CW').src = 'NM_Blank_Page.htm';
        document.getElementById('nmsc_iframe_liga_E_grid_pedidos_CW').src = 'NM_Blank_Page.htm';
        document.getElementById('nmsc_iframe_liga_D_grid_pedidos_CW').src = 'NM_Blank_Page.htm';
        document.getElementById('nmsc_iframe_liga_B_grid_pedidos_CW').src = 'NM_Blank_Page.htm';
        document.getElementById('nmsc_iframe_liga_A_grid_pedidos_CW').style.height = '0px';
        document.getElementById('nmsc_iframe_liga_E_grid_pedidos_CW').style.height = '0px';
        document.getElementById('nmsc_iframe_liga_D_grid_pedidos_CW').style.height = '0px';
        document.getElementById('nmsc_iframe_liga_B_grid_pedidos_CW').style.height = '0px';
        document.getElementById('nmsc_iframe_liga_A_grid_pedidos_CW').style.width  = '0px';
        document.getElementById('nmsc_iframe_liga_E_grid_pedidos_CW').style.width  = '0px';
        document.getElementById('nmsc_iframe_liga_D_grid_pedidos_CW').style.width  = '0px';
        document.getElementById('nmsc_iframe_liga_B_grid_pedidos_CW').style.width  = '0px';
        if (oResp["setValue"]) {
          for (i = 0; i < oResp["setValue"].length; i++) {
