<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idfaccom" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idfaccom']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['fac_compras_new03062022']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="fac_compras_new03062022"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var Crtl_btn_Eliminar = false;
function sc_btn_Eliminar()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Eliminar(); }, 500);
      return;
    }
    if (Crtl_btn_Eliminar) {return;}
    scJs_confirm("<?php echo html_entity_decode('Desea eliminar la compra?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_Eliminar_ok, sc_btn_Eliminar_cancel)
}
function sc_btn_Eliminar_cancel()
{
}
function sc_btn_Eliminar_ok()
{
    Crtl_btn_Eliminar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Eliminar?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
var var_btn_sc_btn_0_gpidcompra = "<?php echo $this->form_encode_input($this->nmgp_dados_form['idfaccom']); ?>";
...
{ 
   var sob_iframe = '';
<?php 
if ($_SESSION['sc_session'][$this->Ini->sc_page]['fac_compras_new03062022']['embutida_form']) {
    echo "   sob_iframe += 'parent.';\r\n";
}
if ($_SESSION['sc_session'][$this->Ini->sc_page]['fac_compras_new03062022']['run_iframe'] == "F" || $_SESSION['sc_session'][$this->Ini->sc_page]['fac_compras_new03062022']['run_iframe'] == "R") {
    echo "   sob_iframe += 'parent.';\r\n";
}
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['fac_compras_new03062022']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['fac_compras_new03062022']['sc_modal']) {
    echo "   sob_iframe += 'parent.';\r\n";
}
?>
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['fac_compras_new03062022']['where_detal']))
  {
?>  
...
{
   nm_field_disabled("pagada=disabled;anulada=disabled", "");
   nm_field_disabled("asentada=disabled", "I");
   
}
function sc_exib_ocult_pag(N_pagina)
{
    document.getElementById('fac_compras_new03062022_form0').style.width='1px';
    document.getElementById('fac_compras_new03062022_form0').style.height='0px';
    document.getElementById('fac_compras_new03062022_form0').style.display='none';
    document.getElementById('fac_compras_new03062022_form0').style.overflow='scroll';
    document.getElementById('fac_compras_new03062022_form1').style.width='1px';
    document.getElementById('fac_compras_new03062022_form1').style.height='0px';
    document.getElementById('fac_compras_new03062022_form1').style.display='none';
    document.getElementById('fac_compras_new03062022_form1').style.overflow='scroll';
<?php
    if (!$_SESSION['sc_session'][$this->Ini->sc_page]['fac_compras_new03062022']['pdf_view']) {
?>
...
?>
   ...
   if ("fac_compras_new03062022_form1" == N_pagina) {
