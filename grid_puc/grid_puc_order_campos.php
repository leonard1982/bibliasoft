<?php
   include_once('grid_puc_session.php');
   session_start();
   $_SESSION['scriptcase']['grid_puc']['glo_nm_path_imag_temp']  = "";
   //check tmp
   if(empty($_SESSION['scriptcase']['grid_puc']['glo_nm_path_imag_temp']))
   {
       $str_path_apl_url = $_SERVER['PHP_SELF'];
       $str_path_apl_url = str_replace("\\", '/', $str_path_apl_url);
       $str_path_apl_url = substr($str_path_apl_url, 0, strrpos($str_path_apl_url, "/"));
       $str_path_apl_url = substr($str_path_apl_url, 0, strrpos($str_path_apl_url, "/")+1);
       /*check tmp*/$_SESSION['scriptcase']['grid_puc']['glo_nm_path_imag_temp'] = $str_path_apl_url . "_lib/tmp";
   }
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['grid_puc']['glo_nm_path_imag_temp'] . "/sc_apl_default_FACILWEBv_2022.txt"))
       {
?>
...
<?php
   ...
   $Ord_Cmp = new grid_puc_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_puc_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_select_orig'] = array();
   }
   $this->restore = isset($_POST['restore']) ? true : false;
   if ($this->restore && !class_exists('Services_JSON')) {
       include_once("grid_puc_json.php");
   }
   ...
   $tab_labels["codigo"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_puc']['labels']["codigo"])) ? $_SESSION['sc_session'][$sc_init]['grid_puc']['labels']["codigo"] : "Código cta.";
   $tab_ger_campos['nombre'] = "on";
   $tab_def_campos['nombre'] = "nombre";
   $tab_labels["nombre"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_puc']['labels']["nombre"])) ? $_SESSION['sc_session'][$sc_init]['grid_puc']['labels']["nombre"] : "Nombre";
   $tab_ger_campos['aux'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['aux'] = "aux";
   }
   else
   {
       $tab_def_campos['aux'] = "";
   }
   $tab_labels["aux"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_puc']['labels']["aux"])) ? $_SESSION['sc_session'][$sc_init]['grid_puc']['labels']["aux"] : "Auxiliar";
   $tab_ger_campos['aux'] = "none";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_puc']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_puc']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_puc']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_puc']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
