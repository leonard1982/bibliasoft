...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="detallecompra_new_080822"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?>
    ...
    {
        do_ajax_detallecompra_new_080822_navigate_form();
    }
    if ("ordem" == x)
    {
        scSetOrderColumn(y);
...
function scForm_submit_single(x) {
	if (x != "excluir")
	{
		...
		if ("incluir" == x || "muda_form" == x || "recarga" == x || "recarga_mobile" == x) {
			...
			Nm_Proc_Atualiz = true;
			do_ajax_detallecompra_new_080822_submit_form();
		}
	}
	if (Nm_submit_ok) {
		Nm_Proc_Atualiz = true;
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_detallecompra_new_080822_submit_form(y, z);
...
function lookup_presentacion_(campo)
{
   ...
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('detallecompra_new_080822')?>/detallecompra_new_080822.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_presentacion_, "presentacion_", dados, false);
}
