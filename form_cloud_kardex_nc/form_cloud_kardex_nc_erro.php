...
$old_error_handler = set_error_handler("form_cloud_kardex_nc_scriptcase_error_handler");

function form_cloud_kardex_nc_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_cloud_kardex_nc']['contr_erro']) && $_SESSION['scriptcase']['form_cloud_kardex_nc']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_cloud_kardex_nc->contr_form_cloud_kardex_nc->Apl_com_erro = true;
            if ($inicial_form_cloud_kardex_nc->contr_form_cloud_kardex_nc->NM_ajax_flag)
            {
                $inicial_form_cloud_kardex_nc->contr_form_cloud_kardex_nc->NM_ajax_info['errList']['geral_form_cloud_kardex_nc'][] = $msg;
                return;
            }
            form_cloud_kardex_nc_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_cloud_kardex_nc_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_cloud_kardex_nc'])  && '' != $_SESSION['scriptcase']['error_icon']['form_cloud_kardex_nc'])  ? $_SESSION['scriptcase']['error_icon']['form_cloud_kardex_nc']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_cloud_kardex_nc']) && '' != $_SESSION['scriptcase']['error_close']['form_cloud_kardex_nc']) ? $_SESSION['scriptcase']['error_close']['form_cloud_kardex_nc'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_cloud_kardex_nc']) && '' != $_SESSION['scriptcase']['error_span_title']['form_cloud_kardex_nc']) ? $_SESSION['scriptcase']['error_span_title']['form_cloud_kardex_nc'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_cloud_kardex_nc']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_cloud_kardex_nc']) ? $_SESSION['scriptcase']['error_icon_title']['form_cloud_kardex_nc'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_cloud_kardex_nc']['error_buffer'] .= ob_get_flush();
}
