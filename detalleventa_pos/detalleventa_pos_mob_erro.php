...
$old_error_handler = set_error_handler("detalleventa_pos_mob_scriptcase_error_handler");

function detalleventa_pos_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['detalleventa_pos_mob']['contr_erro']) && $_SESSION['scriptcase']['detalleventa_pos_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_detalleventa_pos_mob->contr_detalleventa_pos_mob->Apl_com_erro = true;
            if ($inicial_detalleventa_pos_mob->contr_detalleventa_pos_mob->NM_ajax_flag)
            {
                $inicial_detalleventa_pos_mob->contr_detalleventa_pos_mob->NM_ajax_info['errList']['geral_detalleventa_pos_mob'][] = $msg;
                return;
            }
            detalleventa_pos_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function detalleventa_pos_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['detalleventa_pos_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['detalleventa_pos_mob'])  ? $_SESSION['scriptcase']['error_icon']['detalleventa_pos_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['detalleventa_pos_mob']) && '' != $_SESSION['scriptcase']['error_close']['detalleventa_pos_mob']) ? $_SESSION['scriptcase']['error_close']['detalleventa_pos_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['detalleventa_pos_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['detalleventa_pos_mob']) ? $_SESSION['scriptcase']['error_span_title']['detalleventa_pos_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['detalleventa_pos_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['detalleventa_pos_mob']) ? $_SESSION['scriptcase']['error_icon_title']['detalleventa_pos_mob'] : '';
?>
