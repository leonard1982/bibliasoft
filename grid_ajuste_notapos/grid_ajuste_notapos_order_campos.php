<?php
   include_once('grid_ajuste_notapos_session.php');
   session_start();
   $_SESSION['scriptcase']['grid_ajuste_notapos']['glo_nm_path_imag_temp']  = "";
   //check tmp
   if(empty($_SESSION['scriptcase']['grid_ajuste_notapos']['glo_nm_path_imag_temp']))
   {
       $str_path_apl_url = $_SERVER['PHP_SELF'];
       $str_path_apl_url = str_replace("\\", '/', $str_path_apl_url);
       $str_path_apl_url = substr($str_path_apl_url, 0, strrpos($str_path_apl_url, "/"));
       $str_path_apl_url = substr($str_path_apl_url, 0, strrpos($str_path_apl_url, "/")+1);
       /*check tmp*/$_SESSION['scriptcase']['grid_ajuste_notapos']['glo_nm_path_imag_temp'] = $str_path_apl_url . "_lib/tmp";
   }
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['grid_ajuste_notapos']['glo_nm_path_imag_temp'] . "/sc_apl_default_FACILWEBv_2022.txt"))
       {
?>
...
<?php
   ...
   $Ord_Cmp = new grid_ajuste_notapos_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_ajuste_notapos_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_select_orig'] = array();
   }
   $this->restore = isset($_POST['restore']) ? true : false;
   if ($this->restore && !class_exists('Services_JSON')) {
       include_once("grid_ajuste_notapos_json.php");
   }
   ...
   $tab_labels["prefijonota"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['labels']["prefijonota"])) ? $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['labels']["prefijonota"] : "Prefijo";
   $tab_ger_campos['numeronota'] = "on";
   $tab_def_campos['numeronota'] = "numeronota";
   $tab_labels["numeronota"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['labels']["numeronota"])) ? $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['labels']["numeronota"] : "Movimiento #";
   $tab_ger_campos['idtipotran'] = "on";
   $tab_def_campos['idtipotran'] = "idtipotran";
   $tab_labels["idtipotran"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['labels']["idtipotran"])) ? $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['labels']["idtipotran"] : "Concepto";
   $tab_ger_campos['fecha'] = "on";
   $tab_def_campos['fecha'] = "fecha";
   $tab_labels["fecha"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['labels']["fecha"])) ? $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['labels']["fecha"] : "Fecha";
   ...
   $tab_labels["imprimir"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['labels']["imprimir"])) ? $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['labels']["imprimir"] : "Imprimir comprobante";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
<HEAD>
 <TITLE>Ver Notas de Ajuste x existencias Negativas en inventario</TITLE>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_ajuste_notapos_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_ajuste_notapos']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
