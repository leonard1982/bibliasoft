<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idinv_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idinv_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_inventario_fisico_130521"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_inventario_fisico_130521']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_inventario_fisico_130521']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_inventario_fisico_130521']['dashboard_info']['parent_widget'] . "']\")";
...
?> 
var Crtl_btn_Ajustar = false;
function sc_btn_Ajustar()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_Ajustar(); }, 500);
      return;
    }
    if (Crtl_btn_Ajustar) {return;}
    scJs_confirm("<?php echo html_entity_decode('Proceso no se puede reversar, ¿está seguro?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_Ajustar_ok, sc_btn_Ajustar_cancel)
}
function sc_btn_Ajustar_cancel()
{
}
function sc_btn_Ajustar_ok()
{
    Crtl_btn_Ajustar = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Ajustar?@?";
    document.F1.action = "./";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_recalcular_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>', '', '_blank', '0', '0', 'recalcular');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_inventario_fisico_130521']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_form_inventario_fisico_130521_navigate_form();
    }
    if ("ordem" == x)
    {
        scSetOrderColumn(y);
...
function scForm_submit_single(x) {
	if (x != "excluir")
	{
		...
		if ("incluir" == x || "muda_form" == x || "recarga" == x || "recarga_mobile" == x) {
			...
			Nm_Proc_Atualiz = true;
			do_ajax_form_inventario_fisico_130521_submit_form();
		}
	}
	if (Nm_submit_ok) {
		Nm_Proc_Atualiz = true;
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_inventario_fisico_130521_submit_form(y, z);
...
{
   nm_field_disabled("cod_barras_=disabled;idpro_=disabled", "U");
