...
<?php
   ...
   $Ord_Cmp = new grid_productosxtraslado_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_productosxtraslado_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_select_orig'] = array();
   }
   ...
   if ($use_alias == "S")
   {
       $tab_def_campos['fl_numproducc'] = "fl_numproducc";
   }
   else
   {
       $tab_def_campos['fl_numproducc'] = "fl.numproducc";
   }
   $tab_labels["fl_numproducc"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["fl_numproducc"])) ? $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["fl_numproducc"] : "N° Traslado";
   ...
   $tab_labels["p_fecha"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["p_fecha"])) ? $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["p_fecha"] : "Fecha";
   $tab_ger_campos['fl_idpro'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['fl_idpro'] = "fl_idpro";
   }
   else
   {
       $tab_def_campos['fl_idpro'] = "fl.idpro";
   }
   $tab_labels["fl_idpro"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["fl_idpro"])) ? $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["fl_idpro"] : "Materia Prima";
   $tab_ger_campos['fl_idbod'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['fl_idbod'] = "fl_idbod";
   }
   else
   {
       $tab_def_campos['fl_idbod'] = "fl.idbod";
   }
   $tab_labels["fl_idbod"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["fl_idbod"])) ? $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["fl_idbod"] : "Origen";
   ...
   $tab_labels["fl_cantidad"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["fl_cantidad"])) ? $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["fl_cantidad"] : "Cantidad";
   $tab_ger_campos['fl_costop'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['fl_costop'] = "fl_costop";
   }
   else
   {
       $tab_def_campos['fl_costop'] = "fl.costop";
   }
   $tab_labels["fl_costop"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["fl_costop"])) ? $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["fl_costop"] : "Costo";
   $tab_ger_campos['fl_valorpar'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['fl_valorpar'] = "fl_valorpar";
   }
   else
   {
       $tab_def_campos['fl_valorpar'] = "fl.valorpar";
   }
   $tab_labels["fl_valorpar"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["fl_valorpar"])) ? $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['labels']["fl_valorpar"] : "Valor parcial";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
   $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['graf_lab'] = "";
?>
    <script language="javascript"> 
<?php
   if (!$embbed)
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_productosxtraslado_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_productosxtraslado']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
