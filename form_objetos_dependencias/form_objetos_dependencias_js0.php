<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idobjetos_dependencias_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idobjetos_dependencias_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_objetos_dependencias"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?>
    ...
    {
        do_ajax_form_objetos_dependencias_navigate_form();
    }
    if ("ordem" == x)
    {
        scSetOrderColumn(y);
...
{ 
    ...
    Nm_Proc_Atualiz = true;
    do_ajax_form_objetos_dependencias_submit_form(y, z); 
} 
<?php
}
?>
