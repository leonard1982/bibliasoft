...
class grid_historiales_crm_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_historiales_crm']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_historiales_crm']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang , $id_contacto_tercero, $atendido, $asesor;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['contr_total_geral'] = "OK";
   } 
   ...
   function quebra_fecha_sc_free_group_by($fecha, $Where_qb, $Cmp_Name) 
   {
      ...
      global $$Var_name_gb, $id_contacto_tercero, $atendido, $asesor;
      $tot_fecha = array() ;  
      $tot_fecha[0] = $fecha ; 
   }
   //-----  tipo
   ...
   //-----  notas
   function quebra_notas_sc_free_group_by($notas, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $id_contacto_tercero, $atendido, $asesor;
      $tot_notas = array() ;  
   ...
   //-----  asesor
   function quebra_asesor_sc_free_group_by($asesor, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $id_contacto_tercero, $atendido, $asesor;
      $tot_asesor = array() ;  
   ...
   //-----  motivo
   function quebra_motivo_sc_free_group_by($motivo, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $id_contacto_tercero, $atendido, $asesor;
      $tot_motivo = array() ;  
   ...
   {
      global $nm_lang, $cambiar_estado, $id_contacto_tercero, $atendido, $asesor;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('fecha' => "fecha",'tipo' => "tipo",'notas' => "notas",'asesor' => "asesor",'motivo' => "motivo");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('id_historial_crm' => 'N','asesor' => 'N','id_contacto_tercero' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_historiales_crm']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['notas']['reg'] = "S";
      $format_dimensions['notas']['msk'] = "";
      $format_dimensions['asesor']['reg'] = "S";
      $format_dimensions['asesor']['msk'] = "";
      $format_dimensions['motivo']['reg'] = "S";
      $format_dimensions['motivo']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "asesor") {
                  $this->Lookup->lookup_sc_free_group_by_asesor($$Cada_dim,  $asesor);
              }
              if ($Cada_dim == "motivo") {
              }
              if (null === $$Cada_dim)
