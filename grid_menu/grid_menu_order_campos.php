<?php
   ...
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['grid_menu']['glo_nm_path_imag_temp'] . "/sc_apl_default_FACILWEB_FE_73.txt"))
       {
?>
...
<?php
   ...
   $Ord_Cmp = new grid_menu_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_menu_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_select_orig'] = array();
   }
   ...
   if ($use_alias == "S")
   {
       $tab_def_campos['facturas'] = "facturas";
   }
   else
   {
       $tab_def_campos['facturas'] = "";
   }
   $tab_labels["facturas"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_menu']['labels']["facturas"])) ? $_SESSION['sc_session'][$sc_init]['grid_menu']['labels']["facturas"] : "Lista Facturas";
   $tab_ger_campos['notas_credito'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['notas_credito'] = "notas_credito";
   }
   else
   {
       $tab_def_campos['notas_credito'] = "";
   }
   $tab_labels["notas_credito"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_menu']['labels']["notas_credito"])) ? $_SESSION['sc_session'][$sc_init]['grid_menu']['labels']["notas_credito"] : "Notas Crédito";
   $tab_ger_campos['configuracion'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['configuracion'] = "configuracion";
   }
   else
   {
       $tab_def_campos['configuracion'] = "";
   }
   $tab_labels["configuracion"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_menu']['labels']["configuracion"])) ? $_SESSION['sc_session'][$sc_init]['grid_menu']['labels']["configuracion"] : "Configuración";
   $tab_ger_campos['copias_bd'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['copias_bd'] = "copias_bd";
   }
   else
   {
       $tab_def_campos['copias_bd'] = "";
   }
   $tab_labels["copias_bd"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_menu']['labels']["copias_bd"])) ? $_SESSION['sc_session'][$sc_init]['grid_menu']['labels']["copias_bd"] : "Copias de Seguridad";
   $tab_ger_campos['facturas'] = "none";
   $tab_ger_campos['notas_credito'] = "none";
   $tab_ger_campos['configuracion'] = "none";
   $tab_ger_campos['copias_bd'] = "none";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_menu']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_menu']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_menu']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
<HEAD>
 <TITLE><?php echo $this->Nm_lang['lang_othr_grid_title'] ?> cloud_empresas</TITLE>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_menu_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_select'];
...
<?php
  $disp_rest = "none";
  if (isset($_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_menu']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
