...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['control_recuperar_password']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="control_recuperar_password"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_btn_inicio()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_inicio(); }, 500);
      return;
    }
    sc_btn_btn_inicio_ok();
}
function sc_btn_btn_inicio_cancel()
{
}
function sc_btn_btn_inicio_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_blank_iniciar_sesion; ?>', '<?php echo $this->nm_location; ?>', '<?php echo ""; ?>', '', '_self', '0', '0', 'blank_iniciar_sesion');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_recuperar_password']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_control_recuperar_password_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}
...
{
  if (scFormHasChanged) {
    scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', function() { document.F4.target = "_self"; document.F4.action = "control_recuperar_password_fim.php"; document.F4.submit(); }, function() {});
  } else {
    document.F4.target = "_self";
    document.F4.action = "control_recuperar_password_fim.php";
    document.F4.submit();
  }
}
...
{
    NovaJanela = window.open ("control_recuperar_password_help.php?nm_cod_campo=" + campo + "&language=<?php echo $this->Ini->path_lang . $this->Ini->str_lang?>&help_css=<?php echo '../_lib/css/' . $this->Ini->str_schema_all . '_form.css'; ?>", "ScriptCase", "height=100, width=300, resizable, scrollbars");
}
