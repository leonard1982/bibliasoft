...
class grid_pagos_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_pagos']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_pagos']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang , $client, $banco, $id_concepto;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(montocan), sum(ret) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['tot_geral'][0] = "Egresos y retenciones practicadas"; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['contr_total_geral'] = "OK";
   } 
   ...
   //-----  client
   function quebra_client_tercero($client, $arg_sum_client) 
   {
      global $tot_client, $client, $banco, $id_concepto;
      $tot_client = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(montocan), sum(ret) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['where_pesq']; 
      ...
      { 
         $nm_comando .= " where client" . $arg_sum_client ; 
      } 
      else 
      { 
         $nm_comando .= " and client" . $arg_sum_client ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_client[0] = sc_strip_script($client) ; 
      $tot_client[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_client[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_client[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
   } 

   //-----  fecpago
   function quebra_fecpago_fecha($fecpago, $arg_sum_fecpago) 
   {
      global $tot_fecpago, $client, $banco, $id_concepto;
      $tot_fecpago = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(montocan), sum(ret) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['where_pesq']; 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['where_pesq'])) 
      { 
         $nm_comando .= " where " .  $this->Sc_groupby_fecpago . $arg_sum_fecpago ; 
      } 
      else 
      { 
         $nm_comando .= " and " .  $this->Sc_groupby_fecpago . $arg_sum_fecpago ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_fecpago[0] = NM_encode_input(sc_strip_script($fecpago)) ; 
      $tot_fecpago[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_fecpago[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_fecpago[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
   } 
   ...
   {
      global $nm_lang, $imprimir, $client, $banco, $id_concepto;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('client' => "client");
      $cmps_quebra_atual = array("client");
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_tercero($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('idpago' => 'N','numpago' => 'N','client' => 'N','montocan' => 'N','ret' => 'N','descuent' => 'N','iddocapagar' => 'N','saldodocumento' => 'N','banco' => 'N','id_concepto' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(montocan), sum(ret)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2)#@#cmps_quebras#@# from (";
         $comando .= "select montocan as SC_metric1,ret as SC_metric2, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['client']['reg'] = "S";
      $format_dimensions['client']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "client") {
                  $this->Lookup->lookup_tercero_client($$Cada_dim,  $client);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   function Calc_resumo_fecha($destino_resumo)
   {
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pagos']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('fecpago' => "fecpago");
      $cmps_quebra_atual = array("fecpago");
      $ult_cmp_quebra_atual = $cmps_quebra_atual[(count($cmps_quebra_atual) - 1)];
      $arr_tots = "";
      $join     = "";
      $group    = "";
   ...
   function Res_Totaliza_fecha($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $format_dimensions = array();
      $format_dimensions['fecpago']['reg'] = "S";
      $format_dimensions['fecpago']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
