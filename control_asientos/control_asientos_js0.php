...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['control_asientos']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="control_asientos"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_btn_grid_asientos()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_grid_asientos(); }, 500);
      return;
    }
    scJs_confirm("<?php echo html_entity_decode('¿Desea ir a la lista de asientos ya generados?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_btn_grid_asientos_ok, sc_btn_btn_grid_asientos_cancel);
}
function sc_btn_btn_grid_asientos_cancel()
{
}
function sc_btn_btn_grid_asientos_ok()
{
      nm_gp_submit('<?php echo $this->Ini->link_grid_asientos_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo ""; ?>', 'inicio', '_self', '0', '0', 'grid_asientos');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_asientos']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_control_asientos_navigate_form();
    }
} 
...
function scForm_submit_single(x) {
	if (x != "excluir")
	{
		...
		if ("incluir" == x || "muda_form" == x || "recarga" == x || "recarga_mobile" == x) {
			...
			Nm_Proc_Atualiz = true;
			do_ajax_control_asientos_submit_form();
		}
	}
	if (Nm_submit_ok) {
		Nm_Proc_Atualiz = true;
...
{
  if (scFormHasChanged) {
    scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', function() { document.F4.target = "_self"; document.F4.action = "control_asientos_fim.php"; document.F4.submit(); }, function() {});
  } else {
    document.F4.target = "_self";
    document.F4.action = "control_asientos_fim.php";
    document.F4.submit();
  }
}
