  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_control_asientos_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_control_asientos_mob_form();
	}
...
?>
  ...
  // ---------- Validate desde
  function do_ajax_control_asientos_mob_validate_desde()
  {
    ...
    setTimeout(function() {
      var var_desde = scAjaxGetFieldText(nomeCampo_desde);
      x_ajax_control_asientos_mob_validate_desde(var_desde, var_script_case_init, do_ajax_control_asientos_mob_validate_desde_cb);
    }, 200);
  } // do_ajax_control_asientos_mob_validate_desde

  function do_ajax_control_asientos_mob_validate_desde_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_asientos_mob_validate_desde_cb

  // ---------- Validate hasta
  function do_ajax_control_asientos_mob_validate_hasta()
  {
    ...
    setTimeout(function() {
      var var_hasta = scAjaxGetFieldText(nomeCampo_hasta);
      x_ajax_control_asientos_mob_validate_hasta(var_hasta, var_script_case_init, do_ajax_control_asientos_mob_validate_hasta_cb);
    }, 200);
  } // do_ajax_control_asientos_mob_validate_hasta

  function do_ajax_control_asientos_mob_validate_hasta_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "hasta";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_asientos_mob_validate_hasta_cb

  // ---------- Validate regenerar
  function do_ajax_control_asientos_mob_validate_regenerar()
  {
    var nomeCampo_regenerar = "regenerar";
    var var_regenerar = scAjaxGetFieldSelect(nomeCampo_regenerar);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_asientos_mob_validate_regenerar(var_regenerar, var_script_case_init, do_ajax_control_asientos_mob_validate_regenerar_cb);
  } // do_ajax_control_asientos_mob_validate_regenerar

  function do_ajax_control_asientos_mob_validate_regenerar_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "regenerar";
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sAppErrors = scAjaxListErrors(true);
    if ("" == sAppErrors)
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_asientos_mob_validate_regenerar_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_control_asientos_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_control_asientos_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_desde = scAjaxGetFieldText("desde");
    var var_hasta = scAjaxGetFieldText("hasta");
    var var_regenerar = scAjaxGetFieldSelect("regenerar");
    ...
    scAjaxProcOn();
    x_ajax_control_asientos_mob_submit_form(var_desde, var_hasta, var_regenerar, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_control_asientos_mob_submit_form_cb);
  } // do_ajax_control_asientos_mob_submit_form

  function do_ajax_control_asientos_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    ...
    if (scAjaxIsOk())
    {
      ...
      scLigEditLookupCall();
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_asientos_mob']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['control_asientos_mob']['dashboard_info']['under_dashboard']) {
?>
      var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['control_asientos_mob']['dashboard_info']['parent_widget']; ?>']");
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_control_asientos_mob_submit_form_cb_after_alert);
  } // do_ajax_control_asientos_mob_submit_form_cb
  function do_ajax_control_asientos_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_control_asientos_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_control_asientos_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_control_asientos_mob_navigate_form, function() {});
    } else {
      perform_ajax_control_asientos_mob_navigate_form();
    }
  }

  function perform_ajax_control_asientos_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_control_asientos_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['control_asientos_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_control_asientos_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_control_asientos_mob_navigate_form_cb_after_alert);
  } // do_ajax_control_asientos_mob_navigate_form_cb
  function do_ajax_control_asientos_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_control_asientos_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_control_asientos_mob_navigate_form_cb_after_alert
  function sc_hide_control_asientos_mob_form()
  {
  ...
  ajax_field_list[0] = "desde";
  ajax_field_list[1] = "hasta";
  ajax_field_list[2] = "regenerar";
  ...
  var ajax_error_list = {
    ...
    "regenerar": {"label": "Opcional", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  ajax_field_mult["regenerar"][1] = "regenerar";
  ...
  var ajax_field_id = {
    ...
    "regenerar": new Array("hidden_field_label_regenerar", "hidden_field_data_regenerar")
  };
