<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="idempresa" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idempresa']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_empresas']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_empresas"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_empresas']['where_detal']))
  {
?>  
...
?>
    ...
    if (x == "excluir") 
    { 
       if (confirm ("¿Desea borrar la empresa? Tenga cuidado, se borrará la base de datos correspondiente a esa empresa y cualquier cambio que haya hecho en dicha base de datos."))  
       { 
           scAjaxProcOn();
...
function scForm_confirmDelete(callbackOk, callbackCancel) {
	scJs_confirm("¿Desea borrar la empresa? Tenga cuidado, se borrará la base de datos correspondiente a esa empresa y cualquier cambio que haya hecho en dicha base de datos.", callbackOk, callbackCancel);
...
{
    NovaJanela = window.open ("form_empresas_help.php?nm_cod_campo=" + campo + "&language=<?php echo $this->Ini->path_lang . $this->Ini->str_lang?>&help_css=<?php echo '../_lib/css/' . $this->Ini->str_schema_all . '_form.css'; ?>", "ScriptCase", "height=100, width=300, resizable, scrollbars");
}
