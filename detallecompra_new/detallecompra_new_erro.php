...
$old_error_handler = set_error_handler("detallecompra_new_scriptcase_error_handler");

function detallecompra_new_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['detallecompra_new']['contr_erro']) && $_SESSION['scriptcase']['detallecompra_new']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_detallecompra_new->contr_detallecompra_new->Apl_com_erro = true;
            if ($inicial_detallecompra_new->contr_detallecompra_new->NM_ajax_flag)
            {
                $inicial_detallecompra_new->contr_detallecompra_new->NM_ajax_info['errList']['geral_detallecompra_new'][] = $msg;
                return;
            }
            detallecompra_new_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function detallecompra_new_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['detallecompra_new'])  && '' != $_SESSION['scriptcase']['error_icon']['detallecompra_new'])  ? $_SESSION['scriptcase']['error_icon']['detallecompra_new']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['detallecompra_new']) && '' != $_SESSION['scriptcase']['error_close']['detallecompra_new']) ? $_SESSION['scriptcase']['error_close']['detallecompra_new'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['detallecompra_new']) && '' != $_SESSION['scriptcase']['error_span_title']['detallecompra_new']) ? $_SESSION['scriptcase']['error_span_title']['detallecompra_new'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['detallecompra_new']) && '' != $_SESSION['scriptcase']['error_icon_title']['detallecompra_new']) ? $_SESSION['scriptcase']['error_icon_title']['detallecompra_new'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['detallecompra_new']['error_buffer'] .= ob_get_flush();
}
