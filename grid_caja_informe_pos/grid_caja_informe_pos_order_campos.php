<?php
   include_once('grid_caja_informe_pos_session.php');
   session_start();
   $_SESSION['scriptcase']['grid_caja_informe_pos']['glo_nm_path_imag_temp']  = "";
   //check tmp
   if(empty($_SESSION['scriptcase']['grid_caja_informe_pos']['glo_nm_path_imag_temp']))
   {
       $str_path_apl_url = $_SERVER['PHP_SELF'];
       $str_path_apl_url = str_replace("\\", '/', $str_path_apl_url);
       $str_path_apl_url = substr($str_path_apl_url, 0, strrpos($str_path_apl_url, "/"));
       $str_path_apl_url = substr($str_path_apl_url, 0, strrpos($str_path_apl_url, "/")+1);
       /*check tmp*/$_SESSION['scriptcase']['grid_caja_informe_pos']['glo_nm_path_imag_temp'] = $str_path_apl_url . "_lib/tmp";
   }
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['grid_caja_informe_pos']['glo_nm_path_imag_temp'] . "/sc_apl_default_FACILWEBv_2022.txt"))
       {
?>
...
<?php
   ...
   $Ord_Cmp = new grid_caja_informe_pos_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_caja_informe_pos_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_select_orig'] = array();
   }
   $this->restore = isset($_POST['restore']) ? true : false;
   if ($this->restore && !class_exists('Services_JSON')) {
       include_once("grid_caja_informe_pos_json.php");
   }
   ...
   $tab_labels["banco"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['labels']["banco"])) ? $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['labels']["banco"] : "Banco/Caja";
   $tab_ger_campos['fecha'] = "on";
   $tab_def_campos['fecha'] = "fecha";
   $tab_labels["fecha"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['labels']["fecha"])) ? $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['labels']["fecha"] : "Fecha";
   $tab_ger_campos['cantidad'] = "on";
   $tab_def_campos['cantidad'] = "cantidad";
   $tab_labels["cantidad"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['labels']["cantidad"])) ? $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['labels']["cantidad"] : "Facturado";
   $tab_ger_campos['efectivo'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['efectivo'] = "efectivo";
   }
   else
   {
       $tab_def_campos['efectivo'] = "";
   }
   $tab_labels["efectivo"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['labels']["efectivo"])) ? $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['labels']["efectivo"] : "Efectivo";
   ...
   $tab_labels["tarjeta"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['labels']["tarjeta"])) ? $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['labels']["tarjeta"] : "Tarjeta Cred/Deb";
   $tab_ger_campos['cheques'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['cheques'] = "cheques";
   }
   else
   {
       $tab_def_campos['cheques'] = "";
   }
   $tab_labels["cheques"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['labels']["cheques"])) ? $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['labels']["cheques"] : "Cheques u otros";
   $tab_ger_campos['formas_pago'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['formas_pago'] = "formas_pago";
   }
   else
   {
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_caja_informe_pos_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_caja_informe_pos']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
