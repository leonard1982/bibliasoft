...
class grid_terceros_todos_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_terceros_todos']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_terceros_todos']['campos_busca']))
      { 
   ...
   function quebra_geral_vendedor($res_limit=false)
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['where_pesq']; 
      } 
      ...
      if ($rt->fields[0] == 0)
      { 
          if (!isset($Contrl_Interat) && empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['where_pesq_filtro']) && empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['where_pesq_fast']) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['interativ_search']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['interativ_search']))
          {
              $Contrl_Interat = 1;
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['where_pesq']       = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['where_sem_interativ'];
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['interativ_search'] = array();
              $this->quebra_geral_vendedor();
          }
          
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['contr_total_geral'] = "OK";
   } 
   ...
   //-----  loatiende
   function quebra_loatiende_sc_free_group_by($loatiende, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $idmuni, $loatiende;
      $tot_loatiende = array() ;  
      $tot_loatiende[0] = $loatiende ; 
   }
   ...
   {
      global $nm_lang, $facturas, $sc_asigna_vendedor, $idmuni, $loatiende;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['sql_tot_res']);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_vendedor($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "loatiende") {
                  $this->Lookup->lookup_sc_free_group_by_loatiende($$Cada_dim,  $loatiende);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
