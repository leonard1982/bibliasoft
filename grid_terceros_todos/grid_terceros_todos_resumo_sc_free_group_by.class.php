...
class grid_terceros_todos_resumo
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($tipo = "")
   {
      ...
      $this->array_total_loatiende = array();
   ...
   function initializeButtons()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['dashboard_info']['under_dashboard'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['dashboard_info']['maximized']) {
          $tmpDashboardApp = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['dashboard_info']['dashboard_app'];
          if (isset($_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['grid_terceros_todos'])) {
              $tmpDashboardButtons = $_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['grid_terceros_todos'];
      ...
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_terceros_todos']['btn_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['grid_terceros_todos']['btn_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['grid_terceros_todos']['btn_display'] as $NM_cada_btn => $NM_cada_opc)
          {
   ...
   { 
	...
	function generateChartImages() {
		...
		$this->Graf         = new grid_terceros_todos_grafico();
		$this->Graf->Db     = $this->Db;
		$this->Graf->Erro   = $this->Erro;
		$this->Graf->Ini    = $this->Ini;
		$this->Graf->Lookup = $this->Lookup;

		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_step']     = 'image';
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_count']    = 0;
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_total']    = 0;
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_step'] = 'end';
	...
	function zipFileList($fileList, $password = '') {
		$tempDir     = $this->Ini->root . $_SESSION['scriptcase']['grid_terceros_todos']['glo_nm_path_imag_temp'] . '/';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['grid_terceros_todos']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
...
$_SESSION['scriptcase']['grid_terceros_todos']['contr_erro'] = 'off'; 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] != "pdf")
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['where_resumo']);
      }
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['res_hrz'];
      if (isset($_SESSION['scriptcase']['sc_aba_iframe']) && !$this->NM_export)
      {
          foreach ($_SESSION['scriptcase']['sc_aba_iframe'] as $aba => $apls_aba)
          {
              if (in_array("grid_terceros_todos", $apls_aba))
              {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['iframe_menu'] && !$this->NM_export && (!isset($_SESSION['scriptcase']['menu_mobile']) || empty($_SESSION['scriptcase']['menu_mobile'])))
      {
          $this->aba_iframe = true;
      }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['where_resumo'] = "";
      }
      $this->inicializa_vars();
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_Ind_Groupby'];
      $this->Total->$Gb_geral();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['contr_total_geral'] == "OK")
      {
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
          {
              $Arr_tot_name = "array_total_" . $cmp_gb;
              $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['arr_total'][$cmp_gb];
          }
      }
      else
      {
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['tot_geral'][1];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['tot_geral'][1] == 0)
      {
      ...
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] == 'print' || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] == 'pdf') && strpos(" " . $this->Ini->SC_module_export, "resume") === false)
      { }
      else
      {
          $this->drawMatrix();
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['arr_total']['loatiende'] = $this->array_total_loatiende;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['json_use_label'])
       {
          $Lab_loatiende = "loatiende";
       }
       else
       {
       $Lab_loatiende = "Vendedor";
       }
       $prep_field = array();
       $prep_field['loatiende'] = $Lab_loatiende;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['loatiende'] = $Lab_loatiende; 
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
          $Str_gb .= ($Str_gb == "") ? "" : ",";
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['ccnit']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['ccnit'] = "Identificación"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['nombres']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['nombres'] = "Nombres"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['direccion']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['direccion'] = "Dirección"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['idmuni']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['idmuni'] = "Ciudad"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['tel_cel']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['tel_cel'] = "Tel. o celular"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['cliente']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['cliente'] = "Cliente"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['proveedor']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['proveedor'] = "Proveedor"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['empleado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['empleado'] = "Empleado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['si_nomina']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['si_nomina'] = "Nómina"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['idtercero']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['idtercero'] = "Idtercero"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['documento']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['documento'] = "Cédula/Nit"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['nacimiento']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['nacimiento'] = "Fec. Nacimiento"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['sexo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['sexo'] = "Género"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['urlmail']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['urlmail'] = "E-mail"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['fechault']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['fechault'] = "Última comp."; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['saldo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['saldo'] = "Saldo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['afiliacion']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['afiliacion'] = "Cliente desde"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['regimen']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['regimen'] = "Regimen"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['tipo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['tipo'] = "Tipo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['observaciones']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['observaciones'] = "Observaciones"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['loatiende']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['loatiende'] = "Vendedor"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['dv']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['dv'] = "DV"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['contacto']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['contacto'] = "Contacto"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['credito']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['credito'] = "Credito"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['cupo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['cupo'] = "Cupo"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['listaprecios']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['listaprecios'] = "Listaprecios"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['con_actual']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['con_actual'] = "Con Actual"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['efec_retencion']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['efec_retencion'] = "Efec Retencion"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['urlmail_1']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['urlmail_1'] = "Urlmail"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['nombre1']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['nombre1'] = "Nombre 1"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['nombre2']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['nombre2'] = "Nombre 2"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['apellido1']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['apellido1'] = "Apellido 1"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['apellido2']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['apellido2'] = "Apellido 2"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['sucur_cliente']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['sucur_cliente'] = "Sucur Cliente"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['representante']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['representante'] = "Representante"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['es_restaurante']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['es_restaurante'] = "Es Restaurante"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['dias_credito']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['dias_credito'] = "Dias Credito"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['dias_mora']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['dias_mora'] = "Dias Mora"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['cupo_vendedor']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['cupo_vendedor'] = "Cupo Vendedor"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['codigo_ter']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['codigo_ter'] = "Codigo Ter"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['es_cajero']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['es_cajero'] = "Es Cajero"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['autorizado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['autorizado'] = "Autorizado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['zona_clientes']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['zona_clientes'] = "Zona Clientes"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['clasificacion_clientes']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['clasificacion_clientes'] = "Clasificacion Clientes"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['creado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['creado'] = "Creado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['disponible']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['disponible'] = "Disponible"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['id_pedido_tmp']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['id_pedido_tmp'] = "Id Pedido Tmp"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['n_pedido_tmp']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['n_pedido_tmp'] = "N Pedido Tmp"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['total_pedido_tmp']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['total_pedido_tmp'] = "Total Pedido Tmp"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['obs_pedido_tmp']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['obs_pedido_tmp'] = "Obs Pedido Tmp"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['vend_pedido_tmp']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['vend_pedido_tmp'] = "Vend Pedido Tmp"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['ciudad']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['ciudad'] = "Ciudad"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['codigo_postal']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['codigo_postal'] = "Codigo Postal"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['lenguaje']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['lenguaje'] = "Lenguaje"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['nombre_comercil']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['nombre_comercil'] = "Nombre Comercil"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['notificar']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['notificar'] = "Notificar"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['puc_auxiliar_deudores']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['puc_auxiliar_deudores'] = "Puc Auxiliar Deudores"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['puc_retefuente_ventas']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['puc_retefuente_ventas'] = "Puc Retefuente Ventas"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['puc_retefuente_servicios_clie']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['puc_retefuente_servicios_clie'] = "Puc Retefuente Servicios Clie"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['puc_auxiliar_proveedores']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['puc_auxiliar_proveedores'] = "Puc Auxiliar Proveedores"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['puc_retefuente_compras']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['puc_retefuente_compras'] = "Puc Retefuente Compras"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['puc_retefuente_servicios_prov']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['puc_retefuente_servicios_prov'] = "Puc Retefuente Servicios Prov"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['nube']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['nube'] = "Nube"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['tipo_documento']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['tipo_documento'] = "Tipo Documento"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['estado']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['estado'] = "Estado"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['facturas']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['facturas'] = "Listar Cartera"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['sc_asigna_vendedor']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['sc_asigna_vendedor'] = "Lo Atiende"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['imagenter']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['labels']['imagenter'] = "Imagenter"; 
       }
       //-----
       $ix = 0;
       $this->comp_field_nm = array();
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['summarizing_fields_order']['sc_free_group_by']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['summarizing_fields_order']['sc_free_group_by'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['summarizing_fields_order']['sc_free_group_by'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['summarizing_fields_display']['sc_free_group_by'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['summarizing_fields_display']['sc_free_group_by'] as $i_sum => $d_sum)
           {
      ...
      eval ($Str_gb);
       ...
       $Arr_parms = array();
       $Arr_parms['loatiende']['alin'] = "''";
       $Arr_parms['loatiende']['link'] = "S";
       $Arr_parms['loatiende']['fill'] = "true";
       $Arr_parms['loatiende']['order'] = "label";
       $Arr_parms['loatiende']['order_start'] = 'asc';
       $Arr_parms['loatiende']['order_invert'] = 'false';
       $Arr_parms['loatiende']['order_enabled'] = 'false';
       //-----
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
       ...
       //-----
       $prep_links_fl = array();
       $prep_links_fl['loatiende'] = array(0 => 'loatiende', 1 => '');
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_Gb_Free_cmp'] as $cmp_gb => $col_sql)
       {
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_col'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_level'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_sort'] = '';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_col']  = 0;
           }
           ...
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_tabular_label'];
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@grid_terceros_todos@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function makeTabular(&$a_data)
   {
       ...
       {
           $a_labels = array();
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['proc_pdf_vert'])
   {
   ...
   function buildChart()
   {
       ...
       $prep_chart_config = array(
           ...
           '1' => array(
               ...
               'label_x'  => $this->getChartText(strip_tags($Lab_loatiende)),
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_charts']    = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_orig']      = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['comb_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['comb_table_def']  = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['anlt_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['anlt_table_def']  = array();
	...
	function filterChartsByGroupbyLevel() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['chart_level']) && '' !== $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['chart_level']) {
			$applyFilter = true;
		}
		elseif (isset($nmgp_opcao) && 'image_res' == $nmgp_opcao && isset($_POST['chart_level']) && '' !== $_POST['chart_level']) {
			$applyFilter = true;
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['chart_level'] = $_POST['chart_level'];
		}
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_charts'] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_orig'];

		if ($applyFilter) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_charts'] as $chartIndex => $chartData) {
				...
				if ($groupbyLevel > $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['chart_level']) {
					unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_charts'][$chartIndex]);
				}
			}
		}

		$this->comp_chart_data = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_charts'];
	}
	...
	function filterChartsBySummarization() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao)) {
			...
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['summarizing_fields_display']['sc_free_group_by'] as $fieldIndex => $fieldData) {
           ...
           foreach ($a_totals as $i_total => $values)
           {
               ...
               if (isset($this->comp_chart_config[$key_config]))
               {
                   ...
                   foreach ($a_keys as $tmp_key)
                   {
                       ...
                       $xml_links[]  = $this->getFusionLink('sc_grid_terceros_todos_' . session_id() . '_!!!' . implode('_', array_merge(array($group + 1, $i_total), $link_index)) . '!!!.json');
                   }
                   ...
                   $this->comp_chart_data[$key_data]['xml']        = $this->getFusionLink('sc_grid_terceros_todos_' . session_id() . '_!!!' . implode('_', array_merge(array($group, $i_total), $param)) . '!!!.json');
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['arr_export']['data']  = $this->build_data;
           return;
       }
      ...
      $TD_padding = (!$this->Print_All && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] == "pdf") ? " style=\"padding: 0px !important;\"" : "";
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               if (0 <= $columns['level'])
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['embutida'])
                   {
               ...
               $chart   = (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] != "print" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['embutida']) && isset($columns['chart']) && '' != $columns['chart'] && isset($this->comp_chart_data[ $columns['chart'] ]))
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   ...
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialDisplay = '; display: none';
                           $sInitialSrc     = '';
                       }
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['proc_pdf_vert']){ 
   ...
   function resumo_init()
   {
      ...
      if ("out" == $this->NM_tipo)
      {
         $this->monta_html_ini();
         $this->monta_cabecalho();
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] != "pdf" && !$_SESSION['scriptcase']['proc_mobile'])
         {
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['grid_terceros_todos']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['grid_terceros_todos']) . "_";
               } 
           } 
           else 
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['grid_terceros_todos']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['grid_terceros_todos']) . "_";
               } 
           }
       }
   ...
   function inicializa_vars()
   {
      ...
      if ("out" == $this->NM_tipo)
      {
         require_once($this->Ini->path_aplicacao . "grid_terceros_todos_total.class.php"); 
      }
      $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['print_all'];
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['doc_word'] || $this->Ini->sc_export_ajax_img)
      { 
      ...
      if ($this->Print_All)
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] = "print";
      ...
      $this->Total   = new grid_terceros_todos_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['arr_total'][$cmp_gb];
      }
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['dashboard_refresh_after_chart'] = 'resumo';
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['doc_word'])
{
      ...
      { 
           ...
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"grid_terceros_todos_ajax.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida(" </script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"grid_terceros_todos_jquery-3.6.0.min.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"grid_terceros_todos_message.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput2.js\"></script>\r\n");
...
$nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "grid_terceros_todos/grid_terceros_todos_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
           $nm_saida->saida("   <link rel=\"stylesheet\" href=\"" . $this->Ini->path_prod . "/third/jquery_plugin/thickbox/thickbox.css\" type=\"text/css\" media=\"screen\" />\r\n");
           $nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/buttons/" . $this->Ini->Str_btn_css . "\" /> \r\n");
      } 
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['num_css']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['num_css'] = rand(0, 1000);
      }
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_terceros_todos_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_grid_terceros_todos_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_terceros_todos_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['num_css'] . '.css');
...
$nm_saida->saida("      url: \"grid_terceros_todos_save_grid.php\",\r\n");
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if (!$this->Ini->Export_html_zip && $this->Print_All && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['doc_word']) 
      {
      ...
      {
          $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['dashboard_info']['remove_margin'] ? 'margin: 0;' : '';
          $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['dashboard_info']['remove_border'] ? 'border-width: 0;' : '';
          $vertical_center = '';
          $nm_saida->saida(" <BODY id=\"grid_summary\" class=\"" . $this->css_scGridPage . "\" style=\"" . $remove_margin . $vertical_center . "\">\r\n");
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['embutida'])
      {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['embutida'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['doc_word'])
      { 
          $nm_saida->saida("      <STYLE>\r\n");
   ...
   function monta_barra_top_normal()
   {
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['pdf'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Rpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_terceros_todos/grid_terceros_todos_config_pdf.php?nm_opc=pdf_res&nm_target=0&nm_cor=cor&papel=1&orientacao=1&bookmarks=1&largura=1200&conf_larg=S&conf_fonte=10&grafico=S&sc_ver_93=" . s . "&nm_tem_gb=s&nm_res_cons=s&nm_ini_pdf_res=resume,chart&nm_all_modules=grid,resume,chart&password=n&summary_export_columns=N&nm_label_group=S&nm_all_cab=N&nm_all_label=N&pdf_zip=N&nm_orient_grid=2&origem=res&language=es&conf_socor=S&script_case_init=" . $this->Ini->sc_page . "&app_name=grid_terceros_todos&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
         if ($NM_Gbtn)
      ...
      if ($this->nmgp_botoes['chart_conf'] == 'on' && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bconf_graf", "", "", "Rgraf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_terceros_todos/grid_terceros_todos_config_graf_flash.php?nome_apl=grid_terceros_todos&campo_apl=nm_resumo_graf&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=es&tp_apl=cns_htm&KeepThis=true&TB_iframe=true&height=450&width=400&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
      }
      ...
      if ($this->nmgp_botoes['groupby'] == "on" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['opc_psq'] && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
          $Q_free  = false;
          $Q_count = 0;
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_All_Groupby'] as $QB => $Tp)
          {
              if (!in_array($QB, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_Groupby_hide']) && $Tp == "all")
              {
                  $Q_count++;
          ...
          if ($Q_count > 1 || $Q_free)
          {
             $this->nm_btn_exist['groupby'][] = "sel_groupby_top";
              $Cod_Btn = nmButtonOutput($this->arr_buttons, "bgroupby", "scBtnGroupByShow('" . $this->Ini->path_link . "grid_terceros_todos/grid_terceros_todos_sel_groupby.php?opc_ret=resumo&path_img=" . $this->Ini->path_img_global . "&path_btn=" . $this->Ini->path_botoes . "&script_case_init=" . NM_encode_input($this->Ini->sc_page) . "&embbed_groupby=Y&toolbar_pos=top', 'top');", "scBtnGroupByShow('" . $this->Ini->path_link . "grid_terceros_todos/grid_terceros_todos_sel_groupby.php?opc_ret=resumo&path_img=" . $this->Ini->path_img_global . "&path_btn=" . $this->Ini->path_botoes . "&script_case_init=" . NM_encode_input($this->Ini->sc_page) . "&embbed_groupby=Y&toolbar_pos=top', 'top');", "sel_groupby_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "", "", "", "only_text", "text_right", "", "", "", "", "", "", "");
              $nm_saida->saida("           $Cod_Btn \r\n");
          }
              $NM_btn = true;
      }
      if ($this->nmgp_botoes['chart_detail'] == 'on' && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
         ...
         if (is_file("grid_terceros_todos_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
            $Arq_WebHelp = file("grid_terceros_todos_help.txt"); 
   ...
   function monta_html_fim()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['print_all'])
      {
          $this->grafico_pdf();
      }
      $nm_saida->saida("</TABLE>\r\n");
      $nm_saida->saida("<link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/css/" . $this->Ini->str_schema_all . "_sweetalert.css\" />\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['grid_terceros_todos']['glo_nm_path_prod'] . "/third/sweetalert/sweetalert2.all.min.js\"></script>\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['grid_terceros_todos']['glo_nm_path_prod'] . "/third/sweetalert/polyfill.min.js\"></script>\r\n");
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'grid_terceros_todos_config_pivot.php?nome_apl=grid_terceros_todos&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=es&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   $nm_saida->saida("                  action=\"grid_terceros_todos_iframe_prt.php\" \r\n");
   ...
   $nm_saida->saida("   }\r\n");
      ...
      $nm_saida->saida("               nm_submit_modal(\"" . $this->Ini->path_link . "grid_terceros_todos/grid_terceros_todos_export_email.php?script_case_init={$this->Ini->sc_page}&path_img={$this->Ini->path_img_global}&path_btn={$this->Ini->path_botoes}&sType=\"+ str_type +\"&sAdd=__E__nmgp_cor_word=\" + cor + \"__E__SC_module_export=\" + SC_module_export + \"__E__nmgp_password=\" + password + \"&KeepThis=true&TB_iframe=true&modal=true\", bol_param);\r\n");
      $nm_saida->saida("       }\r\n");
      $nm_saida->saida("       else\r\n");
      $nm_saida->saida("       {\r\n");
      ...
      $nm_saida->saida("               document.Fdoc_word.action = \"grid_terceros_todos_export_ctrl.php\";\r\n");
      ...
      $nm_saida->saida("               document.Fprint.action = \"grid_terceros_todos_export_ctrl.php\";\r\n");
      $nm_saida->saida("           }\r\n");
      $nm_saida->saida("           else\r\n");
      $nm_saida->saida("           {\r\n");
      ...
      $nm_saida->saida("               document.Fexport.action = \"grid_terceros_todos_export_ctrl.php\";\r\n");
      ...
      $nm_saida->saida("       document.F3.SC_module_export.value = z;\r\n");
      $nm_saida->saida("  }\r\n");
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['grid_terceros_todos_iframe_params'] = array(
      ...
      $Md5_pdf    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@grid_terceros_todos@SC_par@" . md5($prep_parm_pdf);
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['Md5_pdf'][md5($prep_parm_pdf)] = $prep_parm_pdf;
   ...
   $nm_saida->saida("               nm_submit_modal(\"" . $this->Ini->path_link . "grid_terceros_todos/grid_terceros_todos_export_email.php?script_case_init={$this->Ini->sc_page}&path_img={$this->Ini->path_img_global}&path_btn={$this->Ini->path_botoes}&sType=pdf_res&sAdd=__E__nmgp_tipo_pdf=\" + z + \"__E__sc_parms_pdf=\" + p + \"__E__sc_create_charts=\" + crt + \"__E__sc_graf_pdf=\" + g + \"__E__chart_level=\" + chart_level + \"__E__SC_module_export=\" + SC_module_export + \"__E__use_pass_pdf=\" + use_pass_pdf + \"__E__pdf_all_cab=\" + pdf_all_cab + \"__E__pdf_all_label=\" +  pdf_all_label + \"__E__pdf_label_group=\" +  pdf_label_group + \"__E__pdf_zip=\" +  pdf_zip + \"&nm_opc=pdf&KeepThis=true&TB_iframe=true&modal=true\", '');\r\n");
      ...
      $nm_saida->saida(" }\r\n");
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_Gb_Free_cmp']))
      {
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_terceros_todos_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "grid_terceros_todos/grid_terceros_todos_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['print_all'];
       if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pdf_res'])
       {
           $nm_saida->saida("<TR>\r\n");
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
...
//--- 
 function grafico_pdf()
 {
   ...
   $this->Graf->Ini    = $this->Ini;
   $this->Graf->Lookup = $this->Lookup;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_charts']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['skip_charts']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['skip_charts']))
   {
       ...
       $iChartCount = 1;
       $iChartTotal = sizeof($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_charts']);
 ...
 { 
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['pivot_charts'] as $chart_index => $chart_data)
       {
           if (!$prim_graf)
           {
 ...
 { 
           ...
           if ($this->progress_fp)
           {
               grid_terceros_todos_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "...\n", $this->Ini->Nm_lang, true);
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['this_chart_label'] = '';
       ...
       if ($bChartFP)
       {
           ...
           grid_terceros_todos_pdf_progress_call(100 . "_#NM#_" . 90 . "_#NM#_" . $lang_protect . "...\n", $this->Ini->Nm_lang);
           fwrite($this->progress_fp, 90 . "_#NM#_" . $lang_protect . "...\n");
           fclose($this->progress_fp);
       }
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['charts_html'])
       {
            $nm_saida->saida("<script type=\"text/javascript\">\r\n");
            $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['charts_html']}\r\n");
            $nm_saida->saida("</script>\r\n");
       }
   }
       $nm_saida->saida("</body>\r\n");
...
//--- 
 ...
 {
   global $nm_saida, $nm_lang;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['chart_list']))
   {
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['chart_list'] as $arr_chart)
       {
           $nm_saida->saida("<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>\r\n");
       $nm_saida->saida("<b><div style=\"height:1px;overflow:hidden\"><H1 style=\"font-size:0;padding:1px\">" . $this->Ini->Nm_lang['lang_btns_chrt_pdff_hint'] . "</H1></div></b>\r\n");
           $nm_saida->saida("<table><tr><td>\r\n");
...
//--- 
   ...
   {
      global $nm_saida;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['embutida'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['dashboard_info']['compact_mode'])
      { 
          return;
      } 
      ...
      { 
        ...
        if ($_SESSION['scriptcase']['charset'] != "UTF-8")
        {
          ...
          $documento = $Busca_temp['documento']; 
          $tmp_pos = strpos($documento, "##@@");
          if ($tmp_pos !== false && !is_array($documento))
          {
              $documento = substr($documento, 0, $tmp_pos);
          }
          $documento_2 = $Busca_temp['documento_input_2']; 
          $nombres = $Busca_temp['nombres']; 
          $tmp_pos = strpos($nombres, "##@@");
          if ($tmp_pos !== false && !is_array($nombres))
          {
              $nombres = substr($nombres, 0, $tmp_pos);
          }
          $nombres_2 = $Busca_temp['nombres_input_2']; 
          $loatiende = $Busca_temp['loatiende']; 
          $tmp_pos = strpos($loatiende, "##@@");
          if ($tmp_pos !== false && !is_array($loatiende))
          {
              $loatiende = substr($loatiende, 0, $tmp_pos);
          }
          $estado = $Busca_temp['estado']; 
          $tmp_pos = strpos($estado, "##@@");
          if ($tmp_pos !== false && !is_array($estado))
          {
              $estado = substr($estado, 0, $tmp_pos);
          }
          $clasificacion_clientes = $Busca_temp['clasificacion_clientes']; 
          $tmp_pos = strpos($clasificacion_clientes, "##@@");
          if ($tmp_pos !== false && !is_array($clasificacion_clientes))
          {
              $clasificacion_clientes = substr($clasificacion_clientes, 0, $tmp_pos);
          }
      } 
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['cond_pesq']))
      {  
          $pos       = 0;
          $trab_pos  = false;
          $pos_tmp   = true; 
          $tmp       = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['cond_pesq'];
          while ($pos_tmp)
          {
             $pos = strpos($tmp, "##*@@", $pos);
             if ($pos !== false)
          ...
          $nm_cond_filtro_or  = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['cond_pesq'], $trab_pos + 5) == "or")  ? " " . trim($this->Ini->Nm_lang['lang_srch_orr_cond']) . " " : "";
          $nm_cond_filtro_and = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['cond_pesq'], $trab_pos + 5) == "and") ? " " . trim($this->Ini->Nm_lang['lang_srch_and_cond']) . " " : "";
          $nm_cab_filtro   = substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['cond_pesq'], 0, $trab_pos);
   ...
   //---- 
   function adiciona_registro($quebra_loatiende, $quebra_loatiende_orig)
   {
      $contr_arr = "";
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
      {
          $Name_orig  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_Gb_Free_orig'][$cmp_gb])) ? $_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_Gb_Free_orig'][$cmp_gb] : $cmp_gb;
...
$_SESSION['scriptcase']['grid_terceros_todos']['contr_erro'] = 'off'; 
     ...
     $campos_order = "";
     foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_terceros_todos']['SC_Gb_Free_sql'] as $cmp_var => $resto)
     {
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->ccnit = $rs_res->fields[0] ;  
            $this->nombres = $rs_res->fields[1] ;  
            $this->direccion = $rs_res->fields[2] ;  
            $this->idmuni = $rs_res->fields[3] ;  
            $this->tel_cel = $rs_res->fields[4] ;  
            $this->cliente = $rs_res->fields[5] ;  
            $this->proveedor = $rs_res->fields[6] ;  
            $this->empleado = $rs_res->fields[7] ;  
            $this->si_nomina = $rs_res->fields[8] ;  
            $this->idtercero = $rs_res->fields[9] ;  
            $this->documento = $rs_res->fields[10] ;  
            $this->nacimiento = $rs_res->fields[11] ;  
            $this->sexo = $rs_res->fields[12] ;  
            $this->urlmail = $rs_res->fields[13] ;  
            $this->fechault = $rs_res->fields[14] ;  
            $this->saldo = $rs_res->fields[15] ;  
            $this->saldo =  str_replace(",", ".", $this->saldo);
            $this->afiliacion = $rs_res->fields[16] ;  
            $this->regimen = $rs_res->fields[17] ;  
            $this->tipo = $rs_res->fields[18] ;  
            $this->observaciones = $rs_res->fields[19] ;  
            $this->loatiende = $rs_res->fields[20] ;  
            $this->dv = $rs_res->fields[21] ;  
            $this->contacto = $rs_res->fields[22] ;  
            $this->credito = $rs_res->fields[23] ;  
            $this->cupo = $rs_res->fields[24] ;  
            $this->listaprecios = $rs_res->fields[25] ;  
            $this->con_actual = $rs_res->fields[26] ;  
            $this->efec_retencion = $rs_res->fields[27] ;  
            $this->urlmail_1 = $rs_res->fields[28] ;  
            $this->nombre1 = $rs_res->fields[29] ;  
            $this->nombre2 = $rs_res->fields[30] ;  
            $this->apellido1 = $rs_res->fields[31] ;  
            $this->apellido2 = $rs_res->fields[32] ;  
            $this->sucur_cliente = $rs_res->fields[33] ;  
            $this->representante = $rs_res->fields[34] ;  
            $this->es_restaurante = $rs_res->fields[35] ;  
            $this->dias_credito = $rs_res->fields[36] ;  
            $this->dias_mora = $rs_res->fields[37] ;  
            $this->cupo_vendedor = $rs_res->fields[38] ;  
            $this->cupo_vendedor =  str_replace(",", ".", $this->cupo_vendedor);
            $this->codigo_ter = $rs_res->fields[39] ;  
            $this->es_cajero = $rs_res->fields[40] ;  
            $this->autorizado = $rs_res->fields[41] ;  
            $this->zona_clientes = $rs_res->fields[42] ;  
            $this->clasificacion_clientes = $rs_res->fields[43] ;  
            $this->creado = $rs_res->fields[44] ;  
            $this->disponible = $rs_res->fields[45] ;  
            $this->id_pedido_tmp = $rs_res->fields[46] ;  
            $this->n_pedido_tmp = $rs_res->fields[47] ;  
            $this->total_pedido_tmp = $rs_res->fields[48] ;  
            $this->total_pedido_tmp =  str_replace(",", ".", $this->total_pedido_tmp);
            $this->obs_pedido_tmp = $rs_res->fields[49] ;  
            $this->vend_pedido_tmp = $rs_res->fields[50] ;  
            $this->ciudad = $rs_res->fields[51] ;  
            $this->codigo_postal = $rs_res->fields[52] ;  
            $this->lenguaje = $rs_res->fields[53] ;  
            $this->nombre_comercil = $rs_res->fields[54] ;  
            $this->notificar = $rs_res->fields[55] ;  
            $this->puc_auxiliar_deudores = $rs_res->fields[56] ;  
            $this->puc_retefuente_ventas = $rs_res->fields[57] ;  
            $this->puc_retefuente_servicios_clie = $rs_res->fields[58] ;  
            $this->puc_auxiliar_proveedores = $rs_res->fields[59] ;  
            $this->puc_retefuente_compras = $rs_res->fields[60] ;  
            $this->puc_retefuente_servicios_prov = $rs_res->fields[61] ;  
            $this->nube = $rs_res->fields[62] ;  
            $this->tipo_documento = $rs_res->fields[63] ;  
            $this->estado = $rs_res->fields[64] ;  
            $this->loatiende_orig = $this->loatiende;
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->loatiende, $this->loatiende_orig);
            }
   }
//
