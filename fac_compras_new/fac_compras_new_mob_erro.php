...
$old_error_handler = set_error_handler("fac_compras_new_mob_scriptcase_error_handler");

function fac_compras_new_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['fac_compras_new_mob']['contr_erro']) && $_SESSION['scriptcase']['fac_compras_new_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_fac_compras_new_mob->contr_fac_compras_new_mob->Apl_com_erro = true;
            if ($inicial_fac_compras_new_mob->contr_fac_compras_new_mob->NM_ajax_flag)
            {
                $inicial_fac_compras_new_mob->contr_fac_compras_new_mob->NM_ajax_info['errList']['geral_fac_compras_new_mob'][] = $msg;
                return;
            }
            fac_compras_new_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function fac_compras_new_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['fac_compras_new_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['fac_compras_new_mob'])  ? $_SESSION['scriptcase']['error_icon']['fac_compras_new_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['fac_compras_new_mob']) && '' != $_SESSION['scriptcase']['error_close']['fac_compras_new_mob']) ? $_SESSION['scriptcase']['error_close']['fac_compras_new_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['fac_compras_new_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['fac_compras_new_mob']) ? $_SESSION['scriptcase']['error_span_title']['fac_compras_new_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['fac_compras_new_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['fac_compras_new_mob']) ? $_SESSION['scriptcase']['error_icon_title']['fac_compras_new_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['fac_compras_new']['error_buffer'] .= ob_get_flush();
}
