...
$old_error_handler = set_error_handler("blank_copia_exito_scriptcase_error_handler");

function blank_copia_exito_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['blank_copia_exito']['contr_erro']) && $_SESSION['scriptcase']['blank_copia_exito']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            global $inicial_blank_copia_exito;
            $inicial_blank_copia_exito->contr_blank_copia_exito->Apl_com_erro = true;
            if ($inicial_blank_copia_exito->contr_blank_copia_exito->NM_ajax_flag)
            {
                $inicial_blank_copia_exito->contr_blank_copia_exito->NM_ajax_info['errList']['geral_blank_copia_exito'][] = $msg;
                return;
            }
            blank_copia_exito_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function blank_copia_exito_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['blank_copia_exito'])  && '' != $_SESSION['scriptcase']['error_icon']['blank_copia_exito'])  ? $_SESSION['scriptcase']['error_icon']['blank_copia_exito']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['blank_copia_exito']) && '' != $_SESSION['scriptcase']['error_close']['blank_copia_exito']) ? $_SESSION['scriptcase']['error_close']['blank_copia_exito'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['blank_copia_exito']) && '' != $_SESSION['scriptcase']['error_span_title']['blank_copia_exito']) ? $_SESSION['scriptcase']['error_span_title']['blank_copia_exito'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['blank_copia_exito']) && '' != $_SESSION['scriptcase']['error_icon_title']['blank_copia_exito']) ? $_SESSION['scriptcase']['error_icon_title']['blank_copia_exito'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['blank_copia_exito']['error_buffer'] .= ob_get_flush();
}
