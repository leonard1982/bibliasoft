...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_pagar_venta_pos']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_pagar_venta_pos"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
  nm_gp_submit('<?php echo $this->Ini->link_pdfreport_facturaven_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "SC_glo_par_par_numfacventa*scinpar_numfacventa*scout"; ?>', 'inicio', '_blank', '0', '0', 'pdfreport_facturaven');
}
var Crtl_btn_hacer_pago = false;
function sc_btn_hacer_pago()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_hacer_pago(); }, 500);
      return;
    }
    if (Crtl_btn_hacer_pago) {return;}
    scJs_confirm("<?php echo html_entity_decode('¿Esta seguro?, Pago no se puede deshacer', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_hacer_pago_ok, sc_btn_hacer_pago_cancel)
}
function sc_btn_hacer_pago_cancel()
{
}
function sc_btn_hacer_pago_ok()
{
    Crtl_btn_hacer_pago = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?hacer_pago?@?";
    document.F1.action = "./";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_frm_pos; ?>', '<?php echo $this->nm_location; ?>', '<?php echo ""; ?>', '', '_self', '0', '0', 'frm_pos');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_pagar_venta_pos']['where_detal']))
  {
?>  
...
{
   nm_field_disabled("idcli=disabled;adicional=disabled", "U");
