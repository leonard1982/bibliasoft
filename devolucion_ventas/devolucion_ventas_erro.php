...
$old_error_handler = set_error_handler("devolucion_ventas_scriptcase_error_handler");

function devolucion_ventas_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['devolucion_ventas']['contr_erro']) && $_SESSION['scriptcase']['devolucion_ventas']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            global $inicial_devolucion_ventas;
            $inicial_devolucion_ventas->contr_devolucion_ventas->Apl_com_erro = true;
            if ($inicial_devolucion_ventas->contr_devolucion_ventas->NM_ajax_flag)
            {
                $inicial_devolucion_ventas->contr_devolucion_ventas->NM_ajax_info['errList']['geral_devolucion_ventas'][] = $msg;
                return;
            }
            devolucion_ventas_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function devolucion_ventas_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['devolucion_ventas'])  && '' != $_SESSION['scriptcase']['error_icon']['devolucion_ventas'])  ? $_SESSION['scriptcase']['error_icon']['devolucion_ventas']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['devolucion_ventas']) && '' != $_SESSION['scriptcase']['error_close']['devolucion_ventas']) ? $_SESSION['scriptcase']['error_close']['devolucion_ventas'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['devolucion_ventas']) && '' != $_SESSION['scriptcase']['error_span_title']['devolucion_ventas']) ? $_SESSION['scriptcase']['error_span_title']['devolucion_ventas'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['devolucion_ventas']) && '' != $_SESSION['scriptcase']['error_icon_title']['devolucion_ventas']) ? $_SESSION['scriptcase']['error_icon_title']['devolucion_ventas'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['devolucion_ventas']['error_buffer'] .= ob_get_flush();
}
