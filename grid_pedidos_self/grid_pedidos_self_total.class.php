...
class grid_pedidos_self_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_pedidos_self']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_pedidos_self']['campos_busca']))
      { 
          ...
          $fechadocu_2 = $Busca_temp['fechadocu_input_2']; 
   ...
   {
      global $nada, $nm_lang , $prefijo_ped, $idcli, $vendedor, $usuario;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $nm_comando = "select distinct     idpedido,     numfacven,     nremision,     credito,     fechaven,     fechavenc,     idcli,     subtotal,     valoriva,     total,     facturado,     asentada,     observaciones,     saldo,     adicional,     formapago,     adicional2,     adicional3,     obspago,     vendedor,     dircliente,     numpedido,     prefijo_ped,     tipo_doc,     usuario,     fechadocu,     origen,     iddocumento from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['where_pesq']; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(subtotal), sum(valoriva), sum(total), sum(saldo) from (" . $nm_comando . ") nm_sel_esp "; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(subtotal), sum(valoriva), sum(total), sum(saldo) from (" . $nm_comando . ") nm_sel_esp "; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'][3] = $rt->fields[2]; 
      $rt->fields[3] = str_replace(",", ".", $rt->fields[3]);
      $rt->fields[3] = (string)$rt->fields[3]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'][4] = $rt->fields[3]; 
      $rt->fields[4] = str_replace(",", ".", $rt->fields[4]);
      $rt->fields[4] = (string)$rt->fields[4]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'][5] = $rt->fields[4]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['contr_total_geral'] = "OK";
   } 
