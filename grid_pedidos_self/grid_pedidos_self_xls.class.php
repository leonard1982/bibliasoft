...
class grid_pedidos_self_xls
{
   ...
   function inicializa_vars()
   {
      ...
      { 
          ...
          if (!is_file($this->Ini->root . $this->Ini->path_link . "grid_pedidos_self/grid_pedidos_self_res_xls.class.php"))
          {
              $this->Tem_xls_res  = false;
          }
          if ($this->Tem_xls_res)
          { 
              ...
              $this->Res_xls = new grid_pedidos_self_res_xls();
              $this->prep_modulos("Res_xls");
          } 
          $this->Arquivo    = "sc_xls";
          $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
          $this->Arq_zip    = $this->Arquivo . "_grid_pedidos_self.zip";
          $this->Arquivo   .= "_grid_pedidos_self" . $this->Xls_tp;
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['embutida_label']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['embutida_label'])
      { 
          $this->count_span = 0;
          $this->Xls_row++;
          $this->proc_label();
     ...
     $nm_houve_quebra = "N";
         ...
         if (isset($this->NM_Row_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['embutida'])
         { 
      ...
      $this->xls_set_style();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['embutida'] && $prim_reg)
      { 
          $this->proc_label();
          $this->xls_sub_cons_copy_label($this->Xls_row);
      ...
      if (isset($this->NM_Col_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['embutida'])
      { 
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   { 
      ...
      { 
          ...
          if (!isset($this->NM_cmp_hidden[$Cada_col]) || $this->NM_cmp_hidden[$Cada_col] != "off")
          {
              ...
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallepedido_self']['embutida_label'] = true;
   ...
   function NM_export_fechadocu()
   {
      ...
      if (!empty($this->fechadocu))
      {
         ...
         if ($this->Use_phpspreadsheet) {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->fechadocu, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
         }
         else {
             $this->Nm_ActiveSheet->setCellValueExplicit($current_cell_ref . $this->Xls_row, $this->fechadocu, PHPExcel_Cell_DataType::TYPE_STRING);
         }
         $this->Xls_col++;
   }
   //----- idpedido
   ...
   function NM_export_origen()
   {
         ...
         if (is_numeric($this->origen))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->origen);
         $this->Xls_col++;
   }
   //----- iddocumento
   function NM_export_iddocumento()
   {
         ...
         if (is_numeric($this->iddocumento))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->iddocumento);
         $this->Xls_col++;
   }
   //----- fechaven
   ...
   //----- fechadocu
   function NM_sub_cons_fechadocu()
   {
      if (!empty($this->fechadocu))
      {
         if (substr($this->fechadocu, 10, 1) == "-") 
         ...
         $this->fechadocu = NM_charset_to_utf8($this->fechadocu);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->fechadocu;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "center";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "char";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "";
         $this->Xls_col++;
   ...
   function NM_sub_cons_detalle()
   {
         ...
         $GLOBALS["script_case_init"] = $this->Ini->sc_page;
         $GLOBALS["nmgp_parms"] = "nmgp_opcao?#?xls?@?gidpedid?#?" . str_replace("'", "@aspass@", $this->Orig_idpedido) . "?@?";
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_detallepedido_self']['embutida'] = true;
         if (method_exists($this->grid_detallepedido_self, "controle"))
         {
             $this->grid_detallepedido_self->controle();
   ...
   //----- iddocumento
   function NM_sub_cons_iddocumento()
   {
         $this->iddocumento = NM_charset_to_utf8($this->iddocumento);
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->iddocumento;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "right";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "num";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "#,##0";
         $this->Xls_col++;
   ...
   {
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['nolabel']) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['nolabel'])
       {
   ...
   function quebra_geral_sc_free_total_bot()
   {
       ...
       $prim_cmp = true;
       $mens_tot = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'][0] . "(" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'][1] . ")";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['field_order'] as $Cada_cmp)
       {
           if ($Cada_cmp == "subtotal" && (!isset($this->NM_cmp_hidden['subtotal']) || $this->NM_cmp_hidden['subtotal'] != "off"))
           {
               $Format_Num = "#,##0.00";
               $Vl_Tot     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'][2];
               $prim_cmp = false;
           ...
           {
               $Format_Num = "#,##0.00";
               $Vl_Tot     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'][3];
               $prim_cmp = false;
               $Vl_Tot = NM_charset_to_utf8($Vl_Tot);
               if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['embutida']) {
                   $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $Vl_Tot;
           ...
           {
               $Format_Num = "#,##0.00";
               $Vl_Tot     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'][4];
               $prim_cmp = false;
               $Vl_Tot = NM_charset_to_utf8($Vl_Tot);
               if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['embutida']) {
                   $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $Vl_Tot;
           ...
           elseif ($Cada_cmp == "saldo" && (!isset($this->NM_cmp_hidden['saldo']) || $this->NM_cmp_hidden['saldo'] != "off"))
           {
               $Format_Num = "#,##0.00";
               $Vl_Tot     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['tot_geral'][5];
               $prim_cmp = false;
               $Vl_Tot = NM_charset_to_utf8($Vl_Tot);
               if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['embutida']) {
                   $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $Vl_Tot;
           ...
           elseif (!isset($this->NM_cmp_hidden[$Cada_cmp]) || $this->NM_cmp_hidden[$Cada_cmp] != "off")
           {
               ...
               elseif ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_pedidos_self']['embutida']) {
