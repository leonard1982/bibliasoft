...
*/
    ...
    include_once('grid_produccion_session.php');
    session_start();
    $_SESSION['scriptcase']['grid_produccion']['glo_nm_path_imag_temp']  = "";
    //check tmp
    if(empty($_SESSION['scriptcase']['grid_produccion']['glo_nm_path_imag_temp']))
    {
        $str_path_apl_url = $_SERVER['PHP_SELF'];
        $str_path_apl_url = str_replace("\\", '/', $str_path_apl_url);
        $str_path_apl_url = substr($str_path_apl_url, 0, strrpos($str_path_apl_url, "/"));
        $str_path_apl_url = substr($str_path_apl_url, 0, strrpos($str_path_apl_url, "/")+1);
        /*check tmp*/$_SESSION['scriptcase']['grid_produccion']['glo_nm_path_imag_temp'] = $str_path_apl_url . "_lib/tmp";
    }
    $SC_cod_proj = "FACILWEBv2";
    $SC_apl_proc = "grid_produccion";
    $SC_ver_93   = true;
...
// Charts
        ...
        $isMultiSeriesChart = $is_chart_app && (!isset($_SESSION['sc_session'][$script_case_init]['grid_produccion']['summarizing_drill_down']) || !$_SESSION['sc_session'][$script_case_init]['grid_produccion']['summarizing_drill_down']);
        $isAnalitic         = isset($_SESSION['sc_session'][$script_case_init]['grid_produccion']['graf_opc_atual']) && 2 == $_SESSION['sc_session'][$script_case_init]['grid_produccion']['graf_opc_atual'];

if ($ver_93 == "n")
{
          $sDisplay = !isset($_SESSION['sc_session']['grid_produccion']['show_skip_charts_option']) ||
                     !$_SESSION['sc_session']['grid_produccion']['show_skip_charts_option'] ||
                     (isset($_SESSION['sc_session'][$script_case_init]['grid_produccion']['SC_Ind_Groupby']) && '_NM_SC_' == $_SESSION['sc_session'][$script_case_init]['grid_produccion']['SC_Ind_Groupby']) ||
                     $isMultiSeriesChart ||
                     $isAnalitic ||
                     ($opc == 'pdf_det')
                     ? ' style="display: none"' : '';
...
<?php
        ...
        {
         $sDisplay =
//                    !isset($_SESSION['sc_session']['grid_produccion']['show_skip_charts_option']) ||
//                    !$_SESSION['sc_session']['grid_produccion']['show_skip_charts_option'] ||
                    $isMultiSeriesChart ||
                    $isAnalitic ||
