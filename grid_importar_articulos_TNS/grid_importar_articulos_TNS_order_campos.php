<?php
   include_once('grid_importar_articulos_TNS_session.php');
   session_start();
   $_SESSION['scriptcase']['grid_importar_articulos_TNS']['glo_nm_path_imag_temp']  = "";
   //check tmp
   if(empty($_SESSION['scriptcase']['grid_importar_articulos_TNS']['glo_nm_path_imag_temp']))
   {
       $str_path_apl_url = $_SERVER['PHP_SELF'];
       $str_path_apl_url = str_replace("\\", '/', $str_path_apl_url);
       $str_path_apl_url = substr($str_path_apl_url, 0, strrpos($str_path_apl_url, "/"));
       $str_path_apl_url = substr($str_path_apl_url, 0, strrpos($str_path_apl_url, "/")+1);
       /*check tmp*/$_SESSION['scriptcase']['grid_importar_articulos_TNS']['glo_nm_path_imag_temp'] = $str_path_apl_url . "_lib/tmp";
   }
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['grid_importar_articulos_TNS']['glo_nm_path_imag_temp'] . "/sc_apl_default_FACILWEB_V2.txt"))
       {
?>
...
<?php
   ...
   $Ord_Cmp = new grid_importar_articulos_TNS_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_importar_articulos_TNS_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_select_orig'] = array();
   }
   $this->restore = isset($_POST['restore']) ? true : false;
   if ($this->restore && !class_exists('Services_JSON')) {
       include_once("grid_importar_articulos_TNS_json.php");
   }
   ...
   $tab_labels     = array();
   $tab_ger_campos['m_codigo'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['m_codigo'] = "m_codigo";
   }
   else
   {
       $tab_def_campos['m_codigo'] = "M.CODIGO";
   }
   $tab_labels["m_codigo"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["m_codigo"])) ? $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["m_codigo"] : "CODIGO";
   $tab_ger_campos['descrip'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['descrip'] = "descrip";
   }
   else
   {
       $tab_def_campos['descrip'] = "CAST(SUBSTRING(M.DESCRIP FROM 1 FOR 90) AS CHAR(90))";
   }
   $tab_labels["descrip"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["descrip"])) ? $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["descrip"] : "DESCRIP";
   $tab_ger_campos['m_unidad'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['m_unidad'] = "m_unidad";
   }
   else
   {
       $tab_def_campos['m_unidad'] = "M.UNIDAD";
   }
   $tab_labels["m_unidad"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["m_unidad"])) ? $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["m_unidad"] : "UNIDAD";
   $tab_ger_campos['t_porciva'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['t_porciva'] = "t_porciva";
   }
   else
   {
       $tab_def_campos['t_porciva'] = "T.PORCIVA";
   }
   $tab_labels["t_porciva"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["t_porciva"])) ? $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["t_porciva"] : "PORCIVA";
   $tab_ger_campos['grupocontable'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['grupocontable'] = "grupocontable";
   }
   else
   {
       $tab_def_campos['grupocontable'] = "C.CODIGO";
   }
   $tab_labels["grupocontable"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["grupocontable"])) ? $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["grupocontable"] : "GRUPO_CONTABLE";
   $tab_ger_campos['costos'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['costos'] = "costos";
   }
   else
   {
       $tab_def_campos['costos'] = "(IIF(S.PRECULTPROV IS NOT NULL,S.PRECULTPROV,IIF(S.ULTCOSTPROM IS NOT NULL,S.ULTCOSTPROM,COALESCE(S.COSTO,0))))";
   }
   $tab_labels["costos"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["costos"])) ? $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["costos"] : "COSTOS";
   ...
   $tab_labels["estado"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["estado"])) ? $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['labels']["estado"] : "Estado";
   $tab_ger_campos['estado'] = "none";
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_importar_articulos_TNS']['field_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['grid_importar_articulos_TNS']['field_display']))
   {
       foreach ($_SESSION['scriptcase']['sc_apl_conf']['grid_importar_articulos_TNS']['field_display'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_importar_articulos_TNS_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_select'];
...
<?php
  $disp_rest = "none";
  if (isset($_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_importar_articulos_TNS']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
