...
class grid_cuentaspagar_contado_seleccionar_pesq
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['path_libs_php'] = $this->Ini->path_lib_php;
   ...
   function init()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['opcao'] = "igual";
   }
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_cuentaspagar_contado_seleccionar_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_cuentaspagar_contado_seleccionar_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_cuentaspagar_contado_seleccionar_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == 'autocomp_numfacom')
      {
          $numfacom = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_numfacom($numfacom);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_formapago')
      {
          $formapago = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_formapago($formapago);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_pagada')
      {
          $pagada = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_pagada($pagada);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_saldo')
      {
          $saldo = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_saldo($saldo);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
   ...
   function lookup_ajax_numfacom($numfacom)
   {
      $numfacom = substr($this->Db->qstr($numfacom), 1, -1);
            $numfacom_look = substr($this->Db->qstr($numfacom), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct numfacom from " . $this->Ini->nm_tabela . " where saldo>0 and formapago='CONTADO' and  numfacom like '%" . $numfacom . "%' order by numfacom"; 
      unset($cmp1,$cmp2);
   ...
   function lookup_ajax_formapago($formapago)
   {
      $formapago = substr($this->Db->qstr($formapago), 1, -1);
            $formapago_look = substr($this->Db->qstr($formapago), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct formapago from " . $this->Ini->nm_tabela . " where saldo>0 and formapago='CONTADO' and  formapago like '%" . $formapago . "%' order by formapago"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
   ...
   function lookup_ajax_pagada($pagada)
   {
      $pagada = substr($this->Db->qstr($pagada), 1, -1);
            $pagada_look = substr($this->Db->qstr($pagada), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct pagada from " . $this->Ini->nm_tabela . " where saldo>0 and formapago='CONTADO' and  pagada like '%" . $pagada . "%' order by pagada"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
   ...
   function lookup_ajax_saldo($saldo)
   {
      $saldo = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $saldo);
      $saldo = substr($this->Db->qstr($saldo), 1, -1);
            $saldo_look = substr($this->Db->qstr($saldo), 1, -1); 
      $nmgp_def_dados = array(); 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct saldo from " . $this->Ini->nm_tabela . " where saldo>0 and formapago='CONTADO' and   CAST (saldo AS TEXT)  like '%" . $saldo . "%' order by saldo"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct saldo from " . $this->Ini->nm_tabela . " where saldo>0 and formapago='CONTADO' and   CAST (saldo AS VARCHAR)  like '%" . $saldo . "%' order by saldo"; 
      }
      else
      {
          $nm_comando = "select distinct saldo from " . $this->Ini->nm_tabela . " where saldo>0 and formapago='CONTADO' and  saldo like '%" . $saldo . "%' order by saldo"; 
      }
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
   ...
   function monta_condicao($nome, $condicao, $campo, $campo2 = "", $nome_campo="", $tp_campo="")
   {
      ...
      $nm_ini_lower = "";
      $nm_fim_lower = "";
      $Nm_datas[] = "fechacom";$Nm_datas[] = "fechacom";$Nm_datas[] = "fechavenc";$Nm_datas[] = "fechavenc";$Nm_numeric[] = "idfaccom";$Nm_numeric[] = "idprov";$Nm_numeric[] = "subtotal";$Nm_numeric[] = "valoriva";$Nm_numeric[] = "total";$Nm_numeric[] = "asentada";$Nm_numeric[] = "control";$Nm_numeric[] = "saldo";$Nm_numeric[] = "id_pedidocom";
      ...
      if (in_array($campo_join, $Nm_numeric))
      {
         ...
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['decimal_db'] == ".")
         {
            $nm_aspas  = "";
            $nm_aspas1 = "";
         }
      ...
      $Nm_datas[] = "fechacom";$Nm_datas[] = "fechacom";$Nm_datas[] = "fechavenc";$Nm_datas[] = "fechavenc";
      if (in_array($campo_join, $Nm_datas))
      {
          ...
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['SC_sep_date']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['SC_sep_date']))
          {
              $nm_aspas  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['SC_sep_date'];
              $nm_aspas1 = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['SC_sep_date1'];
          }
      }
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
<HTML>
<HEAD>
 <TITLE>FACTURAS CON SALDO POR PAGAR</TITLE>
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_cuentaspagar_contado_seleccionar/grid_cuentaspagar_contado_seleccionar_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
<?php
$vertical_center = '';
?>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_cuentaspagar_contado_seleccionar_ajax_search.js"></script>
 <script type="text/javascript" src="grid_cuentaspagar_contado_seleccionar_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_cuentaspagar_contado_seleccionar_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_cuentaspagar_contado_seleccionar']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_cuentaspagar_contado_seleccionar']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
if (count($this->nm_mens_alert) || count($this->Ini->nm_mens_alert)) {
   ...
   if (isset($this->nm_mens_alert) && !empty($this->nm_mens_alert))
   {
       foreach ($this->nm_mens_alert as $i_alert => $mensagem)
       {
           ...
           if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['ajax_nav'])
           { 
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_numfacom_cond#NMF#' + search_get_sel_txt('SC_numfacom_cond') + '@NMF@';
  str_out += 'SC_numfacom#NMF#' + search_get_text('SC_numfacom') + '@NMF@';
  str_out += 'id_ac_numfacom#NMF#' + search_get_title('select2-id_ac_numfacom-container') + '@NMF@';
  str_out += 'SC_formapago_cond#NMF#' + search_get_sel_txt('SC_formapago_cond') + '@NMF@';
  str_out += 'SC_formapago#NMF#' + search_get_text('SC_formapago') + '@NMF@';
  str_out += 'id_ac_formapago#NMF#' + search_get_title('select2-id_ac_formapago-container') + '@NMF@';
  str_out += 'SC_fechacom_cond#NMF#' + search_get_sel_txt('SC_fechacom_cond') + '@NMF@';
  str_out += 'SC_fechacom_dia#NMF#' + search_get_sel_txt('SC_fechacom_dia') + '@NMF@';
  str_out += 'SC_fechacom_mes#NMF#' + search_get_sel_txt('SC_fechacom_mes') + '@NMF@';
  str_out += 'SC_fechacom_ano#NMF#' + search_get_sel_txt('SC_fechacom_ano') + '@NMF@';
  str_out += 'SC_fechacom_input_2_dia#NMF#' + search_get_sel_txt('SC_fechacom_input_2_dia') + '@NMF@';
  str_out += 'SC_fechacom_input_2_mes#NMF#' + search_get_sel_txt('SC_fechacom_input_2_mes') + '@NMF@';
  str_out += 'SC_fechacom_input_2_ano#NMF#' + search_get_sel_txt('SC_fechacom_input_2_ano') + '@NMF@';
  str_out += 'SC_pagada_cond#NMF#' + search_get_sel_txt('SC_pagada_cond') + '@NMF@';
  str_out += 'SC_pagada#NMF#' + search_get_text('SC_pagada') + '@NMF@';
  str_out += 'id_ac_pagada#NMF#' + search_get_title('select2-id_ac_pagada-container') + '@NMF@';
  str_out += 'SC_saldo_cond#NMF#' + search_get_sel_txt('SC_saldo_cond') + '@NMF@';
  str_out += 'SC_saldo#NMF#' + search_get_text('SC_saldo') + '@NMF@';
  str_out += 'id_ac_saldo#NMF#' + search_get_title('select2-id_ac_saldo-container') + '@NMF@';
  str_out += 'SC_saldo_input_2#NMF#' + search_get_text('SC_saldo_input_2') + '@NMF@';
...
<script type="text/javascript">
 $(function() {
  $(".sc-ui-autocomp-numfacom").on("focus", function() {
  }).on("blur", function() {
   ...
   ajax: {
    ...
    data: function (params) {
     var query = {
      ...
      nmgp_parms: "NM_ajax_opcao?#?autocomp_numfacom",
      max_itens: "10",
   ...
   $("#SC_numfacom").val(e.params.data.id);
  });
  $(".sc-ui-autocomp-formapago").on("focus", function() {
  }).on("blur", function() {
  }).on("keydown", function(e) {
   if(e.keyCode == $.ui.keyCode.TAB && $(".ui-autocomplete").filter(":visible").length) {
    e.keyCode = $.ui.keyCode.DOWN;
   ...
   ajax: {
    ...
    data: function (params) {
     var query = {
      q: params.term,
      nmgp_opcao: "ajax_autocomp",
      nmgp_parms: "NM_ajax_opcao?#?autocomp_formapago",
      max_itens: "10",
      script_case_init: <?php echo $this->Ini->sc_page ?>
     }
     return query;
   ...
   $("#SC_formapago").val(e.params.data.id);
  });
  $(".sc-ui-autocomp-pagada").on("focus", function() {
  }).on("blur", function() {
  }).on("keydown", function(e) {
   if(e.keyCode == $.ui.keyCode.TAB && $(".ui-autocomplete").filter(":visible").length) {
    e.keyCode = $.ui.keyCode.DOWN;
   ...
   ajax: {
    ...
    data: function (params) {
     var query = {
      q: params.term,
      nmgp_opcao: "ajax_autocomp",
      nmgp_parms: "NM_ajax_opcao?#?autocomp_pagada",
      max_itens: "10",
      script_case_init: <?php echo $this->Ini->sc_page ?>
     }
     return query;
   ...
   $("#SC_pagada").val(e.params.data.id);
  });
  $(".sc-ui-autocomp-saldo").on("focus", function() {
  }).on("blur", function() {
  }).on("keydown", function(e) {
   if(e.keyCode == $.ui.keyCode.TAB && $(".ui-autocomplete").filter(":visible").length) {
    e.keyCode = $.ui.keyCode.DOWN;
   ...
   ajax: {
    ...
    data: function (params) {
     var query = {
      q: params.term,
      nmgp_opcao: "ajax_autocomp",
      nmgp_parms: "NM_ajax_opcao?#?autocomp_saldo",
      max_itens: "10",
      script_case_init: <?php echo $this->Ini->sc_page ?>
     }
     return query;
...
<?php
   ...
   {
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div style="width: 100%">
 ...
 <div style="height:37px; border-width:0px 0px 1px 0px;  border-style: dashed; border-color:#ddd; display: block">
 	<table style="width:100%; border-collapse:collapse; padding:0;">
    	<tr>
        	<td id="lin1_col1" class="scFilterHeaderFont"><span>FACTURAS CON SALDO POR PAGAR</span></td>
...
<?php
   ...
   {
      global 
             $numfacom_cond, $numfacom, $numfacom_autocomp,
             $formapago_cond, $formapago, $formapago_autocomp,
             $fechacom_cond, $fechacom, $fechacom_dia, $fechacom_mes, $fechacom_ano, $fechacom_input_2_dia, $fechacom_input_2_mes, $fechacom_input_2_ano,
             $pagada_cond, $pagada, $pagada_autocomp,
             $saldo_cond, $saldo, $saldo_input_2, $saldo_autocomp,
             $nm_url_saida, $nm_apl_dependente, $nmgp_parms, $bprocessa, $nmgp_save_name, $NM_operador, $NM_filters, $nmgp_save_option, $NM_filters_del, $Script_BI;
      $Script_BI = "";
      $this->nmgp_botoes['clear'] = "on";
      $this->nmgp_botoes['save'] = "on";
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_cuentaspagar_contado_seleccionar']['btn_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['grid_cuentaspagar_contado_seleccionar']['btn_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['grid_cuentaspagar_contado_seleccionar']['btn_display'] as $NM_cada_btn => $NM_cada_opc)
          {
      ...
      if (isset($_SESSION['scriptcase']['sc_aba_iframe']))
      {
          ...
          {
              if (in_array("grid_cuentaspagar_contado_seleccionar", $apls_aba))
              {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['iframe_menu'] && (!isset($_SESSION['scriptcase']['menu_mobile']) || empty($_SESSION['scriptcase']['menu_mobile'])))
      {
          $this->aba_iframe = true;
      }
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $numfacom = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['numfacom']; 
          $numfacom_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['numfacom_cond']; 
          $formapago = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['formapago']; 
          $formapago_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['formapago_cond']; 
          $fechacom_dia = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_dia']; 
          $fechacom_mes = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_mes']; 
          $fechacom_ano = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_ano']; 
          $fechacom_input_2_dia = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_input_2_dia']; 
          $fechacom_input_2_mes = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_input_2_mes']; 
          $fechacom_input_2_ano = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_input_2_ano']; 
          $fechacom_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_cond']; 
          $pagada = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['pagada']; 
          $pagada_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['pagada_cond']; 
          $saldo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['saldo']; 
          $saldo_input_2 = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['saldo_input_2']; 
          $saldo_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['saldo_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['NM_operador']; 
      } 
      if (!isset($numfacom_cond) || empty($numfacom_cond))
      {
         $numfacom_cond = "eq";
      }
      if (!isset($formapago_cond) || empty($formapago_cond))
      {
         $formapago_cond = "eq";
      }
      if (!isset($fechacom_cond) || empty($fechacom_cond))
      {
         $fechacom_cond = "eq";
      }
      ...
      if (!isset($saldo_cond) || empty($saldo_cond))
      {
         $saldo_cond = "gt";
      }
      ...
      $str_hide_numfacom = (in_array($numfacom_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_formapago = (in_array($formapago_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_fechacom = (in_array($fechacom_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_pagada = (in_array($pagada_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_saldo = (in_array($saldo_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;

      $str_display_numfacom = ('bw' == $numfacom_cond) ? $display_aberto : $display_fechado;
      $str_display_formapago = ('bw' == $formapago_cond) ? $display_aberto : $display_fechado;
      $str_display_fechacom = ('bw' == $fechacom_cond) ? $display_aberto : $display_fechado;
      $str_display_pagada = ('bw' == $pagada_cond) ? $display_aberto : $display_fechado;
      $str_display_saldo = ('bw' == $saldo_cond) ? $display_aberto : $display_fechado;

      if (!isset($numfacom) || $numfacom == "")
      {
          $numfacom = "";
      }
      if (isset($numfacom) && !empty($numfacom))
      {
      ...
      if (isset($formapago) && !empty($formapago))
      {
         $tmp_pos = strpos($formapago, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $formapago = substr($formapago, 0, $tmp_pos);
         }
      }
      if (!isset($fechacom) || $fechacom == "")
      {
          $fechacom = "";
      }
      if (isset($fechacom) && !empty($fechacom))
      {
      ...
      {
         $tmp_pos = strpos($saldo, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $saldo = substr($saldo, 0, $tmp_pos);
         }
      }
?>
 <TR align="center">
 ...
 $nmgp_tab_label .= "numfacom?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
...
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br>
      <SELECT class="SC_Cond_Selector scFilterObjectOdd" id="SC_numfacom_cond" name="numfacom_cond" onChange="nm_campos_between(document.getElementById('id_vis_numfacom'), this, 'numfacom')">
       <OPTION value="eq" <?php if ("eq" == $numfacom_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="df" <?php if ("df" == $numfacom_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_dife'] ?></OPTION>
      </SELECT>
      <br><span id="id_hide_numfacom"  <?php echo $str_hide_numfacom?>><?php
      if ($numfacom != "")
      {
      $numfacom_look = substr($this->Db->qstr($numfacom), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct numfacom from " . $this->Ini->nm_tabela . " where saldo>0 and formapago='CONTADO' and numfacom = '$numfacom_look' order by numfacom"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$numfacom]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$numfacom];
      }
      else
      {
          $sAutocompValue = $numfacom;
      }
?>
<INPUT  type="text" id="SC_numfacom" name="numfacom" value="<?php echo NM_encode_input($numfacom) ?>"  size=20 alt="{datatype: 'text', maxLength: 20, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectOdd sc-ui-autocomp-numfacom" id="id_ac_numfacom" name="numfacom_autocomp"><?php if ('' !=  $numfacom) { ?><option value="<?php echo $numfacom ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
 </TD>
 ...
 $nmgp_tab_label .= "formapago?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
?>
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br>
      <SELECT class="SC_Cond_Selector scFilterObjectEven" id="SC_formapago_cond" name="formapago_cond" onChange="nm_campos_between(document.getElementById('id_vis_formapago'), this, 'formapago')">
       <OPTION value="eq" <?php if ("eq" == $formapago_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
      </SELECT>
      <br><span id="id_hide_formapago"  <?php echo $str_hide_formapago?>><?php
      if ($formapago != "")
      {
      $formapago_look = substr($this->Db->qstr($formapago), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct formapago from " . $this->Ini->nm_tabela . " where saldo>0 and formapago='CONTADO' and formapago = '$formapago_look' order by formapago"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$formapago]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$formapago];
      }
      else
      {
          $sAutocompValue = $formapago;
      }
?>
<INPUT  type="text" id="SC_formapago" name="formapago" value="<?php echo NM_encode_input($formapago) ?>"  size=15 alt="{datatype: 'text', maxLength: 15, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectEven sc-ui-autocomp-formapago" id="id_ac_formapago" name="formapago_autocomp"><?php if ('' !=  $formapago) { ?><option value="<?php echo $formapago ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
 </TD>
 ...
 $nmgp_tab_label .= "fechacom?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
?>
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br>
      <SELECT class="SC_Cond_Selector scFilterObjectOdd" id="SC_fechacom_cond" name="fechacom_cond" onChange="nm_campos_between(document.getElementById('id_vis_fechacom'), this, 'fechacom')">
       <OPTION value="eq" <?php if ("eq" == $fechacom_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="gt" <?php if ("gt" == $fechacom_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr'] ?></OPTION>
       <OPTION value="lt" <?php if ("lt" == $fechacom_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less'] ?></OPTION>
       <OPTION value="bw" <?php if ("bw" == $fechacom_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_betw'] ?></OPTION>
      </SELECT>
      <br><span id="id_hide_fechacom"  <?php echo $str_hide_fechacom?>>
<?php
  $Form_base = "ddmmyyyy";
  $date_format_show = "";
...
<?php
  ...
  {
?>
<span id='id_date_part_fechacom_DD' style='display: inline-block'>
<INPUT class="sc-js-input scFilterObjectOdd" type="text" id="SC_fechacom_dia" name="fechacom_dia" value="<?php echo NM_encode_input($fechacom_dia); ?>" size="2" alt="{datatype: 'mask', maskList: '99', alignRight: true, maxLength: 2, autoTab: true, enterTab: false}">
</span>
...
<?php
  ...
  {
?>
<span id='id_date_part_fechacom_MM' style='display: inline-block'>
<INPUT class="sc-js-input scFilterObjectOdd" type="text" id="SC_fechacom_mes" name="fechacom_mes" value="<?php echo NM_encode_input($fechacom_mes); ?>" size="2" alt="{datatype: 'mask', maskList: '99', alignRight: true, maxLength: 2, autoTab: true, enterTab: false}">
</span>
...
<?php
  ...
  {
?>
<span id='id_date_part_fechacom_AAAA' style='display: inline-block'>
<INPUT class="sc-js-input scFilterObjectOdd" type="text" id="SC_fechacom_ano" name="fechacom_ano" value="<?php echo NM_encode_input($fechacom_ano); ?>" size="4" alt="{datatype: 'mask', maskList: '9999', alignRight: true, maxLength: 4, autoTab: true, enterTab: false}">
 </span>
...
?>
        <SPAN id="id_css_fechacom"  class="scFilterFieldFontOdd">
 <br><?php echo $date_format_show ?>         </SPAN>
                  <br />
        <SPAN id="id_vis_fechacom"  <?php echo $str_display_fechacom; ?> class="scFilterFieldFontOdd">
...
<?php
  ...
  {
?>
<span id='id_date_part_fechacom_input_2_DD' style='display: inline-block'>
<INPUT class="sc-js-input scFilterObjectOdd" type="text" id="SC_fechacom_input_2_dia" name="fechacom_input_2_dia" value="<?php echo NM_encode_input($fechacom_input_2_dia); ?>" size="2" alt="{datatype: 'mask', maskList: '99', alignRight: true, maxLength: 2, autoTab: true, enterTab: false}">
</span>
...
<?php
  ...
  {
?>
<span id='id_date_part_fechacom_input_2_MM' style='display: inline-block'>
<INPUT class="sc-js-input scFilterObjectOdd" type="text" id="SC_fechacom_input_2_mes" name="fechacom_input_2_mes" value="<?php echo NM_encode_input($fechacom_input_2_mes); ?>" size="2" alt="{datatype: 'mask', maskList: '99', alignRight: true, maxLength: 2, autoTab: true, enterTab: false}">
</span>
...
<?php
  ...
  {
?>
<span id='id_date_part_fechacom_input_2_AAAA' style='display: inline-block'>
<INPUT class="sc-js-input scFilterObjectOdd" type="text" id="SC_fechacom_input_2_ano" name="fechacom_input_2_ano" value="<?php echo NM_encode_input($fechacom_input_2_ano); ?>" size="4" alt="{datatype: 'mask', maskList: '9999', alignRight: true, maxLength: 4, autoTab: true, enterTab: false}">
 </span>
...
?>
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br>
      <SELECT class="SC_Cond_Selector scFilterObjectEven" id="SC_pagada_cond" name="pagada_cond" onChange="nm_campos_between(document.getElementById('id_vis_pagada'), this, 'pagada')">
       <OPTION value="eq" <?php if ("eq" == $pagada_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
      </SELECT>
      <br><span id="id_hide_pagada"  <?php echo $str_hide_pagada?>><?php
      if ($pagada != "")
      {
      $pagada_look = substr($this->Db->qstr($pagada), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct pagada from " . $this->Ini->nm_tabela . " where saldo>0 and formapago='CONTADO' and pagada = '$pagada_look' order by pagada"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$pagada]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$pagada];
      }
      else
      {
          $sAutocompValue = $pagada;
      }
?>
<INPUT  type="text" id="SC_pagada" name="pagada" value="<?php echo NM_encode_input($pagada) ?>"  size=10 alt="{datatype: 'text', maxLength: 10, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectEven sc-ui-autocomp-pagada" id="id_ac_pagada" name="pagada_autocomp"><?php if ('' !=  $pagada) { ?><option value="<?php echo $pagada ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
 </TD>
...
?>
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br>
      <SELECT class="SC_Cond_Selector scFilterObjectOdd" id="SC_saldo_cond" name="saldo_cond" onChange="nm_campos_between(document.getElementById('id_vis_saldo'), this, 'saldo')">
       <OPTION value="gt" <?php if ("gt" == $saldo_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr'] ?></OPTION>
       <OPTION value="lt" <?php if ("lt" == $saldo_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $saldo_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="bw" <?php if ("bw" == $saldo_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_betw'] ?></OPTION>
      </SELECT>
      <br><span id="id_hide_saldo"  <?php echo $str_hide_saldo?>><?php
      if ($saldo != "")
      {
      $saldo_look = substr($this->Db->qstr($saldo), 1, -1); 
      $nmgp_def_dados = array(); 
   ...
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct saldo from " . $this->Ini->nm_tabela . " where saldo>0 and formapago='CONTADO' and saldo = $saldo_look order by saldo"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct saldo from " . $this->Ini->nm_tabela . " where saldo>0 and formapago='CONTADO' and saldo = $saldo_look order by saldo"; 
      ...
      if (isset($nmgp_def_dados[0][$saldo]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$saldo];
      }
      else
      {
            nmgp_Form_Num_Val($saldo, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          $sAutocompValue = $saldo;
      }
?>
<INPUT  type="text" id="SC_saldo" name="saldo" value="<?php echo NM_encode_input($saldo) ?>" size=12 alt="{datatype: 'integer', maxLength: 12, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectOdd sc-ui-autocomp-saldo" id="id_ac_saldo" name="saldo_autocomp"><?php if ('' !=  $saldo) { ?><option value="<?php echo $saldo ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>         <BR>
        <SPAN id="id_vis_saldo"  <?php echo $str_display_saldo; ?> class="scFilterFieldFontOdd">
         <?php echo $date_sep_bw ?>&nbsp;
         <BR>
         <INPUT type="text" id="SC_saldo_input_2" name="saldo_input_2" value="<?php echo NM_encode_input($saldo_input_2) ?>"  size=12 alt="{datatype: 'integer', maxLength: 12, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" class="sc-js-input sc-js-input scFilterObjectOdd">         </SPAN>
...
?>
<?php
   if (is_file("grid_cuentaspagar_contado_seleccionar_help.txt"))
   {
      $Arq_WebHelp = file("grid_cuentaspagar_contado_seleccionar_help.txt"); 
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_cuentaspagar_contado_seleccionar']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_cuentaspagar_contado_seleccionar']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_cuentaspagar_contado_seleccionar']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_cuentaspagar_contado_seleccionar']['start'] == 'filter')
   {
?>
...
?>
   <INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.numfacom_cond.value = 'eq';
   nm_campos_between(document.getElementById('id_vis_numfacom'), document.F1.numfacom_cond, 'numfacom');
   document.F1.numfacom.value = "";
   document.F1.numfacom_autocomp.value = "";
   $('#select2-id_ac_numfacom-container').html('<?php echo $this->Val_init_numfacom['desc'] ?>');;
   document.F1.formapago_cond.value = 'eq';
   nm_campos_between(document.getElementById('id_vis_formapago'), document.F1.formapago_cond, 'formapago');
   document.F1.formapago.value = "";
   document.F1.formapago_autocomp.value = "";
   $('#select2-id_ac_formapago-container').html('<?php echo $this->Val_init_formapago['desc'] ?>');;
   document.F1.fechacom_cond.value = 'eq';
   nm_campos_between(document.getElementById('id_vis_fechacom'), document.F1.fechacom_cond, 'fechacom');
   document.F1.fechacom_dia.value = "";
   document.F1.fechacom_mes.value = "";
   document.F1.fechacom_ano.value = "";
   document.F1.fechacom_input_2_dia.value = "";
   document.F1.fechacom_input_2_mes.value = "";
   document.F1.fechacom_input_2_ano.value = "";
   ...
   document.F1.pagada_autocomp.value = "";
   $('#select2-id_ac_pagada-container').html('<?php echo $this->Val_init_pagada['desc'] ?>');;
   document.F1.saldo_cond.value = 'gt';
   nm_campos_between(document.getElementById('id_vis_saldo'), document.F1.saldo_cond, 'saldo');
   document.F1.saldo.value = "";
   document.F1.saldo_autocomp.value = "";
   $('#select2-id_ac_saldo-container').html('<?php echo $this->Val_init_saldo['desc'] ?>');;
   document.F1.saldo_input_2.value = "";
   Sc_carga_select2('all');
 }
 ...
 {
    $('#SC_fechacom_dia').bind('change', function() {sc_grid_cuentaspagar_contado_seleccionar_valida_dia(this)});
    $('#SC_fechacom_input_2_dia').bind('change', function() {sc_grid_cuentaspagar_contado_seleccionar_valida_dia(this)});
    $('#SC_fechacom_input_2_mes').bind('change', function() {sc_grid_cuentaspagar_contado_seleccionar_valida_mes(this)});
    $('#SC_fechacom_mes').bind('change', function() {sc_grid_cuentaspagar_contado_seleccionar_valida_mes(this)});
 }
 function sc_grid_cuentaspagar_contado_seleccionar_valida_dia(obj)
 {
 ...
 function sc_grid_cuentaspagar_contado_seleccionar_valida_mes(obj)
 {
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "FACILWEBv_2022/grid_cuentaspagar_contado_seleccionar";
   ...
   function inicializa_vars()
   {
      ...
      $this->nm_data = new nm_data("es");
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] = "";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function salva_filtro($nmgp_save_origem)
   {
      global $NM_filters_save, $nmgp_save_name, $nmgp_save_option, $script_case_init;
          ...
          $NM_patch .= "grid_cuentaspagar_contado_seleccionar/";
          if (!is_dir($NM_patch))
          {
              $NMdir = mkdir($NM_patch, 0755);
          }
   ...
   function recupera_filtro($NM_filters)
   {
      ...
      $tp_fields['SC_numfacom_cond'] = 'cond';
      $tp_fields['SC_numfacom'] = 'text_aut';
      $tp_fields['id_ac_numfacom'] = 'select2_aut';
      $tp_fields['SC_formapago_cond'] = 'cond';
      $tp_fields['SC_formapago'] = 'text_aut';
      $tp_fields['id_ac_formapago'] = 'select2_aut';
      $tp_fields['SC_fechacom_cond'] = 'cond';
      $tp_fields['SC_fechacom_dia'] = 'text';
      $tp_fields['SC_fechacom_mes'] = 'text';
      $tp_fields['SC_fechacom_ano'] = 'text';
      $tp_fields['SC_fechacom_input_2_dia'] = 'text';
      $tp_fields['SC_fechacom_input_2_mes'] = 'text';
      $tp_fields['SC_fechacom_input_2_ano'] = 'text';
      $tp_fields['SC_pagada_cond'] = 'cond';
      $tp_fields['SC_pagada'] = 'text_aut';
      $tp_fields['id_ac_pagada'] = 'select2_aut';
      $tp_fields['SC_saldo_cond'] = 'cond';
      $tp_fields['SC_saldo'] = 'text_aut';
      $tp_fields['id_ac_saldo'] = 'select2_aut';
      $tp_fields['SC_saldo_input_2'] = 'text';
   ...
   {
      global $numfacom_cond, $numfacom, $numfacom_autocomp,
             $formapago_cond, $formapago, $formapago_autocomp,
             $fechacom_cond, $fechacom, $fechacom_dia, $fechacom_mes, $fechacom_ano, $fechacom_input_2_dia, $fechacom_input_2_mes, $fechacom_input_2_ano,
             $pagada_cond, $pagada, $pagada_autocomp,
             $saldo_cond, $saldo, $saldo_input_2, $saldo_autocomp, $nmgp_tab_label;

      $C_formatado = true;
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_gp_limpa.php", "F", "nm_limpa_valor") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_conv_dados.php", "F", "nm_conv_limpa_dado") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_edit.php", "F", "nmgp_Form_Num_Val") ; 
      if (!empty($numfacom_autocomp) && empty($numfacom))
      {
          $numfacom = $numfacom_autocomp;
      }
      if (!empty($formapago_autocomp) && empty($formapago))
      {
          $formapago = $formapago_autocomp;
      }
      if (!empty($pagada_autocomp) && empty($pagada))
      {
          $pagada = $pagada_autocomp;
      }
      if (!empty($saldo_autocomp) && empty($saldo))
      {
          $saldo = $saldo_autocomp;
      }
      $numfacom_cond_salva = $numfacom_cond; 
      if (!isset($numfacom_input_2) || $numfacom_input_2 == "")
      {
          $numfacom_input_2 = $numfacom;
      }
      $formapago_cond_salva = $formapago_cond; 
      if (!isset($formapago_input_2) || $formapago_input_2 == "")
      {
          $formapago_input_2 = $formapago;
      }
      $fechacom_cond_salva = $fechacom_cond; 
      if (!isset($fechacom_input_2_dia) || $fechacom_input_2_dia == "")
      {
          $fechacom_input_2_dia = $fechacom_dia;
      }
      if (!isset($fechacom_input_2_mes) || $fechacom_input_2_mes == "")
      {
          $fechacom_input_2_mes = $fechacom_mes;
      }
      if (!isset($fechacom_input_2_ano) || $fechacom_input_2_ano == "")
      {
          $fechacom_input_2_ano = $fechacom_ano;
      }
      ...
      $saldo_cond_salva = $saldo_cond; 
      if (!isset($saldo_input_2) || $saldo_input_2 == "")
      {
          $saldo_input_2 = $saldo;
      }
      if ($saldo_cond != "in")
      {
          nm_limpa_numero($saldo, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      if ($saldo_cond != "in")
      {
          nm_limpa_numero($saldo_input_2, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['numfacom'] = $numfacom; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['numfacom_cond'] = $numfacom_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['formapago'] = $formapago; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['formapago_cond'] = $formapago_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_dia'] = $fechacom_dia; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_mes'] = $fechacom_mes; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_ano'] = $fechacom_ano; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_input_2_dia'] = $fechacom_input_2_dia; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_input_2_mes'] = $fechacom_input_2_mes; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_input_2_ano'] = $fechacom_input_2_ano; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_cond'] = $fechacom_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['pagada'] = $pagada; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['pagada_cond'] = $pagada_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['saldo'] = $saldo; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['saldo_input_2'] = $saldo_input_2; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['saldo_cond'] = $saldo_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca'] = $Temp_Busca;
      }
      if ($saldo_cond != "in" && $saldo_cond != "bw" && !empty($saldo) && !is_numeric($saldo))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : Saldo";
      }
      if ($saldo_cond == "bw" && ((!empty($saldo) && !is_numeric($saldo)) || (!empty($saldo_input_2) && !is_numeric($saldo_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : Saldo";
      }
      if (!empty($this->Campos_Mens_erro)) 
    ...
    if ($numfacom != '') {
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['numfacom'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          ...
          $this->cmp_formatado['numfacom'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['numfacom'] = $numfacom;
      }
      $nmgp_def_dados = array();
    if ($formapago != '') {
      $formapago_look = substr($this->Db->qstr($formapago), 1, -1); 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['formapago'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['formapago'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['formapago'] = $formapago;
      }
      $nmgp_def_dados = array();
    if ($pagada != '') {
      $pagada_look = substr($this->Db->qstr($pagada), 1, -1); 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['pagada'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['pagada'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['pagada'] = $pagada;
      }
      $nmgp_def_dados = array();
    if ($saldo != '') {
      $saldo_look = substr($this->Db->qstr($saldo), 1, -1); 
   ...
   { 
      ...
      $Conteudo = $saldo_input_2;
      if (strtoupper($saldo_cond) != "II" && strtoupper($saldo_cond) != "QP" && strtoupper($saldo_cond) != "NP" && strtoupper($saldo_cond) != "IN") 
      { 
      ...
      $this->cmp_formatado['saldo_input_2'] = $Conteudo;

      //----- $numfacom
      $this->Date_part = false;
      if (isset($numfacom) || $numfacom_cond == "nu" || $numfacom_cond == "nn" || $numfacom_cond == "ep" || $numfacom_cond == "ne")
      {
         $this->monta_condicao("numfacom", $numfacom_cond, $numfacom, "", "numfacom");
      }

      //----- $formapago
      $this->Date_part = false;
      if (isset($formapago) || $formapago_cond == "nu" || $formapago_cond == "nn" || $formapago_cond == "ep" || $formapago_cond == "ne")
      {
         $this->monta_condicao("formapago", $formapago_cond, $formapago, "", "formapago");
      }

      //----- $fechacom
      $this->Date_part = false;
      if ($fechacom_cond != "bi_TP")
      {
          $fechacom_cond = strtoupper($fechacom_cond);
          $Dtxt = "";
          $val  = array();
          $Dtxt .= $fechacom_ano;
          $Dtxt .= $fechacom_mes;
          $Dtxt .= $fechacom_dia;
          $val[0]['ano'] = $fechacom_ano;
          $val[0]['mes'] = $fechacom_mes;
          $val[0]['dia'] = $fechacom_dia;
          if ($fechacom_cond == "BW")
          {
              $val[1]['ano'] = $fechacom_input_2_ano;
              $val[1]['mes'] = $fechacom_input_2_mes;
              $val[1]['dia'] = $fechacom_input_2_dia;
          }
          $this->Operador_date_part = "";
          $this->Lang_date_part     = "";
          if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_oracle))
          {
              $this->nm_prep_date($val, "DT", "DATETIME", $fechacom_cond, "", "data");
          }
          else
          {
              $this->nm_prep_date($val, "DT", "DATE", $fechacom_cond, "", "data");
          }
          if (!$this->Date_part) {
              $val[0] = $this->Ini->sc_Date_Protect($val[0]);
          }
          $fechacom = $val[0];
          $this->cmp_formatado['fechacom'] = $val[0];
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom'] = $val[0];
          $this->nm_data->SetaData($this->cmp_formatado['fechacom'], "YYYY-MM-DD");
          $this->cmp_formatado['fechacom'] = $this->nm_data->FormataSaida($this->nm_data->FormatRegion("DT", "dmY"));
          if ($fechacom_cond == "BW")
          {
              if (!$this->Date_part) {
                  $val[1] = $this->Ini->sc_Date_Protect($val[1]);
              }
              $fechacom_input_2     = $val[1];
              $this->cmp_formatado['fechacom_input_2'] = $val[1];
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']['fechacom_input_2'] = $val[1];
              $this->nm_data->SetaData($this->cmp_formatado['fechacom_input_2'], "YYYY-MM-DD");
              $this->cmp_formatado['fechacom_input_2'] = $this->nm_data->FormataSaida($this->nm_data->FormatRegion("DT", "dmY"));
          }
          if (!empty($Dtxt) || $fechacom_cond == "NU" || $fechacom_cond == "NN"|| $fechacom_cond == "EP"|| $fechacom_cond == "NE")
          {
              $this->monta_condicao("fechacom", $fechacom_cond, $fechacom, $fechacom_input_2, 'fechacom', 'DATE');
          }
      }

      //----- $pagada
      $this->Date_part = false;
      if (isset($pagada) || $pagada_cond == "nu" || $pagada_cond == "nn" || $pagada_cond == "ep" || $pagada_cond == "ne")
      {
      ...
      //----- $saldo
      $this->Date_part = false;
      if (isset($saldo) || $saldo_cond == "nu" || $saldo_cond == "nn" || $saldo_cond == "ep" || $saldo_cond == "ne")
      {
         $this->monta_condicao("saldo", $saldo_cond, $saldo, $saldo_input_2, "saldo");
      }
   }
   ...
   {
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_pesq_fast'] = "";
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['fast_search']);
      if ("" == $this->comando_filtro)
      {
          $this->comando = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_orig'];
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_pesq']         = $this->comando . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['opcao']              = "pesq";
      if ("" == $this->comando_filtro)
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_pesq_filtro'] = "";
      }
      else
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['cond_pesq'] .= $this->NM_operador;
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['contr_array_resumo'] = "NAO";
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['contr_total_geral']  = "NAO";
         unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['tot_geral']);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_pesq_ant'] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_cuentaspagar_contado_seleccionar']['where_pesq'];
