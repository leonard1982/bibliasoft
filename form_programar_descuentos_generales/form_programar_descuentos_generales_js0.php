<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="id_programar" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['id_programar']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_programar_descuentos_generales']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_programar_descuentos_generales"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_programar_descuentos_generales']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_form_programar_descuentos_generales_navigate_form();
    }
} 
...
?>
    ...
    nm_pack('cajas_afectadas_dest', 'cajas_afectadas');
    if (x == "excluir") 
    { 
    ...
    { 
       ...
       {
           Nm_Proc_Atualiz = true;
           do_ajax_form_programar_descuentos_generales_submit_form();
       }
    } 
    if (Nm_submit_ok)
    { 
...
function scForm_packMultiSelect_multi() {
	...
	for (var Nm_x = 1; Nm_x < NM_count_mult; Nm_x++) {
		nm_pack('cajas_afectadas' + Nm_x + '_dest', 'cajas_afectadas' + Nm_x);
	}
