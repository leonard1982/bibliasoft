<?php
class form_detalleventa_111121_form extends form_detalleventa_111121_apl
{
function Form_Init()
{
...
<HEAD>
 <TITLE><?php if ('novo' == $this->nmgp_opcao) { echo strip_tags("" . $this->Ini->Nm_lang['lang_othr_frmi_titl'] . " - detalleventa"); } else { echo strip_tags("" . $this->Ini->Nm_lang['lang_othr_frmu_titl'] . " - detalleventa"); } ?></TITLE>
...
<?php
if ($this->Embutida_form && isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['sc_modal'] && isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['sc_redir_atualiz']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['sc_redir_atualiz'] == 'ok')
{
?>
...
<script type="text/javascript" src="<?php echo $this->Ini->path_prod ?>/third/jquery_plugin/select2/js/select2.full.min.js"></script>
 ...
 <?php
 if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['embutida_pdf']))
 {
 ?>
...
?>
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>form_detalleventa_111121/form_detalleventa_111121_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
...
<?php
include_once("form_detalleventa_111121_sajax_js.php");
?>
<script type="text/javascript">
...
var Nav_binicio_macro_disabled  = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['first']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['first'] : 'off'); ?>";
var Nav_bavanca_macro_disabled  = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['forward']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['forward'] : 'off'); ?>";
var Nav_bretorna_macro_disabled = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['back']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['back'] : 'off'); ?>";
var Nav_bfinal_macro_disabled   = "<?php echo (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['last']) ? $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['last'] : 'off'); ?>";
...
function navpage_atualiza(str_navpage)
{
 ...
 function nm_field_disabled_reset() {
  for (var i = 0; i < iAjaxNewLine; i++) {
    ...
    nm_field_disabled("elcolor_=enabled", "", i);
    nm_field_disabled("latalla_=enabled", "", i);
    nm_field_disabled("elsabor_=enabled", "", i);
  }
 } // nm_field_disabled_reset
<?php

include_once('form_detalleventa_111121_jquery.php');
...
?>
 ...
 function addAutocomplete(elem) {
  ...
  $(".sc-ui-autocomp-idpro_", elem).on("focus", function() {
   ...
   width: "300px",
   ajax: {
    url: "form_detalleventa_111121.php",
    dataType: "json",
   ...
   var sId = $(this).attr("id").substr(6), sRow = "idpro_" != sId ? sId.substr(6) : "";
   sc_form_detalleventa_111121_idpro__onfocus("id_sc_field_" + sId, sRow);
...
<?php
$str_iframe_body = ('F' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] || 'R' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe']) ? 'margin: 2px;' : '';
...
if ($GLOBALS["erro_incl"] == 1)
{
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['opc_ant'] = "novo";
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['recarga'] = "novo";
}
if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['recarga']))
{
    $opcao_botoes = $this->nmgp_opcao;
}
else
{
    $opcao_botoes = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['recarga'];
}
if ('novo' == $opcao_botoes && $this->Embutida_form)
{
    $opcao_botoes = 'inicio';
}
    $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['remove_margin'] ? 'margin: 0; ' : '';
    $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['remove_border'] ? 'border-width: 0; ' : '';
    $vertical_center = '';
?>
...
<?php
 include_once("form_detalleventa_111121_js0.php");
?>
<script type="text/javascript"> 
nmdg_enter_tab = true;
...
<?php
$_SESSION['scriptcase']['error_span_title']['form_detalleventa_111121'] = $this->Ini->Error_icon_span;
$_SESSION['scriptcase']['error_icon_title']['form_detalleventa_111121'] = '' != $this->Ini->Err_ico_title ? $this->Ini->path_icones . '/' . $this->Ini->Err_ico_title : '';
?>
...
<?php
  if (!$this->Embutida_call && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['mostra_cab']) || $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['mostra_cab'] != "N") && (!$_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['under_dashboard'] || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['compact_mode'] || $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['maximized']))
  {
?>
<tr><td>
<style>
...
<div style="width: 100%">
 ...
 <div style="height:37px; border-width:0px 0px 1px 0px;  border-style: dashed; border-color:#ddd; display: block">
 	<table style="width:100%; border-collapse:collapse; padding:0;">
    	<tr>
        	<td id="lin1_col1" class="scFormHeaderFont"><span><?php if ($this->nmgp_opcao == "novo") { echo "" . $this->Ini->Nm_lang['lang_othr_frmi_titl'] . " - detalleventa"; } else { echo "" . $this->Ini->Nm_lang['lang_othr_frmu_titl'] . " - detalleventa"; } ?></span></td>
...
<tr><td>
<?php
if (($this->Embutida_form || !$this->Embutida_call || $this->Grid_editavel || $this->Embutida_multi || ($this->Embutida_call && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['embutida_liga_form_btn_nav'])) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "F" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "R")
{
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['new']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['new']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['new']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['new']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['new'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['insert']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['insert']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['insert']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['insert']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['insert'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['bcancelar']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['bcancelar']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['bcancelar']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['bcancelar']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['bcancelar'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['balterarsel']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['balterarsel']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['balterarsel']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['balterarsel']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['balterarsel'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['bexcluirsel']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['bexcluirsel']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['bexcluirsel']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['bexcluirsel']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['bexcluirsel'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['help']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['help']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['help']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['help']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['help'];
        }
?>
...
<?php
    ...
    if (($opcao_botoes == "novo") && (isset($_SESSION['scriptcase']['nm_sc_retorno']) && !empty($_SESSION['scriptcase']['nm_sc_retorno']) && ($nm_apl_dependente != 1 || $this->nm_Start_new) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "F" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "R") && (!$this->Embutida_call) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['under_dashboard']))) {
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['exit']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['exit']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['exit']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['exit']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['exit'];
        }
?>
...
<?php
    ...
    if (($opcao_botoes == "novo") && (!isset($_SESSION['scriptcase']['nm_sc_retorno']) || empty($_SESSION['scriptcase']['nm_sc_retorno']) || $nm_apl_dependente == 1 || $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] == "F" || $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] == "R") && (!$this->Embutida_call) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['under_dashboard']))) {
...
<?php
    ...
    if (($opcao_botoes != "novo") && (!$this->Embutida_call) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['under_dashboard'] || (isset($this->is_calendar_app) && $this->is_calendar_app)))) {
        $sCondStyle = (isset($_SESSION['scriptcase']['nm_sc_retorno']) && !empty($_SESSION['scriptcase']['nm_sc_retorno']) && $nm_apl_dependente != 1 && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "F" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "R" && !$this->aba_iframe && $this->nmgp_botoes['exit'] == "on") ? '' : 'display: none;';
?>
<?php
...
<?php
    ...
    if (($opcao_botoes != "novo") && (!$this->Embutida_call) && ((!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['under_dashboard']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['under_dashboard'] || (isset($this->is_calendar_app) && $this->is_calendar_app)))) {
        $sCondStyle = (!isset($_SESSION['scriptcase']['nm_sc_retorno']) || empty($_SESSION['scriptcase']['nm_sc_retorno']) || $nm_apl_dependente == 1 || $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] == "F" || $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] == "R" || $this->aba_iframe || $this->nmgp_botoes['exit'] != "on") && ($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "F" && $this->nmgp_botoes['exit'] == "on") && ($nm_apl_dependente == 1 && $this->nmgp_botoes['exit'] == "on") ? '' : 'display: none;';
?>
<?php
...
<?php
  ...
  {
       if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['where_filter']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['empty_filter'] = true;
       }
       echo "<tr><td>";
  }
?>
...
?>
<?php
   if (!isset($this->nmgp_cmp_hidden['numfac_']))
   {
       $this->nmgp_cmp_hidden['numfac_'] = 'off';
   }
...
<?php
 ...
 ?>
   ...
   <?php if ($this->Embutida_form && $this->nmgp_botoes['insert'] != "on") {?>
    <TD class="scFormLabelOddMult"  width="10"> &nbsp; </TD>
   <?php }?>
   <?php if (isset($this->nmgp_cmp_hidden['numfac_']) && $this->nmgp_cmp_hidden['numfac_'] == 'off') { $sStyleHidden_numfac_ = 'display: none'; }
      if (1 || !isset($this->nmgp_cmp_hidden['numfac_']) || $this->nmgp_cmp_hidden['numfac_'] == 'on') {
      if (!isset($this->nm_new_label['numfac_'])) {
          $this->nm_new_label['numfac_'] = "Numfac"; } ?>

    <TD class="scFormLabelOddMult css_numfac__label" id="hidden_field_label_numfac_" style="<?php echo $sStyleHidden_numfac_; ?>" > <?php echo $this->nm_new_label['numfac_'] ?> </TD>
   <?php } ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['desc_']) && $this->nmgp_cmp_hidden['desc_'] == 'off') { $sStyleHidden_desc_ = 'display: none'; }
      if (1 || !isset($this->nmgp_cmp_hidden['desc_']) || $this->nmgp_cmp_hidden['desc_'] == 'on') {
      if (!isset($this->nm_new_label['desc_'])) {
          $this->nm_new_label['desc_'] = "Desc. Art."; } ?>

    <TD class="scFormLabelOddMult css_desc__label" id="hidden_field_label_desc_" style="<?php echo $sStyleHidden_desc_; ?>" > <?php echo $this->nm_new_label['desc_'] ?> </TD>
   <?php } ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['obs_']) && $this->nmgp_cmp_hidden['obs_'] == 'off') { $sStyleHidden_obs_ = 'display: none'; }
      ...
      if (!isset($this->nm_new_label['obs_'])) {
          $this->nm_new_label['obs_'] = "Comp. Descr."; } ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['existencia_']) && $this->nmgp_cmp_hidden['existencia_'] == 'off') { $sStyleHidden_existencia_ = 'display: none'; }
      ...
      if (!isset($this->nm_new_label['existencia_'])) {
          $this->nm_new_label['existencia_'] = "Existencia detallada"; } ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['elcolor_']) && $this->nmgp_cmp_hidden['elcolor_'] == 'off') { $sStyleHidden_elcolor_ = 'display: none'; }
      if (1 || !isset($this->nmgp_cmp_hidden['elcolor_']) || $this->nmgp_cmp_hidden['elcolor_'] == 'on') {
      if (!isset($this->nm_new_label['elcolor_'])) {
          $this->nm_new_label['elcolor_'] = "Colores"; } ?>

    <TD class="scFormLabelOddMult css_elcolor__label" id="hidden_field_label_elcolor_" style="<?php echo $sStyleHidden_elcolor_; ?>" > <?php echo $this->nm_new_label['elcolor_'] ?> </TD>
   <?php } ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['latalla_']) && $this->nmgp_cmp_hidden['latalla_'] == 'off') { $sStyleHidden_latalla_ = 'display: none'; }
      if (1 || !isset($this->nmgp_cmp_hidden['latalla_']) || $this->nmgp_cmp_hidden['latalla_'] == 'on') {
      if (!isset($this->nm_new_label['latalla_'])) {
          $this->nm_new_label['latalla_'] = "Tallas"; } ?>

    <TD class="scFormLabelOddMult css_latalla__label" id="hidden_field_label_latalla_" style="<?php echo $sStyleHidden_latalla_; ?>" > <?php echo $this->nm_new_label['latalla_'] ?> </TD>
   <?php } ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['elsabor_']) && $this->nmgp_cmp_hidden['elsabor_'] == 'off') { $sStyleHidden_elsabor_ = 'display: none'; }
      if (1 || !isset($this->nmgp_cmp_hidden['elsabor_']) || $this->nmgp_cmp_hidden['elsabor_'] == 'on') {
      if (!isset($this->nm_new_label['elsabor_'])) {
          $this->nm_new_label['elsabor_'] = "Sabores"; } ?>

    <TD class="scFormLabelOddMult css_elsabor__label" id="hidden_field_label_elsabor_" style="<?php echo $sStyleHidden_elsabor_; ?>" > <?php echo $this->nm_new_label['elsabor_'] ?> </TD>
   <?php } ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['unidadmayor_']) && $this->nmgp_cmp_hidden['unidadmayor_'] == 'off') { $sStyleHidden_unidadmayor_ = 'display: none'; }
    ...
    <TD class="scFormLabelOddMult css_unidadmayor__label" id="hidden_field_label_unidadmayor_" style="<?php echo $sStyleHidden_unidadmayor_; ?>" > <?php echo $this->nm_new_label['unidadmayor_'] ?> <span class="scFormRequiredOddMult">*</span> </TD>
   <?php } ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['stockubica_']) && $this->nmgp_cmp_hidden['stockubica_'] == 'off') { $sStyleHidden_stockubica_ = 'display: none'; }
      ...
      if (!isset($this->nm_new_label['stockubica_'])) {
          $this->nm_new_label['stockubica_'] = "Stock Ubic."; } ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['valorunit_']) && $this->nmgp_cmp_hidden['valorunit_'] == 'off') { $sStyleHidden_valorunit_ = 'display: none'; }
    ...
    <TD class="scFormLabelOddMult css_valorunit__label" id="hidden_field_label_valorunit_" style="<?php echo $sStyleHidden_valorunit_; ?>" > <?php echo $this->nm_new_label['valorunit_'] ?> <span class="scFormRequiredOddMult">*</span> </TD>
   <?php } ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['factor_']) && $this->nmgp_cmp_hidden['factor_'] == 'off') { $sStyleHidden_factor_ = 'display: none'; }
    ...
    <TD class="scFormLabelOddMult css_factor__label" id="hidden_field_label_factor_" style="<?php echo $sStyleHidden_factor_; ?>" > <?php echo $this->nm_new_label['factor_'] ?> <span class="scFormRequiredOddMult">*</span> </TD>
   <?php } ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['costop_']) && $this->nmgp_cmp_hidden['costop_'] == 'off') { $sStyleHidden_costop_ = 'display: none'; }
    ...
    <TD class="scFormLabelOddMult css_costop__label" id="hidden_field_label_costop_" style="<?php echo $sStyleHidden_costop_; ?>" > <?php echo $this->nm_new_label['costop_'] ?> <span class="scFormRequiredOddMult">*</span> </TD>
   <?php } ?>
...
{ 
   ...
   { 
       ob_start();
       $iStart = sizeof($this->form_vert_form_detalleventa_111121);
       $guarda_nmgp_opcao = $this->nmgp_opcao;
       $guarda_form_vert_form_detalleventa_111121 = $this->form_vert_form_detalleventa_111121;
       $this->nmgp_opcao = 'novo';
   } 
   if ($this->Embutida_form && empty($this->form_vert_form_detalleventa_111121))
   {
       $sc_seq_vert = 0;
   }
   foreach ($this->form_vert_form_detalleventa_111121 as $sc_seq_vert => $sc_lixo)
   {
       $this->loadRecordState($sc_seq_vert);
       $this->iddet_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['iddet_'];
       $this->remision_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['remision_'];
       $this->devuelto_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['devuelto_'];
       $this->numdevo_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['numdevo_'];
       $this->iddeta_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['iddeta_'];
       if (isset($this->Embutida_ronly) && $this->Embutida_ronly && !$Line_Add)
       {
           $this->nmgp_cmp_readonly['numfac_'] = true;
           $this->nmgp_cmp_readonly['idpro_'] = true;
           $this->nmgp_cmp_readonly['desc_'] = true;
           $this->nmgp_cmp_readonly['obs_'] = true;
           $this->nmgp_cmp_readonly['existencia_'] = true;
           $this->nmgp_cmp_readonly['colores_'] = true;
           $this->nmgp_cmp_readonly['elcolor_'] = true;
           $this->nmgp_cmp_readonly['tallas_'] = true;
           $this->nmgp_cmp_readonly['latalla_'] = true;
           $this->nmgp_cmp_readonly['sabor_'] = true;
           $this->nmgp_cmp_readonly['elsabor_'] = true;
       ...
       elseif ($Line_Add)
       {
           if (!isset($this->nmgp_cmp_readonly['numfac_']) || $this->nmgp_cmp_readonly['numfac_'] != "on") {$this->nmgp_cmp_readonly['numfac_'] = false;}
           if (!isset($this->nmgp_cmp_readonly['idpro_']) || $this->nmgp_cmp_readonly['idpro_'] != "on") {$this->nmgp_cmp_readonly['idpro_'] = false;}
           if (!isset($this->nmgp_cmp_readonly['desc_']) || $this->nmgp_cmp_readonly['desc_'] != "on") {$this->nmgp_cmp_readonly['desc_'] = false;}
           ...
           if (!isset($this->nmgp_cmp_readonly['elcolor_']) || $this->nmgp_cmp_readonly['elcolor_'] != "on") {$this->nmgp_cmp_readonly['elcolor_'] = false;}
           if (!isset($this->nmgp_cmp_readonly['tallas_']) || $this->nmgp_cmp_readonly['tallas_'] != "on") {$this->nmgp_cmp_readonly['tallas_'] = false;}
           if (!isset($this->nmgp_cmp_readonly['latalla_']) || $this->nmgp_cmp_readonly['latalla_'] != "on") {$this->nmgp_cmp_readonly['latalla_'] = false;}
           if (!isset($this->nmgp_cmp_readonly['sabor_']) || $this->nmgp_cmp_readonly['sabor_'] != "on") {$this->nmgp_cmp_readonly['sabor_'] = false;}
           if (!isset($this->nmgp_cmp_readonly['elsabor_']) || $this->nmgp_cmp_readonly['elsabor_'] != "on") {$this->nmgp_cmp_readonly['elsabor_'] = false;}
        ...
        $this->numfac_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['numfac_']; 
       $numfac_ = $this->numfac_; 
       if (!isset($this->nmgp_cmp_hidden['numfac_']))
       {
           $this->nmgp_cmp_hidden['numfac_'] = 'off';
       }
       $sStyleHidden_numfac_ = '';
       if (isset($sCheckRead_numfac_))
       {
           unset($sCheckRead_numfac_);
       }
       if (isset($this->nmgp_cmp_readonly['numfac_']))
       {
           $sCheckRead_numfac_ = $this->nmgp_cmp_readonly['numfac_'];
       }
       if (isset($this->nmgp_cmp_hidden['numfac_']) && $this->nmgp_cmp_hidden['numfac_'] == 'off')
       {
           unset($this->nmgp_cmp_hidden['numfac_']);
           $sStyleHidden_numfac_ = 'display: none;';
       }
       $bTestReadOnly_numfac_ = true;
       $sStyleReadLab_numfac_ = 'display: none;';
       $sStyleReadInp_numfac_ = '';
       if (isset($this->nmgp_cmp_readonly['numfac_']) && $this->nmgp_cmp_readonly['numfac_'] == 'on')
       {
           $bTestReadOnly_numfac_ = false;
           unset($this->nmgp_cmp_readonly['numfac_']);
           $sStyleReadLab_numfac_ = '';
           $sStyleReadInp_numfac_ = 'display: none;';
       }
       $this->idpro_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['idpro_']; 
       $idpro_ = $this->idpro_; 
       $sStyleHidden_idpro_ = '';
       ...
       $this->desc_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['desc_']; 
       $desc_ = $this->desc_; 
       $sStyleHidden_desc_ = '';
       if (isset($sCheckRead_desc_))
       {
           unset($sCheckRead_desc_);
       }
       if (isset($this->nmgp_cmp_readonly['desc_']))
       {
           $sCheckRead_desc_ = $this->nmgp_cmp_readonly['desc_'];
       }
       if (isset($this->nmgp_cmp_hidden['desc_']) && $this->nmgp_cmp_hidden['desc_'] == 'off')
       {
           unset($this->nmgp_cmp_hidden['desc_']);
           $sStyleHidden_desc_ = 'display: none;';
       }
       $bTestReadOnly_desc_ = true;
       $sStyleReadLab_desc_ = 'display: none;';
       $sStyleReadInp_desc_ = '';
       if (isset($this->nmgp_cmp_readonly['desc_']) && $this->nmgp_cmp_readonly['desc_'] == 'on')
       {
           $bTestReadOnly_desc_ = false;
           unset($this->nmgp_cmp_readonly['desc_']);
           $sStyleReadLab_desc_ = '';
           $sStyleReadInp_desc_ = 'display: none;';
       }
       $this->obs_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['obs_']; 
       $obs_ = $this->obs_; 
       ...
       $this->existencia_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['existencia_']; 
       ...
       $this->colores_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['colores_']; 
       ...
       $this->elcolor_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['elcolor_']; 
       $elcolor_ = $this->elcolor_; 
       $sStyleHidden_elcolor_ = '';
       if (isset($sCheckRead_elcolor_))
       {
           unset($sCheckRead_elcolor_);
       }
       if (isset($this->nmgp_cmp_readonly['elcolor_']))
       {
           $sCheckRead_elcolor_ = $this->nmgp_cmp_readonly['elcolor_'];
       }
       if (isset($this->nmgp_cmp_hidden['elcolor_']) && $this->nmgp_cmp_hidden['elcolor_'] == 'off')
       {
           unset($this->nmgp_cmp_hidden['elcolor_']);
           $sStyleHidden_elcolor_ = 'display: none;';
       }
       $bTestReadOnly_elcolor_ = true;
       $sStyleReadLab_elcolor_ = 'display: none;';
       $sStyleReadInp_elcolor_ = '';
       if (isset($this->nmgp_cmp_readonly['elcolor_']) && $this->nmgp_cmp_readonly['elcolor_'] == 'on')
       {
           $bTestReadOnly_elcolor_ = false;
           unset($this->nmgp_cmp_readonly['elcolor_']);
           $sStyleReadLab_elcolor_ = '';
           $sStyleReadInp_elcolor_ = 'display: none;';
       }
       $this->tallas_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['tallas_']; 
       ...
       $this->latalla_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['latalla_']; 
       $latalla_ = $this->latalla_; 
       $sStyleHidden_latalla_ = '';
       if (isset($sCheckRead_latalla_))
       {
           unset($sCheckRead_latalla_);
       }
       if (isset($this->nmgp_cmp_readonly['latalla_']))
       {
           $sCheckRead_latalla_ = $this->nmgp_cmp_readonly['latalla_'];
       }
       if (isset($this->nmgp_cmp_hidden['latalla_']) && $this->nmgp_cmp_hidden['latalla_'] == 'off')
       {
           unset($this->nmgp_cmp_hidden['latalla_']);
           $sStyleHidden_latalla_ = 'display: none;';
       }
       $bTestReadOnly_latalla_ = true;
       $sStyleReadLab_latalla_ = 'display: none;';
       $sStyleReadInp_latalla_ = '';
       if (isset($this->nmgp_cmp_readonly['latalla_']) && $this->nmgp_cmp_readonly['latalla_'] == 'on')
       {
           $bTestReadOnly_latalla_ = false;
           unset($this->nmgp_cmp_readonly['latalla_']);
           $sStyleReadLab_latalla_ = '';
           $sStyleReadInp_latalla_ = 'display: none;';
       }
       $this->sabor_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['sabor_']; 
       $sabor_ = $this->sabor_; 
       $sStyleHidden_sabor_ = '';
       ...
       $this->elsabor_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['elsabor_']; 
       $elsabor_ = $this->elsabor_; 
       $sStyleHidden_elsabor_ = '';
       if (isset($sCheckRead_elsabor_))
       {
           unset($sCheckRead_elsabor_);
       }
       if (isset($this->nmgp_cmp_readonly['elsabor_']))
       {
           $sCheckRead_elsabor_ = $this->nmgp_cmp_readonly['elsabor_'];
       }
       if (isset($this->nmgp_cmp_hidden['elsabor_']) && $this->nmgp_cmp_hidden['elsabor_'] == 'off')
       {
           unset($this->nmgp_cmp_hidden['elsabor_']);
           $sStyleHidden_elsabor_ = 'display: none;';
       }
       $bTestReadOnly_elsabor_ = true;
       $sStyleReadLab_elsabor_ = 'display: none;';
       $sStyleReadInp_elsabor_ = '';
       if (isset($this->nmgp_cmp_readonly['elsabor_']) && $this->nmgp_cmp_readonly['elsabor_'] == 'on')
       {
           $bTestReadOnly_elsabor_ = false;
           unset($this->nmgp_cmp_readonly['elsabor_']);
           $sStyleReadLab_elsabor_ = '';
           $sStyleReadInp_elsabor_ = 'display: none;';
       }
       $this->idbod_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['idbod_']; 
       $idbod_ = $this->idbod_; 
       $sStyleHidden_idbod_ = '';
       ...
       $this->unidadmayor_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['unidadmayor_']; 
       ...
       $this->stockubica_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['stockubica_']; 
       ...
       $this->unidad_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['unidad_']; 
       ...
       $this->cantidad_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['cantidad_']; 
       ...
       $this->valorunit_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['valorunit_']; 
       ...
       $this->valorpar_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['valorpar_']; 
       ...
       $this->descuento_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['descuento_']; 
       ...
       $this->adicional_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['adicional_']; 
       ...
       $this->adicional1_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['adicional1_']; 
       ...
       $this->factor_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['factor_']; 
       ...
       $this->iva_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['iva_']; 
       $iva_ = $this->iva_; 
       $sStyleHidden_iva_ = '';
       ...
       $this->costop_ = $this->form_vert_form_detalleventa_111121[$sc_seq_vert]['costop_']; 
...
<?php echo nmButtonOutput($this->arr_buttons, "bmd_novo", "do_ajax_form_detalleventa_111121_add_new_line(" . $sc_seq_vert . ")", "do_ajax_form_detalleventa_111121_add_new_line(" . $sc_seq_vert . ")", "sc_new_line_" . $sc_seq_vert . "", "", "", "display: none", "", "", "", $this->Ini->path_botoes, "", "", "", "", "");?>
<?php }?>
<?php
  $Style_add_line = (!$Line_Add) ? "display: none" : "";
?>
<?php echo nmButtonOutput($this->arr_buttons, "bmd_cancelar", "do_ajax_form_detalleventa_111121_cancel_insert(" . $sc_seq_vert . ")", "do_ajax_form_detalleventa_111121_cancel_insert(" . $sc_seq_vert . ")", "sc_canceli_line_" . $sc_seq_vert . "", "", "", "" . $Style_add_line . "", "", "", "", $this->Ini->path_botoes, "", "", "", "", "");?>
<?php echo nmButtonOutput($this->arr_buttons, "bmd_cancelar", "do_ajax_form_detalleventa_111121_cancel_update(" . $sc_seq_vert . ")", "do_ajax_form_detalleventa_111121_cancel_update(" . $sc_seq_vert . ")", "sc_cancelu_line_" . $sc_seq_vert . "", "", "", "display: none", "", "", "", $this->Ini->path_botoes, "", "", "", "", "");?>
 </TD>
   <?php }?>
   <?php if (isset($this->nmgp_cmp_hidden['numfac_']) && $this->nmgp_cmp_hidden['numfac_'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="numfac_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($numfac_) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOddMult css_numfac__line" id="hidden_field_data_numfac_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_numfac_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_numfac__line" style="vertical-align: top;padding: 0px">
<?php if ($bTestReadOnly_numfac_ && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["numfac_"]) &&  $this->nmgp_cmp_readonly["numfac_"] == "on") { 

 ?>
<input type="hidden" name="numfac_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($numfac_) . "\">" . $numfac_ . ""; ?>
<?php } else { ?>
<span id="id_read_on_numfac_<?php echo $sc_seq_vert ?>" class="sc-ui-readonly-numfac_<?php echo $sc_seq_vert ?> css_numfac__line" style="<?php echo $sStyleReadLab_numfac_; ?>"><?php echo $this->form_format_readonly("numfac_", $this->form_encode_input($this->numfac_)); ?></span><span id="id_read_off_numfac_<?php echo $sc_seq_vert ?>" class="css_read_off_numfac_<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_numfac_; ?>">
 <input class="sc-js-input scFormObjectOddMult css_numfac__obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_numfac_<?php echo $sc_seq_vert ?>" type=text name="numfac_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($numfac_) ?>"
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=20"; } ?> alt="{datatype: 'integer', maxLength: 20, thousandsSep: '<?php echo str_replace("'", "\'", $this->field_config['numfac_']['symbol_grp']); ?>', thousandsFormat: <?php echo $this->field_config['numfac_']['symbol_fmt']; ?>, allowNegative: false, onlyNegative: false, negativePos: <?php echo (4 == $this->field_config['numfac_']['format_neg'] ? "'suffix'" : "'prefix'") ?>, alignment: 'left', enterTab: true, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddMultWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_numfac_<?php echo $sc_seq_vert; ?>_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_numfac_<?php echo $sc_seq_vert; ?>_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
$aLookup = array();
   ...
   $nm_nao_carga = false;
   $nmgp_def_dados = "" ; 
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_idpro_']))
   {
       $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_idpro_'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_idpro_']); 
   }
   else
   {
       $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_idpro_'] = array(); 
    }
   ...
   if ($nm_comando != "" && $rs = $this->Db->SelectLimit($nm_comando, 10, 0))
   {
       ...
       { 
              ...
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_idpro_'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
...
<select id="id_ac_idpro_<?php echo $sc_seq_vert; ?>" class="scFormObjectOddMult sc-ui-autocomp-idpro_ css_idpro__obj sc-js-input"><?php if ('' != $this->idpro_) { ?><option value="<?php echo $this->idpro_ ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select></span><?php } ?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['desc_']) && $this->nmgp_cmp_hidden['desc_'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="desc_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($desc_) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOddMult css_desc__line" id="hidden_field_data_desc_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_desc_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_desc__line" style="padding: 0px"><?php
          ...
          { 
              ...
              $prt_desc_ = "sc_" . $out_desc_ . "_desc__3030_" . $img_time . "_grp__NM__ico__NM__icons8-factura-32.png";
              ...
              if ($this->Ini->Export_img_zip) {
                  $this->Ini->Img_export_zip[] = $this->Ini->root . $out_desc_;
                  $desc_ = "<img  border=\"0\" src=\"" . $prt_desc_ . "\"/>" ; 
              }
              else {
...
?>
<span id="id_imghtml_desc_<?php echo $sc_seq_vert; ?>"><a href="javascript:nm_gp_submit('<?php echo $this->Ini->link_form_detalleventa_observ_edit . "', '$this->nm_location', 'SC_glo_par_par_numfacventa*scinpar_numfacventa*scoutdetalle*scin" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dados_form'][$sc_seq_vert]['iddet_'] . "*scoutNM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout', '', '_self', '0', '0', 'form_detalleventa_observ')\"><font color=\"" . $this->Ini->cor_link_dados . "\">" . $desc_ ; ?></font></a></span>
<?php if ($bTestReadOnly_desc_ && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["desc_"]) &&  $this->nmgp_cmp_readonly["desc_"] == "on") { 

 ?>
<input type="hidden" name="desc_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($desc_) . "\">" . $desc_ . ""; ?>
<?php } else { ?>
<span id="id_read_on_desc_<?php echo $sc_seq_vert ?>" class="sc-ui-readonly-desc_<?php echo $sc_seq_vert ?> css_desc__line" style="<?php echo $sStyleReadLab_desc_; ?>"><?php echo $this->form_format_readonly("desc_", $this->form_encode_input($this->desc_)); ?></span><span id="id_read_off_desc_<?php echo $sc_seq_vert ?>" class="css_read_off_desc_<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_desc_; ?>"></span><?php } ?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_desc_<?php echo $sc_seq_vert; ?>_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_desc_<?php echo $sc_seq_vert; ?>_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
<span id="id_read_on_obs_<?php echo $sc_seq_vert ?>" class="sc-ui-readonly-obs_<?php echo $sc_seq_vert ?> css_obs__line" style="<?php echo $sStyleReadLab_obs_; ?>"><?php echo $this->form_format_readonly("obs_", $this->form_encode_input($obs__val)); ?></span><span id="id_read_off_obs_<?php echo $sc_seq_vert ?>" class="css_read_off_obs_<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_obs_; ?>">
 ...
 alt="{datatype: 'text', maxLength: 600, allowedChars: '<?php echo $this->allowedCharsCharset("") ?>', lettersCase: 'upper', enterTab: true, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddMultWm', maskChars: '(){}[].,;:-+/ '}" >
<?php echo $obs_; ?>
</textarea>
</span><?php } ?>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOddMult css_existencia__line" id="hidden_field_data_existencia_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_existencia_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_existencia__line" style="padding: 0px"><?php
          ...
          { 
              if ($this->Ini->Export_img_zip) {
                  ...
                  $existencia_ = "<img border=\"0\" src=\"scriptcase__NM__ico__NM__shopping_bag_24.png\"/>" ; 
              }
              else {
...
?>
<span id="id_imghtml_existencia_<?php echo $sc_seq_vert; ?>"><a href="javascript:nm_gp_submit('<?php echo $this->Ini->link_grid_stock_cons . "', '$this->nm_location', 'SC_glo_par_par_idproducto*scinarticulo*scoutNMSC_modal*scinok*scout', 'inicio', 'modal', '500', '500', 'grid_stock')\"><font color=\"" . $this->Ini->cor_link_dados . "\">" . $existencia_ ; ?></font></a></span>
<?php if ($bTestReadOnly_existencia_ && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["existencia_"]) &&  $this->nmgp_cmp_readonly["existencia_"] == "on") { 

 ?>
<input type="hidden" name="existencia_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($existencia_) . "\">" . $existencia_ . ""; ?>
<?php } else { ?>
<span id="id_read_on_existencia_<?php echo $sc_seq_vert ?>" class="sc-ui-readonly-existencia_<?php echo $sc_seq_vert ?> css_existencia__line" style="<?php echo $sStyleReadLab_existencia_; ?>"><?php echo $this->form_format_readonly("existencia_", $this->form_encode_input($this->existencia_)); ?></span><span id="id_read_off_existencia_<?php echo $sc_seq_vert ?>" class="css_read_off_existencia_<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_existencia_; ?>"></span><?php } ?>
...
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_colores_']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_colores_'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_colores_']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_colores_'] = array(); 
}
if ($this->idpro_ != "")
{ 
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              ...
              $nmgp_def_dados .= $rs->fields[1] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_colores_'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
...
<?php
   ...
   echo "\r" ; 
   $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_colores_'][] = '0'; 
...
<?php  }?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['elcolor_']) && $this->nmgp_cmp_hidden['elcolor_'] == 'off') { $sc_hidden_yes++; ?>
<input type=hidden name="elcolor_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($this->elcolor_) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOddMult css_elcolor__line" id="hidden_field_data_elcolor_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_elcolor_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_elcolor__line" style="vertical-align: top;padding: 0px">
<?php if ($bTestReadOnly_elcolor_ && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["elcolor_"]) &&  $this->nmgp_cmp_readonly["elcolor_"] == "on") { 
 
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_elcolor_']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_elcolor_'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_elcolor_']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_elcolor_'] = array(); 
}
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
   { 
       $GLOBALS["NM_ERRO_IBASE"] = 1;  
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       while (!$rs->EOF) 
       { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_elcolor_'][] = $rs->fields[0];
   ...
   while (!empty($todo[$x])) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
          if ($cadaselect[1] == "@ ") {$cadaselect[1]= trim($cadaselect[1]); } ; 
          if (isset($this->Embutida_ronly) && $this->Embutida_ronly && isset($this->elcolor__1))
          {
              foreach ($this->elcolor__1 as $tmp_elcolor_)
              {
                  if (trim($tmp_elcolor_) === trim($cadaselect[1])) { $elcolor__look .= $cadaselect[0] . '__SC_BREAK_LINE__'; }
              }
          }
          elseif (trim($this->elcolor_) === trim($cadaselect[1])) { $elcolor__look .= $cadaselect[0]; } 
          $x++; 
   }

?>
<input type="hidden" name="elcolor_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($elcolor_) . "\">" . $elcolor__look . ""; ?>
<?php } else { ?>
<?php
   $todo = $this->Form_lookup_elcolor_();
   $x = 0 ; 
   $elcolor__look = ""; 
   while (!empty($todo[$x])) 
   {
          ...
          if (empty($elcolor__look))
          {
              $elcolor__look = $this->elcolor_;
          }
   $x = 0; 
   echo "<span id=\"id_read_on_elcolor_" . $sc_seq_vert . "\" class=\"css_elcolor__line\" style=\"" .  $sStyleReadLab_elcolor_ . "\">" . $this->form_format_readonly("elcolor_", $this->form_encode_input($elcolor__look)) . "</span><span id=\"id_read_off_elcolor_" . $sc_seq_vert . "\" class=\"css_read_off_elcolor_" . $this->classes_100perc_fields['span_input'] . "\" style=\"white-space: nowrap; " . $sStyleReadInp_elcolor_ . "\">";
   echo " <span id=\"idAjaxSelect_elcolor_" .  $sc_seq_vert . "\" class=\"" . $this->classes_100perc_fields['span_select'] . "\"><select class=\"sc-js-input scFormObjectOddMult css_elcolor__obj" . $this->classes_100perc_fields['input'] . "\" style=\"\" id=\"id_sc_field_elcolor_" . $sc_seq_vert . "\" name=\"elcolor_" . $sc_seq_vert . "\" size=\"1\" alt=\"{type: 'select', enterTab: true}\">" ; 
   echo "\r" ; 
   while (!empty($todo[$x]) && !$nm_nao_carga) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
          if ($cadaselect[1] == "@ ") {$cadaselect[1]= trim($cadaselect[1]); } ; 
          echo "  <option value=\"$cadaselect[1]\"" ; 
          if (trim($this->elcolor_) === trim($cadaselect[1])) 
          {
              echo " selected" ; 
          }
          if (strtoupper($cadaselect[2]) == "S") 
          {
              if (empty($this->elcolor_)) 
              {
                  echo " selected" ;
              } 
           } 
...
?> 
<?php  }?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_elcolor_<?php echo $sc_seq_vert; ?>_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_elcolor_<?php echo $sc_seq_vert; ?>_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_tallas_']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_tallas_'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_tallas_']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_tallas_'] = array(); 
}
if ($this->idpro_ != "")
{ 
   $this->nm_clear_val("idpro_");
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              ...
              $nmgp_def_dados .= $rs->fields[1] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_tallas_'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
...
<?php
   ...
   echo "\r" ; 
   $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_tallas_'][] = '0'; 
   echo "  <option value=\"0\">" . str_replace("<", "&lt;"," ") . "</option>" ; 
   while (!empty($todo[$x]) && !$nm_nao_carga) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
...
<?php  }?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['latalla_']) && $this->nmgp_cmp_hidden['latalla_'] == 'off') { $sc_hidden_yes++; ?>
<input type=hidden name="latalla_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($this->latalla_) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOddMult css_latalla__line" id="hidden_field_data_latalla_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_latalla_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_latalla__line" style="vertical-align: top;padding: 0px">
<?php if ($bTestReadOnly_latalla_ && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["latalla_"]) &&  $this->nmgp_cmp_readonly["latalla_"] == "on") { 
 
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_latalla_']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_latalla_'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_latalla_']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_latalla_'] = array(); 
}
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
   { 
       $GLOBALS["NM_ERRO_IBASE"] = 1;  
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       while (!$rs->EOF) 
       { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_latalla_'][] = $rs->fields[0];
              $nmgp_def_dados .= $rs->fields[0] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $rs->MoveNext() ; 
       } 
   ...
   while (!empty($todo[$x])) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
          if ($cadaselect[1] == "@ ") {$cadaselect[1]= trim($cadaselect[1]); } ; 
          if (isset($this->Embutida_ronly) && $this->Embutida_ronly && isset($this->latalla__1))
          {
              foreach ($this->latalla__1 as $tmp_latalla_)
              {
                  if (trim($tmp_latalla_) === trim($cadaselect[1])) { $latalla__look .= $cadaselect[0] . '__SC_BREAK_LINE__'; }
              }
          }
          elseif (trim($this->latalla_) === trim($cadaselect[1])) { $latalla__look .= $cadaselect[0]; } 
          $x++; 
   }

?>
<input type="hidden" name="latalla_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($latalla_) . "\">" . $latalla__look . ""; ?>
<?php } else { ?>
<?php
   $todo = $this->Form_lookup_latalla_();
   $x = 0 ; 
   $latalla__look = ""; 
   while (!empty($todo[$x])) 
   {
          ...
          if (empty($latalla__look))
          {
              $latalla__look = $this->latalla_;
          }
   $x = 0; 
   echo "<span id=\"id_read_on_latalla_" . $sc_seq_vert . "\" class=\"css_latalla__line\" style=\"" .  $sStyleReadLab_latalla_ . "\">" . $this->form_format_readonly("latalla_", $this->form_encode_input($latalla__look)) . "</span><span id=\"id_read_off_latalla_" . $sc_seq_vert . "\" class=\"css_read_off_latalla_" . $this->classes_100perc_fields['span_input'] . "\" style=\"white-space: nowrap; " . $sStyleReadInp_latalla_ . "\">";
   echo " <span id=\"idAjaxSelect_latalla_" .  $sc_seq_vert . "\" class=\"" . $this->classes_100perc_fields['span_select'] . "\"><select class=\"sc-js-input scFormObjectOddMult css_latalla__obj" . $this->classes_100perc_fields['input'] . "\" style=\"\" id=\"id_sc_field_latalla_" . $sc_seq_vert . "\" name=\"latalla_" . $sc_seq_vert . "\" size=\"1\" alt=\"{type: 'select', enterTab: true}\">" ; 
   echo "\r" ; 
   while (!empty($todo[$x]) && !$nm_nao_carga) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
          if ($cadaselect[1] == "@ ") {$cadaselect[1]= trim($cadaselect[1]); } ; 
          echo "  <option value=\"$cadaselect[1]\"" ; 
          if (trim($this->latalla_) === trim($cadaselect[1])) 
          {
              echo " selected" ; 
          }
          if (strtoupper($cadaselect[2]) == "S") 
          {
              if (empty($this->latalla_)) 
              {
                  echo " selected" ;
              } 
           } 
...
?> 
<?php  }?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_latalla_<?php echo $sc_seq_vert; ?>_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_latalla_<?php echo $sc_seq_vert; ?>_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_sabor_']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_sabor_'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_sabor_']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_sabor_'] = array(); 
}
if ($this->idpro_ != "")
{ 
   $this->nm_clear_val("idpro_");
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              ...
              $nmgp_def_dados .= $rs->fields[1] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_sabor_'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
...
<?php
   ...
   echo "\r" ; 
   $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_sabor_'][] = '0'; 
   echo "  <option value=\"0\">" . str_replace("<", "&lt;"," ") . "</option>" ; 
   while (!empty($todo[$x]) && !$nm_nao_carga) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
...
<?php  }?>
   ...
   <?php if (isset($this->nmgp_cmp_hidden['elsabor_']) && $this->nmgp_cmp_hidden['elsabor_'] == 'off') { $sc_hidden_yes++; ?>
<input type=hidden name="elsabor_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($this->elsabor_) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOddMult css_elsabor__line" id="hidden_field_data_elsabor_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_elsabor_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_elsabor__line" style="vertical-align: top;padding: 0px">
<?php if ($bTestReadOnly_elsabor_ && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["elsabor_"]) &&  $this->nmgp_cmp_readonly["elsabor_"] == "on") { 
 
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_elsabor_']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_elsabor_'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_elsabor_']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_elsabor_'] = array(); 
}
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
   { 
       $GLOBALS["NM_ERRO_IBASE"] = 1;  
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       while (!$rs->EOF) 
       { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_elsabor_'][] = $rs->fields[0];
              $nmgp_def_dados .= $rs->fields[0] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $rs->MoveNext() ; 
       } 
   ...
   while (!empty($todo[$x])) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
          if ($cadaselect[1] == "@ ") {$cadaselect[1]= trim($cadaselect[1]); } ; 
          if (isset($this->Embutida_ronly) && $this->Embutida_ronly && isset($this->elsabor__1))
          {
              foreach ($this->elsabor__1 as $tmp_elsabor_)
              {
                  if (trim($tmp_elsabor_) === trim($cadaselect[1])) { $elsabor__look .= $cadaselect[0] . '__SC_BREAK_LINE__'; }
              }
          }
          elseif (trim($this->elsabor_) === trim($cadaselect[1])) { $elsabor__look .= $cadaselect[0]; } 
          $x++; 
   }

?>
<input type="hidden" name="elsabor_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($elsabor_) . "\">" . $elsabor__look . ""; ?>
<?php } else { ?>
<?php
   $todo = $this->Form_lookup_elsabor_();
   $x = 0 ; 
   $elsabor__look = ""; 
   while (!empty($todo[$x])) 
   {
          ...
          if (empty($elsabor__look))
          {
              $elsabor__look = $this->elsabor_;
          }
   $x = 0; 
   echo "<span id=\"id_read_on_elsabor_" . $sc_seq_vert . "\" class=\"css_elsabor__line\" style=\"" .  $sStyleReadLab_elsabor_ . "\">" . $this->form_format_readonly("elsabor_", $this->form_encode_input($elsabor__look)) . "</span><span id=\"id_read_off_elsabor_" . $sc_seq_vert . "\" class=\"css_read_off_elsabor_" . $this->classes_100perc_fields['span_input'] . "\" style=\"white-space: nowrap; " . $sStyleReadInp_elsabor_ . "\">";
   echo " <span id=\"idAjaxSelect_elsabor_" .  $sc_seq_vert . "\" class=\"" . $this->classes_100perc_fields['span_select'] . "\"><select class=\"sc-js-input scFormObjectOddMult css_elsabor__obj" . $this->classes_100perc_fields['input'] . "\" style=\"\" id=\"id_sc_field_elsabor_" . $sc_seq_vert . "\" name=\"elsabor_" . $sc_seq_vert . "\" size=\"1\" alt=\"{type: 'select', enterTab: true}\">" ; 
   echo "\r" ; 
   while (!empty($todo[$x]) && !$nm_nao_carga) 
   {
          $cadaselect = explode("?#?", $todo[$x]) ; 
          if ($cadaselect[1] == "@ ") {$cadaselect[1]= trim($cadaselect[1]); } ; 
          echo "  <option value=\"$cadaselect[1]\"" ; 
          if (trim($this->elsabor_) === trim($cadaselect[1])) 
          {
              echo " selected" ; 
          }
          if (strtoupper($cadaselect[2]) == "S") 
          {
              if (empty($this->elsabor_)) 
              {
                  echo " selected" ;
              } 
           } 
...
?> 
<?php  }?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_elsabor_<?php echo $sc_seq_vert; ?>_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_elsabor_<?php echo $sc_seq_vert; ?>_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
$nmgp_def_dados = "" ; 
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_idbod_']))
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_idbod_'] = array_unique($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_idbod_']); 
}
else
{
    $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_idbod_'] = array(); 
}
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
   { 
       $GLOBALS["NM_ERRO_IBASE"] = 1;  
   ...
   if ($nm_comando != "" && $rs = $this->Db->Execute($nm_comando))
   {
       ...
       { 
              ...
              $nmgp_def_dados .= $rs->fields[1] . "?#?" ; 
              $nmgp_def_dados .= $rs->fields[0] . "?#?N?@?" ; 
              $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_idbod_'][] = $rs->fields[0];
              $rs->MoveNext() ; 
       } 
       $rs->Close() ; 
   } 
...
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_unidadmayor_'][] = 'NO'; ?>
 <option  value="SI" <?php  if ($this->unidadmayor_ == "SI") { echo " selected" ;} ?>>SI</option>
<?php $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['Lookup_unidadmayor_'][] = 'SI'; ?>
 </select></span>
</span><?php  }?>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOddMult css_stockubica__line" id="hidden_field_data_stockubica_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_stockubica_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_stockubica__line" style="vertical-align: top;padding: 0px"><input type="hidden" name="stockubica_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($stockubica_); ?>"><span id="id_ajax_label_stockubica_<?php echo $sc_seq_vert; ?>"><?php echo nl2br($stockubica_); ?></span>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOddMult css_unidad__line" id="hidden_field_data_unidad_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_unidad_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_unidad__line" style="vertical-align: top;padding: 0px"><input type="hidden" name="unidad_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($unidad_); ?>"><span id="id_ajax_label_unidad_<?php echo $sc_seq_vert; ?>"><?php echo nl2br($unidad_); ?></span>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOddMult css_cantidad__line" id="hidden_field_data_cantidad_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_cantidad_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_cantidad__line" style="padding: 0px">
...
<span id="id_read_on_cantidad_<?php echo $sc_seq_vert ?>" class="sc-ui-readonly-cantidad_<?php echo $sc_seq_vert ?> css_cantidad__line" style="<?php echo $sStyleReadLab_cantidad_; ?>"><?php echo $this->form_format_readonly("cantidad_", $this->form_encode_input($this->cantidad_)); ?></span><span id="id_read_off_cantidad_<?php echo $sc_seq_vert ?>" class="css_read_off_cantidad_<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_cantidad_; ?>">
 ...
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=5"; } ?> alt="{datatype: 'decimal', maxLength: 12, precision: 2, decimalSep: '<?php echo str_replace("'", "\'", $this->field_config['cantidad_']['symbol_dec']); ?>', thousandsSep: '<?php echo str_replace("'", "\'", $this->field_config['cantidad_']['symbol_grp']); ?>', thousandsFormat: <?php echo $this->field_config['cantidad_']['symbol_fmt']; ?>, manualDecimals: true, allowNegative: false, onlyNegative: false, negativePos: <?php echo (4 == $this->field_config['cantidad_']['format_neg'] ? "'suffix'" : "'prefix'") ?>, alignment: 'center', enterTab: true, enterSubmit: false, autoTab: false, selectOnFocus: true, watermark: '', watermarkClass: 'scFormObjectOddMultWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOddMult css_valorunit__line" id="hidden_field_data_valorunit_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_valorunit_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%;float:right"><tr><td  class="scFormDataFontOddMult css_valorunit__line" style="vertical-align: top;padding: 0px">
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOddMult css_valorpar__line" id="hidden_field_data_valorpar_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_valorpar_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%;float:right"><tr><td  class="scFormDataFontOddMult css_valorpar__line" style="padding: 0px"><input type="hidden" name="valorpar_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($valorpar_); ?>"><span id="id_ajax_label_valorpar_<?php echo $sc_seq_vert; ?>"><?php echo nl2br($valorpar_); ?></span>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOddMult css_descuento__line" id="hidden_field_data_descuento_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_descuento_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_descuento__line" style="padding: 0px"><input type="hidden" name="descuento_<?php echo $sc_seq_vert ?>" value="<?php echo $this->form_encode_input($descuento_); ?>"><span id="id_ajax_label_descuento_<?php echo $sc_seq_vert; ?>"><?php echo nl2br($descuento_); ?></span>
...
<?php } else { $sc_hidden_no++; ?>
    ...
    <TD class="scFormDataOddMult css_adicional1__line" id="hidden_field_data_adicional1_<?php echo $sc_seq_vert; ?>" style="<?php echo $sStyleHidden_adicional1_; ?>"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td  class="scFormDataFontOddMult css_adicional1__line" style="vertical-align: top;padding: 0px">
...
<?php   
        if (isset($sCheckRead_numfac_))
       {
           $this->nmgp_cmp_readonly['numfac_'] = $sCheckRead_numfac_;
       }
       if ('display: none;' == $sStyleHidden_numfac_)
       {
           $this->nmgp_cmp_hidden['numfac_'] = 'off';
       }
       if (isset($sCheckRead_idpro_))
       ...
       if (isset($sCheckRead_desc_))
       {
           $this->nmgp_cmp_readonly['desc_'] = $sCheckRead_desc_;
       }
       if ('display: none;' == $sStyleHidden_desc_)
       {
           $this->nmgp_cmp_hidden['desc_'] = 'off';
       }
       if (isset($sCheckRead_obs_))
       {
       ...
       if (isset($sCheckRead_elcolor_))
       {
           $this->nmgp_cmp_readonly['elcolor_'] = $sCheckRead_elcolor_;
       }
       if ('display: none;' == $sStyleHidden_elcolor_)
       {
           $this->nmgp_cmp_hidden['elcolor_'] = 'off';
       }
       if (isset($sCheckRead_tallas_))
       {
       ...
       if (isset($sCheckRead_latalla_))
       {
           $this->nmgp_cmp_readonly['latalla_'] = $sCheckRead_latalla_;
       }
       if ('display: none;' == $sStyleHidden_latalla_)
       {
           $this->nmgp_cmp_hidden['latalla_'] = 'off';
       }
       if (isset($sCheckRead_sabor_))
       {
       ...
       if (isset($sCheckRead_elsabor_))
       {
           $this->nmgp_cmp_readonly['elsabor_'] = $sCheckRead_elsabor_;
       }
       if ('display: none;' == $sStyleHidden_elsabor_)
       {
           $this->nmgp_cmp_hidden['elsabor_'] = 'off';
       }
       if (isset($sCheckRead_idbod_))
       {
   ...
   { 
       ...
       $this->form_vert_form_detalleventa_111121 = $guarda_form_vert_form_detalleventa_111121;
   } 
   if ($Table_refresh) 
   { 
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['birpara']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['birpara']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['birpara']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['birpara']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['birpara'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['first']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['first']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['first']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['first']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['first'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['back']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['back']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['back']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['back']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['back'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['forward']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['forward']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['forward']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['forward']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['forward'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['last']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_disabled']['last']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['last']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['last']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['btn_label']['last'];
        }
?>
...
<?php if (('novo' != $this->nmgp_opcao || $this->Embutida_form) && !$this->nmgp_form_empty && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "F") { if ('parcial' == $this->form_paginacao) {?><script>summary_atualiza(<?php echo ($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['reg_start'] + 1). ", " . $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['reg_qtd'] . ", " . ($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['total'] + 1)?>);</script><?php }} ?>
<?php if (('novo' != $this->nmgp_opcao || $this->Embutida_form) && !$this->nmgp_form_empty && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "F") { if ('total' == $this->form_paginacao) {?><script>summary_atualiza(1, <?php echo $this->sc_max_reg . ", " . $this->sc_max_reg?>);</script><?php }} ?>
<?php if (('novo' != $this->nmgp_opcao || $this->Embutida_form) && !$this->nmgp_form_empty && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "R" && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_iframe'] != "F") { ?><script>navpage_atualiza('<?php echo $this->SC_nav_page ?>');</script><?php } ?>
</td></tr> 
</table> 
</div> 
</td> 
...
<?php
if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_modal']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['run_modal'])
{
?>
...
?>
<?php
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['masterValue']))
{
    if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['under_dashboard']) {
?>
var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['parent_widget']; ?>']");
if (dbParentFrame && dbParentFrame[0] && dbParentFrame[0].contentWindow.scAjaxDetailValue)
{
<?php
        foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['masterValue'] as $cmp_master => $val_master)
        {
?>
    dbParentFrame[0].contentWindow.scAjaxDetailValue('<?php echo $cmp_master ?>', '<?php echo $val_master ?>');
<?php
        }
        unset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['masterValue']);
?>
}
<?php
    }
...
<script>
 var dbParentFrame = $(parent.document).find("[name='<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['dashboard_info']['parent_widget']; ?>']");
 dbParentFrame[0].contentWindow.scAjaxDetailStatus("form_detalleventa_111121");
</script>
<?php
    }
    else {
...
?>
<script>
 parent.scAjaxDetailStatus("form_detalleventa_111121");
 parent.scAjaxDetailHeight("form_detalleventa_111121", <?php echo $sTamanhoIframe; ?>);
</script>
<?php
    }
}
...
{
<?php
if ($this->lig_edit_lookup && isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['sc_modal'])
{
?>
...
<script type="text/javascript">
	function scBtnFn_sys_format_inc() {
		if ($("#sc_b_new_t.sc-unique-btn-1").length && $("#sc_b_new_t.sc-unique-btn-1").is(":visible")) {
			...
			do_ajax_form_detalleventa_111121_add_new_line(); return false;
			 return;
		}
...
<?php
$_SESSION['sc_session'][$this->Ini->sc_page]['form_detalleventa_111121']['buttonStatus'] = $this->nmgp_botoes;
?>
<script type="text/javascript">
