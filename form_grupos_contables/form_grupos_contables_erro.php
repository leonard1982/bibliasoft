...
$old_error_handler = set_error_handler("form_grupos_contables_scriptcase_error_handler");

function form_grupos_contables_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_grupos_contables']['contr_erro']) && $_SESSION['scriptcase']['form_grupos_contables']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_grupos_contables->contr_form_grupos_contables->Apl_com_erro = true;
            if ($inicial_form_grupos_contables->contr_form_grupos_contables->NM_ajax_flag)
            {
                $inicial_form_grupos_contables->contr_form_grupos_contables->NM_ajax_info['errList']['geral_form_grupos_contables'][] = $msg;
                return;
            }
            form_grupos_contables_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_grupos_contables_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_grupos_contables'])  && '' != $_SESSION['scriptcase']['error_icon']['form_grupos_contables'])  ? $_SESSION['scriptcase']['error_icon']['form_grupos_contables']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_grupos_contables']) && '' != $_SESSION['scriptcase']['error_close']['form_grupos_contables']) ? $_SESSION['scriptcase']['error_close']['form_grupos_contables'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_grupos_contables']) && '' != $_SESSION['scriptcase']['error_span_title']['form_grupos_contables']) ? $_SESSION['scriptcase']['error_span_title']['form_grupos_contables'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_grupos_contables']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_grupos_contables']) ? $_SESSION['scriptcase']['error_icon_title']['form_grupos_contables'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_grupos_contables']['error_buffer'] .= ob_get_flush();
}
