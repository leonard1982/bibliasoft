...
$old_error_handler = set_error_handler("devfac_compras_scriptcase_error_handler");

function devfac_compras_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['devfac_compras']['contr_erro']) && $_SESSION['scriptcase']['devfac_compras']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_devfac_compras->contr_devfac_compras->Apl_com_erro = true;
            if ($inicial_devfac_compras->contr_devfac_compras->NM_ajax_flag)
            {
                $inicial_devfac_compras->contr_devfac_compras->NM_ajax_info['errList']['geral_devfac_compras'][] = $msg;
                return;
            }
            devfac_compras_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function devfac_compras_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['devfac_compras'])  && '' != $_SESSION['scriptcase']['error_icon']['devfac_compras'])  ? $_SESSION['scriptcase']['error_icon']['devfac_compras']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['devfac_compras']) && '' != $_SESSION['scriptcase']['error_close']['devfac_compras']) ? $_SESSION['scriptcase']['error_close']['devfac_compras'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['devfac_compras']) && '' != $_SESSION['scriptcase']['error_span_title']['devfac_compras']) ? $_SESSION['scriptcase']['error_span_title']['devfac_compras'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['devfac_compras']) && '' != $_SESSION['scriptcase']['error_icon_title']['devfac_compras']) ? $_SESSION['scriptcase']['error_icon_title']['devfac_compras'] : '';
?>
