...
class grid_facturaven_cliente_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_facturaven_cliente']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_facturaven_cliente']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang , $resolucion, $idcli;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_cliente']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_cliente']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(saldo) from ((SELECT      idfacven,     numfacven,     nremision,     credito,     fechaven,     fechavenc,     idcli,     subtotal,     valoriva,     total,     pagada,     asentada,     observaciones,     saldo,     adicional,     adicional2,     adicional3, resolucion FROM      facturaven WHERE      idcli = " . $_SESSION['par_cliente'] . " ORDER BY idfacven desc) UNION (SELECT      idfacven,     numfacven,     nremision,     credito,     fechaven,     fechavenc,     idcli,     subtotal,     valoriva,     total,     pagada,     asentada,     observaciones,     saldo,     adicional,     adicional2,     adicional3, resolucion FROM      remisiones WHERE      idcli = " . $_SESSION['par_cliente'] . " ORDER BY idfacven DESC) ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_cliente']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(saldo) from ((SELECT      idfacven,     numfacven,     nremision,     credito,     fechaven,     fechavenc,     idcli,     subtotal,     valoriva,     total,     pagada,     asentada,     observaciones,     saldo,     adicional,     adicional2,     adicional3, resolucion FROM      facturaven WHERE      idcli = " . $_SESSION['par_cliente'] . " ORDER BY idfacven desc) UNION (SELECT      idfacven,     numfacven,     nremision,     credito,     fechaven,     fechavenc,     idcli,     subtotal,     valoriva,     total,     pagada,     asentada,     observaciones,     saldo,     adicional,     adicional2,     adicional3, resolucion FROM      remisiones WHERE      idcli = " . $_SESSION['par_cliente'] . " ORDER BY idfacven DESC) ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_cliente']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_cliente']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_cliente']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_cliente']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_facturaven_cliente']['contr_total_geral'] = "OK";
   } 
