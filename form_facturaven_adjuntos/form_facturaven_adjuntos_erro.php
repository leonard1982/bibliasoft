...
$old_error_handler = set_error_handler("form_facturaven_adjuntos_scriptcase_error_handler");

function form_facturaven_adjuntos_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_facturaven_adjuntos']['contr_erro']) && $_SESSION['scriptcase']['form_facturaven_adjuntos']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_facturaven_adjuntos->contr_form_facturaven_adjuntos->Apl_com_erro = true;
            if ($inicial_form_facturaven_adjuntos->contr_form_facturaven_adjuntos->NM_ajax_flag)
            {
                $inicial_form_facturaven_adjuntos->contr_form_facturaven_adjuntos->NM_ajax_info['errList']['geral_form_facturaven_adjuntos'][] = $msg;
                return;
            }
            form_facturaven_adjuntos_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_facturaven_adjuntos_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_facturaven_adjuntos'])  && '' != $_SESSION['scriptcase']['error_icon']['form_facturaven_adjuntos'])  ? $_SESSION['scriptcase']['error_icon']['form_facturaven_adjuntos']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_facturaven_adjuntos']) && '' != $_SESSION['scriptcase']['error_close']['form_facturaven_adjuntos']) ? $_SESSION['scriptcase']['error_close']['form_facturaven_adjuntos'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_facturaven_adjuntos']) && '' != $_SESSION['scriptcase']['error_span_title']['form_facturaven_adjuntos']) ? $_SESSION['scriptcase']['error_span_title']['form_facturaven_adjuntos'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_facturaven_adjuntos']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_facturaven_adjuntos']) ? $_SESSION['scriptcase']['error_icon_title']['form_facturaven_adjuntos'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_facturaven_adjuntos']['error_buffer'] .= ob_get_flush();
}
