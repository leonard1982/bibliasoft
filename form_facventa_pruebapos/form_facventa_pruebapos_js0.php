...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_facventa_pruebapos']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_facventa_pruebapos"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_Vender_par_numfacventa = "<?php echo $this->form_encode_input($this->nmgp_dados_form['numfacven']); ?>";
var var_btn_Vender_sum_iva = "<?php echo $this->form_encode_input($this->nmgp_dados_form['valoriva']); ?>";
function sc_btn_Vender()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Vender(); }, 500);
      return;
    }
  tb_show('', '<?php echo $this->Ini->link_seleccionarproducto_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&script_case_session=<?php echo session_id() ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "par_numfacventa*scin' + var_btn_Vender_par_numfacventa + '*scoutt_iva*scin0*scoutsum_iva*scin' + var_btn_Vender_sum_iva + '*scoutNM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinS*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=300&width=900&modal=true', '');
}
var var_btn_Pagar_idfacven = "<?php echo $this->form_encode_input($this->nmgp_dados_form['idfacven']); ?>";
var var_btn_Pagar_par_numfacventa = "<?php echo $this->form_encode_input($this->nmgp_dados_form['numfacven']); ?>";
function sc_btn_Pagar()
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_facturaven_pagar_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "idfacven?#?' + var_btn_Pagar_idfacven + '?@?par_numfacventa?#?' + var_btn_Pagar_par_numfacventa + '?@?NM_btn_insert?#?S?@?NM_btn_update?#?S?@?NM_btn_delete?#?S?@?NM_btn_navega?#?S?@?"; ?>', '', '_self', '0', '0', 'form_facturaven_pagar');
}
var var_btn_sc_btn_0_par_numfacventa = "<?php echo $this->form_encode_input($this->nmgp_dados_form['numfacven']); ?>";
function sc_btn_sc_btn_0()
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_detalleven_pos_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "par_numfacventa?#?' + var_btn_sc_btn_0_par_numfacventa + '?@?NM_btn_insert?#?S?@?NM_btn_update?#?S?@?NM_btn_delete?#?S?@?NM_btn_navega?#?N?@?"; ?>', '', '_self', '0', '0', 'form_detalleven_pos');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_facventa_pruebapos']['where_detal']))
  {
?>  
...
{
   nm_field_disabled("idcli=disabled", "U");
