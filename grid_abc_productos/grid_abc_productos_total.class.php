...
class grid_abc_productos_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_abc_productos']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_abc_productos']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_grupo($res_limit=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(vendido), sum(porcentaje) from (select p.nompro as producto,sum(d.valorpar-d.iva) as vendido,round(((sum(d.valorpar-d.iva)*100)/(select sum(v.valorpar-v.iva) from detalleventa v)),2) as porcentaje,(select sum(v.valorpar-v.iva) from detalleventa v) as total, g.nomgrupo from productos p inner join detalleventa d on d.idpro=p.idprod inner join grupo g on p.idgrup=g.idgrupo GROUP by p.nompro ORDER by sum(d.valorpar-d.iva) desc) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(vendido), sum(porcentaje) from (select p.nompro as producto,sum(d.valorpar-d.iva) as vendido,round(((sum(d.valorpar-d.iva)*100)/(select sum(v.valorpar-v.iva) from detalleventa v)),2) as porcentaje,(select sum(v.valorpar-v.iva) from detalleventa v) as total, g.nomgrupo from productos p inner join detalleventa d on d.idpro=p.idprod inner join grupo g on p.idgrup=g.idgrupo GROUP by p.nompro ORDER by sum(d.valorpar-d.iva) desc) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['contr_total_geral'] = "OK";
   } 
   ...
   //-----  nomgrupo
   function quebra_nomgrupo_grupo($nomgrupo, $arg_sum_nomgrupo) 
   {
      global $tot_nomgrupo;
      $tot_nomgrupo = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq'])) 
      { 
         $nm_comando .= " where nomgrupo" . $arg_sum_nomgrupo ; 
      } 
      else 
      { 
         $nm_comando .= " and nomgrupo" . $arg_sum_nomgrupo ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_nomgrupo[0] = sc_strip_script($nomgrupo) ; 
      $tot_nomgrupo[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_nomgrupo[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_nomgrupo[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
   } 

   //-----  nomgrupo
   function quebra_nomgrupo_sc_free_group_by($nomgrupo, $Where_qb, $Cmp_Name) 
   {
      ...
      $tot_nomgrupo[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_nomgrupo;
   } 


   //----- 
   function Calc_resumo_grupo($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("es");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('nomgrupo' => "nomgrupo");
      $cmps_quebra_atual = array("nomgrupo");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_grupo($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_grupo($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('vendido' => 'N','porcentaje' => 'N','total' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(vendido), sum(porcentaje)#@#cmps_quebras#@# from (select p.nompro as producto,sum(d.valorpar-d.iva) as vendido,round(((sum(d.valorpar-d.iva)*100)/(select sum(v.valorpar-v.iva) from detalleventa v)),2) as porcentaje,(select sum(v.valorpar-v.iva) from detalleventa v) as total, g.nomgrupo from productos p inner join detalleventa d on d.idpro=p.idprod inner join grupo g on p.idgrup=g.idgrupo GROUP by p.nompro ORDER by sum(d.valorpar-d.iva) desc) sc_sel_esp " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2)#@#cmps_quebras#@# from (";
         $comando .= "select vendido as SC_metric1,porcentaje as SC_metric2, " . $cmps_quebras1 . " from (select p.nompro as producto,sum(d.valorpar-d.iva) as vendido,round(((sum(d.valorpar-d.iva)*100)/(select sum(v.valorpar-v.iva) from detalleventa v)),2) as porcentaje,(select sum(v.valorpar-v.iva) from detalleventa v) as total, g.nomgrupo from productos p inner join detalleventa d on d.idpro=p.idprod inner join grupo g on p.idgrup=g.idgrupo GROUP by p.nompro ORDER by sum(d.valorpar-d.iva) desc) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (select p.nompro as producto,sum(d.valorpar-d.iva) as vendido,round(((sum(d.valorpar-d.iva)*100)/(select sum(v.valorpar-v.iva) from detalleventa v)),2) as porcentaje,(select sum(v.valorpar-v.iva) from detalleventa v) as total, g.nomgrupo from productos p inner join detalleventa d on d.idpro=p.idprod inner join grupo g on p.idgrup=g.idgrupo GROUP by p.nompro ORDER by sum(d.valorpar-d.iva) desc) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['nomgrupo']['reg'] = "S";
      $format_dimensions['nomgrupo']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'grupo', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "nomgrupo") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_grupo($where_ok);
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
