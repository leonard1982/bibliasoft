...
class grid_abc_productos_pesq
{
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['path_libs_php'] = $this->Ini->path_lib_php;
   ...
   function init()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] = "igual";
   }
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_abc_productos_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_abc_productos_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_abc_productos_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
   ...
   function lookup_ajax_porcentaje($porcentaje)
   {
      ...
      {
          $nm_comando = "select distinct porcentaje from (select p.nompro as producto,sum(d.valorpar-d.iva) as vendido,round(((sum(d.valorpar-d.iva)*100)/(select sum(v.valorpar-v.iva) from detalleventa v)),2) as porcentaje,(select sum(v.valorpar-v.iva) from detalleventa v) as total, g.nomgrupo from productos p inner join detalleventa d on d.idpro=p.idprod inner join grupo g on p.idgrup=g.idgrupo GROUP by p.nompro ORDER by sum(d.valorpar-d.iva) desc) nm_sel_esp where   CAST (porcentaje AS TEXT)  like '%" . $porcentaje . "%'"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct porcentaje from (select p.nompro as producto,sum(d.valorpar-d.iva) as vendido,round(((sum(d.valorpar-d.iva)*100)/(select sum(v.valorpar-v.iva) from detalleventa v)),2) as porcentaje,(select sum(v.valorpar-v.iva) from detalleventa v) as total, g.nomgrupo from productos p inner join detalleventa d on d.idpro=p.idprod inner join grupo g on p.idgrup=g.idgrupo GROUP by p.nompro ORDER by sum(d.valorpar-d.iva) desc) nm_sel_esp where   CAST (porcentaje AS VARCHAR)  like '%" . $porcentaje . "%'"; 
      }
      else
      {
          $nm_comando = "select distinct porcentaje from (select p.nompro as producto,sum(d.valorpar-d.iva) as vendido,round(((sum(d.valorpar-d.iva)*100)/(select sum(v.valorpar-v.iva) from detalleventa v)),2) as porcentaje,(select sum(v.valorpar-v.iva) from detalleventa v) as total, g.nomgrupo from productos p inner join detalleventa d on d.idpro=p.idprod inner join grupo g on p.idgrup=g.idgrupo GROUP by p.nompro ORDER by sum(d.valorpar-d.iva) desc) nm_sel_esp where  porcentaje like '%" . $porcentaje . "%'"; 
      }
      foreach ($this->Ini->nm_col_dinamica as $nm_cada_col => $nm_nova_col)
      {
          $nm_comando = str_replace($nm_cada_col, $nm_nova_col, $nm_comando); 
      ...
      { 
         while (!$rs->EOF) 
         { 
            nmgp_Form_Num_Val($rs->fields[0], ",", ".", "0", "S", "2", "", "N:3", "-") ; 
            $cmp1 = NM_charset_to_utf8(trim($rs->fields[0]));
            $nmgp_def_dados[] = array($cmp1 => $cmp1); 
            $rs->MoveNext() ; 
         } 
   ...
   function monta_condicao($nome, $condicao, $campo, $campo2 = "", $nome_campo="", $tp_campo="")
   {
      ...
      $nm_ini_lower = "";
      $nm_fim_lower = "";
      $Nm_numeric[] = "vendido";$Nm_numeric[] = "porcentaje";$Nm_numeric[] = "total";$Nm_numeric[] = "vendido";$Nm_numeric[] = "porcentaje";$Nm_numeric[] = "total";
      ...
      if (in_array($campo_join, $Nm_numeric))
      {
         ...
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['decimal_db'] == ".")
         {
            $nm_aspas  = "";
            $nm_aspas1 = "";
         }
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_abc_productos/grid_abc_productos_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
<?php
$vertical_center = '';
?>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_abc_productos_ajax_search.js"></script>
 <script type="text/javascript" src="grid_abc_productos_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_abc_productos_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_abc_productos']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_abc_productos']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_nomgrupo_cond#NMF#' + search_get_sel_txt('SC_nomgrupo_cond') + '@NMF@';
  str_out += 'SC_nomgrupo#NMF#' + search_get_select('SC_nomgrupo') + '@NMF@';
  ...
  str_out += 'SC_porcentaje_input_2#NMF#' + search_get_text('SC_porcentaje_input_2') + '@NMF@';
...
<?php
   ...
   {
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div style="width: 100%">
 ...
 <div style="height:37px; border-width:0px 0px 1px 0px;  border-style: dashed; border-color:#ddd; display: block">
 	<table style="width:100%; border-collapse:collapse; padding:0;">
    	<tr>
        	<td id="lin1_col1" class="scFilterHeaderFont"><span>ABC DE PRODUCTOS</span></td>
            <td id="lin1_col2" class="scFilterHeaderFont"><span><IMG src="<?php echo $this->Ini->path_imag_cab; ?>/scriptcase__NM__ico__NM__components_32.png" border="0"></span></td>
        </tr>
    </table>		 
 </div>
</div>
...
<?php
   ...
   {
      global 
             $nomgrupo_cond, $nomgrupo,
             $producto_cond, $producto, $producto_autocomp,
             $porcentaje_cond, $porcentaje, $porcentaje_input_2, $porcentaje_autocomp, $porcentaje_IN,
      ...
      if (isset($_SESSION['scriptcase']['sc_aba_iframe']))
      {
          ...
          {
              if (in_array("grid_abc_productos", $apls_aba))
              {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['iframe_menu'] && (!isset($_SESSION['scriptcase']['menu_mobile']) || empty($_SESSION['scriptcase']['menu_mobile'])))
      {
          $this->aba_iframe = true;
      }
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $nomgrupo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['nomgrupo']; 
          $nomgrupo_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['nomgrupo_cond']; 
          $producto = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['producto']; 
          $producto_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['producto_cond']; 
          $porcentaje = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['porcentaje']; 
          $porcentaje_IN = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['porcentaje_IN']; 
          $porcentaje_input_2 = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['porcentaje_input_2']; 
          $porcentaje_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['porcentaje_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['NM_operador']; 
      } 
      if (!isset($nomgrupo_cond) || empty($nomgrupo_cond))
      {
         $nomgrupo_cond = "eq";
      }
      ...
      $str_hide_nomgrupo = (in_array($nomgrupo_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      ...
      $str_display_nomgrupo = ('bw' == $nomgrupo_cond) ? $display_aberto : $display_fechado;
      ...
      if (!isset($nomgrupo) || $nomgrupo == "")
      {
          $nomgrupo = "";
      }
      if (isset($nomgrupo) && !empty($nomgrupo))
      {
 ...
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr>
      ...
      <INPUT type="hidden" id="SC_nomgrupo_cond" name="nomgrupo_cond" value="eq">

    <TD nowrap class="scFilterLabelOdd" style="vertical-align: top" > <?php
 $SC_Label = (isset($this->New_label['nomgrupo'])) ? $this->New_label['nomgrupo'] : "Grupo/Familia";
 $nmgp_tab_label .= "nomgrupo?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
...
?>
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br><span id="id_hide_nomgrupo"  <?php echo $str_hide_nomgrupo?>>
<?php
      $nomgrupo_look = substr($this->Db->qstr($nomgrupo), 1, -1); 
      $nmgp_def_dados = "" ; 
      $nm_comando = "SELECT nomgrupo, nomgrupo  FROM grupo  ORDER BY nomgrupo"; 
      foreach ($this->Ini->nm_col_dinamica as $nm_cada_col => $nm_nova_col)
      {
          $nm_comando = str_replace($nm_cada_col, $nm_nova_col, $nm_comando); 
      }
      ...
      if ($rs = $this->Db->Execute($nm_comando)) 
      { 
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['psq_check_ret']['nomgrupo'] = array();
         while (!$rs->EOF) 
         { 
            $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['psq_check_ret']['nomgrupo'][] = trim($rs->fields[0]);
...
?>
   <span id="idAjaxSelect_nomgrupo">
      <SELECT class="scFilterObjectOdd" id="SC_nomgrupo" name="nomgrupo"  size="1">
...
<?php
      ...
      foreach ($nm_opcoes as $nm_opcao)
      {
         if (!empty($nm_opcao))
         {
            ...
            if ("" != $nomgrupo)
            {
                    $nomgrupo_sel = ($nm_opc_cod === $nomgrupo) ? "selected" : "";
            }
            else
            {
               $nomgrupo_sel = ("S" == $nm_opc_sel) ? "selected" : "";
            }
            $nm_sc_valor = $nm_opc_val;
            $nm_opc_val = $nm_sc_valor;
?>
       <OPTION value="<?php echo NM_encode_input($nm_opc_cod . $delimitador . $nm_opc_val); ?>" <?php echo $nomgrupo_sel; ?>><?php echo $nm_opc_val; ?></OPTION>
<?php
         }
      }
?>
...
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br><span id="id_hide_producto"  <?php echo $str_hide_producto?>><?php
      ...
      $producto_look = substr($this->Db->qstr($producto), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct producto from (select p.nompro as producto,sum(d.valorpar-d.iva) as vendido,round(((sum(d.valorpar-d.iva)*100)/(select sum(v.valorpar-v.iva) from detalleventa v)),2) as porcentaje,(select sum(v.valorpar-v.iva) from detalleventa v) as total, g.nomgrupo from productos p inner join detalleventa d on d.idpro=p.idprod inner join grupo g on p.idgrup=g.idgrupo GROUP by p.nompro ORDER by sum(d.valorpar-d.iva) desc) nm_sel_esp where producto = '$producto_look'"; 
      foreach ($this->Ini->nm_col_dinamica as $nm_cada_col => $nm_nova_col)
      {
          $nm_comando = str_replace($nm_cada_col, $nm_nova_col, $nm_comando); 
      }
...
?>
<INPUT  type="text" id="SC_producto" name="producto" value="<?php echo NM_encode_input($producto) ?>"  size=50 alt="{datatype: 'text', maxLength: 600, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
...
<span class="SC_Field_label_Mob"><?php echo $SC_Label ?></span><br>
      <SELECT class="SC_Cond_Selector scFilterObjectOdd" id="SC_porcentaje_cond" name="porcentaje_cond" onChange="nm_campos_between(document.getElementById('id_vis_porcentaje'), this, 'porcentaje')">
       ...
       <OPTION value="bw" <?php if ("bw" == $porcentaje_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_betw'] ?></OPTION>
       <OPTION value="ii" <?php if ("ii" == $porcentaje_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_stts_with'] ?></OPTION>
       <OPTION value="qp" <?php if ("qp" == $porcentaje_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="df" <?php if ("df" == $porcentaje_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_dife'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $porcentaje_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="ge" <?php if ("ge" == $porcentaje_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr_equl'] ?></OPTION>
       <OPTION value="le" <?php if ("le" == $porcentaje_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less_equl'] ?></OPTION>
       <OPTION value="in" <?php if ("in" == $porcentaje_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_betw_sevr'] ?></OPTION>
       <OPTION value="nu" <?php if ("nu" == $porcentaje_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_null'] ?></OPTION>
       <OPTION value="nn" <?php if ("nn" == $porcentaje_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_nnul'] ?></OPTION>
      </SELECT>
      <br><span id="id_hide_porcentaje"  <?php echo $str_hide_porcentaje?>><?php
      if ($porcentaje != "")
      {
      $porcentaje_look = substr($this->Db->qstr($porcentaje), 1, -1); 
      $nmgp_def_dados = array(); 
   ...
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct porcentaje from (select p.nompro as producto,sum(d.valorpar-d.iva) as vendido,round(((sum(d.valorpar-d.iva)*100)/(select sum(v.valorpar-v.iva) from detalleventa v)),2) as porcentaje,(select sum(v.valorpar-v.iva) from detalleventa v) as total, g.nomgrupo from productos p inner join detalleventa d on d.idpro=p.idprod inner join grupo g on p.idgrup=g.idgrupo GROUP by p.nompro ORDER by sum(d.valorpar-d.iva) desc) nm_sel_esp where porcentaje = $porcentaje_look"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct porcentaje from (select p.nompro as producto,sum(d.valorpar-d.iva) as vendido,round(((sum(d.valorpar-d.iva)*100)/(select sum(v.valorpar-v.iva) from detalleventa v)),2) as porcentaje,(select sum(v.valorpar-v.iva) from detalleventa v) as total, g.nomgrupo from productos p inner join detalleventa d on d.idpro=p.idprod inner join grupo g on p.idgrup=g.idgrupo GROUP by p.nompro ORDER by sum(d.valorpar-d.iva) desc) nm_sel_esp where porcentaje = $porcentaje_look"; 
      ...
      {
            nmgp_Form_Num_Val($porcentaje, ",", ".", "0", "S", "2", "", "N:3", "-") ; 
          $sAutocompValue = $porcentaje;
      }
?>

<?php
$style_cond_in = ($porcentaje_cond == "in") ? " style=\"display: none;\"" : ""; 
?>
<INPUT  type="text" id="SC_porcentaje" name="porcentaje" value="<?php echo NM_encode_input($porcentaje) ?>" size=4 alt="{datatype: 'integer', maxLength: 4, thousandsSep: ',', allowNegative: true, onlyNegative: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectOdd sc-ui-autocomp-porcentaje" id="id_ac_porcentaje" name="porcentaje_autocomp"<?php echo $style_cond_in ?>><?php if ('' !=  $porcentaje) { ?><option value="<?php echo $porcentaje ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
<?php
$style_cond_in = ($porcentaje_cond != "in") ? " style=\"display: none;\"" : ""; 
?>
<INPUT  type="text" id="SC_porcentaje_IN" name="porcentaje_IN" value="<?php echo NM_encode_input($porcentaje_IN) ?>" size=4 alt="{datatype: 'text', maxLength: 4, allowedChars: '0123456789,-', lettersCase: '', autoTab: false, enterTab: false}" class="sc-js-input scFilterObjectOdd"<?php echo $style_cond_in ?>>         <BR>
        <SPAN id="id_vis_porcentaje"  <?php echo $str_display_porcentaje; ?> class="scFilterFieldFontOdd">
         <?php echo $date_sep_bw ?>&nbsp;
         <BR>
         <INPUT type="text" id="SC_porcentaje_input_2" name="porcentaje_input_2" value="<?php echo NM_encode_input($porcentaje_input_2) ?>"  size=4 alt="{datatype: 'integer', maxLength: 4, thousandsSep: ',', allowNegative: true, onlyNegative: false, enterTab: false}" class="sc-js-input sc-js-input scFilterObjectOdd">         </SPAN>
...
<?php
   if (is_file("grid_abc_productos_help.txt"))
   {
...
?>
<?php
   if ($nm_apl_dependente == 1 || (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opc_psq'] && !$this->aba_iframe))
   {
...
<?php
       ...
       elseif (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['under_dashboard'])
       { }
       else 
       { 
?>
...
<?php
   ...
   elseif ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opc_psq'])
   {
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['sc_modal'])
       {
?>
...
?>
   ...
   <INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.nomgrupo_cond.value = 'eq';
   nm_campos_between(document.getElementById('id_vis_nomgrupo'), document.F1.nomgrupo_cond, 'nomgrupo');
   document.F1.nomgrupo.value = "";
   ...
   document.F1.porcentaje_input_2.value = "";
   document.F1.porcentaje_IN.value = "";
   Sc_carga_select2('all');
 }
 function Sc_carga_select2(Field)
 {
    if (Field == 'all' || Field == 'nomgrupo') {
       Sc_carga_select2_nomgrupo();
    }
 }
 function Sc_carga_select2_nomgrupo()
 {
  $("#SC_nomgrupo").select2(
    {
      language: {
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "FACILWEBv_2022/grid_abc_productos";
   ...
   function inicializa_vars()
   {
      ...
      $this->nm_data = new nm_data("es");
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] = "";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function recupera_filtro($NM_filters)
   {
      ...
      $tp_fields['SC_nomgrupo_cond'] = 'cond';
      $tp_fields['SC_nomgrupo'] = 'select';
      ...
      $tp_fields['SC_porcentaje_input_2'] = 'text';
   ...
   {
      global $nomgrupo_cond, $nomgrupo,
             $producto_cond, $producto, $producto_autocomp,
             $porcentaje_cond, $porcentaje, $porcentaje_input_2, $porcentaje_autocomp, $porcentaje_IN, $nmgp_tab_label;
      ...
      $nomgrupo_cond_salva = $nomgrupo_cond; 
      if (!isset($nomgrupo_input_2) || $nomgrupo_input_2 == "")
      {
          $nomgrupo_input_2 = $nomgrupo;
      }
      ...
      if ($tmp_pos === false) {
          ...
          $L_lookup = substr($nomgrupo, 0, $tmp_pos);
      }
      if ($this->NM_ajax_opcao != "ajax_grid_search_change_fil" && !empty($L_lookup) && !in_array($L_lookup, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['psq_check_ret']['nomgrupo'])) {
          if (!empty($this->Campos_Mens_erro)) {$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= "Grupo/Familia : " . $this->Ini->Nm_lang['lang_errm_ajax_data'];
      }
      if ($porcentaje_cond != "in")
      {
          nm_limpa_numero($porcentaje, ",") ; 
      }
      if ($porcentaje_cond != "in")
      {
          nm_limpa_numero($porcentaje_input_2, ",") ; 
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['nomgrupo'] = $nomgrupo; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['nomgrupo_cond'] = $nomgrupo_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['producto'] = $producto; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['producto_cond'] = $producto_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['porcentaje'] = $porcentaje; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['porcentaje_input_2'] = $porcentaje_input_2; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['porcentaje_IN'] = $porcentaje_IN; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['porcentaje_cond'] = $porcentaje_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca'] = $Temp_Busca;
      }
      ...
      $this->cmp_formatado['nomgrupo'] = $Conteudo;
      $nmgp_def_dados = array();
    if ($producto != '') {
      $producto_look = substr($this->Db->qstr($producto), 1, -1); 
      $nmgp_def_dados = array(); 
   ...
   { 
      ...
      {
          $cmp1 = $porcentaje;
          nmgp_Form_Num_Val($cmp1, ",", ".", "0", "S", "2", "", "N:3", "-") ; 
          $this->cmp_formatado['porcentaje'] = $cmp1;
      }
      $Conteudo = $porcentaje_input_2;
      ...
      $this->cmp_formatado['porcentaje_input_2'] = $Conteudo;

      //----- $nomgrupo
      $this->Date_part = false;
      if (isset($nomgrupo))
      {
         $this->monta_condicao("nomgrupo", $nomgrupo_cond, $nomgrupo, "", "nomgrupo");
      }
      ...
      {
         $this->monta_condicao("producto", $producto_cond, $producto, "", "producto");
      }
      ...
      {
         $this->monta_condicao("porcentaje", $porcentaje_cond, $porcentaje, $porcentaje_input_2, "porcentaje");
      }
      if ($porcentaje_cond == "in")
      {
          $porcentaje = $Cmp_save;
   ...
   {
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq_fast'] = "";
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['fast_search']);
      if ("" == $this->comando_filtro)
      {
          $this->comando = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_orig'];
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq']         = $this->comando . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao']              = "pesq";
      if ("" == $this->comando_filtro)
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq_filtro'] = "";
      }
      else
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'] .= $this->NM_operador;
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['contr_array_resumo'] = "NAO";
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['contr_total_geral']  = "NAO";
         unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['tot_geral']);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq_ant'] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq'];
   ...
   function css_obj_select_ajax($Obj)
   {
      switch ($Obj)
      {
         case "nomgrupo" : return ('class="scFilterObjectOdd"'); break;
         default       : return ("");
      }
   }
