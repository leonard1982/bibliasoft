...
class grid_abc_productos_resumo
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   var $sum_vendido;
   var $sum_porcentaje;
   var $sc_proc_quebra_nomgrupo;
   var $count_nomgrupo;
   var $sum_nomgrupo_vendido;
   var $sum_nomgrupo_porcentaje;
   ...
   function __construct($tipo = "")
   {
      ...
      $this->array_total_nomgrupo = array();
   ...
   function initializeButtons()
   {
      ...
      $this->nmgp_botoes['chart_exit'] = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['under_dashboard'] ? "off" : "on";

      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['under_dashboard'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['maximized']) {
          $tmpDashboardApp = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['dashboard_app'];
          if (isset($_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['grid_abc_productos'])) {
              $tmpDashboardButtons = $_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['grid_abc_productos'];
      ...
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_abc_productos']['btn_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['grid_abc_productos']['btn_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['grid_abc_productos']['btn_display'] as $NM_cada_btn => $NM_cada_opc)
          {
   ...
   { 
	...
	function generateChartImages() {
		...
		$this->Graf         = new grid_abc_productos_grafico();
		$this->Graf->Db     = $this->Db;
		$this->Graf->Erro   = $this->Erro;
		$this->Graf->Ini    = $this->Ini;
		$this->Graf->Lookup = $this->Lookup;

		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_step']     = 'image';
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_count']    = 0;
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_total']    = 0;
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_step'] = 'end';
	...
	function zipFileList($fileList, $password = '') {
		$tempDir     = $this->Ini->root . $_SESSION['scriptcase']['grid_abc_productos']['glo_nm_path_imag_temp'] . '/';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['grid_abc_productos']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
   ...
   function monta_resumo($b_export = false)
   {
      ...
      $this->sc_where_orig   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_orig'];
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq'];
      $this->sc_where_filtro = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq_filtro'];
     if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca']))
     { 
         $Busca_temp = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['campos_busca'];
      ...
      $this->nm_where_dinamico = "";
       ob_start(); 
      $_SESSION['scriptcase']['grid_abc_productos']['contr_erro'] = 'on';
...
$_SESSION['scriptcase']['grid_abc_productos']['contr_erro'] = 'off'; 
       $this->SC_Buf_onInit = ob_get_clean();; 
         if  (!empty($this->nm_where_dinamico)) 
         {   
             $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq'] .= $this->nm_where_dinamico;
         }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] != "pdf")
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_resumo']);
      }
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['res_hrz'];
      $dir_raiz          = strrpos($_SERVER['PHP_SELF'],"/") ;  
      $dir_raiz          = substr($_SERVER['PHP_SELF'], 0, $dir_raiz + 1) ;  
      $this->nm_location = $this->Ini->sc_protocolo . $this->Ini->server . $dir_raiz; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_resumo'] = "";
      }
      $this->inicializa_vars();
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Ind_Groupby'];
      $this->Total->$Gb_geral();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['contr_total_geral'] == "OK")
      {
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
          {
              $Arr_tot_name = "array_total_" . $cmp_gb;
              $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['arr_total'][$cmp_gb];
          }
      }
      else
      {
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['tot_geral'][1];
      $this->array_total_geral[1] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['tot_geral'][2];
      $this->array_total_geral[2] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['tot_geral'][3];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['tot_geral'][1] == 0)
      {
      ...
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] == 'print' || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] == 'pdf') && strpos(" " . $this->Ini->SC_module_export, "resume") === false)
      { }
      else
      {
          $this->drawMatrix();
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['arr_total']['nomgrupo'] = $this->array_total_nomgrupo;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['json_use_label'])
       {
          $Lab_nomgrupo = "nomgrupo";
       }
       else
       {
       $Lab_nomgrupo = "Grupo/Familia";
       }
       $prep_field = array();
       $prep_field['nomgrupo'] = $Lab_nomgrupo;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['labels']['nomgrupo'] = $Lab_nomgrupo; 
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
          $Str_gb .= ($Str_gb == "") ? "" : ",";
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['labels']['nomgrupo']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['labels']['nomgrupo'] = "Grupo/Familia"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['labels']['producto']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['labels']['producto'] = "Producto"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['labels']['vendido']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['labels']['vendido'] = "Vendido"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['labels']['porcentaje']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['labels']['porcentaje'] = "Porcentaje"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['labels']['total']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['labels']['total'] = "Total"; 
       }
       //-----
       $ix = 0;
       $this->comp_field_nm = array();
       ...
       $this->comp_sum = array(
           ...
           3 => "Vendido(" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sumt'] . ")",
           4 => "Porcentaje(" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sumt'] . ")",
       );
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['summarizing_fields_order']['sc_free_group_by']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['summarizing_fields_order']['sc_free_group_by'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['summarizing_fields_order']['sc_free_group_by'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['summarizing_fields_display']['sc_free_group_by'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['summarizing_fields_display']['sc_free_group_by'] as $i_sum => $d_sum)
           {
       ...
       $this->comp_sum_lnk = array(
           ...
           3 => array('field' => "vendido", 'show' => true),
           4 => array('field' => "porcentaje", 'show' => true),
       );
      ...
      eval ($Str_gb);
       ...
       $Arr_parms = array();
       $Arr_parms['nomgrupo']['alin'] = "''";
       $Arr_parms['nomgrupo']['link'] = "S";
       $Arr_parms['nomgrupo']['fill'] = "true";
       $Arr_parms['nomgrupo']['order'] = "label";
       $Arr_parms['nomgrupo']['order_start'] = 'asc';
       $Arr_parms['nomgrupo']['order_invert'] = 'false';
       $Arr_parms['nomgrupo']['order_enabled'] = 'false';
       //-----
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
       ...
       //-----
       $prep_links_fl = array();
       $prep_links_fl['nomgrupo'] = array(0 => 'nomgrupo', 1 => '@aspass@');
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Gb_Free_cmp'] as $cmp_gb => $col_sql)
       {
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_col'] = 3;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_level'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_sort'] = 'd';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_col']  = 0;
           }
           else
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_tabular_label'];
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@grid_abc_productos@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function makeTabular(&$a_data)
   {
       ...
       {
           $a_labels = array();
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['proc_pdf_vert'])
   {
   ...
   function buildChart()
   {
       ...
       $prep_chart_config = array(
           ...
           '1' => array(
               ...
               'label_x'  => $this->getChartText(strip_tags($Lab_nomgrupo)),
           ...
           '3' => array(
               'title'    => $this->getChartText(strip_tags("Porcentaje(" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sumt'] . ")")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'nomgrupo',
               'field_y'  => $this->comp_sum_nm[4],
               'label_x'  => $this->getChartText(strip_tags($Lab_nomgrupo)),
               'label_y'  => $this->getChartText(strip_tags("Porcentaje(" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sumt'] . ")")),
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_charts']    = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_orig']      = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['comb_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['comb_table_def']  = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['anlt_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['anlt_table_def']  = array();
	...
	function filterChartsByGroupbyLevel() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['chart_level']) && '' !== $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['chart_level']) {
			$applyFilter = true;
		}
		elseif (isset($nmgp_opcao) && 'image_res' == $nmgp_opcao && isset($_POST['chart_level']) && '' !== $_POST['chart_level']) {
			$applyFilter = true;
			$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['chart_level'] = $_POST['chart_level'];
		}
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_charts'] = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_orig'];

		if ($applyFilter) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_charts'] as $chartIndex => $chartData) {
				...
				if ($groupbyLevel > $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['chart_level']) {
					unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_charts'][$chartIndex]);
				}
			}
		}

		$this->comp_chart_data = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_charts'];
	}
	...
	function filterChartsBySummarization() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao)) {
			...
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['summarizing_fields_display']['sc_free_group_by'] as $fieldIndex => $fieldData) {
           ...
           foreach ($a_totals as $i_total => $values)
           {
               ...
               if (isset($this->comp_chart_config[$key_config]))
               {
                   ...
                   foreach ($a_keys as $tmp_key)
                   {
                       ...
                       $xml_links[]  = $this->getFusionLink('sc_grid_abc_productos_' . session_id() . '_!!!' . implode('_', array_merge(array($group + 1, $i_total), $link_index)) . '!!!.json');
                   }
                   ...
                   $this->comp_chart_data[$key_data]['xml']        = $this->getFusionLink('sc_grid_abc_productos_' . session_id() . '_!!!' . implode('_', array_merge(array($group, $i_total), $param)) . '!!!.json');
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['arr_export']['data']  = $this->build_data;
           return;
       }

       $nm_saida->saida("<tr id=\"summary_body\">\r\n");
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['ajax_nav'])
       { 
           $_SESSION['scriptcase']['saida_html'] = "";
       } 
      $TD_padding = (!$this->Print_All && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] == "pdf") ? " style=\"padding: 0px !important;\"" : "";
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               if (0 <= $columns['level'])
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['embutida'])
                   {
               ...
               $chart   = (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] != "print" && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['embutida']) && isset($columns['chart']) && '' != $columns['chart'] && isset($this->comp_chart_data[ $columns['chart'] ]))
   ...
   function drawMatrixLabels()
   {
       ...
       $nm_saida->saida("    <script type=\"text/javascript\">\r\n");
       if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['embutida'])
       { 
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   ...
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialDisplay = '; display: none';
                           $sInitialSrc     = '';
                       }
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['proc_pdf_vert']){ 
   ...
   function resumo_init()
   {
      ...
      if ("out" == $this->NM_tipo)
      {
         $this->monta_html_ini();
         $this->monta_cabecalho();
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] != "pdf" && !$_SESSION['scriptcase']['proc_mobile'])
         {
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['grid_abc_productos']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['grid_abc_productos']) . "_";
               } 
           } 
           else 
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['grid_abc_productos']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['grid_abc_productos']) . "_";
               } 
           }
       }
   ...
   function inicializa_vars()
   {
      ...
      if ("out" == $this->NM_tipo)
      {
         require_once($this->Ini->path_aplicacao . "grid_abc_productos_total.class.php"); 
      }
      $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['print_all'];
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['doc_word'] || $this->Ini->sc_export_ajax_img)
      { 
      ...
      if ($this->Print_All)
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] = "print";
      ...
      $this->Total   = new grid_abc_productos_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['arr_total'][$cmp_gb];
      }
...
if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['doc_word'])
{
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_refresh_after_chart'] = 'resumo';
      $nm_saida->saida("<HTML" . $_SESSION['scriptcase']['reg_conf']['html_dir'] . ">\r\n");
      $nm_saida->saida("<HEAD>\r\n");
      $nm_saida->saida(" <TITLE>ABC DE PRODUCTOS</TITLE>\r\n");
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['doc_word'])
{
      ...
      { 
           ...
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"grid_abc_productos_ajax.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida(" </script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"grid_abc_productos_jquery-3.6.0.min.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"grid_abc_productos_message.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput2.js\"></script>\r\n");
...
$nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "grid_abc_productos/grid_abc_productos_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
           $nm_saida->saida("   <link rel=\"stylesheet\" href=\"" . $this->Ini->path_prod . "/third/jquery_plugin/thickbox/thickbox.css\" type=\"text/css\" media=\"screen\" />\r\n");
           $nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/buttons/" . $this->Ini->Str_btn_css . "\" /> \r\n");
      } 
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['num_css']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['num_css'] = rand(0, 1000);
      }
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_abc_productos_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_grid_abc_productos_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_abc_productos_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['num_css'] . '.css');
...
$nm_saida->saida("      url: \"grid_abc_productos_save_grid.php\",\r\n");
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if (!$this->Ini->Export_html_zip && $this->Print_All && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['doc_word']) 
      {
      ...
      {
          $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['remove_margin'] ? 'margin: 0;' : '';
          $remove_border = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['remove_border'] ? 'border-width: 0;' : '';
          $vertical_center = '';
          $nm_saida->saida(" <BODY id=\"grid_summary\" class=\"" . $this->css_scGridPage . "\" style=\"" . $remove_margin . $vertical_center . "\">\r\n");
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['embutida'])
      {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['embutida'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['doc_word'])
      { 
          $nm_saida->saida("      <STYLE>\r\n");
   ...
   function monta_barra_top_normal()
   {
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['word'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bword", "", "", "Rword_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_abc_productos/grid_abc_productos_config_word.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_cor=AM&nm_res_cons=s&nm_ini_word_res=resume&nm_all_modules=grid,resume,chart&password=n&origem=res&language=es&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['print'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Rprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_abc_productos/grid_abc_productos_config_print.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_opc=resumo&nm_cor=PB&nm_res_cons=s&nm_ini_prt_res=resume&nm_all_modules=grid,resume,chart&password=n&origem=res&language=es&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      if ($this->nmgp_botoes['chart_conf'] == 'on' && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bconf_graf", "", "", "Rgraf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_abc_productos/grid_abc_productos_config_graf_flash.php?nome_apl=grid_abc_productos&campo_apl=nm_resumo_graf&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=es&tp_apl=cns_htm&KeepThis=true&TB_iframe=true&height=450&width=400&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
      }
      ...
      if ($this->nmgp_botoes['groupby'] == "on" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['opc_psq'] && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
          $Q_free  = false;
          $Q_count = 0;
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_All_Groupby'] as $QB => $Tp)
          {
              if (!in_array($QB, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Groupby_hide']) && $Tp == "all")
              {
                  $Q_count++;
          ...
          if ($Q_count > 1 || $Q_free)
          {
             $this->nm_btn_exist['groupby'][] = "sel_groupby_top";
              $Cod_Btn = nmButtonOutput($this->arr_buttons, "bgroupby", "scBtnGroupByShow('" . $this->Ini->path_link . "grid_abc_productos/grid_abc_productos_sel_groupby.php?opc_ret=resumo&path_img=" . $this->Ini->path_img_global . "&path_btn=" . $this->Ini->path_botoes . "&script_case_init=" . NM_encode_input($this->Ini->sc_page) . "&embbed_groupby=Y&toolbar_pos=top', 'top');", "scBtnGroupByShow('" . $this->Ini->path_link . "grid_abc_productos/grid_abc_productos_sel_groupby.php?opc_ret=resumo&path_img=" . $this->Ini->path_img_global . "&path_btn=" . $this->Ini->path_botoes . "&script_case_init=" . NM_encode_input($this->Ini->sc_page) . "&embbed_groupby=Y&toolbar_pos=top', 'top');", "sel_groupby_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "", "", "", "only_text", "text_right", "", "", "", "", "", "", "");
              $nm_saida->saida("           $Cod_Btn \r\n");
          }
              $NM_btn = true;
      }
      if ($this->nmgp_botoes['chart_detail'] == 'on' && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
         ...
         if (is_file("grid_abc_productos_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
            $Arq_WebHelp = file("grid_abc_productos_help.txt"); 
   ...
   function monta_html_fim()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['print_all'])
      {
          $this->grafico_pdf();
      }
      $nm_saida->saida("</TABLE>\r\n");
      $nm_saida->saida("<link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/css/" . $this->Ini->str_schema_all . "_sweetalert.css\" />\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['grid_abc_productos']['glo_nm_path_prod'] . "/third/sweetalert/sweetalert2.all.min.js\"></script>\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['grid_abc_productos']['glo_nm_path_prod'] . "/third/sweetalert/polyfill.min.js\"></script>\r\n");
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'grid_abc_productos_config_pivot.php?nome_apl=grid_abc_productos&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=es&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   $nm_saida->saida("                  action=\"grid_abc_productos_iframe_prt.php\" \r\n");
   ...
   $nm_saida->saida("   }\r\n");
      ...
      $nm_saida->saida("               nm_submit_modal(\"" . $this->Ini->path_link . "grid_abc_productos/grid_abc_productos_export_email.php?script_case_init={$this->Ini->sc_page}&path_img={$this->Ini->path_img_global}&path_btn={$this->Ini->path_botoes}&sType=\"+ str_type +\"&sAdd=__E__nmgp_cor_word=\" + cor + \"__E__SC_module_export=\" + SC_module_export + \"__E__nmgp_password=\" + password + \"&KeepThis=true&TB_iframe=true&modal=true\", bol_param);\r\n");
      $nm_saida->saida("       }\r\n");
      $nm_saida->saida("       else\r\n");
      $nm_saida->saida("       {\r\n");
      ...
      $nm_saida->saida("               document.Fdoc_word.action = \"grid_abc_productos_export_ctrl.php\";\r\n");
      ...
      $nm_saida->saida("               document.Fprint.action = \"grid_abc_productos_export_ctrl.php\";\r\n");
      $nm_saida->saida("           }\r\n");
      $nm_saida->saida("           else\r\n");
      $nm_saida->saida("           {\r\n");
      ...
      $nm_saida->saida("               document.Fexport.action = \"grid_abc_productos_export_ctrl.php\";\r\n");
      ...
      $nm_saida->saida("       document.F3.SC_module_export.value = z;\r\n");
      $nm_saida->saida("  }\r\n");
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['grid_abc_productos_iframe_params'] = array(
      ...
      $Md5_pdf    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@grid_abc_productos@SC_par@" . md5($prep_parm_pdf);
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['Md5_pdf'][md5($prep_parm_pdf)] = $prep_parm_pdf;
   ...
   $nm_saida->saida("               nm_submit_modal(\"" . $this->Ini->path_link . "grid_abc_productos/grid_abc_productos_export_email.php?script_case_init={$this->Ini->sc_page}&path_img={$this->Ini->path_img_global}&path_btn={$this->Ini->path_botoes}&sType=pdf_res&sAdd=__E__nmgp_tipo_pdf=\" + z + \"__E__sc_parms_pdf=\" + p + \"__E__sc_create_charts=\" + crt + \"__E__sc_graf_pdf=\" + g + \"__E__chart_level=\" + chart_level + \"__E__SC_module_export=\" + SC_module_export + \"__E__use_pass_pdf=\" + use_pass_pdf + \"__E__pdf_all_cab=\" + pdf_all_cab + \"__E__pdf_all_label=\" +  pdf_all_label + \"__E__pdf_label_group=\" +  pdf_label_group + \"__E__pdf_zip=\" +  pdf_zip + \"&nm_opc=pdf&KeepThis=true&TB_iframe=true&modal=true\", '');\r\n");
      ...
      $nm_saida->saida(" }\r\n");
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Gb_Free_cmp']))
      {
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_grid_abc_productos_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "grid_abc_productos/grid_abc_productos_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['print_all'];
       if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pdf_res'])
       {
           $nm_saida->saida("<TR>\r\n");
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
...
//--- 
 function grafico_pdf()
 {
   ...
   $this->Graf->Ini    = $this->Ini;
   $this->Graf->Lookup = $this->Lookup;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_charts']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['skip_charts']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['skip_charts']))
   {
       ...
       $iChartCount = 1;
       $iChartTotal = sizeof($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_charts']);
 ...
 { 
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['pivot_charts'] as $chart_index => $chart_data)
       {
           if (!$prim_graf)
           {
 ...
 { 
           ...
           if ($this->progress_fp)
           {
               grid_abc_productos_pdf_progress_call($this->progress_tot . "_#NM#_" . $this->progress_now . "_#NM#_" . $sChartLang . " " . $iChartCount . "...\n", $this->Ini->Nm_lang, true);
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['this_chart_label'] = '';
       ...
       if ($bChartFP)
       {
           ...
           grid_abc_productos_pdf_progress_call(100 . "_#NM#_" . 90 . "_#NM#_" . $lang_protect . "...\n", $this->Ini->Nm_lang);
           fwrite($this->progress_fp, 90 . "_#NM#_" . $lang_protect . "...\n");
           fclose($this->progress_fp);
       }
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['charts_html'])
       {
            $nm_saida->saida("<script type=\"text/javascript\">\r\n");
            $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['charts_html']}\r\n");
            $nm_saida->saida("</script>\r\n");
       }
   }
       $nm_saida->saida("</body>\r\n");
...
//--- 
 ...
 {
   global $nm_saida, $nm_lang;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['chart_list']))
   {
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['chart_list'] as $arr_chart)
       {
           $nm_saida->saida("<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>\r\n");
       $nm_saida->saida("<b><div style=\"height:1px;overflow:hidden\"><H1 style=\"font-size:0;padding:1px\">" . $this->Ini->Nm_lang['lang_btns_chrt_pdff_hint'] . "</H1></div></b>\r\n");
           $nm_saida->saida("<table><tr><td>\r\n");
...
//--- 
   ...
   {
      global $nm_saida;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['embutida'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['dashboard_info']['compact_mode'])
      { 
          return;
      } 
      ...
      { 
        ...
        if ($_SESSION['scriptcase']['charset'] != "UTF-8")
        {
          ...
          $nomgrupo = $Busca_temp['nomgrupo']; 
          $tmp_pos = strpos($nomgrupo, "##@@");
          if ($tmp_pos !== false && !is_array($nomgrupo))
          {
              $nomgrupo = substr($nomgrupo, 0, $tmp_pos);
          }
      ...
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq']))
      {  
          $pos       = 0;
          $trab_pos  = false;
          $pos_tmp   = true; 
          $tmp       = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'];
          while ($pos_tmp)
          {
             $pos = strpos($tmp, "##*@@", $pos);
             if ($pos !== false)
          ...
          $nm_cond_filtro_or  = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'], $trab_pos + 5) == "or")  ? " " . trim($this->Ini->Nm_lang['lang_srch_orr_cond']) . " " : "";
          $nm_cond_filtro_and = (substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'], $trab_pos + 5) == "and") ? " " . trim($this->Ini->Nm_lang['lang_srch_and_cond']) . " " : "";
          $nm_cab_filtro   = substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['cond_pesq'], 0, $trab_pos);
     ...
     {
         $this->Ini->Img_export_zip[] = $this->Ini->root . $this->Ini->path_imag_cab . "/scriptcase__NM__ico__NM__components_32.png";
         $img_LIN1_COL2 = "scriptcase__NM__ico__NM__components_32.png";
     }
     elseif ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['doc_word'] || $this->Ini->sc_export_ajax_img)
     {
         $loc_img_word = $this->Ini->root . $this->Ini->path_imag_cab . "/scriptcase__NM__ico__NM__components_32.png";
     ...
     {
         $img_LIN1_COL2 = $this->NM_raiz_img . $this->Ini->path_imag_cab . "/scriptcase__NM__ico__NM__components_32.png";
     }
      $nm_saida->saida("<style>\r\n");
      ...
      $nm_saida->saida("    	<tr>\r\n");
      $nm_saida->saida("        	<td id=\"lin1_col1\" class=\"" . $this->css_scGridHeaderFont . "\"><span>ABC DE PRODUCTOS</span></td>\r\n");
   ...
   //---- 
   function adiciona_registro($sum_vendido, $sum_porcentaje, $quebra_nomgrupo, $quebra_nomgrupo_orig)
   {
      $contr_arr = "";
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
      {
          $Name_orig  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Gb_Free_orig'][$cmp_gb])) ? $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Gb_Free_orig'][$cmp_gb] : $cmp_gb;
          ...
          $cmp_look   = "Cmp_formt";
          $cmp_orig   = "Cmp_arg";
          if (empty($sum_vendido)) {$sum_vendido = '0';}
          if (empty($sum_porcentaje)) {$sum_porcentaje = '0';}
          eval ('
          if (!isset($this->' . $arr_name . '))
          {
              $this->' . $arr_name . '[0] = 1;
              $this->' . $arr_name . '[1] = ' . $sum_vendido . ';
              $this->' . $arr_name . '[2] = ' . $sum_porcentaje . ';
              $this->' . $arr_name . '[3] = 1;
   ...
   function finaliza_arrays()
   {
      ...
      $this->sum_vendido  = $this->array_total_geral[1];
      $this->sum_porcentaje  = $this->array_total_geral[2];
      $contr_cmp_gb = array();
      $contr_ordem  = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
      {
 ...
 /*-----*/ 
      if (isset($contr_cmp_gb[0]))
      {
         ...
         if (isset($contr_ordem[$contr_cmp_gb[0]]) && $contr_ordem[$contr_cmp_gb[0]] == "asc") {
             ksort($arr_name_0);
         }
         if (isset($contr_ordem[$contr_cmp_gb[0]]) && $contr_ordem[$contr_cmp_gb[0]] == "desc") {
             krsort($arr_name_0);
         }
         foreach ($arr_name_0 as $cmp_name_0  => $dados_gb)
         {
           $prep = $contr_cmp_gb[0];
           $this->$prep = $dados_gb[4];
           $prep = "count_" . $contr_cmp_gb[0];
           $this->$prep = $dados_gb[0];
           $prep = "sum_" . $contr_cmp_gb[0] . "_vendido";
           $this->$prep = $dados_gb[1];
           $prep = "sum_" . $contr_cmp_gb[0] . "_porcentaje";
           $this->$prep = $dados_gb[2];
           $this->NM_count = "count_" . $contr_cmp_gb[0];
           $this->NM_sum_vendido  = "sum_" . $contr_cmp_gb[0] . "_vendido";
           $this->NM_sum_porcentaje  = "sum_" . $contr_cmp_gb[0] . "_porcentaje";
           $prep = "sc_proc_quebra_" . $contr_cmp_gb[0];
           $this->$prep = true;
           if ($contr_cmp_gb[0] == "nomgrupo")
           {
           $_SESSION['scriptcase']['grid_abc_productos']['contr_erro'] = 'on';
if (!isset($_SESSION['gtotalporcentaje'])) {$_SESSION['gtotalporcentaje'] = "";}
if (!isset($this->sc_temp_gtotalporcentaje)) {$this->sc_temp_gtotalporcentaje = (isset($_SESSION['gtotalporcentaje'])) ? $_SESSION['gtotalporcentaje'] : "";}
...
$_SESSION['scriptcase']['grid_abc_productos']['contr_erro'] = 'off'; 
           ...
           $this->$prep = false;
            eval ("\$this->array_total_" . $contr_cmp_gb[0] . "['" . $cmp_name_0 . "'][1] = \$this->sum_" . $contr_cmp_gb[0] . "_vendido;");
            eval ("\$this->array_total_" . $contr_cmp_gb[0] . "['" . $cmp_name_0 . "'][2] = \$this->sum_" . $contr_cmp_gb[0] . "_porcentaje;");
         } 
      } 
      $this->array_total_geral[0] = $this->count_ger;
      $this->array_total_geral[1] = $this->sum_vendido;
      $this->array_total_geral[2] = $this->sum_porcentaje;
   }
...
$_SESSION['scriptcase']['grid_abc_productos']['contr_erro'] = 'off'; 
     ...
     { 
         $nmgp_select = "SELECT nomgrupo, producto, vendido, porcentaje, total from (select p.nompro as producto,sum(d.valorpar-d.iva) as vendido,round(((sum(d.valorpar-d.iva)*100)/(select sum(v.valorpar-v.iva) from detalleventa v)),2) as porcentaje,(select sum(v.valorpar-v.iva) from detalleventa v) as total, g.nomgrupo from productos p inner join detalleventa d on d.idpro=p.idprod inner join grupo g on p.idgrup=g.idgrupo GROUP by p.nompro ORDER by sum(d.valorpar-d.iva) desc) nm_sel_esp"; 
     } 
     ...
     $nmgp_select .= " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['where_pesq']; 
     $campos_order = "";
     foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_abc_productos']['SC_Gb_Free_sql'] as $cmp_var => $resto)
     {
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->nomgrupo = $rs_res->fields[0] ;  
            $this->producto = $rs_res->fields[1] ;  
            $this->vendido = $rs_res->fields[2] ;  
            $this->vendido =  str_replace(",", ".", $this->vendido);
            $this->porcentaje = $rs_res->fields[3] ;  
            $this->porcentaje =  str_replace(",", ".", $this->porcentaje);
            $this->total = $rs_res->fields[4] ;  
            $this->total =  str_replace(",", ".", $this->total);
            $this->nomgrupo_orig = $this->nomgrupo;
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->vendido, $this->porcentaje, $this->nomgrupo, $this->nomgrupo_orig);
            }
   }
//
