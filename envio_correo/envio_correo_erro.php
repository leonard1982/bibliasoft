...
$old_error_handler = set_error_handler("envio_correo_scriptcase_error_handler");

function envio_correo_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['envio_correo']['contr_erro']) && $_SESSION['scriptcase']['envio_correo']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_envio_correo->contr_envio_correo->Apl_com_erro = true;
            if ($inicial_envio_correo->contr_envio_correo->NM_ajax_flag)
            {
                $inicial_envio_correo->contr_envio_correo->NM_ajax_info['errList']['geral_envio_correo'][] = $msg;
                return;
            }
            envio_correo_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function envio_correo_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['envio_correo'])  && '' != $_SESSION['scriptcase']['error_icon']['envio_correo'])  ? $_SESSION['scriptcase']['error_icon']['envio_correo']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['envio_correo']) && '' != $_SESSION['scriptcase']['error_close']['envio_correo']) ? $_SESSION['scriptcase']['error_close']['envio_correo'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['envio_correo']) && '' != $_SESSION['scriptcase']['error_span_title']['envio_correo']) ? $_SESSION['scriptcase']['error_span_title']['envio_correo'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['envio_correo']) && '' != $_SESSION['scriptcase']['error_icon_title']['envio_correo']) ? $_SESSION['scriptcase']['error_icon_title']['envio_correo'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['envio_correo']['error_buffer'] .= ob_get_flush();
}
