...
class grid_ventas_por_familia_resumo
{
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      {
          $this->array_total_vendedor = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['arr_total']['vendedor'];
          $this->array_total_grupo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['arr_total']['grupo'];
      }
      else
      {
          $this->array_total_vendedor = array();
   ...
   function completeMatrix()
   {
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['summarizing_fields_order']['asesor_grupo']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['summarizing_fields_order']['asesor_grupo'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['summarizing_fields_order']['asesor_grupo'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['summarizing_fields_display']['asesor_grupo'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['summarizing_fields_display']['asesor_grupo'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['summarizing_fields_display']['asesor_grupo'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['summarizing_fields_display']['asesor_grupo'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['summarizing_fields_display']['asesor_grupo'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['summarizing_fields_display']['asesor_grupo'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['summarizing_fields_display']['asesor_grupo'] as $i_sum => $d_sum)
           {
       ...
       foreach ($this->array_total_vendedor as $i_vendedor => $d_vendedor) {
           ...
           foreach ($this->array_total_grupo[$i_vendedor] as $i_grupo => $d_grupo) {
               if (!in_array($i_grupo, $this->comp_label[1][ $d_vendedor[6] ], true)) {
                   $this->comp_index[1][ $d_grupo[6] ] = $d_grupo[5];
                   $this->comp_label[1][ $d_vendedor[6] ][ $d_grupo[6] ] = $d_grupo[5];
               }
           }
       }
       ...
       //-----
       $this->comp_links_fl = array(
           array('name' => 'vendedor', 'prot' => '@aspass@'),
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['pivot_order_level'] = -1;
       }
       ...
       foreach ($this->comp_label[0] as $i_vendedor => $l_vendedor) {
           ...
           foreach ($this->comp_label[1][$i_vendedor] as $i_grupo => $l_grupo) {
               if (isset($this->array_total_grupo[$i_vendedor][$i_grupo])) {
                   $this->comp_group[$i_vendedor][$i_grupo] = array();
               }
           }
       }
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_ventas_por_familia']['proc_pdf_vert']){ 
   ...
   function totaliza()
   {
      $this->Total->Calc_resumo_asesor_grupo("res");
...
//--- 
   ...
   //---- 
   function adiciona_registro($sum_base, $sum_f_valoriva, $sum_neto, $quebra_vendedor, $quebra_grupo, $quebra_vendedor_orig, $quebra_grupo_orig)
   {
      //----- Vendedor
      if (!isset($this->array_total_vendedor[$quebra_vendedor_orig]))
      {
         $this->array_total_vendedor[$quebra_vendedor_orig][0] = 1;
         $this->array_total_vendedor[$quebra_vendedor_orig][1] = $sum_base;
         $this->array_total_vendedor[$quebra_vendedor_orig][2] = $sum_f_valoriva;
         $this->array_total_vendedor[$quebra_vendedor_orig][3] = $sum_neto;
      ...
      //----- Grupo
      if (!isset($this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig]))
      {
         $this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig][0] = 1;
         $this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig][1] = $sum_base;
         $this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig][2] = $sum_f_valoriva;
         $this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig][3] = $sum_neto;
         $this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig][4] = 1;
         $this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig][5] = $quebra_grupo;
         $this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig][6] = $quebra_grupo_orig;
      }
      else
      {
         $this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig][0]++;
         $this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig][1] += $sum_base;
         $this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig][2] += $sum_f_valoriva;
         $this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig][3] += $sum_neto;
         $this->array_total_grupo[$quebra_vendedor_orig][$quebra_grupo_orig][4]++;
      }
   }
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            nmgp_Form_Num_Val($this->cantidad, "", "", "0", "", "", "", "N:", "") ; 
            nmgp_Form_Num_Val($this->base, "", "", "", "", "", "", "V::", "") ; 
            nmgp_Form_Num_Val($this->neto, "", "", "", "", "", "", "V::", "") ; 
            nmgp_Form_Num_Val($this->periodo, "", "", "0", "", "", "", "N:", "") ; 
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->base, $this->f_valoriva, $this->neto, $this->vendedor, $this->grupo, $this->vendedor_orig,  $this->grupo_orig);
            }
   }
//
