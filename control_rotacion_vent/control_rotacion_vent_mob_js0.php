...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['control_rotacion_vent_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="control_rotacion_vent_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_Ventas()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Ventas(); }, 500);
      return;
    }
    sc_btn_Ventas_ok();
}
function sc_btn_Ventas_cancel()
{
}
function sc_btn_Ventas_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_grid_detalleventa_arti_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "SC_glo_par_fmen*scinfmen*scoutSC_glo_par_fmay*scinfmen*scout"; ?>', 'inicio', '_self', '0', '0', 'grid_detalleventa_arti');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['control_rotacion_vent_mob']['where_detal']))
  {
?>  
...
?>
    ...
    {
        do_ajax_control_rotacion_vent_mob_navigate_form();
    }
} 
...
function scForm_submit_single(x) {
	if (x != "excluir")
	{
		...
		if ("incluir" == x || "muda_form" == x || "recarga" == x || "recarga_mobile" == x) {
			...
			Nm_Proc_Atualiz = true;
			do_ajax_control_rotacion_vent_mob_submit_form();
		}
	}
	if (Nm_submit_ok) {
		Nm_Proc_Atualiz = true;
...
function nm_saida_glo()
{
  ...
  document.F4.action = "control_rotacion_vent_mob_fim.php";
  document.F4.submit();
}
