<form name="F2" method=post 
               action="./" 
               target="_self"> 
<input type="hidden" name="iddetallecombo_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['iddetallecombo_']); ?>">
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_detallecombos"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  ...
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_detallecombos']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_detallecombos']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_detallecombos']['dashboard_info']['parent_widget'] . "']\")";
...
?>
    ...
    {
        do_ajax_form_detallecombos_navigate_form();
    }
    if ("ordem" == x)
    {
        scSetOrderColumn(y);
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_detallecombos_submit_form(y, z);
