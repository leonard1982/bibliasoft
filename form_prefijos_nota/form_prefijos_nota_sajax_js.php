  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_prefijos_nota" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_prefijos_nota_form();
	}
...
?>
  ...
  // ---------- Validate prefijo_n_
  function do_ajax_form_prefijos_nota_validate_prefijo_n_(iNumLinha)
  {
    var nomeCampo_prefijo_n_ = "prefijo_n_" + iNumLinha;
    var var_prefijo_n_ = scAjaxGetFieldText(nomeCampo_prefijo_n_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_prefijos_nota_validate_prefijo_n_(var_prefijo_n_, iNumLinha, var_script_case_init, do_ajax_form_prefijos_nota_validate_prefijo_n__cb);
  } // do_ajax_form_prefijos_nota_validate_prefijo_n_

  function do_ajax_form_prefijos_nota_validate_prefijo_n__cb(sResp)
  {
    ...
    if ("" != iLineNumber)
    {
      sFieldValid = "prefijo_n_" + iLineNumber;
    }
    else
    {
      sFieldValid = "prefijo_n_";
    }
    ...
    if ("" == sFieldErrors)
    {
      ...
      scErrorLineOff(iLineNumber, "prefijo_n_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "prefijo_n_");
    }
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_prefijos_nota_validate_prefijo_n__cb

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_form_prefijos_nota_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_form_prefijos_nota_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_form_prefijos_nota_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_form_prefijos_nota_add_new_line_cb);
  } // do_ajax_form_prefijos_nota_add_new_line

  function do_ajax_form_prefijos_nota_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
    if ("{" == sResp.substr(0, 1)) {
        oResp = scAjaxResponse(sResp);
  ...
  } // do_ajax_form_prefijos_nota_add_new_line_cb

  // ---------- backup_line
  function do_ajax_form_prefijos_nota_backup_line(iNumLinha)
  {
    var var_idprefijo_ = scAjaxGetFieldText("idprefijo_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_prefijos_nota_backup_line(var_idprefijo_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_form_prefijos_nota_backup_line_cb);
  } // do_ajax_form_prefijos_nota_backup_line

  function do_ajax_form_prefijos_nota_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    if (!scAjaxHasError())
  ...
  } // do_ajax_form_prefijos_nota_backup_line_cb

  function do_ajax_form_prefijos_nota_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
  ...
  } // do_ajax_form_prefijos_nota_cancel_insert

  function do_ajax_form_prefijos_nota_cancel_update(iSeqVert)
  {
    do_ajax_form_prefijos_nota_backup_line(iSeqVert);
...
?>
  } // do_ajax_form_prefijos_nota_cancel_update

  function do_ajax_form_prefijos_nota_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_form_prefijos_nota_restore_buttons

  // ---------- table_refresh
  function do_ajax_form_prefijos_nota_table_refresh()
  {
    var var_idprefijo_ = document.F2.idprefijo_.value;
    ...
    scAjaxProcOn();
    x_ajax_form_prefijos_nota_table_refresh(var_idprefijo_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_prefijos_nota_table_refresh_cb);
  } //  do_ajax_form_prefijos_nota_table_refresh

  function do_ajax_form_prefijos_nota_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    document.F2.idprefijo_.value = scAjaxGetKeyValue("idprefijo_");
    ...
    scAjaxAlert(do_ajax_form_prefijos_nota_table_refresh_cb_after_alert);
  } // do_ajax_form_prefijos_nota_table_refresh_cb
  function do_ajax_form_prefijos_nota_table_refresh_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    ...
    sc_insert_open = false;
  } // do_ajax_form_prefijos_nota_table_refresh_cb_after_alert
...
function scJs_sweetalert_params(params) {
  ...
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_form_prefijos_nota_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_form_prefijos_nota_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
    sc_num_ult_line = iNumLinha;
    sc_num_ult_tr   = indexTR;
    scAjaxHideMessage();
    var var_prefijo_n_ = scAjaxGetFieldText("prefijo_n_" + iNumLinha);
    var var_nm_form_submit = document.F1.nm_form_submit.value;
...
?>
    x_ajax_form_prefijos_nota_submit_form(var_prefijo_n_, var_nmgp_refresh_row, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_prefijos_nota_submit_form_cb);
  } // do_ajax_form_prefijos_nota_submit_form

  function do_ajax_form_prefijos_nota_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
      ...
      scAjaxHideErrorDisplay("table");
      scAjaxHideErrorDisplay("prefijo_n_" + sc_num_ult_line);
<?php
if (isset($this->Embutida_form) && $this->Embutida_form) {
?>
      scAjaxSetReadonly();
...
?>
    ...
    scAjaxSetLabel(true);
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_form_prefijos_nota_submit_form_cb_after_alert);
  } // do_ajax_form_prefijos_nota_submit_form_cb
  function do_ajax_form_prefijos_nota_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_prefijos_nota_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_prefijos_nota_navigate_form()
  {
    perform_ajax_form_prefijos_nota_navigate_form();
  }

  function perform_ajax_form_prefijos_nota_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    {
      scAjaxHideErrorDisplay("prefijo_n_" + iNavForm);
    }
    var var_idprefijo_ = document.F2.idprefijo_.value;
    var var_nm_form_submit = document.F2.nm_form_submit.value;
    var var_nmgp_opcao = document.F2.nmgp_opcao.value;
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_form_prefijos_nota_navigate_form(var_idprefijo_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_prefijos_nota_navigate_form_cb);
  } // do_ajax_form_prefijos_nota_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_prefijos_nota']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_prefijos_nota_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.idprefijo_.value = scAjaxGetKeyValue("idprefijo_" + var_last_index);
    ...
    scErrorLineReset();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_prefijos_nota_navigate_form_cb_after_alert);
  } // do_ajax_form_prefijos_nota_navigate_form_cb
  function do_ajax_form_prefijos_nota_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
...
?>
  ...
  } // do_ajax_form_prefijos_nota_navigate_form_cb_after_alert
  function sc_hide_form_prefijos_nota_form()
  {
...
?>
  ...
  {
    ajax_field_list[iTotCampos] = "prefijo_n_" + iNumLinha;
    iTotCampos++;
    ajax_error_list["prefijo_n_" + iNumLinha] = {"label": "Prefijo<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_field_mult["prefijo_n_"][iNumLinha] = "prefijo_n_" + iNumLinha;
    ajax_field_id["prefijo_n_" + iNumLinha] = new Array("hidden_field_label_prefijo_n_", "hidden_field_data_prefijo_n_" + iNumLinha);
    ajax_error_count["prefijo_n_" + iNumLinha] = "off";
<?php
if (!$this->Grid_editavel)
{
?>
    ajax_read_only["prefijo_n_" + iNumLinha] = "off";
<?php
}
else
{
?>
    ajax_read_only["prefijo_n_" + iNumLinha] = "on";
<?php
}
?>
  }
  ...
  {
    ajax_error_list["prefijo_n_" + iNumLinha] = null;
  }
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ajax_error_list["prefijo_n_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["prefijo_n_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["prefijo_n_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["prefijo_n_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["prefijo_n_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
?>
  ...
  {
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['prefijo_n_'])) ? $this->nmgp_cmp_readonly['prefijo_n_'] : 'off';
?>
    scAjaxFieldRead("prefijo_n_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }

  function mdCloseObjects(iSeq)
  {
    scAjaxFieldRead("prefijo_n_" + iSeq, "on");
  }
...
?>
  ...
  function scErrorLineOff(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("off" == ajax_error_count["prefijo_n_" + iRow]))
    {
      if (bErrorRow)
      {
        ajax_error_count["prefijo_n_" + iRow] = "off";
      }
      ...
      $("#hidden_field_data_prefijo_n_" + iRow).removeClass("scFormErrorLine");
    }
  }
