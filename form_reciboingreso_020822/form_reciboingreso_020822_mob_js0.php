<form name="F2" method=post 
               action="form_reciboingreso_020822_mob.php" 
               target="_self"> 
<input type="hidden" name="idrecibo" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['idrecibo']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_reciboingreso_020822_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_reciboingreso_020822_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_imprime_par_numero = "<?php echo $this->form_encode_input($this->nmgp_dados_form['idrecibo']); ?>";
function sc_btn_imprime()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_imprime(); }, 500);
      return;
    }
    sc_btn_imprime_ok();
}
function sc_btn_imprime_cancel()
{
}
function sc_btn_imprime_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_pdfreport_recibocaja_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "par_numero*scin' + var_btn_imprime_par_numero + '*scout"; ?>', 'inicio', '_blank', '0', '0', 'pdfreport_recibocaja');
}
...
function sc_btn_btn_asentar()
{
    ...
    scJs_confirm("<?php echo html_entity_decode('¿Desea asentar el documento?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_btn_asentar_ok, sc_btn_btn_asentar_cancel)
}
...
function sc_btn_btn_asentar_ok()
{
    ...
    document.F1.action = "form_reciboingreso_020822_mob.php";
...
function sc_btn_btn_reversar()
{
    ...
    scJs_confirm("<?php echo html_entity_decode('¿Desea reversar el documento?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_btn_reversar_ok, sc_btn_btn_reversar_cancel)
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_reciboingreso_020822_mob']['where_detal']))
  {
?>  
...
{
    NovaJanela = window.open ("form_reciboingreso_020822_mob_help.php?nm_cod_campo=" + campo + "&language=<?php echo $this->Ini->path_lang . $this->Ini->str_lang?>&help_css=<?php echo '../_lib/css/' . $this->Ini->str_schema_all . '_form.css'; ?>", "ScriptCase", "height=100, width=300, resizable, scrollbars");
}
...
{
   //si no esta asentado y tiene cuenta
if(document.getElementById('id_sc_field_ncuenta_tercero').value!='')
{
//document.getElementById('id_sc_field_cliente').readOnly = true;
document.getElementById('id_ac_cliente').disabled = true;
}
else
{
//document.getElementById('id_sc_field_cliente').readOnly = false;
document.getElementById('id_ac_cliente').disabled = false;
}
}
function sc_asentado_onchange()
{
   nm_atualiza ('alterar');
}
function sc_asentado_onclick()
{
   
}
function sc_valcobrar_onchange()
{
...
{
   nm_field_disabled("nurecibo=disabled", "U");
